/*
参考：https://atcoder.jp/contests/past202107-open/editorial
・自力ならず。。結構惜しかった。
・平面走査、座圧、遅延セグ木、区間和取得・区間01反転
・だいたい方針は合ってた。区間和取得・区間01反転の操作を、
　座圧した値に適用できなくて詰んだのだが、
　普通に1の代わりに座圧した重みを値として入れればいいだけだった。。
　今回は区間に後から加算とかする訳じゃないからかな、
　普通に値として持てばいいんだね。区間長とかじゃなくて。
*/
...
void solve() {
    ...
    ll N = cpx.size();
    ll M = cpy.size();
    vector<vector<tuple<ll, ll, ll>>> events(M);
    for (auto& [a, b, c, d] : qs) {
        a = cpx[a], c = cpx[c];
        b = cpy[b], d = cpy[d];
        events[b].pb({0, a, c});
    ...
    rep(x, N-1) {
        seg.set(x, {cpx.unzip(x+1)-cpx.unzip(x), 0});
    }
    seg.build();

    rep(y, M-1) {
        ll szy = cpy.unzip(y+1)-cpy.unzip(y);
        for (auto [op, l, r] : events[y]) {
            seg.update(l, r, 1);
        }
        ll szx = seg.query(0, N);
        ans += szx*szy;
    }
    print(ans);
}
