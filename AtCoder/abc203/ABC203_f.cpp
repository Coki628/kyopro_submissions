/*
・ABC高難度埋め
・自力AC！嬉しい。それなりに時間かかったし本番中なら厳しいんだけどね。。
・ソートしてDP、遷移先は二分探索で探す。
・なんか決め打ちにぶたんとかで軽く迷走してガチャガチャ色々いじった後、DP方針を考えて、
　高橋君の操作回数がlog回くらいで終わるからこれ添字に持てるじゃん、ってなって解けた。
　操作回数がlog回くらいなことは考察序盤で考えたのに、DP方針に転換した後しばらく
　そこを添字に持ってこれることに気付けなかったのはタイムロスだったなー。
　なんか値に2人の最小値ペア持たせようとしたりしてた。まあ最終的に解けたからいいけど。
*/
...
void solve() {
    ...
    dp[0][0] = 0;
    rep(i, N) {
        ll k = bisearch_min(i, N, [&](ll m) {
            return A[m] <= A[i]/2;
        });
        rep(j, M) {
            if (dp[i][j]+1 <= K) chmin(dp[i+1][j], dp[i][j]+1);
            if (j+1 < M) chmin(dp[k][j+1], dp[i][j]);
        }
    }
    rep(j, M) {
        if (dp[N][j] < INF) {
