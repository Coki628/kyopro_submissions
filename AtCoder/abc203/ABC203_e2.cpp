...
・やってみた。やっぱこっちのが簡潔に書けるし楽。
　位置だけ分かればよくでindexいらないなら、やっぱsetのが楽だよねこういうの。
*/
...
void solve() {
    ...
    for (auto& [x, li] : XY) {
        vector<ll> rm, add;
        for (ll y : li) {
            if (se.count(y) and not se.count(y-1) and not se.count(y+1)) {
                rm.pb(y);
            }
            if (not se.count(y) and (se.count(y-1) or se.count(y+1))) {
                add.pb(y);
            }
        }
        for (ll y : rm) {
