/*
参考：https://atcoder.jp/contests/arc122/editorial
　　　https://note.com/syamashi/n/ndc9f6be90359
・ARC青diff残埋め
・自力ならず。これは本番中解けなくて、後からフィボナッチだよって話だったの聞いたのも覚えてたんだけどね。
・構築、フィボナッチ
・まず、3,4の操作はフィボナッチになる。これに気付かなきゃスタート地点立ててない。
　(本番中は散々悩んで気付かなかった気がする。)
・で、フィボナッチと分かった所で、もうひと考察必要。Nぴったりに合わせるためにどうするか。
　実は途中で+1をやるのは、そこから始まるフィボナッチを独立にもう1個作ると考えることができる。
　同じ操作で足していくだけなんだから、言われると確かにそうかなと思う。
　これなら最初に終了位置を決めておけば、今+1するとそれが最終的にどこまで増えるフィボナッチになるか
　予想できるようになるので、これがN以下である限り貪欲に足せばいい。
　終了位置は1つのフィボナッチでN以下最大になるところにしておけば足りなくなることはない。
・どんな数であってもぴったり合わせられるのは、まあ2冪で上から足してくのと同じような感じだろう。
　フィボナッチはすごい数列なので、そういうのよくできてるとしても全然不思議じゃない。
　(まじで数学強い人に呆れられそうな理屈だな。。)
*/
