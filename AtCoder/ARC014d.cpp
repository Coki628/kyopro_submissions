/*
参考：https://www.slideshare.net/hamayanhamayan/atcoder-regular-contest-014
・試験管青diff残埋め
・自力ならず。結構むずくない？これこの時代で青diffなのかー。。
・差分、ソート、二分探索、総和と個数管理するやつ
・この発想に至らなかった。結構思いつきにくくない？…そうでもないのか。
・両側から広がる行をx+yとすると、行間の行数を取ってソートしておけば、
　どこまでは埋まり切ってどこが余白があるかは二分探索で求まる。
　これが分かれば、後は総和と個数*長さでやるいつものやつ。
　端だけは両側からではなく片側から埋まる感じになるので別途処理。
*/
...
void solve() {
    ...
    rep(_, M) {
        ll x, y;
        cin >> x >> y;
        ll i = bisect_left(B, x+y);
        ll ans1 = N;
        ll ans2 = acc.query(0, i) + (N-i-1)*(x+y);
        ll ans3 = min(x, A[0]-1);
        ll ans4 = min(y, L-A[N-1]);
