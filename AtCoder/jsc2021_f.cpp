/*
参考：https://atcoder.jp/contests/jsc2021/editorial
・自力ならず。。
　行と列でセグ木2本とかしようとしてて、
　そもそも値の方を乗せる発想に至らなかったため、完全に考察負け。
・座標圧縮、BIT2本持って個数と和を管理
・BIT2本持って個数と和を管理するやつ。座圧するし2次元だからちょいややこしい。
　とはいえ考察が整えば実装は結構すんなりだった。
・ある値が更新された時、影響を受けるのは1行か1列だから、
　そこがどう変化するか分かるようにデータを持つ。
　今回はその値以下ならその値、そうでないなら逆側が使われるので、
　値の個数と和を管理すれば、両方分かるようになって答えが分かる。
*/
...
void solve() {
    ...
    BIT<ll> bitcnta(L), bitsma(L), bitcntb(L), bitsmb(L);
    bitcnta.add(0, N);
    bitcntb.add(0, M);
    vector<ll> A(N), B(M);
    ll ans = 0;
    for (auto [t, x, y] : TXY) {
        ...
        if (t == 1) {
            bitcnta.add(A[x], -1);
            bitcnta.add(y, 1);
            bitsma.add(A[x], -cp.unzip(A[x]));
            bitsma.add(y, cp.unzip(y));

            ans -= bitcntb.query(0, A[x])*cp.unzip(A[x]) + bitsmb.query(A[x], L);
            ans += bitcntb.query(0, y)*cp.unzip(y) + bitsmb.query(y, L);
            print(ans);
            ...
            bitcntb.add(B[x], -1);
            bitcntb.add(y, 1);
            bitsmb.add(B[x], -cp.unzip(B[x]));
            bitsmb.add(y, cp.unzip(y));

            ans -= bitcnta.query(0, B[x])*cp.unzip(B[x]) + bitsma.query(B[x], L);
            ans += bitcnta.query(0, y)*cp.unzip(y) + bitsma.query(y, L);
            print(ans);
