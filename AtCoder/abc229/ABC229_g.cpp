/*
参考：https://twitter.com/kyopro_friends/status/1464593018451750919/photo/1
　　　https://note.com/syamashi/n/n233de5d9bbfc
　　　https://www.youtube.com/watch?v=_EudxS2QDIs
・自力ならず。もうちょっとかと思ったら全然むずかった。
・二分探索、累積和の累積和、累積和応用、中央値
・まず、元の配列そのままだとやりづらくて、これを、
　「Yのある所だけにして、間にある.の数を値に持った配列」みたいなやつにする。
　これだとindexが1つ動くとYの数も1つ増減みたいになって都合がいい。
・この状態で、スワップしてYを集めてくる(.を外側に追い出す)みたいな操作は、
　.の数の累積和の累積和みたいのを作っておくと求まる。
　ただしこのまま区間和を取るだけではダメで、手前から累積してきた余計な分を差し引く必要がある。
　つまり下の方の長方形部分を引いて、上に残った三角形部分だけ使うイメージ。これが右に抜ける分。
　左に抜ける分は向きが逆になるが、これは逆に長方形から累積和を引くと余事象みたいな
　いい感じの三角形が出てくる。(自分ノート図も参照)
　あとこの左右に抜く時起点にするのは中央値の位置。まあこれは自分でも分かったからいいけど。
・添字がぐちゃぐちゃになる。結局+1とか何もない綺麗な形にまとまったけど、
　これはすぬけさん解説動画のものをほぼ借用しているからそうなっただけ。。
　配列が持っているのは、その手前までの.の数なので、左端は余計だと思って+1してたのが間違い。
　実際は矩形の足し引きで左端を0位置にしちゃうから、ここは使わない位置でいい。
*/
...
void solve() {
    ...
    ll ans = bisearch_max(0, N+1, [&](ll x) {
        ll res = INF;
        rep(l, N-x+1) {
            ll r = l+x;
            ll m = (l+r)/2;
            ll lcnt = (m-l)*acc[m] - acc.query(l, m);
            ll rcnt = acc.query(m, r) - (r-m)*acc[m];
            chmin(res, lcnt+rcnt);
        }
        return res <= K;
    });
