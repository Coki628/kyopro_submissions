...
・ダイクストラ応用、三分探索
・コスト計算がちょっと特殊なダイクストラ。
　最初にぶたんで落ちて、三分探索+周辺100でWAが3つまで減ったので周辺500調べたらTLギリギリAC。
・でもやっぱりこれは嘘解法っぽかったみたい。
　本当は実験したり微分したりすると、√Dらへんが良いってなるんだって。
　三分探索+周辺をやるにしても、floor外してからさぶたんした方が近い値になるっぽい。
・小数でやってみた。これだと探索100回で周辺は10探す、でACした。TLも余裕できたし。
*/
...
ll calc(ll t, ll c, ll d) {
    auto [i, val] = trisearch_min(t, INF, [&](ld m) {
        return (m-t)+d/(m+1);
    }, 100);
    ll offset = 10;
    ll mn = val;
    rep(m, max((ll)i-offset, t), min((ll)i+offset, INF)) {
        chmin(mn, (m-t)+d/(m+1));
    }
    return c+mn;
}
