/**
 * ・E8さん精選100+50問埋め：逆元を使う問題
 * ・なんとか自力AC！本番解けなくて悔しかったのを覚えてた問題。
 * ・nCr系数え上げ、二分探索
 * ・なんとなく方針は覚えてたものの、やはり一筋縄ではいかない。
 * 　とりあえず自明にNGな2パターンを場合分け。ここまではいい。
 * 　(x+2,y+1)と(x+1,y+2)をどう振り分けるか、これは考えた。
 * 　なんか式変形とかできそうだなーとも思ったけど、単調姓はあるとみなして二分探索した。
 * 　「x側に+2をm回やった時、そのx側の値がx以下になる最大回数」みたいな感じ。
 * 　最初にNGケースを弾いてあるので、ぴったり来る場所があるのは確定。
 * 　ここで出た回数をx+2にする回数として、nCrやればOK。
 * ・過去のやつ見た。やっぱり式変形だったね…。まあ、通せれば正義。
 */
...
int main() {
    ...
    ll N = (x+y)/3;
    ll res = bisearch_max(0, N, [&](ll m) { return m*2+(N-m) <= x; });
    ModTools mt(MOD);
    mt.build(N);
    auto ans = mt.nCr(N, res);
    print(ans);
    return 0;
}
