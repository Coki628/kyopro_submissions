/*
参考：https://blog.hamayanhamayan.com/entry/2021/04/03/160910
・セグ木でDP高速化、式変形
・公式解は頭いいダイクストラに帰着してたけど、このはまやんさんの解法は自分のにまだ近い。
　基本方針はだいたい同じだけど、式変形することでセグ木での区間最大値の遷移を可能にしてる。
・DPの遷移は max(dp[l]+acc.query(l, r)) だけど、これは実際は
　max(dp[l]+acc[r]-acc[l]) に分解できて、rが固定なので、
　max(dp[l]-acc[l])+acc[r] r部分を外側に出せる。
　これならmax内の添字が1つになったので、セグ木で値を持っておけば
　区間最大値取得として高速に処理できる。
・これが出れば平方分割頑張るより実装も全然軽いし良かったんだけどなぁ。
*/
...
void solve() {
    ...
    rep(r, 1, N+1) {
        ...
        for (auto [l, c] : adj[r]) {
            ...
            // }
            chmax(dp[r], seg.query(l, r)+acc.acc[r]-c);
        }
        seg.update(r, dp[r]-acc.acc[r]);
    }
    ll ans = dp[N];
    print(ans);
}
