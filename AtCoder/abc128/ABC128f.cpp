...
　　　https://www.youtube.com/watch?v=m-Nov1EvGoc&t=6294s
　　　https://qiita.com/vain0x/items/e81a76a7b7c27f17654b
　　　https://kyopro.hateblo.jp/entry/2019/05/27/173455
・ABC高難度埋め
・むずい。
・難しい式変形を要するDP、調和級数
・まず、題意の移動を繰り返すと、a-bという形は常に出てくるのでこれをcとする。
　すると、0,a,c,c+a,2c,2c+a...みたいになってだいぶ見通しが良くなる。
　さらに終点がN-1であることから、このcをいくつか進めた kc+a = N-1 という式も立ち、
　これを式変形すれば a = N-1-kc となる。偶数番目は kc なので、
　indexでは N-1-kc と kc を交互に踏むようなジグザグな遷移をすると分かる。
　さらにcを固定してkを1つずつ進めれば、前の結果に追加で2マス増える感じなので
　前の結果を使いまわして値を確認できる。
・これらを踏まえると、外ループでcを固定して内ループでkを進めていけば
　dp[c][k]の値を網羅できる。この時kもcもNくらいまでの値を取りうるが、
　cが大きくなればkは大きくならないみたいな形をしているので、
　結果的に2重ループがNlogNくらいで回るので間に合う。(調和級数)
・基本的にkとcを動かしていくが、実際のa,bの状態も気にする。
　普通に進めるとbが負数になったりするのでそれは式変形してチェックして弾く。
・今回は訪問済のマスを踏めないルールもあるので、それは内ループで
　setに訪問済管理させて重複が出たところで切ればOK。
・式変形むずいし、非自明な感じだし、コーナーもあるしで、
　到底まだこれを自力で詰めれる気がしない。橙diff強い。。
*/
...
void solve() {
    ...
    rep(c, 1, N) {
        ...
        rep(k, 1, N) {
            ...
            ll a = N-1-k*c;
            ll b = a-c;
            // bが負数になるとbでも前に進むみたいなっておかしくなるのでダメ
            if (b <= 0) break;
            // 訪問済を踏むような移動は不可
            if (a == k*c or se.count(a) or se.count(k*c)) break;
            se.insert(a), se.insert(k*c);
            dp[k] = dp[k-1]+A[k*c]+A[a];
            chmax(ans, dp[k]);
        }
    }
    print(ans);
