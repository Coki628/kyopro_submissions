/*
参考：https://zenn.dev/tsugitta/articles/atcoder-agc-051
・AGC青diff以下残埋め
・自力ならず。まあこれも数学色強すぎるのでしょうがない。。
・正12角形、数え上げ
・分かりやすい解説をしてくれてる人がいて良かった。
　外側から決めていくとすると、実はある頂点で三角形と四角形をどっち向きにおくかの2択を決めると、
　その外周の配置は一意になる。で、その時三角形に面していた方の辺は長さが1短くなって、
　1個内側がまた外周と同じような動きをしてこれが再帰的に繰り返されていく。
　式で表すと、辺の長さが交互にa,bだとして f(a, b) = f(a-1, b) + f(a, b-1) となる。
　これをさらに観察(てか多分実験かね)すると、二項係数かなって感じの値が出てくる。
　実際、f(a, b) = f(a+b, a) となり、これはnCrで一発で解ける。
*/
