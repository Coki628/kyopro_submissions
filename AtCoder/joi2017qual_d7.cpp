 ...
 * ・きっちり自力AC！
 * ・bitDP、累積和で前計算
 * ・これは最初に解いた時難しくて解けなかったのよく覚えてる。
 * ・同じ種類のぬいぐるみは連続する必要があるので、並べ方はM!になる。
 * 　この順列全探索をbitDPに落とす。
 * ・「今置き終わった種類」の集合があると、各種類の個数を前計算しておけば、次を置き始める位置が分かる。
 * 　さらに累積和で区間内に現れる各種類の個数を前計算しておけば、今回移動の必要がない個数も分かる。
 * 　これで今回置く時のコストが分かるのでbitDPを進められる。
 * ・過去の記録を見ると、どうやら2回目の挑戦時にも自力通せなかったらしい。やっとできたね。
 */
