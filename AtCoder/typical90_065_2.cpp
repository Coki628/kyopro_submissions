/*
参考：https://github.com/E869120/kyopro_educational_90/blob/main/editorial/065-02.jpg
　　　https://github.com/E869120/kyopro_educational_90/blob/main/editorial/065-03.jpg
・E8さん典型90問埋め
・余事象、FFT
・2種類の玉に条件が課せられている所を余事象で1つの玉の条件に読み替える。
・するとそれぞれの玉がその個数で使えるかどうかが独立になるので、
　nCrで個数毎に前計算しておける。
・これなら2種類の玉について、全ペアの掛け合わせにできるので、
　FFTに突っ込めば高速に計算できる。
*/
...
void solve() {
    ...
    ModTools mt(MOD, max({R, G, B, K}));
    // mint ans = 0;
    // rep(r, min(R, K)+1) {
    //     rep(g, min(G, K)+1) {
    //         ll b = K-r-g;
    //         if (b > min(B, K)) continue;
    //         if (b < 0) break;
    //         if (b >= K-X and r >= K-Y and g >= K-Z) {
    //             ans += mt.nCr(R, r)*mt.nCr(G, g)*mt.nCr(B, b);
    //         }
    //     }
    // }
    // print(ans);

    // R,Gについて個数毎の通り数を前計算しておく
    vector<int> C1(K+1), C2(K+1);
    rep(r, K-Y, min(R, K)+1) {
        C1[r] = mt.nCr(R, r);
    }
    rep(g, K-Z, min(G, K)+1) {
    ...
    // FFTで全ペアの通り数にする
    auto fft = convolution<MOD>(C1, C2);

    mint ans = 0;
    rep(b, K-X, min(B, K)+1) {
        if (K-b >= fft.size()) continue;
        // R,Gの結果をBと掛け合わせる
        ans += mt.nCr(B, b)*(mint)fft[K-b];
    }
    print(ans);
}
