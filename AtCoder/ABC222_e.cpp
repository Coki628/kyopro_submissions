/*
・きっちり自力AC！これは結構速かった。
・HLD、区間加算BIT、辺属性、mapで部分和DP
・制約小さいからHLD要らなかったけど、DFSとかで毎回パス取ってくるのもだるそうだったのでこれで。
・これで寄与が分かれば、それぞれをどっちの色に振り分けるか、なので、これは部分和DPできる。
　ちゃんと式変形なりすれば正数の範囲でやれそうだったけど、考える時間ももったいないので、mapで負数ありのままやった。
*/
...
void solve() {
    ...
    BIT2<ll> bit(N);
    rep(i, M-1) {
        hld.add(A[i], A[i+1], [&](ll a, ll b) { bit.add(a, b, 1); }, true);
    }
