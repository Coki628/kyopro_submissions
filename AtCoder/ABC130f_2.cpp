...
・2倍して正数でいけるようにして無事AC。でもすぐじゃなくて、色々あった。
　if (t < INF) だと、INFよりちょい小さいみたいになったやつを拾ってしまってバグるぽい。
　INF/2にしたら通った。
・なんていうか、こういうのは到底自力で通せるようになるビジョンが見えないよね。。
　考察見えないもん。。
*/
...
void solve() {
    ...
    ll x_static_mn = INF, x_static_mx = -INF, y_static_mn = INF, y_static_mx = -INF;
    ll x_toleft_mn = INF, x_toleft_mx = -INF, y_todown_mn = INF, y_todown_mx = -INF;
    ll x_toright_mn = INF, x_toright_mx = -INF, y_toup_mn = INF, y_toup_mx = -INF;
    rep(i, N) {
        ll x, y;
        char d;
        cin >> x >> y >> d;
    ...
    ld ans = INF;
    for (ll t : T) {
        chmin(ans, (get_x_mx(t)-get_x_mn(t))*(get_y_mx(t)-get_y_mn(t))/(ld)4);
    }
    print(ans);
}
