/*
参考：https://maspypy.com/%E6%95%B0%E5%AD%A6-n%E4%B9%97%E3%81%AE%E8%A8%88%E7%AE%97#toc8
・なんとか自力AC！いやー大変だった。
・整数論、フェルマーの小定理、冪乗の冪乗
・ググり力。maspyさんに感謝。上記の参考記事に、
　a^x≡1となるようなxを指数部のmod取りに使えばいいよってなことが書いてある。
　(本当はもっとすごい難しく書いてあるけど、要約すると多分そう)
　で、これはフェルマーよりmodが素数ならa^(mod-1)≡1とできるはずで、
　つまり指数部のmod取りにはmod-1を使えばいい。
　それとARC113_bから情報を得たんだけど、0乗だとおかしくなるので場合分けする。
　でもこれでもC++がなんかWAになったので、途中実験で使ってたpythonを書き直してそっちでまずAC。
・終了後、TLから情報得たけど、WAの原因はただのオーバーフローだった。。
　modpowのライブラリで最初にmod取りするようにしたら無事AC。
*/
...
void solve() {
    ...
    mint ans = pow(M, pow(K, N, MOD-1) != 0 ? pow(K, N, MOD-1) : MOD-1, MOD);
    print(ans);
}
