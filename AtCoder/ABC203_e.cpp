...
・後日自力AC！本番こっちやればよかった。。
・座圧BIT
・実は白ポーンが行ける列はそんなに多くなくて、元位置のN以外だと、
　黒ポーンがいるところにしか行けない。
　つまり最大でMくらいにしかならない。ここは全部見れる。
・で、黒ポーンがいないところは同じ列をひたすら進むだけなので考えなくてよく、
　各黒ポーンの位置について、1つ前の行で真上と斜め上が到達可だったかを見れば、
　その行で黒ポーンのいるマスに着けるかどうかを判定できる。
　これを上から順に見ていきたいので、黒ポーンを行毎にまとめてソートしておく。
・各列の到達可否の状態を座圧BITで持ったけど、公式がsetでやってて
　多分こっちのが楽そうなので後でやってみる。
*/
...
void solve() {
    ...
    for (auto& [x, li] : XY) {
        ...
        for (ll y : li) {
            ll cpy = cp[y];
            if (bit[cpy] == 1 and (cp.unzip(cpy-1) != y-1 or bit[cpy-1] == 0) and (cp.unzip(cpy+1) != y+1 or bit[cpy+1] == 0)) {
                rm.pb(cpy);
            }
            if (bit[cpy] == 0 and (cp.unzip(cpy-1) == y-1 and bit[cpy-1] == 1 or cp.unzip(cpy+1) == y+1 and bit[cpy+1] == 1)) {
                add.pb(cpy);
            }
        }
        for (ll cpy : rm) {
