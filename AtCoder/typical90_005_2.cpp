/*
参考：https://twitter.com/e869120/status/1378527948563615746/photo/3
　　　https://github.com/E869120/kyopro_educational_90/blob/main/sol/005-03.cpp
・E8さん典型90問、残りの高難度埋め
・ダブリング応用
・むずい。頑張ってコード読み解きながらどうなってるか理解を進めた。
・ダブリング的に2冪毎の情報を持っておいて、それを組み合わせて大きい数の場合を取得するんだけど、
　何が普通のダブリングと違うかということを確かめながらやった。
・まず今回のこれが何でできるかなんだけど、2冪先に対する全状態の遷移先が分かるからできる。(多分)
　普通のダブリングはこれが1つの値に対してやるのに対して、今回は行全部の状態を一気にやる感じ。
　内側で行全部の更新のためにK^2かかるけど、処理はlogN回くらいしかやらなくていいので間に合う。
・各2冪に対する遷移結果は前計算しておく。この時、10の2冪乗？みたいなやつが必要になるけど、
　これはさらに前計算しておく。多分これやらないとlogもう1個乗るからとても重い。
*/
...
void solve() {
    ...
    const ll MXLOG = 62;
    vector<ll> pow10(MXLOG);
    rep(i, MXLOG) {
        pow10[i] = pow(10, pow(2, i), K);
    }

    auto f = [&](const vector<mint> &A, const vector<mint> &B, ll k) {
        ll K = A.size();
        vector<mint> res(K);
        rep(i, K) {
            rep(j, K) {
                res[(i*pow10[k]+j)%K] += A[i]*B[j];
            }
        }
        return res;
    };

    auto dp = list2d<mint>(MXLOG, K, 0);
    rep(i, M) {
        dp[0][C[i]%K] += 1;
    }
    rep(i, MXLOG-1) {
        dp[i+1] = f(dp[i], dp[i], i);
    }

    auto db = get_doubling(dp, f);
    vector<mint> init(K);
    init[0] = 1;
    auto ans = db.get(init, N);
    print(ans[0]);
}
