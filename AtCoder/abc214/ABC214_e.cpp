/*
参考：https://www.youtube.com/watch?v=RCB7EZaddEQ&list=PLLeJZg4opYKYOl0X8hz1R94K5kNOUkIG7
・自力ならず。終了後、考えたけどダメだった。
・区間スケジューリングに帰着、座標圧縮
・自分では箱とボールでペアにするフローに見えたりで、
　前から貪欲で区間スケジューリングっていう発想には至らなかった。
・左端から見ていって、使える状態になったやつをpriqueに詰めて、
　右端が早いやつから貪欲に割り当てていく、っていう、
　やってみればほんと区間スケジューリングの基本みたいなやつだった。。
・でもそれが分かっても、区間全体が10憶で長いので、そこは工夫が要る。
　人によって区間setでやったりしてたけど、自分は座圧で次の左端までのサイズを使って処理した。
　公式解説はなんか賢い実装をしていた。。
・しばらくWAが取れなくて悩んだけど、最後に終わった後のキューの残チェックが必要だった。
*/
...
void solve() {
    ...
    rep(l, M-1) {
        ...
        ll sz = cp.unzip(l+1)-cp.unzip(l);
        while (sz and que.size()) {
            ll r = que.pop();
            if (r <= l) {
                No();
                return;
