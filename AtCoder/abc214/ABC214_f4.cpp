/*
・部分文字列DP応用、in-place DP、累積和同時進行DP
・ARC125_dをちょっといじると、文字種類数が多い場合でも対応できるものが作れる。
　結構微調整に苦労したけど無事AC。
・mapをvectorにして、BITパートを累積和にしてlogを取った。
*/
...
void solve() {
    ...
    // dp[i] := 最後にi番目を使った通り数
    vector<mint> dp(N+1), acc(N+2);
    dp[0] = 1, acc[1] = 1;
    rep(i, N) {
        // i == 0 で例外的に遷移させる分、1では遷移させない調整
        if (prv[i] == 1) prv[i]++;
        dp[i+1] = acc[(i == 0 ? i+1 : i)] - acc[(prv[i] != -1 ? prv[i]-1 : 0)];
        // dp.update(i+1, dp.query((prv[i] != -1 ? prv[i]-1 : 0), (i == 0 ? i+1 : i)));
        acc[i+2] = dp[i+1] + acc[i+1];
    }
    mint ans = acc[N+1] - acc[1];
