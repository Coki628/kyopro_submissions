...
・きっちり自力AC！
・順列、数え上げ、辞書順
・割と最近すげー悩んで通したABC202_dと同じ考え方で解ける。
　そこにその値が置かれた時、
　その数より辞書順で前なのが確定するパターンがいくつ増えるか、
　を前から順に見ていく。具体的には、
　(A[i]-cnt-1)*(N-i-1)! : まだ出ていない自分より小さい数 * 残りを自由に置く
　みたいになる。
・自分より小さい値がいくつ出現済かを管理する必要があるので、
　それは転倒数の要領でBITでやる。
*/
...
void solve() {
    ...
    mint ans = 0;
    rep(i, N) {
        ll cnt = bit.query(0, A[i]);
        ans += mt.fact[N-i-1]*(A[i]-cnt-1);
        bit.add(A[i], 1);
    }
    ans += 1;
    print(ans);
