/*
参考：https://www.ioi-jp.org/joig/2020/tasks/2021-joig-t6-review.pdf
・自力ならず。。悔しいね。
・前計算、式変形、尺取り法、座圧
・前計算の方針は合ってたけど、式変形で横幅を固定できるのが見えなかった。ほんと式変形弱い。。
　で、その後の尺取りパートも、なんかイメージできなくて大変だった。
　index保持してsetに出し入れとか考えたけどそれは要らなくて、
　右端が何個入ったか、左端が何個出たか、が分かればいい。
　どの長方形の左端かとかは区別つかなくなっても今回の場合問題ない。
　後はベースになる横幅より長い区間は邪魔なので予め省いておくと、
　尺取りパートがかなり簡潔に書けた。
・1000*1000ペアで約50万の縦に、横の尺取りで種類数256が掛かるので約1億3000万、
　この計算量は制約1秒もあってかなりきつい。なんとかAC0.94秒。
・公式解の実装みたく座圧を入れた。これだと縦の1000*1000/2が256^2=65000くらいになるのでだいぶ違う。
　閉区間と半開区間の兼ね合いが座圧の時ガチャガチャして面倒だったけど。AC0.42秒。
*/
...
void solve() {
    ...
    vector<ll> L(M, INF), R(M, -INF), U(M, INF), D(M, -INF);
    rep(i, H) {
        rep(j, W) {
            chmin(L[A[i][j]], j);
            chmax(R[A[i][j]], j);
    ...
    Compress<ll> cpu(U), cpd(D);
    U = cpu.zip(U), D = cpd.zip(D);

    ll ans = INF;
    for (ll uh : U) {
        for (ll dh : D) {
            ll hlen = cpd.unzip(dh)-cpu.unzip(uh)+1;
            if (hlen <= 0) continue;
            ll wlen = S/hlen;
            vector<ll> vl, vr;
            rep(i, M) {
