/*
参考：https://atcoder.jp/contests/abc229/editorial
・自力ならず。。
・二部グラフのDP、円環のDP
・これは全然DPの方針に行けなかったな。。こういうグラフのDPもアリか。
・基本方針は、ある頂点を二部のどっちに置くかを状態に持って決めていく感じ。
　ただし今回は円環なのもあって、最後に頂点Nと1の関係性をやる必要がある。
　また、頂点0と繋ぐ辺と、隣接頂点を繋ぐ辺でコストを処理する時微妙に添字がずれるので、
　揃えてやるとうまくいかない。公式の実装見て読み取ったけど、
　自分でDP方針に行っててもこれが詰められなったように思う。。
*/
...
void solve() {
    ...
    rep(i, 1, N) {
        rep(j, 2) {
            rep(k, 2) {
                rep(nxt, 2) {
                    chmin(dp[i][j][nxt], dp[i-1][j][k] + (nxt == k ? B[i-1] : 0) + (nxt == 0 ? A[i] : 0));
                }
            }
        }
    }
    ll ans = INF;
    rep(j, 2) rep(k, 2) chmin(ans, dp[N-1][j][k] + (j == k ? B[N-1] : 0));
    print(ans);
}
