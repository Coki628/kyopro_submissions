...
"""
参考：https://www.ioi-jp.org/camp/2013/2013-sp-tasks/2013-sp-day1-joi_poster-review.pdf
・自力ならず。方針は合ってたけど誤差に負けて解説とかテストケースとか色々見た。
・幾何、円と円の交点、誤差
・ただ誤差との壮絶な戦いだった。そして負けた。
"""
...
for i in range(N):
    ...
    for j in range(N):
        if i == j:
            continue
        x2, y2 = XY[j]
        r1 = hypot(x1-x2, y1-y2)
        # はみ出る
        if (x1 - r1 < 0 or x1 + r1 > W) \
                or (y1 - r1 < 0 or y1 + r1 > H):
            continue
        for k in range(N):
            if i == k or j == k:
                continue
            ...
            for l in range(N):
                if i == l or j == l or k == l:
                    continue
                x4, y4 = XY[l]
                r2 = hypot(x3-x4, y3-y4)
                # はみ出る
                if (x3 - r2 < 0 or x3 + r2 > W) \
                        or (y3 - r2 < 0 or y3 + r2 > H):
                    continue
                # 2円間が遠い
                if not gm.intersectCC((x1, y1, r1), (x3, y3, r2)):
                    continue
                # 2円間の交点
                if r1 < r2:
                    # 小さい方の円を少し大きくして、誤差により誤って内包判定されるのを防ぐ
                    res = gm.get_cross_pointCC((x1, y1, r1+gm.EPS), (x3, y3, r2))
                else:
                    res = gm.get_cross_pointCC((x1, y1, r1), (x3, y3, r2+gm.EPS))
                # 遠い以外で交点がなかったら、どちらかが内包している
                if res == -1:
                    ans += 1
print(ans // 2)
