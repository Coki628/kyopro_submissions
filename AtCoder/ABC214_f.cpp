...
・終了後upsolveした。自力ならず。。
・部分文字列DP
・自分で考察した時は、通り数ならともかく種類数は重複しまくりでDPで数えられなそうだし、
　包除とか二項係数でゴニョゴニョやるなら分が悪いなって捨てちゃったんだよね。。
　その種類数を重複なく数え上げるDPこそがこの部分文字列DPだった。
　各文字種類について、次に出現する最短indexだけを取ることで重複をなくせるって考え方は、
　多分この記事読んだ当時は言われてもピンと来なかったんだよね。
　今なら多少は納得できる。(abc171_fの考え方を思い出した。)
・これが思い出せればもうほぼそのままで、追加の条件として、
　2文字目以降は隣り合うものを取らないように遷移先取得の左端を+1するってのをやるくらい。
・前計算配列作るのサボってBIT26本生やしてにぶたんしたけどちゃんとAC。
　でも20万のNにしては遅くて、AC1.19秒。BITをllからintにしたら1.06秒。
　BITにぶたんはlog2つ乗っても相当速い印象なんだけど、MOD数え上げだから重いんかな？
　って思ったけど、文字種類数の26もかかってるから実質log3つみたいなもんか。
*/
...
void solve() {
    ...
    rep(i, N) {
        rep(c, M) {
            ll j = bit[c].bisearch_fore(i+(i > 0), N-1, 1);
            if (j != -1) {
                dp[j+1] += dp[i];
            }
        }
