/*
参考：https://atcoder.jp/contests/arc124/editorial
・公式解。結構方針違って、これも大事な考察典型かと思えたのでやっておく。
・候補を絞って全探索
・さっきのBとだいぶ似た話になるが、最初の要素A[0],B[0]を必ず使うので、
　最終的なGCDはこれらの約数以外にはなり得ない。このことを使って、約数全探索する。
・判定パートは、各要素が決め打ちしたGCDのどちらかで割り切れるかどうかを2パターン試して、
　両方ともダメならNG、といったものをN個進めていく。
*/
...
void solve() {
    ...
    auto divs1 = divisors(AB[0].first);
    auto divs2 = divisors(AB[0].second);
    ll ans = 1;
    for (auto d1 : divs1) {
        for (auto d2 : divs2) {
            bool ok = 1;
            for (auto [a, b] : AB) {
                if (not (a%d1 == 0 and b%d2 == 0 or b%d1 == 0 and a%d2 == 0)) {
                    ok = 0;
                    break;
                }
            }
