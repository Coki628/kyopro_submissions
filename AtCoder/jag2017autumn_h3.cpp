 ...
 * 　　　https://atcoder.jp/contests/jag2017autumn/submissions/17666912
 * ・蟻本演習4-7-3
 * ・文字列検索、Aho-Corasick法
 * ・公式解説ではアホコラやるだけくらいの話だったので分からず、人様の提出を見て使い方を理解。
 * 　アホコラ上の頂点を1文字ずつ移動していくと、その位置でちょうど終わる単語が全部分かるのでこれを使う。
 * 　普通のTrie木だと行き止まったらそこで終了だけど、
 * 　アホコラは一番都合いい位置にワープしてくれるからね。ほんとに賢い。
 */
...
int main() {
    ...
    ll v = 0;
    rep(i, 1, M+1) {
        // i文字目の手前まで終わった時のアホコラ上での位置v
        v = aho.move(T[i-1], v).second;
        // この位置で終わる単語だけを遷移の対象にする
        for (auto id : aho.nodes[v].accept) {
            ll j = sz[id];
            dp[i] += dp[i-j];
            dp[i] %= MOD;
        }
