/**
 * ・きっちり自力AC！これも方針はすんなり整った。
 * ・mex、セグ木にぶたん
 * ・ある区間のmexを高速に求めたい。区間は1つずつずれていくので、値の変化は1つ減って1つ増えるだけ。
 * 　これをセグ木で個数管理して、左から最初に個数0が出現する場所はにぶたんで探せる。
 */
...
void solve() {
    ...
    SegTree<ll> seg(N+1, [](ll a, ll b) { return min(a, b); }, INF);
    ...
    ll ans = INF;
    rep(i, N-K+1) {
        chmin(ans, seg.bisearch_fore(0, N, 0, less_equal<ll>()));
        seg.update(A[i], seg[A[i]]-1);
        if (i+K < N) seg.update(A[i+K], seg[A[i+K]]+1);
    }
    print(ans);
}
