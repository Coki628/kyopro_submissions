/*
参考：https://twitter.com/SSRS_cp/status/1429337813812617216
・自力ならず。。累積和取って、差分にして、とかやったけどうまくいかず。。
・正解は累積和取ってソート。公式解説のシグマの式は心が折れる。。
　それが累積和なんだとさえ書いてくれてない。数式で察せってことなんだろうけど。。
・数列ペアの絶対値がソートすると外れるみたいな話は、いつかのABCのCあたりでもあった気がする。
　なんだけど、累積和って順序が関係するものだと思うし、ソートしちゃって大丈夫なの？って思った。
　でもよくよく考察すると、今回この累積和は累積和で区間和とかが使いたい言うよりは、
　右端-左端ペアで考えた時に絶対値を外したい訳で、そのために左端が常に小さければ、
　絶対値つける必要なくなるでしょ？って感じっぽい。それで位置が入れ替わっていても、
　結局全ペア見た合計が欲しいんだから、どう足しても最終的に同じでしょ？ってなる。
・絶対値が取れてしまえば、あとは各位置が左端と右端で使われる回数が寄与で使えるから線形で解ける。
・あとWA出したんだけど、オーバーフローが気付きにくい。
　普段ならそもそも最初からmintでやっちゃうんだけど、後でソートする都合上最初はllでやることになるし、
　数列20万で最大値10^9だと最大10^14くらいにしかならないから大丈夫かなー、って思ったら、
　累積和取るからこの時点で個々の値が10^14くらいまでありえて、
　これを*Nしてるから確かに10^19超くらいになってた。。
*/
...
void solve() {
    ...
    mint ans = 0;
    rep(i, N+1) {
        ans += (mint)acc[i]*i - (mint)acc[i]*(N-i);
    }
    print(ans);
}
