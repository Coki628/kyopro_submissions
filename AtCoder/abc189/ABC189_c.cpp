/**
 * ・自力AC！危ない。。
 * ・N^2乗ループの内側で区間minが取れればいいんだけど、制約からN^2=1億になるのできつい。
 * 　logが乗ると無理な感じなのでセグ木の代わりにSparse Tableを持ってくると、無事AC0.17秒。
 * 　試しにセグ木で投げると、やっぱりTLEしたので、Sparse Tableちゃんと持っててよかった。。
 * ・でも普通に右端伸ばしながら累積min更新していけばよかったんだよね。
 * ・もっと言えば、これ最大長方形そのものだからスタック使ってO(N)もできるんだね。。
 */
...
int main() {
    ...
    SparseTable st(A, [](ll a, ll b) { return min(a, b); });
    ll ans = 0;
    rep(i, 0, N) {
        rep(j, i+1, N+1) {
            ll mn = st.query(i, j);
