 ...
 * ・自力ならず…。これは悔しい。
 * ・グリッド、再帰全探索
 * ・3^Nの再帰全探索は考えてたのに適切に詰められなかった。
 * 　なんか先に3^N全列挙して、ここは半畳、ここは縦、ここは横、
 * 　みたいにやろうとしたんだけど、既に置かれた場所どうしようとかなって詰んだ。
 * ・結局、考え方だいたい同じで、状態に一畳の使用個数持って、遷移できるなら遷移、
 * 　最後に数合ってたら答えに追加、みたいなやつ雑に作ったら通った。
 * 　前もって全状態列挙とかしたのがよくなくて、
 * 　その場その場で遷移できるように遷移させてったらうまくいったね…。
 */
...
void solve() {
    ...
    auto rec = [&](auto&& f, vvi &grid, ll acnt, ll i) -> void {
        if (i == N) {
            // 条件に合うように最後まで行ったら答えにカウントする
            if (acnt == 0) ans++;
            return;
        }
        ...
        if (grid[h][w]) {
            ...
            if (acnt) {
                // 一畳を縦に置く
                if (h+1 < H and grid[h+1][w] == 0) {
                    grid[h+1][w] = 1;
                    f(f, grid, acnt-1, i+1);
                    grid[h+1][w] = 0;
                }
                // 一畳を横に置く
                if (w+1 < W and grid[h][w+1] == 0) {
                    grid[h][w+1] = 1;
                    f(f, grid, acnt-1, i+1);
                    grid[h][w+1] = 0;
                }
