/*
・きっちり自力AC！
・ソート、にぶたん、Wavelet Matrix
・ぶん殴った。Aについてソートしておけば、A[i]の位置より手前の区間で、
　区間[0,i)でB[i]以下の個数みたいので解ける。同値があるので添字境界がちとめんどい。
・普通はBITに座圧したBの情報を入れ込んでくやつっぽい。
　そっちでやるとやっぱり同値があるので境界処理が面倒そうって感じ。。
*/
...
void solve() {
    ...
    ll ans = 0;
    rep(i, N) {
        ll j = bisect_right(A, A[i]);
        ans += wm.range_freq(0, j, B[i], INF);
    }
    print(ans);
}
