...
・マラソン精進
・とりあえずこれは初期解。
　やり方は、マス数の最短を縦移動と横移動だけランダムで辿るだけ。
　結構この初期解出すための実装が重め。まあ長期マラソンだしね。
　インタラクティブ風で、ローカルと提出で挙動変える必要のある個所が結構ある。
・Rustはもう導入済みなので、テストケース作成はすぐできたけど、
　ファイル読み込みパートを作ってたら、色々文字化けとかにはまった。
　なんか const char* がstringじゃないから結構クセあるっぽい。
・とりあえずこれを叩き台に改善方針を考えていく。今スコア614億くらい。
*/
