/*
参考：https://scrapbox.io/procon-kirokuyou/ARC125_C_-_LIS_to_Original_Sequence_(600)
　　　https://atcoder.jp/contests/arc125/editorial
・ARC青diff残埋め
・自力ならず。
・LIS、辞書順最小
・難しい。しかもググっても解説記事が出なくて情報が少ない。。
・とりあえずA[i]を置いて、A[i]未満のものを降順において、A[i+1]を置いて、を繰り返して、
　ってやってA[K-1]より大きいのだけA[K-1]の手前に降順で置く、ってやるとLISの条件は達成できる。(と思う)
　でもこれだと辞書順最小にはならない。
・で、どうすればいいかと言うと、A[i]の直後にはA[i]未満でまだ使っていないもののうち、一番小さいものだけ置く。
　これを繰り返して、余ったやつは最後に降順に全部置く。
　だいたいこれでOKなんだけど、最後のA[K-1]をやる時だけは、小さいのを後ろに置いてはいけない。
　やると最後に余ったやつ置いていく時にLISが増えるから。
・いやー、こういうのも無理だね！笑
*/
...
void solve() {
    ...
    // K == 1 だとA[i]の直後に1つ置くのがそもそもアウトなので場合分け
    if (K == 1) {
        vector<ll> ans(N);
        iota(ALL(ans), 1);
        reverse(ALL(ans));
    ...
    // 数列Aに含まれない値はsetで使用済管理する
    my_set<ll> se;
    rep(i, 1, N+1) se.insert(i);
    rep(i, K) se.erase(A[i]);
    vector<ll> ans;
    rep(i, K) {
        // A[K-1]より大きい値は、A[K-1]の直前に降順で置く
        // (辞書順的には末尾に置きたいけどそれだとLISが増えるのでここがベスト)
        if (i == K-1) {
            rep(j, N, A[K-1], -1) {
                se.erase(j);
                ans.eb(j);
        ...
        ans.eb(A[i]);
        // 辞書順最小を目指すため、A[i]より小さい残る要素から一番小さいもの1つだけここに置く
        // ※最後にこれをやると残りを降順で置く時にLISが増えるので最後はやらない
        if (i != K-1 and se.size() and se.front() < A[i]) {
            ans.eb(se.pop_front());
        }
    }
    // 残り全部(A[K-1]より小さい)をLISを増やさないように降順で置く
