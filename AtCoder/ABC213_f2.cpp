...
・stoqさんのBeatsが速そうだったので拝借。1秒以上早くなった！AC1.72秒！
　100万のNにlog(2つ？)乗るのにこれはいい。かなり殴り力高そう。
*/
...
void solve() {
    ...
    SegmentTreeBeats seg(lcp);
    vector<ll> ans(N);
    rep(i, 1, N) {
        seg.update_min(0, i, lcp[i-1]);
        ans[sa[i]] += seg.query_sum(0, i);
    }
    seg = SegmentTreeBeats(lcp);
    rep(i, N-2, -1, -1) {
        seg.update_min(i, N-1, lcp[i]);
        ans[sa[i]] += seg.query_sum(i, N-1);
    }
    rep(i, N) {
        ans[i] += N-i;
    }
