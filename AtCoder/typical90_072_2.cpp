/*
参考：https://github.com/NASU41/tenkei90/tree/main/solution/Q072
・E8さん典型90問埋め
・bitDP、グリッド
・自分もやろうとしたbitDP方針。
　これが詰めきれていれば、再帰全探索方針が生えなくても解けたのにね。。
・bitDPパートは自分でやろうとしてたのとだいたい同じ。
　ただし通った頂点数はpopcountで分かるので、最大値とかも持つ必要なくてboolでいい。
・始点に戻るパートでは、ある始点sから始まって、隣接するtならOKなので、
　sを決め打った後そこから4方向見てtとしてdp[bit][s][t]をチェックする。
・言われたら確かにー、ってなるのに、どうしてこれを自分で詰めれなかったかねー。。。
*/
...
void solve() {
    ...
    auto dp = list3d(N, N, 1LL<<N, false);
    rep(i, N) {
    ...
    rep(bit, 1LL<<N) {
        rep(s, N) {
            auto [h, w] = idtogrid(s, W);
            for (auto [dh, dw] : directions) {
                ll nh = h+dh;
                ll nw = w+dw;
                if (nh < 0 or nw < 0 or nh >= H or nw >= W) continue;
                if (grid[nh][nw] == '#') continue;
                ll t = gridtoid(nh, nw, W);
                if (popcount(bit) > 2 and dp[s][t][bit]) {
