/*
参考：https://img.atcoder.jp/abc156/editorial.pdf
・ABC高難度埋め
・むずい。考察全く歯が立たない。実装はまあやったことある感じの集まりなんだけどね。。
・余事象、MODの性質
・とりあえず各クエリで全部mod mしておく。これはやったけど、
　そんなに便利な性質につながるとは思わなかったな。
・Ai < Ai+1 を直接数えるのではなく、Ai = Ai+1 と Ai > Ai+1 を数える。
　Ai = Ai+1 はなんとなく数えやすそうだが、まあ実際そうで、
　mod取った差分の列をBとして、B[i] = 0 の場所はAが変化しないので該当する。
　N回中何回出てくるかは、K回中の合計を数えておいてdivmodでまとめる分と余り分を分けて数える。
・Ai < Ai+1 はmodの境界を越えて次の周期に入ったところが該当しそうというのは思ったけど、
　加算分が大きすぎると、1周通り越して現在位置より先に来てしまう場合があるので判別は無理に思えた。
　なんだけど、ここで最初にmod m取ったことが効いてきて、これをやっておくと、
　何周通り越してもそれは無視して現在位置からの増分を数えてくれるので、
　飛び越えた分は数えずにちゃんと Ai < Ai+1 になる場所だけを見つけられる。
　周期の境界を何回越えるかは、割り切れる回数の変化で確認できるので、
　r/x-(l-1)/x みたいな形でちょくちょく見る、区間内にxの倍数がいくつあるかみたいなやつで数える。
　なお左端は自明に初期値A[0]になるが、右端はさっきと同様にdivmodで数えられる。
*/
