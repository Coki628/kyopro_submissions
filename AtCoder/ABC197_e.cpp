/*
・自力AC！しょうもないケアレスミスしてなければもうちょい早かったんだけど。。
・DP。dp[i][j] := i色目まで見て位置jにいる時の最小値、でjをmapで持った。
・結局右か左に思いっきり動くが最善だと思い、それなら状態は少なそう、
　となり、座標位置をmapで持ってDPした。
・cin >> c; と c--; を前後逆に書いてるというしょうもないミスで時間が溶けた…。
*/
