/*
参考：https://atcoder.jp/contests/abc222/editorial
・頂点拡張、木の直径、ダイクストラ
・公式解の方針、やった。
・頂点拡張するのは自分の方針と同じだった。
　そこから、どの頂点から見ても、一番遠いのは木の直径のどっちかって話になる。確かにそうっぽい。
・今回の条件として自分の所の超頂点は使えないので、それが直径に絡んでる時は場合分け。
・少し前に作ったけどまだ実戦投入全然してなかったダイクストラのライブラリ使ってみた。
　こういう同じグラフで何回か最短距離取らないといけない時はかなり簡潔に書けて良さそう。
*/
...
void solve() {
    ...
    vector<vector<pair<int, ll>>> nodes(N*2);
    rep(i, N-1) {
        ll u, v, c;
        cin >> u >> v >> c;
        u--; v--;
    ...
    auto dk = get_dijkstra(nodes);

    auto dist0 = dk.solve(0);
    ll v1 = max_element(ALL(dist0)) - dist0.begin();
    auto dist1 = dk.solve(v1);
    ll v2 = max_element(ALL(dist1)) - dist1.begin();
    auto dist2 = dk.solve(v2);
