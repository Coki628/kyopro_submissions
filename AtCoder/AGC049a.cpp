/*
参考：https://atcoder.jp/contests/agc049/editorial
・AGC青diff以下残埋め
・自力ならず。これはSCCと見せかけてそうじゃない、みたいのは覚えてたけど、それでも解けず。。
・期待値の線形性、ワーシャルフロイド
・解説読むと確かにそうだよねーなんだけど、その方針に頭が向くには結構な考察力がないと。。
　操作回数の期待値なので「その頂点を操作する確率」を全頂点で総和したものと一致する。
　全頂点操作するとしたら、操作回数は確実にNになる訳で期待値もN、まあそうだろうとなる。
　今回は「その頂点に到達できる頂点」が操作されると頂点は消えてしまうので、この分少なくなる。
　例えばそんな頂点が(自分以外に)4個あれば、その4個が先に選ばれてしまえば
　自分が選ばれることは永遠にない訳で、自分が選ばれる確率は1/5になる。
　これを全頂点で見ていって総和すれば答え。実装はワーシャルフロイドが簡単。
*/
