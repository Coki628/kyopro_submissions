...
・E8さん典型90問埋め
・なんとか自力AC！危なかった。。
・約数列挙、状態を減らして全探索、オーバーフロー
・時間かかった。Ratedで出てたら泣いてた。
　素因数分解して3グループに分ける方法を数えようとしたら
　重複処理でどん詰まり。うまくいかず。
・結局、a,b,cの候補になる数はKの約数に限られるので、
　約数列挙してそこから全探索。
・思った以上にTL厳しくて、枝刈りしたり、重複判定setじゃなくてvector+sortにしたり、
　最後にオーバーフロー判定のキャストをldじゃなくてdoubleにしてなんとかAC、1.74秒。
・小数型へのキャストが重いのを疑って、オーバーフロー検知の関数を導入。(from えびちゃんの記事)
　でもあんま変わらなくて、AC1.65秒。
・追記。解説見た。10^12以下で多い所だと約数6000個くらいあるそうで、
　それじゃ3重ループきつい訳だ。普通無理。むしろよく枝刈りで通ったねって感じ。
　お年玉方式で式変形すれば2重ループで3つ目は一意に定まる。
　言われると確かにそうじゃん、って感じ。約数に絞り込んで安心して、
　それができるって見方をすっかり見落としてた。ダメだねー。。
*/
...
void solve() {
    ...
    rep(i, N) {
        ...
        rep(j, i, N) {
            ll b = divs[j];
            if (mul_overflow(a, b) or a*b > K) break;
            rep(k, j, N) {
                ll c = divs[k];
                if (mul_overflow(a*b, c) or a*b*c > K) break;
                if (a*b*c == K) {
                    vector<ll> tmp = {a, b, c};
                    sort(ALL(tmp));
                    V.pb(tmp);
