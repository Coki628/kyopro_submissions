...
・求めるXを今回必要な勝ち数じゃなくて、今回までに必要な合計の勝ち数にしたら、式が簡単になった。
・式変形：今回までに必要な合計の勝ち数をXとして、
　そこまでの総勝ち数/そこまでの総ゲーム数 < X/(そこまでの総ゲーム数+今回のゲーム数)
　をXについて解く。
...
# 今回機嫌を良くする(勝率を上げる)ために必要な合計の勝利数を計算
...
for i in range(N):
    for j in range(N+1):
        ...
        if j <= N:
            # 今回機嫌を良くする遷移
            dp[i+1][j+1] = min(dp[i+1][j+1], calc(game_cnts[i], dp[i][j], A[i]))
