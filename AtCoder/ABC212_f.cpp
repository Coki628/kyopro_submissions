/*
・ABC高難度埋め
・自力AC！！黄diff上位通せたのは嬉しいけど、2時間くらいかかってるので本番じゃあまだまだ…。
・クエリ先読み、イベントソート、UnionFind
・状況の変化が複雑だが、イベントソートしてクエリ先読みでいけそうな雰囲気。実際それでいけた。
　基本的には状況を時系列で前からシミュしていって、クエリ回答の時点が来たら答える、で進める。
・ただしこれだけだと、全クエリで10万回バスに乗るみたいな時に2乗になってダメ。
　そこで、同じ時間・場所に合流したクエリはその後答えの変化が同じなので、
　UnionFindを使って同一視していいクエリをまとめながら進めていく。
・最後に気を付けるのが同時刻のイベントの処理順で、今回は例えば、
　時刻3にクエリ開始→3.5のバスに乗る、とか
　時刻3.5にバス到着→3.5のバスに乗る、とかができるので、これらができるような順番で並べる。
・座圧して隣接リストにしちゃったけど、今回はそのままでいけたっぽいな。。
*/
...
void solve() {
    ...
    T = cp.zip(T);
    ll L = cp.size();
    vector<vector<tuple<ll, ll, ll, ll>>> events(L);
    rep(i, M) {
        auto [u, v, s, t] = bus[i];
        events[cp[s]].eb(3, u, v, i);
        events[cp[t]].eb(2, u, v, i);
    }
    rep(i, Q) {
        auto [x, y, z] = qs[i];
        events[cp[x]].eb(0, y, -1, i);
        events[cp[z]].eb(1, -1, -1, i);
    }
    rep(t, L) sort(ALL(events[t]));

    vector<ll> anscity(Q), stay(N, -1), ride(M, -1);
    vector<pll> ansbus(Q);
    UnionFind uf(Q);
    vector<string> ans(Q);
    rep(t, L) {
        for (auto [op, a, b, i] : events[t]) {
            // 人の出発
            if (op == 0) {
                ll q = i;
                if (stay[a] != -1) {
            ...
            // クエリ回答
                ...
                ll q = uf.find(i);
                if (anscity[q] != -1) {
                    ans[i] = tostr(anscity[q]+1);
                } else {
                    ans[i] = tostr(ansbus[q].first+1) + " " + tostr(ansbus[q].second+1);
                }
            // バスの到着
            } elif (op == 2) {
                if (ride[i] != -1) {
