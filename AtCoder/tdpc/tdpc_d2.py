...
　みたいな考察部分のが大事な気もする。
・添字を普通に4次元リスト管理にした版。こっちのが安定して速かった。python0.3秒pypy0.4秒でAC。
"""
...
dp = list4d(N+1, need2+1, need3+1, need5+1, 0)
# 初期化：0回振って全部0の確率は100%(そりゃそうだ)
dp[0][0][0][0] = 1
for i in range(N):
    for j in range(need2+1):
        for k in range(need3+1):
            for l in range(need5+1):
                # 今回の目が1〜6の場合をそれぞれ遷移(必要数をオーバーした分は同一とみなす)
                nxt = dp[i][j][k][l] / 6
                dp[i+1][j][k][l] += nxt
                dp[i+1][min(need2, j+1)][k][l] += nxt
                dp[i+1][j][min(need3, k+1)][l] += nxt
                dp[i+1][min(need2, j+2)][k][l] += nxt
                dp[i+1][j][k][min(need5, l+1)] += nxt
                dp[i+1][min(need2, j+1)][min(need3, k+1)][l] += nxt
