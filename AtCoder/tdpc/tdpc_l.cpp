...
・自力AC！いい感じ！
・後ろから累積和で前計算してDP
・dp[i][j] := i番目まで見て、j番目の猫まで隣接させた時の最大値
　みたいなDPを作る。添字でN^2と遷移でNでN^3だとまだ間に合わないから高速化して、
　って思ったら通ってしまった。よく考えると、内側ループ2つがそれぞれ半分くらいしか回らず、
　多分この形は定数倍が/6とかしているので、10億/6=約1.7億くらいでC++だと
　全然間に合うのだろう。AC0.23秒くらいだった。
*/
...
void solve() {
    ...
    vvl acc = list2d(N, N, 0LL);
    rep(i, N) {
        rep(j, i-1, -1, -1) {
            acc[i][j] = acc[i][j+1]+F[i][j];
        }
    }

    auto dp = list2d(N+1, N, -INF);
    dp[0][0] = 0;
    rep(i, N) {
        rep(j, i+1) {
            rep(k, j+1) {
                chmax(dp[i+1][j], dp[i][k]+acc[i][j]*2);
            }
        }
    }
    ll ans = max(dp[N]);
