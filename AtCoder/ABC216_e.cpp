/*
・自力AC！これ通せてまだ良かった。けどこれ緑diff？
・二分探索、等差数列の和
・Kが20万くらいならやるだけなのに、10憶あってみんなが涙した。
　どう高速化するか考えていく。
・大きい方から順に進めていくとすると、1つ下の値まで下がった所で
　状態をそっちとまとめてしまえば、同じことを*2とかでやっていけばよくなる。
・K回に達しない間は、1ずつ下がっていくのを等差数列の和とみなしてまとめてやる。
　最後にK回の直前で止めて、そこだけ愚直に足してbreakする。
・口で言うのは簡単だけど、境界の実装めっちゃバグりそうだし、
　サンプル激弱だし、祈るように投げた。無事一発ACで良かった。。
*/
...
void solve() {
    ...
    rep(i, N) {
        ll cnt = A[i]-A[i+1];
        ll mx = bisearch_max(0, cnt+1, [&](ll m) {
            return m*(i+1) <= K;
        });
        ans += get_sum(A[i]-mx+1, 1, mx)*(i+1);
        K -= mx*(i+1);
        if (mx < cnt) {
            ans += K*(A[i]-mx);
            break;
