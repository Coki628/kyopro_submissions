/*
参考：https://blog.hamayanhamayan.com/entry/2019/01/12/160257
・自力ならず。悔しい。包除ほんと解けないね。。
・数え上げ、DPと包除原理
・結局、後から思いついた包除の方針のが、まだ座圧グリッドDPよりは近かった。
　それでも、時間かけたとして詰め切れた気はしないけどね。。
・壁に注目して壁毎の通り数を数えるでよかったけど、
　この時に「j個以上壁を通った」の状態を持つのがキーだった。
　確かにこの「○○以上」の形は包除原理によくハマる。
　そしてこれを求めるDPは、(○○以上部分を偶奇だけにすれば)状態Nの遷移NでN^2で間に合う。
　てか、HとWが10万なのに気を取られて、Nが2乗間に合う制約だってことを全然意識できてなかったので、良くないね。
　あと、各遷移は二項係数で増える通り数掛ける感じね。
　にしてもこの始点と終点もNに含めるとうまくいくのとかも、自分じゃ考えつかなそう。。
・さてDPで状態が最後まで集まったら、偶奇で足し引きすればOK。
　この時どっちがどっちかは、かなり強い人でもガチャガチャやるって聞くし、
　とりあえず難しく考えなくてもいいかも。
　今回は、1個壁を通った、だったらそれは実際は始点から終点に直行したやつなので、
　じゃあ通常とは逆で奇数が+かな、くらいの気持ちでいる。
*/
...
void solve() {
    ...
    // dp[i][j] := i番目の壁まで見て、j個(偶奇のみ)以上の壁を通った通り数
    auto dp = list2d<mint>(N, 2, 0);
    dp[0][0] = 1;
    rep(i, N) {
        auto [r1, c1] = RC[i];
        rep(j, 2) {
            rep(k, i+1, N) {
                ...
                ll r = r2-r1;
                ll c = c2-c1;
                dp[k][(j+1)%2] += dp[i][j]*mt.nCr(r+c, r);
            }
        }
    }
