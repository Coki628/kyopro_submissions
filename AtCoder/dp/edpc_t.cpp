...
　とすると、遷移の仕方は同じなのでこれらの状態を同一視できる。
　順列は大小関係さえ押さえておけば、そこから自ずと色々分かる、みたいなところある。
・後は状態の変化に従って遷移させて、貰うDPにして累積和で遷移高速化すればOK。
*/
...
void solve() {
    ...
    rep(i, N-1) {
        ...
        rep(j, N) {
            if (j > rest) break;
            if (S[i] == '<') {
                dp[i+1][j] = acc.query(0, j+1);
                // rep(k, j+1) {
                //     dp[i+1][j] += dp[i][k];
                // }
            } else {
                dp[i+1][j] = acc.query(j+1, rest+1);
                // rep(k, j+1, rest+1) {
                //     dp[i+1][j] += dp[i][k];
                // }
            }
