...
・やりきった。maspyさんに直接聞いちゃった。方針はだいたい合ってた。
　logに変換した時、係数が負数だと指数が奇数の時だけ正負が入れ替わるので、
　元々偶奇交互なのと合わさって結果的に全部負になる。
　これは自分じゃ気付けなかったな…。思ってた方針で通せて嬉しい。
*/
...
void solve() {
    ...
    res[0] = 1;
    // Π(1-x^(a+1)) => exp(Σ(log(1-x^(a+1))))
    rep(a, 1, K+2) {
        if (C[a] == 0) continue;
        ll k = 1;
        while (a*k <= K+1) {
            fps[a*k] -= mt.div(C[a], k);
            k++;
        }
    }
    res *= fps.exp(K+2);
