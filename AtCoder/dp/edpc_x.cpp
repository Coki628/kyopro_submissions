/*
参考：https://blog.hamayanhamayan.com/entry/2019/01/12/154729
　　　https://ikatakos.com/pot/programming_algorithm/contest_history/atcoder/2019/0106_educational_dp_4
・自力ならず。ソートの条件作れなかった。。
・ソートしてナップザックDP、難しいソートしてDP、式変形
・ソートの条件作るんだろうとは分かったものの、作れず。。
・本当はw+sだけでいいみたいだけど、自分でも導きやすそうな方針で解いておいた。
・今の和がxで、a,bの順に置く場合とb,aの順に置く場合の条件をそれぞれ式にしてみる。
　すると、x <= as and x+aw <= bs とその逆になって、これを式変形して1本にすると、
　x <= min(as, bs-aw) (とその逆)にできる。大きい方がx以上となるために明らかに有利なので、
　min(as, bs-aw) > min(bs, as-bw) とすれば、これをソートの条件に使える。
・後は普通にナップザック。強いて言えば制約が大きくてまともに2次元持つと2億くらいあってメモリが死ぬので、
　配列再利用の節約テクは必要になる。速度は2億回しても0.05秒で爆速だった。
*/
...
void solve() {
    ...
    sort(ALL(WSV), [](tuple<ll, ll, ll> a, tuple<ll, ll, ll> b) {
        return min(get<1>(a), get<1>(b)-get<0>(a)) > min(get<1>(b), get<1>(a)-get<0>(b));
    });

    auto dp = list2d(2, M, -INF);
    dp[0][0] = 0;
    rep(i, N) {
        auto [w, s, v] = WSV[i];
        rep(j, M) {
