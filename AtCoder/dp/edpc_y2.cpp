...
　最後のサンプルが合わず。
・全然別の方針にしてみた。包除でうまいこと重複を、、うまいことできてなかった。。
*/
...
void solve() {
    ...
    rep(i, N) {
        ...
        ll h2 = H-r;
        ll w2 = W-c;
        C1[i] = mt.nCr(h1+w1, h1) * mt.nCr(h2+w2, h2);
    }
    ...
    rep(h, 1, H+1) {
        for (auto [w, i] : adj[h]) {
            C2[i] = bit.query(0, w+1);
            bit.add(w, 1);
        }
    }

    mint ans = mt.nCr(H+W-2, H-1);
    rep(i, N) {
        ans -= C1[i] * pow(-1, C2[i]%2);
    }
    print(ans);
}
