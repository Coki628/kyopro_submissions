/*
参考：https://blog.hamayanhamayan.com/entry/2019/01/12/153435
　　　https://ferin-tech.hatenablog.com/entry/2019/01/09/101410
　　　https://www.creativ.xyz/dp-practice/
　　　https://ikatakos.com/pot/programming_algorithm/contest_history/atcoder/2019/0106_educational_dp_4
・自力ならず。解説は、遷移の理屈についてはikatakosさんのやつが詳しくて参考になった。
・遷移の作り方が難しいDP、DP高速化、遅延セグ木、区間最大値取得・区間加算
・右端ソートして左から決めていく。これはやったんだけど、いい感じの遷移が全然見えて来ず。
・dp[i] := 位置iで最後に'1'を使った時の最大値 を状態に持って、
　iでの最大値を手前から区間最大値で取ったら、右端がiの区間について手前に区間加算していく、
　といったことをする。なんだけど、これが説明聞いてもなかなか入って来なかった。
　なんか、右端来た時に区間加算とかしちゃったら、それ以降左から最大取る時、
　常にその区間分の値含まれちゃって、途中が'0'になるケース拾えなくない？って。
・でも手元でそういうケースをシミュレートすると、だんだん分かってきた。
　最大値取得する場面で、自分より手前全部を見て一番いい所を取ってくる訳だけど、
　この時遷移元と遷移先の間になった区間がまさに'0'にした区間なんだ。(多分)
　左から常に最適を作ってきてるから、そこより左は最適なのが保証されてて、
　その時また余計な部分をすっ飛ばして遷移するから、また新しい位置で最適を作れる。
　その手前までの部分集合からは最適を既に集めてるから、今回見てる位置を含めた上で
　またその位置までの最適な部分集合を確定する、みたいのを繰り返してく。
　この貰う遷移の仕組みは最近ABC201_fでやったものと似ていると感じた。
・さて、遷移が終わっても更新もいまいち直観的でない。
　右端が来たら手前に区間加算、って口で言うのは簡単だけど、これ本当に正しいの？ってなる。
　既に確定したはずの自分より手前に更新をかけに行ってしまうのはDPとしてだいぶ気持ち悪く思えるし。
　でもやってみるとこれで正しい動きをするので、そういうものだと思うしかない。
*/
...
void solve() {
    ...
    auto dp = get_Lazy_segment_tree(f, g, h, T, E);
    dp.build(vector<ll>(N+1, 0));
    rep(r, 1, N+1) {
        dp.update(r, dp.query(0, r));
        for (auto [l, a] : adj[r]) {
            dp.update(l, r+1, a);
        }
    }
