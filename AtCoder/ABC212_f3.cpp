/*
参考：https://blog.hamayanhamayan.com/entry/2021/08/01/012316
・ABC高難度埋め
・ダブリング二分探索、辺を頂点として見る
・公式解の方針。解説は相変わらずはまやんさんが易しいのでそっちで。
・第一印象としてこういうARC060eみたいなダブリングにぶたんはイメージしたけど、
　移動の後の待ち時間とかもあるし、ある町から移動できる場所は一意にならない。
　で、ダメかなって方針転換しちゃったんだけど、やっぱりできるのね。。
・必要だったのは、町ではなくバスの方を頂点として移動を考えることだった。
　確かにこれだと次に乗るバスは一意なので、ダブリングに乗る。
　ダブリングが構築できれば、あとはバスの乗り換え回数で二分探索。
　乗り換え回数は答えと直接関係がないので、一見使えなそうに思えてしまうが、
　これでその乗り換えを開始・終了する時刻と始点・終点となる町が分かるので、
　時刻zの手前まで飛べば、後は場合分けで答えが出る。
*/
...
void solve() {
    ...
    // 乗り継ぐバスが一意なので、乗り継いだ回数でダブリングする
    vector<ll> A(M+1);
    rep(i, M) {
        // 1回乗り継いだ先
        auto [_, j] = *lower_bound(ALL(adj[V[i]]), mkp(T[i], 0LL));
        A[i] = j;
    }
    // もうその後にバスがない状態を番兵として用意しておく
    A[M] = M;
    const ll MXLOG = 20;
    const ll MX = 1LL<<MXLOG;
    auto db = get_doubling(MXLOG, A, -1LL, f);
    ...
    rep(i, Q) {
        ...
        ll _, st;
        tie(_, st) = *lower_bound(ALL(adj[y]), mkp(x, 0LL));
        if (st == M) {
            print(y+1);
            continue;
        }
        ll j = bisearch_max(0, MX, [&](ll m) {
            ll g = db.get(st, m);
            if (g == M) return false;
            return S[g] < z;
        });
        ll g = db.get(st, j);
        if (z <= S[g]) print(U[g]+1);
        elif (S[g] < z and z <= T[g]) print(mkp(U[g]+1, V[g]+1));
        else print(V[g]+1);
    }
}
