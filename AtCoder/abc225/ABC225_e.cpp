/*
参考：みんなのツイート
　　　https://atcoder.jp/contests/abc225/submissions/26908463
・自力ならず。。全然方針立たなくて悔しい。
・幾何、偏角ソート、区間スケジューリング、誤差対策
・本番中は、幾何っぽいし険しいな、って敬遠しちゃったんだけど、
　終了後、偏角ソートと区間スケジューリングって2つのキーワード拾ったら結構すぐ方針立った。
　こういう、ひと考察要るけどそれ乗り越えれば典型みたいなやつ、ちゃんと解きたいよね。。
・さて、思ったように実装したらWAが取れないし、EPS調整しても取れないから、
　ldのまま通したって言ってたうしさんのコードを見に行くと、EPSを20まで上げてた。
　でもこれを自分の所で同じにやると、同値の所で少しも値が変わらない判定になってしまうらしく、
　a+EPS > b を a+EPS >= b にするとちゃんとACした。イコール付きはこっちで判定した方がいいかも。
*/
...
void solve() {
    ...
    vector<pair<ld, ld>> RL;
    rep(i, N) {
        auto [x, y] = XY[i];
        ld l = get_degree<ld>({1, 0}, {0, 0}, {x-1, y});
        ld r = get_degree<ld>({1, 0}, {0, 0}, {x, y-1});
        RL.eb(90-r, 90-l);
    }
    sort(ALL(RL));
