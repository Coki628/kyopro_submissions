 ...
 * ・自力AC！これはまあ、ね。
 * ・ローリングハッシュ、二分探索
 * ・5000^2/2=約1300万ならワンチャン2重ループ愚直通らないかなと思ったけど無理だった。
 * ・昔やったのを思い出しつつ、区間長固定でにぶたん。
 * 　まあこれで普通にsubstrしても通るって話があるけどね。。
 */
...
void solve() {
   ...
   ll ans = bisearch_max(0, N/2+1, [&](ll x) -> bool {
        map<uint64_t, ll> mp;
        rep(l, N-x+1) {
            ll r = l+x;
            if (mp.count(rh.query(table, l, r))) {
                if (mp[rh.query(table, l, r)] <= l) {
                    return 1;
                }
            } else {
                mp[rh.query(table, l, r)] = r;
            }
        }
        return 0;
    });
