/*
参考：https://atcoder.jp/contests/arc130/editorial
・自力ならず。アドホックむずい。
・構築、小さい所を全探索、貪欲
・とりあえず繰り上がりさえ起きなければどう置こうと桁和は変わらないはずなので、
　これは自由に数字を並び替えて繰り上がりする場所を最大化する+構築みたいな問題になる。
・で、どうした方がいいか考えると、なるべく無駄がないようにペアを組んでいきたい。
　1箇所は10以上にならないと繰り上がりが発生しないので和が10以上のペアが必要で、
　後はそこを起点にすれば9以上で繰り上がってくれるので9以上で作る。
　長い方で9が余っている場合は繰り上がり直後に置けば単体で繰り上がるのでそれも適切に置く。
　あとはどのペアで10以上を作るかだけ選択が必要だが、これは和が10～18になるペア高々9通りを全探索できる。
　繰り上がらないものは雑に置いてOK。まとめると、最終的に上から、
　適当(長い方だけ)→残った9(長い方だけ)→和が9以上ペア→和が10以上(1ペアだけ)→繰り上がらない適当ペア
　みたいにする。
*/
...
void solve() {
    ...
    auto org1 = Counter(S);
    auto org2 = Counter(T);
    string ans1 = S, ans2 = T;
    ll mx = 0;
    // 繰り上がりの末尾桁に使う和が10以上のペアを全探索
    rep(i, 10, 19) {
        rep(j, 1, 10) {
            ll k = i-j;
            char c1 = '0'+j, c2 = '0'+k;
            ll use = min(org1[c1], org2[c2]);
            if (use) {
                ll cnt = 0;
                string cur1, cur2;
                auto C1 = org1;
                ...
                C2[c2]--;
                cnt++;
                // 残りから和が9以上のペアを組めるだけ組む
                rep(i, 9, 19) {
                    rep(j, 1, 10) {
                        ll k = i-j;
                        char c1 = '0'+j, c2 = '0'+k;
                        ll use = min(C1[c1], C2[c2]);
                        cur1 += string(use, c1);
                        cur2 += string(use, c2);
                        C1[c1] -= use;
                        C2[c2] -= use;
                ...
                // 先頭側に長い方の残りを付けるのでまた反転
                reverse(ALL(cur1));
                reverse(ALL(cur2));
                // 9は単体で繰り上がるかもしれないので先に使う
                cnt += C1['9'];
                cur1 += string(C1['9'], '9');
                C1['9'] = 0;
                // 残りを使い切る
