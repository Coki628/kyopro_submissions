 ...
 * ・蟻本演習4-7-2
 * ・Trie木上の木DP
 * ・当選番号を文字列で表して、Trie木に乗せることで、部分一致を効率よく処理する。
 * ・この時各頂点に、その頂点の合計と最大値みたいな2つの値を持たせるんだけど、
 * 　合計はその頂点にぴったり一致する分だけ、にして、
 * 　最大値はその頂点より先(葉側、その頂点を含まない)が持つ最大値、にするとうまくいく。
 * 　最大値に自分の頂点の分を含めちゃったりするとバグる。。
 */
...
int main() {
    ...
    ll M = trie.size();
    // val[i] := 頂点iに該当する番号の賞金額
    // mx[i]  := 頂点iより先(頂点iの分を含まない)で得られる賞金合計の最大値
    vector<ll> val(M), mx(M);
    for (auto& p : AB) {
        string a;
        ll b;
        tie(a, b) = p;
        ll sz = a.size();
        auto rec = [&](auto&& f, ll v, ll i) -> ll {
            if (i == sz) {
                val[v] += b;
                return mx[v]+val[v];
            }
            ll res = f(f, trie.next(v, a[i]), i+1);
            chmax(mx[v], res);
            return mx[v]+val[v];
        };
        ll res = rec(rec, trie.next(0, a[0]), 1);
        chmax(mx[0], res);
        print(mx[0]);
    }
    return 0;
