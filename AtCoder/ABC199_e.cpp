...
・きっちり自力AC！
・bitDP
・制約はbitDPって言ってる。でも流れを考えてみると、x番目までに値が含まれるか、
　みたいな条件チェックをしないといけないので、終わった値の集合があっても、
　順番が分からなくなってると無理じゃないか？となる。
・でもさらに考えると、集合に新しく要素を追加する時に、
　追加前の状態までは条件を満たしているはずなので、xが数列の長さ未満の条件は
　考慮しなくていい、となり、これなら出来上がった部分の順番はもう必要がなくなり、
　bitDPが成立するよね、となる。
・条件チェックパートはソートにぶたんしたけど、もっと上手にやれそう。
　とはいえ、これでもちゃんとACしたのでまあOK。AC0.64秒。
　2^18*100=約2621万ににぶたんが乗るので、
　continueやbreakが結構多いとはいえ、まあ結構かかるのは分かる。
*/
