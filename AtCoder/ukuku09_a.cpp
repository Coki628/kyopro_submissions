...
・なんとか自力AC！
・辞書順最小、上界を見積もる、実験
・なるべく前が小さい物を考えると、(1, N-1)とすると必ずうまくいくことが分かり、
　前が2になることなはい、となる。
・後ろに使える数を考える。小さい数を実験してみると、
　x=1,2,3..それぞれの同じ項を比べると、綺麗に同じ数(x-1項目の値)ずつ変化する。
　これを使ってmodが一致するかでぴったりになるかを判定して、
　うまくいくようならその時の値を答えに使う。
　この漸化式はフィボナッチみたいなやつですぐ大きくなるのでNを超えるまで見て十分間に合う。
*/
