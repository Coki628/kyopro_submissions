...
・ちょっと時間かかったけど自力AC！
・まあこれは何度も解いてるし。。
・とはいえC++ではやってなかったし、幾何ライブラリ整備できてよかった。
*/
...
void solve() {
    ...
    rep(i, N) {
        vector<ld> degs;
        rep(j, i+1, i+N) {
            degs.pb(get_degree(P[(i+1)%N], P[i], P[j%N]));
        }
        rep(i, N-1) degs.pb(degs[i]+360);
        sort(ALL(degs));
        rep(j, N-1) {
            ll l = bisect_left(degs, degs[j]+90-EPS);
            ll m = bisect_right(degs, degs[j]+90+EPS);
            ll r = bisect_left(degs, degs[j]+180-EPS);
            ans[1] += m-l;
            ans[2] += r-m;
        }
    }
