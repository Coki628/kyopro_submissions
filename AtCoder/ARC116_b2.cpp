/*
参考：https://www.youtube.com/watch?v=x_ZBB8MjTbI
・自力ならず。。やっぱ数え上げダメだなー。。
・MOD数え上げ、シグマの式変形
・時間はかかったけど、あと一歩までは来てた。
・連続しなくていい部分列なので、どこでも選べる。ソートしても影響はない。
　最小値と最大値を固定すると、間の部分を自由に選べる2冪みたいにして、
　2重ループなら解ける状態を作れる。具体的には、
　ΣΣA[i]*A[j]*2^(j-i-1) とできる。i == j のところは含めると
　2冪の計算に場合分けが出て面倒になるので、後から別途A[i]^2を足すことにする。
　ここまでは行った。ここからシグマをA[i]を外に出して ΣA[i]ΣA[j]*2^(j-i-1)
　とするまでは式変形できたんだけど、2冪部分をどうするか分からず。
・公式解説は後ろからやって*2してくみたいな賢い解法で解いてたけど、
　それは自分で導ける気がせず、しっくり来るものがないか悩んでいると、
　輪さんの動画で、いい感じの式変形をしてた。これだ。
・2^(j-i-1) は 2^j*2^(-i-1) に分解できる。言われると確かにそうだ。
　冪乗ってばらせるじゃん。。
・これなら内側はA[j]*2^jだけとなり、これは累積和なりで前計算しておける。
　*2^(-i-1) は /2(i+1) と変形できるので、これで全部できる。
・powをmintの方に統一してみた。速度とかは特に変わらずAC0.11秒。
*/
...
void solve() {
    ...
    sort(ALL(A));
    vector<mint> tmp(N);
    rep(i, N) tmp[i] = A[i]*mint(2).pow(i);
    Accumulate<mint> acc(tmp);

    mint ans = 0;
    rep(i, N-1) {
        ans += A[i]/mint(2).pow(i+1)*acc.query(i+1, N);
    }
    rep(i, N) ans += A[i].pow(2);
    print(ans);
}
