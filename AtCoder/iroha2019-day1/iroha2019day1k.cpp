...
void solve() {
    ...
    vector<map<ll, mint>> memo(N+1);
    auto rec = [&](auto&& f, ll i, ll d) -> mint {
        if (i < 0) return 0;
        if (memo[i].count(d)) return memo[i][d];
        mint res = 0;
        rep(j, sz[i]) {
            res += f(f, i-1, d+tostr(A[i][j]).size()) + A[i][j]*pow(10, d, MOD);
        }
        res /= sz[i];
        memo[i][d] = res;
        return res;
    };
    auto ans = rec(rec, N-1, 0);
    rep(i, N) {
        ans *= sz[i];
    }
    print(ans);
