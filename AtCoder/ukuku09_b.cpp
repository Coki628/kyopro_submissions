...
・自力AC！
・セグ木、優先度付きキュー
・方針は割とすんなり立ったのに、なぜかやってみるとTLE。。
　原因は、最小を取るindexを見つける所で、x == mn ってやってたのがダメで、
　x <= mn にしたら正しく動いた。これ前もやった気がするなぁ。
　その時も何でイコールじゃダメなんだろーって思った気がする。
*/
...
void solve() {
    ...
    my_priority_queue<tuple<ll, ll, ll>, vector<tuple<ll, ll, ll>>> que;
    auto seg = get_lazy_segment_tree(f, g, h, T, E);
    seg.build(A);
    que.push({N, 0, N});
    ll ans = 0;
    while (M > 0 and que.size()) {
        auto [len, l, r] = que.pop();
        ll mn = seg.query(l, r);
        ...
        ll r2 = r;
        if (r1-l1 > 0) {
            que.push({r1-l1, l1, r1});
        }
        if (r2-l2 > 0) {
            que.push({r2-l2, l2, r2});
        }
    }
    print(ans);
}
