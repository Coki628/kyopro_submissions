/*
参考：https://atcoder.jp/contests/arc125/editorial
・自力ならず。数学問つらい。
・式変形
・自分で導けたのは x^2-z^2 = y ってとこまで。でもこの後の
　(x+z)(x-z) = y に気付いてたとして、その先に進めなかったとは思う。
・答えを導くためにいくつか重要な式変形がある。以下、公式解説に合わせて
　p = x+z, q = x-z と置く。
　・(p = x+z) + (q = x-z) -> p+q = 2x -> x = (p+q)/2
　　ここから、xを整数とするために、p,qの偶奇の一致が必要と分かる。
　・1 <= y <= N, y = p*q -> p*q <= N -> p <= N/q
　　ここから、pの上界がN/qと分かる。(下界は自明にq)
　この2つの式を作れると、qを固定した時のpの範囲として、
　q <= p <= N/q が分かり、偶奇の一致が必要なことも分かるので、
　それを満たすようなpを各qについて√Nくらいまで見ればOK。
・でもこんなん思いつかんよ。。思ったとして、これで十分って確証も持てないし。。
*/
