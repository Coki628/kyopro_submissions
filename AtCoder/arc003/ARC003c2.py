...
・pythonTLE。。pypyもTLEだしそもそもメモリ制限64MがつらくてMLE。。
"""
...
# グリッドダイクストラ(コストは普通に時間を持って、明るさがmに満たない場所は壁みたいに行けなくする)
def dijkstra(m):
        ...
        for direction in directions:
            ...
            if grid[nh][nw] in numbers:
                ...
                if light >= m:
                    if t < memo[nh][nw]:
                        memo[nh][nw] = t
                        heappush(que, (t, nh, nw))
            # ゴールマスはキューに詰めないで記録のみ
            elif grid[nh][nw] == 'g':
                memo[nh][nw] = min(memo[nh][nw], t)
    # ゴール地点への到達時間を返却
    return memo[gh][gw]

# 明るさで二分探索して最大値を探す
res = bisearch_max(-1.0, 9.0, lambda m: dijkstra(m) != INF)
if res == -1.0:
    print(-1)
else:
    print(res)
