...
・結構すんなり自力AC！これは嬉しい。
・クエリ系、座圧、BITにぶたん
・題意の操作を高速にシミュする。始点として使う各S[i]を座圧して持っておいて、
　間の区間長をBITで各座標に重みとして持たせる。操作ではこれを削り取っていくイメージ。
　S[i]以降でまだ白マスが残っている最小位置をBITにぶたんで探して、削除して前に進む。
　全体で削除量が多ければ、早々に最後の位置以外が空っぽになるので計算量は増えない。
*/
...
void solve() {
    ...
    rep(i, N) {
        ll j = S[i];
        while (C[i] > 0) {
            j = bit.bisearch_fore(j, M-1, 1);
            ll use = min(C[i], bit[j]);
            C[i] -= use;
            bit.add(j, -use);
        }
        ll res = cp.unzip(j)+(H[j]-bit[j])-1;
        print(res);
    }
}
