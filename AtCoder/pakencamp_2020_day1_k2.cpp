...
・なんとか自力AC！時間かかった。。
・候補を絞って全探索、区切るDP
・色々な区切り方を考えると通り数は膨大なので、最終的な答えになるGCDの候補を絞っていく方針を取る。
　なんかGCDだとありえる数が限られるから、その候補を絞って全探索みたいのはよく見る感じがする。
・最初はそれでも候補が数列の総和の最大値で9万くらいあるから、
　その内側で判定の処理に区切るDPできないなぁとか思ってたんだけど、
　よくよくサンプルとか見てたら、きつい(区切る回数が多い)制約で達成できた値は、
　緩い方でも必ず達成できそうなことに気付く。この単調性は使えそう。
　ある区切られた状態をマージしても、例えば3の倍数と3の倍数を足し合わせたら必ず3の倍数を維持できる。
　これを辿ると、最終的に全部つながった1つの固まりになった時は、下の方で見た全ての数の倍数になる。
　ここで思ったのが、じゃあ逆に答えとしてはその1つになったやつの約数しかなり得ないんじゃないか、ってこと。
　これが当たりで、大きく状態数を減らせた。
・さて、判定処理は区切るDPで最後まで到達できるか見ようとしてたんだけど、
　途中までやっててうまくDPが合わないなーってなってた時、
　これ前から貪欲に区切るだけでもいいんじゃないか？となる。
　確かに区切れるだけ区切ったとして、そこを区切らなかったとしてもある数の倍数であることは維持されるので、
　損はなさそう。やってみる。合う。これなら速いはず。投げたら無事AC。
・解説見たけど、判定パート貪欲でNしかかからないから、
　全探索対象は約数に絞らなくても総和全部試して間に合ったんだね。。
・区切るDPも合わせてみた。なんか区切らない遷移みたいな要らんことしてた。
　区切らないのはそもそも遷移させないでよかった。
　これだと約数の個数*N^3くらいかかるからきついと思ったけど、なんとか間に合ってAC1.31秒。
　多分内側のループ2つが半分くらいしか回らないから定数倍が効いてるんだと思う。
*/
...
void solve() {
    ...
    Accumulate<ll> acc(A);
    auto divs = divisors(sum(A));

    vector<ll> ans(N, 1);
    for (ll x : divs) {
        auto dp = list2d(N+1, N+1, false);
        dp[0][0] = 1;
        rep(i, 1, N+1) {
            rep(j, 1, i+1) {
                rep(k, i) {
                    if (dp[k][j-1] and acc.query(k, i)%x == 0) {
                        dp[i][j] = 1;
                    }
                }
            }
