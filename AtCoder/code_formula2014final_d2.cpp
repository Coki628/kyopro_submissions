/*
参考：https://torus711.hatenablog.com/entry/20141009/1412868963
・試験管青diff残埋め
・自力ならず。大変だった。解けるべきDP問題なんだけど、工夫が必要。
・状態と遷移を工夫するDP、区間スケジューリング
・状態N^2と遷移Nをかければ割と思いつきやすいDP。
　(まーそっちも自力じゃうまくまとまらなかったんだけど。)
　そこからどうするかだけど、思った以上に状態が減らせて、思った以上に遷移はやっていい。
　あとDP高速化と言えば貰うDPだけど、今回は配るDPでできた。逆に起点が固定のがやりやすいと思った。
・状態についてだが、何回連続したかを状態に持ちたくなるのだが、実はこれを持たなくてもいい。
　というのも、遷移の方で毎回行けるところまで連続させる場合を試せるからだ。これで状態は1次元になる。
・大きく分けて、違う種類に遷移するものと同じ種類に遷移するものがある。
　違う種類に対する遷移は、自分より先の遷移できる場所にすればいいのでまあ普通に。
　前述の通り、状態が1次元になっているので、この候補の遷移先は全て試せる。
　問題は同じ種類に遷移する場合だが、これは同じ種類だけでまとめて
　予め右端ソートした配列を作っておけば、区間スケジューリングの要領で、
　行ける中で一番近い場所への遷移を繰り返せる。毎回やれるところまでやっても、
　せいぜい遷移はN回だし、各位置でこれは同じaに対してだけやればいいので
　1つのiからの遷移で結局トータルN回くらいまでしかない。
・あと今回、右端ソートすべきか左端ソートすべきか迷ったんだけど、
　DPの遷移ってことで言えばここはどっちでもいいっぽい。なんとなく、
　貰うDPが右端ソートで配るDPが左端ソートしたい気持ちになったんだけど、
　今回配るDPで右端ソートしてるけど問題なくACしてる。
　それより今回は遷移のパートで区間スケジューリングっぽいことをしないといけないので、
　こっちは右端ソートが必須。
*/
...
void solve() {
    ...
    rep(i, N) {
        ...
        // 同じ種類で2連続以上する遷移
        ll cnt = 2;
        for (auto [r2, l2, j] : adj[a1]) {
            if (r1 <= l2) {
                chmax(dp[j], dp[i]+acc.query(1, cnt));
                cnt++;
                l1 = l2;
                r1 = r2;
            }
