/**
 * ・自力ならず。。A,B,Cでこれが考察一番むずくないか？
 * ・ビット、偶奇
 * ・まず、条件に合うのはどういう状況かと言うと、
 * 　ビットの01が異なる箇所が奇数個あるペアになる。
 * 　異なる箇所が偶数個であれば、ちょうど半分になるので、同点になる可能性はある。　
 * ・で、これが分かっても、全ペアについてそのチェックはできない。
 * 　結局、立ってるビットの数の偶奇が異なることが、
 * 　ビットの状態の異なる箇所が奇数個あることに繋がるので、
 * 　立ってるビットの数の偶奇で2グループに分けて別グループ同士を掛ければいい。
 * ・いやまじでこれがA,B,Cの中で一番むずいって。
 * 　これが茶diffになって全人類解いたと思うとつらい気持ちになるな。
 */
