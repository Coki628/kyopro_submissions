/**
 * ・E8さん精選100+50問埋め：その他のテクニック：圧縮
 * ・区間加算BIT、二分探索
 * ・また別解で通した。データ構造力。
 * 　片方の色が連続する区間がどこまで続くかは二分探索できる。
 */
...
int main() {
    ...
    rep(i, 1, N) {
        if (i%2 == 0) {
            ...
            if (bit[A[i-1]][i-1] == 1) {
                // 区間の1の総和 = 区間長 となる区間を左に伸ばす
                ll j = bisearch_min(-1, i-1, [&](ll m) {
                    return bit[A[i-1]].query(m, i) == i-m;
                });
                bit[A[i-1]].add(j, i, -1);
                bit[A[i]].add(j, i, 1);
            }
