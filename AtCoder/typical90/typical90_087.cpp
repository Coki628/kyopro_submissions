...
・単調性はありそうなのでにぶたんする。
　ぴったりを求めるので2回にぶたんする。
　最初、K個以上の最大値とK個以下の最小値の差分取ったけど、
　<=と<でやった差分取るのが同じ処理でいいしバグりにくそうね。
・方針は合ってたんだけど、WAが取れずにしばらく悩んでた。
　結局、INF個ある判定のところで、K個ちょうどかどうかのチェックをしてなかったのが原因で、
　確かにそれだとK個ぴったりが全くなくてもINF個ある判定になってしまう。
　そこをちゃんとチェックしたら無事AC。
*/
...
void solve() {
    ...
    ll MX = 1000000000000;
    ll r = bisearch_max(0, MX+1, [&](ll x) {
        return calc(x) >= K;
    });
    if (r == MX) {
        ll x = calc(MX);
        if (x == K) print("Infinity");
        else print(0);
        return;
    } elif (r == 0) {
        print(0);
