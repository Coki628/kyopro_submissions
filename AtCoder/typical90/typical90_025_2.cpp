...
・E8さん典型90問埋め
・自力ならず。むずかった。
・工夫して全列挙、重複組み合わせ列挙、式変形
・重要な考察として、f(x)の値は各桁の順序が変わっても変化しないので、
　これをまとめられる。そしてこの入れ替わった順序が表すxは、
　違う順序で同じxになったりはしないので、そのxを取りうるかの確認ができれば、
　答えは+1するかどうかだけになる。
・式変形も必要で、x-f(x) = B -> x = f(x)+B とすることで、
　f(x)が決まればxであるべき値を一意に出来る。
・後は先行0の関係もあるので、1〜11桁の全てについて、
　重複組み合わせでありうるパターンを列挙すれば、
　それを1つずつチェックしていける。
・重複組み合わせ全列挙ライブラリ化した。pythonのやつっぽく。
*/
...
void solve() {
    ...
    rep(len, 1, MX+1) {
        auto combs = combinations_with_replacement(A, len);
        for (auto &comb : combs) {
            V.pb(comb);
        }
    }
