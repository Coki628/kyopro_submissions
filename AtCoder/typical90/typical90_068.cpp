...
・E8さん典型90問埋め
・なんとか自力AC！めっちゃ時間かかった。。
・重み付きUnionFind、式変形、偶奇
・明らかに重み付きUFな見た目をしているが、そのままだと繋げない。
　式変形を頑張ると、2つ先同士であれば距離を定めて繋げられると分かる。
　indexが奇数個離れているところは、隣まで移動して、
　これもまた式変形に基づいて頑張る。色々添字も頑張る。
　端とか、どっち隣から行くとかも場合分け頑張る。
*/
...
void solve() {
    ...
    rep(_, Q) {
        ...
        if (op == 0) {
            V[x] = v;
            if (x-1 >= 0 and x+1 < N and V[x-1] != -1) {
                // A[i-1]+(V[i]-V[i-1]) = A[i+1]
                uf.merge(x-1, x+1, V[x]-V[x-1]);
            }
            if (x+2 < N and V[x+1] != -1) {
                // A[i]+(V[i+1]-V[i]) = A[i+2]
                uf.merge(x, x+2, V[x+1]-V[x]);
            }
        } else {
            if (abs(x-y)%2 == 0) {
                if (uf.is_same(x, y)) {
                ...
                if (y-1 >= 0 and V[y-1] != -1 and uf.is_same(x, y-1)) {
                    auto d = uf.diff(x, y-1);
                    // A[i] = V[i-1]-A[i-1]
                    print(V[y-1]-(v+d));
                } elif (y+1 < N and V[y] != -1 and uf.is_same(x, y+1)) {
                    auto d = uf.diff(x, y+1);
                    // A[i] = V[i]-A[i+1]
                    print(V[y]-(v+d));
                } else {
