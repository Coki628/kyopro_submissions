/*
参考：https://github.com/E869120/kyopro_educational_90/blob/main/editorial/047-01.jpg
　　　https://github.com/E869120/kyopro_educational_90/blob/main/editorial/047-02.jpg
・E8さん典型90問埋め
・3種類を0,1,2に置き換え、ローリングハッシュ
・ロリハに行くまでの考察がきつい。
・まず0,1,2で管理するとだいぶ扱いやすくなる。これはやった。
・次に、気付けなかった考察で、判定を3文字ともいっぺんにやる必要はなくて、
　斜めの列でRが合うか、Bが合うか、って3回やるようにするとだいぶ見通しが立つ。
　これならその時注目してない2種については、どちらかでスワップしておけば、
　部分文字列の一致判定に帰着できる。ここまで来ればロリハで高速に判定できる。
*/
...
void solve() {
    ...
    rep(c, 3) {
        ...
        auto table2 = rh.build(B2);
        rep(i, N) {
            ll l1 = i;
            ll r1 = N;
            ll l2 = 0;
            ll r2 = N-i;
            if (rh.query(table1, l1, r1) == rh.query(table2, l2, r2)) {
                ans++;
            }
            if (i > 0 and rh.query(table2, l1, r1) == rh.query(table1, l2, r2)) {
                ans++;
            }
        }
    }
