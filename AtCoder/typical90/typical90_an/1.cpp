...
・E8さん典型90問埋め
・なんとか自力AC…？最近燃やす埋めるが出たって話も聞いてたのに、めちゃくちゃ時間かかった。。
・最大流、最小カット、燃やす埋める
・ある家を選ぶor選ばないで燃やす埋めるに帰着させる。
・コストの乗せ方は診断人さんのスライドを参考に、教科書通りに、
　利得の大きい方を0、少ない方をそこからの差額、とする。
・同時に選ぶ時の制約は、INFの辺を張る。この辺はARC085eを参考に。
　ここで無限にバグらせたんだけど、この辺は、
　「Aを選ぶならBも選ばないといけない」だったら、A→Bに向かって張る。
　これがなんか直感だと逆になってて、ずっと間違って張ってた。。
*/
...
void solve() {
    ...
    rep(u, N) {
        ...
        rep(_, K) {
            ...
            cin >> v;
            v--;
            // vを選ぶならuも選ばないといけない、の辺
            mf.add_edge(v, u, INF);
        }
    }

    auto res = total-mf.flow(s, t);
    print(res);
}
