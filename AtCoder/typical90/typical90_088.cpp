...
・自力ならず。鳩の巣、気付いたことなし。。
・鳩の巣原理、再帰全探索、バックトラック
・総和が約9000と限定されているので、条件を満たすパターンが1万個以上くらいあれば
　その中に重複が出て、答えとして妥当な2つのカードの集合を用意できる。
・そして条件を満たすかどうかだが、それを使った時NGになるカードを隣接リストで管理しておいて、
　今何がNGかどうかは愚直にmultisetに出し入れして感じとかそんなんで通った。
　このmultisetと答えに使う数列はバックトラックで状態戻しする。
　NG条件のクエリが最大で全ペア分くらいあるけど、
　多分条件が多ければそれだけ分岐が減るから速くなって大丈夫なんだと思う。
・取りうる状態が少ないのがほんとに非自明で難しいなーって思った。
*/
...
void solve() {
    ...
    vvl ans(MX+1);
    multiset<ll> ng;
    auto rec = [&](auto&& f, vector<ll> &V1, ll sm, ll x) {
        if (x == N) {
            if (ans[sm].size()) {
                auto V2 = ans[sm];
                print(V1.size());
