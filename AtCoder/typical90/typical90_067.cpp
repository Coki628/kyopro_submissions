...
・なんとか自力AC、、、Pythonで。
・N進数変換
・K<=100だし、文字列もせいぜい20桁なので、そんなに計算量ないはずなんだけど、
　なぜか3件TLEする。値が7以下になったらもう変化ないはずなので、
　そこでbreakしたらめちゃ速くなったけど今度は2件WA。。
・ライブラリがバグってるとかを疑ってPythonで投げてみたらあっさり通った。
　でもPythonで使った古い方のdton使っても結果変わらず。
　なんなんだろう。。。
・終了後、テストケース確認した。Nが8進数で書かれた8^20<Nの数だから、
　20桁あってllじゃ受けれないじゃん。。そりゃPythonなら通るわ。
　結局、しょうもないケアレスミスで、進数ライブラリのバグじゃなかったからまあ良かったか。
*/
