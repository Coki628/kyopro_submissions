/*
参考：https://twitter.com/e869120/status/1406429317249126402/photo/2
　　　https://twitter.com/e869120/status/1406429317249126402/photo/3
　　　https://github.com/E869120/kyopro_educational_90/blob/main/sol/071-03.cpp
・E8さん典型90問、残りの高難度埋め
・トポロジカルソート応用、再帰全探索(を途中で打ち切る)、バックトラック
・まずトポソが何をしているのか知っているのは前提としてあって、
　最大10通り示さないといけないので、貼るだけではダメで中身をいじる。
・トポソの最中に「今使える頂点から1つ選ぶ」というパートがあって、ここを再帰全探索にかける。
　当然全部やったら指数時間で無理なのだが、最大10通りで打ち切るのならできる。
　やり方はfor文の内側で再帰させて、いわゆるバックトラックで状態を戻すが、まあまあ戻す物が多くてややこしい。
・この時、setだとindexでループを回せず、途中で要素抜くとイテレータが壊れたので、vectorに変えた。
　setでも毎回コピー作ればループは回せたが、これだと最悪は10万くらいのsetを毎回複製してしまうので無理。
・これでもTLEが取れないので、公式解説のコードとか眺めながら何がいけないか考えた。
　答えは閉路があってトポソできない時の処理が原因、だった。即時終了条件をK個集まったら、にしてあったので、
　閉路があるパターンで打ち切れてなかった。つまり、終わり方を3種類、
　・K個集まった　→　trueを返して再帰探索打ち切り
　・まだ集まってない　→　falseを返して処理継続
　・閉路があった　→　-1出力してexit(0)で即終了
　みたいに用意する必要があった。これをやったら無事AC。
・exit(0)終了は最後の手段みたいなとこがあるので、普通に再帰打ち切りでこのケースも処理できるようにしてみた。
　エラー終了っぽいのに成功と同じtrue返すのがちょっと気持ち悪いが、まあ仕方ない。
　再帰が終わった後で、答えがK個集まってるかどうかで成否を判定してる。
*/
