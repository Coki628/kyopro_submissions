...
・これは部分2
・インタラクティブ、三分探索
・整数の三分探索は、同値がなければ実数と同じにできると思ってたけど、
　どうやら多分割る3切り捨てでm1,m2が正確じゃなくなるせいだと思うんだけど、
　前後調べないとWAになる。なのでデフォルトで前後1は調べるようにした。
・今回みたいに回数制限がある場合は、実数の方みたいに回数指定でめいっぱいやって、
　その中で最大を取る、みたいにするのが良さそう。(公式はそうやってた。)
*/
...
void solve() {
    ...
    auto [id, val] = trisearch_max(0, N, [](ll m) {
       return ask(m);
    });
    answer(val);
}
