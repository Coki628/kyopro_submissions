/*
・E8さん典型90問埋め
・部分点3を頑張ってたらACコードになった。解説ちら読みでヒントは得ているので自力ではない。
・区切るDP、尺取り、座圧BITで転倒数
・さっきの区切るDPをベースに、区間転倒数取得の高速化を考える。
　すると、転倒数の増減は区間の伸縮に対して単調性があるので、
　尺取り法の要領で高速に取得していける。(まあ尺取り使うのはヒントで知ってたんだけど。。)
・ここで、尺取りはいつも左端固定で右端を動かす配るDPみたいな遷移をさせるのに対して、
　区切るDPは基本的に貰うDPっぽく動かす方が慣れてて具合がいいので、
　どっちをどっちに寄せるか悩んだ。最初、区切るDPを配るDPっぽくする方にしたんだけど
　散々にバグって、尺取りを右端固定で左端動かす感じにしたら割とすんなり行った。
・それができれば、遷移できると分かった区間に対する一括遷移はこれもBITでできるので、
　全体でも計算量NlogNくらいにできてAC。
*/
