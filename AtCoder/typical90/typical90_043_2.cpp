...
・きっちり自力AC！
・グリッドダイクストラ、超頂点
・直前が縦移動か横移動かの状態を増やしてダイクストラする。
・100万マス*2で状態200万のダイクストラだけど、結構遅くてAC0.80秒。
・C++で書いてなかった01BFSを書いてみた。
　案外あんま早くならなくて、AC0.57秒。
*/
...
void solve() {
    ...
    auto res = bfs01(nodes, {s, N+s});
    ll g = gridtoid(gh, gw, W);
    ll ans = min(res[g], res[N+g]);
    print(ans);
}
