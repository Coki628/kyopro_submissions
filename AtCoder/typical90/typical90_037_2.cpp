...
・きっちり自力AC！
・貰うDP、セグ木で遷移高速化
・Functionなしセグ木が普通にやるとvectorに入らなくて困った。
　とりあえず今回は後ろから更新すれば1本で使いまわせたのでOK。
　でも複数本生やしたい場合も絶対出てくるので、方法は要確認だな。
・↑やってみた。ラムダなくして関数ベタ書きすればとりあえず大丈夫。
　まーこっちのがメモリは食うし遅いし、再利用できる時はした方がいいけどね。
*/
...
struct SegmentTree {
    ...
    SegmentTree(const Monoid &M1) : M1(M1) {}
...
template<typename Monoid>
SegmentTree<Monoid> get_segment_tree(int N, const Monoid& M1) {
    return {N, M1};
}

template<typename Monoid>
SegmentTree<Monoid> get_segment_tree(const Monoid& M1) {
    return {M1};
}
...
void solve() {
    ...
    // auto dp = list2d(N+1, W+1, -INF);
    // dp[0][0] = 0;
    // rep(i, N) {
    //     auto [l, r, v] = LRV[i];
    //     rep(j, W+1) {
    //         chmax(dp[i+1][j], dp[i][j]);
    //         rep(k, max(j-r, 0LL), j-l+1) {
    //             if (dp[i][k] > -INF) chmax(dp[i+1][j], dp[i][k]+v);
    //         }
    //     }
    // }
    // ll ans = dp[N][W];

    vector<SegmentTree<ll>> dp(N+1, get_segment_tree(W+1, -INF));
    dp[0].update(0, 0);
    rep(i, N) {
        auto [l, r, v] = LRV[i];
        rep(j, W+1) {
            dp[i+1].update(j, dp[i][j]);
            ll mx = dp[i].query(max(j-r, 0LL), j-l+1);
            if (mx > -INF) {
                dp[i+1].update(j, max(dp[i+1][j], mx+v));
            }
        }
    }
    ll ans = dp[N][W];
