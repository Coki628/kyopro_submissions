...
・すんなり自力AC！
・半分全列挙
・TLで最近これ出たのは聞いちゃってたけど、
　まあさすがにこれはノーヒントでも制約で気付くでしょ、きっと。
・2^20=100万ちょいににぶたんのlog乗っても、さすがにC++は安定AC0.24秒。
*/
...
void solve() {
    ...
    rep(bit, 1LL<<N1) {
        ...
        adj[popcount(bit)].pb(cur);
    }
    rep(i, N1+1) sort(ALL(adj[i]));
    ...
    rep(bit, 1LL<<N2) {
        ...
        ll cnt = K-popcount(bit);
        if (0 <= cnt and cnt <= N1) {
            ans += bisect_right(adj[cnt], P-cur);
        }
    }
    print(ans);
}
