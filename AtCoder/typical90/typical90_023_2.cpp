/*
参考：https://twitter.com/e869120/status/1386138990361726978/photo/3
・E8さん典型90問埋め
・部分点3
・スライドbitDP
・bitDPで、直前の1行の状態を持って、1ビットずつスライドさせるやつ。
　そういえばこのやり方あったな。忘れてた。
　1ビット(1マス)ずつずらしていくと、いい感じに状態を持てるんだよね。
*/
...
void solve() {
    ...
    ll N = H*W;
    auto dp = list2d(N+1, 1LL<<(W+1), (mint)0);
    dp[0][0] = 1;
    ll msk = (1LL<<(W+1))-1;
    rep(i, N) {
        rep(bit, 1LL<<(W+1)) {
            dp[i+1][msk&(bit<<1)] += dp[i][bit];
