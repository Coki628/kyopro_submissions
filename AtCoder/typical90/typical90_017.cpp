/*
・E8さん典型90問埋め
・きっちり自力AC！星7をかなり早く通せたから嬉しい。
・BIT、イベントソート
・円のままだと分かりにくいので、直線で考えてみる。
　s < t のものだけ数えるとのことなので、点0をまたぐような物は考えなくてよさそう。
　必要なら2周しようかと思ったけど、とりあえず1周のままやってみる。
・方針としては、始点が来たらBITに足す、終点が来たらその手前までで
　始点が来ていてまだ終点が来ていないもの(つまりBIT上に残っているもの)
　を集計して答えに足す。自分は終点が来たので取り除く。
・やってみると、最後のサンプルが合わないのでもう少し考える。
　同時に終点が来たやつを数えてしまっている可能性があることに気付く。
　取り除く処理と集計処理のイベントを分けて、
　同値のものは全て集計が後になるようにすると、これでAC。
*/
...
void solve() {
    ...
    BIT<ll> bit(N);
    ll ans = 0;
    for (auto [d, op, i] : events) {
        if (op == 0) {
            bit.add(L[i], -1);
        } elif (op == 1) {
            bit.add(d, 1);
        } else {
            ans += bit.query(L[i]+1, d);
        }
    }
    print(ans);
}
