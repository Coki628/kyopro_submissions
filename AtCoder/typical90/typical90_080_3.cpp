/*
参考：https://twitter.com/momomorgentau/status/1410398600132063233
・E8さん典型90問埋め
・bitDP、ビット毎に見る
・別解。もももさんのツイートから。これができるなら包除見えなくても通せるべきだったな。。
・dp[i][S] := iビット目まで見て、条件を満たした要素の集合がSの時の通り数
　みたいなbitDPにする。遷移はそのビットを立てる/立てないの2通りで、
　立てる方の遷移をする時には、今見ているビットが立っている要素を取り込む感じになる。
・本当はその遷移のパートが前計算で高速化できるけど、愚直でも通るのはさすがC++。
　計算量がD*2^N*Nで12億5800万くらいあるのにAC1.58秒。。
・前計算高速化した版。これだと計算量はNが取れて6290万くらい。AC0.42秒。
*/
...
void solve() {
    ...
    auto dp = list2d(D+1, 1<<N, 0LL);
    dp[0][0] = 1;
    rep(i, D) {
        rep(bit, 1<<N) {
            dp[i+1][bit] += dp[i][bit];
            dp[i+1][bit|prep[i]] += dp[i][bit];
        }
    }
    ll ans = dp[D][(1<<N)-1];
    print(ans);
