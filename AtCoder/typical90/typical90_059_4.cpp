...
・実は辺の向きが片方しかない制約だったので、SCCいらなかった。
　そのままDAGとしてメモ化再帰でOK。でもなぜかこっちのが遅くてAC1.93秒。。
...
void solve() {
    ...
    rep(x, 0, N, offset) {
        ...
        auto dfs = [&](auto&& f, ll u) -> bitset<offset> {
            ...
            bitset<offset> res;
            if (u != N and x <= u and u < x+offset) res.set(u-x);
            for (ll v : nodes[u]) {
                res |= f(f, v);
            }
