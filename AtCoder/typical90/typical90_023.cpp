...
void solve() {
    ...
    vector<mint> dp(1LL<<W);
    dp[0] = 1;
    rep(i, 1, H-1) {
        vector<mint> nxt(1LL<<W);
        rep(bit1, 1LL<<W) {
            if (dp[bit1] == 0) continue;
            rep(bit2, 1LL<<W) {
                if (bit2>>0 & 1 or bit2>>(W-1) & 1) continue;
                bool ok = 1;
                rep(j, 1, W-1) {
                    if (grid[i][j] == '#' and bit2>>j & 1) {
                        ok = 0;
                        break;
                    }
                    ...
                    if (bit2>>j & 1) {
                        if (bit1>>j & 1 or bit1>>(j-1) & 1 or bit1>>(j+1) & 1) {
                            ok = 0;
                            break;
                        }
                    }
