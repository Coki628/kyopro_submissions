/**
 * ・E8さん精選100+50問埋め：座標圧縮
 * ・きっちり自力AC！
 * ・座標圧縮、2次元いもす、グリッドBFS
 * ・やる。座圧は半開区間を意識すればバグらない。BFSする時には一番外側をマスとして使わないように、1つ小さくやる。
 */
...
void bfs(vvl &grid, vector<vector<bool>> &visited, vector<pii> src) {
...
void solve() {
    ...
    auto [zpx, unzpx] = compress(S1);
    auto [zpy, unzpy] = compress(S2);
    ll M1 = zpx.size();
    ll M2 = zpy.size();

    auto imos = list2d(M2, M1, 0LL);
    rep(i, N) {
        imos[zpy[Y1[i]]][zpx[X1[i]]]++;
        imos[zpy[Y2[i]]][zpx[X2[i]]]++;
        imos[zpy[Y1[i]]][zpx[X2[i]]]--;
        imos[zpy[Y2[i]]][zpx[X1[i]]]--;
    }
    rep(i, M2) {
        rep(j, M1-1) {
            imos[i][j+1] += imos[i][j];
    ...
    auto visited = list2d(M2, M1, false);
    ll ans = 0;
    rep(i, M2-1) {
        rep(j, M1-1) {
            if (!imos[i][j] and !visited[i][j]) {
                ans++;
                bfs(imos, visited, {{i, j}});
            }
        }
    }
    print(ans);
