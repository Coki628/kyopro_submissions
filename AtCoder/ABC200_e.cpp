...
・自力ならず。これは完全に負け。見当違いの方面で迷走してた。
　二項係数で数えそうだけど上限Nがあるからどうしよう、、とかやってたけど、
　普通にDP高速化すれば和の列挙ができるね。。その先も自力考察無理だったけど…。
・貰うDP、DP高速化、式変形、ループを減らす
・まず、a+b+c = X とすると、このX毎にまとめたい気持ちになる。
　これを二項係数ぐりぐりするのかと思ったのがそもそも間違いで、
　選ぶ回数が3回しかないので、列挙する和が100万以上あってもDPできる。
　遷移は多いけど1〜Nどの数でも使えるから規則的で、
　累積和取って貰うDPすれば1回で済む。
　これを和の小さい方から足して行って、次でちょうどK以上になる、
　って所まで進めれば、K番目になる時の和が分かる。
・で、この和が分かった後も結構考察大変で、aを固定してループするんだけど、
　aと和が分かっていると、b,cをそれぞれ多いか少ない方に全振りするとすれば
　bの下限と上限が分かる。例えば下限なら、c=Nとして式変形して、
　a+b+N = sm -> b = sm-a-N として、実際には0以下は取れないのでb=1とmaxを取る。
・この上限-下限+1がこのaを使う場合の通り数になるので、
　これで足りないならまだaを進めるし、足りるなら止めて、
　現在のカウントとKの差分から、何番目のbを使うかを決める。
　これでcも一意になるので、答えが出せる。
*/
...
void solve() {
    ...
    auto dp = list2d(M+1, N*M+1, 0LL);
    dp[0][0] = 1;
    rep(i, M) {
        Accumulate<ll> acc(dp[i]);
        rep(j, N*M+1) {
            // rep(k, max(j-N, 0LL), j) {
            //     dp[i+1][j] += dp[i][k];
            // }
            dp[i+1][j] += acc.query(max(j-N, 0LL), j);
        }
    }
