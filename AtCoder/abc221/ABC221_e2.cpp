/*
参考：https://twitter.com/titia_til/status/1444298109836619776
　　　https://twitter.com/bayashiko_kpr/status/1444297784375459840
・自力ならず。。悔しい。
・座圧してBIT、区間の積
・全然この方針に向かえてなかった。でもこういう方針に行きかけた時、
　自分より手前でかつ小さい要素の個数は分かっても、個々の要素の位置によって増える通り数は異なるし、
　取れるindexは区間にもならずにバラバラなので、これじゃあまとめられないと思ってしまった。
・右端を軸に進めていくとすると、自分の処理が終わった後に自分を左端候補としてBITなりに詰めれば、
　自分より位置が手前でかつ小さい要素の個数とかは取れる。でも今回は位置によって増える通り数が異なるので、
　このままではダメで、重みを付ける。具体的には、2冪でその位置を左端として一番右まで取り切った場合の通り数を使う。
　こうすると、右端側でまとめて取った時には、全部が一番右まで取り切ったものとしての和が取れるので、
　区間和で自分より後ろを引く時みたいな感じで、2冪は積なので区間積とみなせて、
　自分より後ろで割るということをやる。これを全部の位置でやればOK。
・連続した区間じゃなくても、その位置の分の更新の時に一番右までの体で重み付けして、
　右端の時にまとめて後ろ切って帳尻合わせる感じのやつは前にもやったことがあったので、
　これは気付いてちゃんと解けるようになりたいな。
*/
...
void solve() {
    ...
    rep(i, N) {
        if (i > 0) {
            mint sm = bit.query(0, A[i]+1);
            ans += sm / pow2[N-i];
        }
        bit.add(A[i], pow2[N-i-1]);
    }
    print(ans);
}
