/*
参考：https://twitter.com/keijak/status/1444302023931871233
・keijakさんの別解。こっちのがデータ構造強めだけど考察浅めで解けるかもしれない。
・遅延セグ木、区間和取得・区間乗算・区間更新
・indexが進む毎に、左端候補は全て等しく*2されると考えていいので、
　全体を*2、1点で+1、区間和取得の3つができれば操作が陽にできて、余分な右を割り戻すみたいな考察は要らなくなる。
　で、これはしばらく前に作った区間乗算モノイドが使える。
*/
...
void solve() {
    ...
    A = cp.zip(A);
    ll M = cp.size();
    auto seg = get_lazy_segment_tree(M, f, g, h, T, E);
    mint ans = 0;
    rep(i, N) {
        ans += seg.query(0, A[i]+1);
        // 全体に*2
        seg.update(0, M, {2, 0, 0});
        // 位置A[i]に+1
        seg.update(A[i], {1, seg[A[i]]+1, 1});
    }
    print(ans);
}
