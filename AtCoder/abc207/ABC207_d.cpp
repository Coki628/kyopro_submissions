/*
参考：https://blog.hamayanhamayan.com/entry/2021/06/27/005703
・ABC高難度埋め
・自力ならず。。
・幾何、平行移動、回転移動
・やっぱりはまやんさん方針は考え方が近いことが多い。今回も参考にさせていただいた。
　ソートして一致判定とか、まあ割と自分の方針で近いところまで来ていたんだけど、細部が詰めれず。
・角度取得時の座標の向きを勘違いしてたっぽいので、分かりやすくなるようコメントを修正した。
・さて、まずはA,Bから基準の点を1つ(pa1,pb1とする)決め打ってそこを同じ位置に来るペアとみなして平行移動をやる。
　以後、この差分diffを使ってB側の座標を動かして話を進めていく。
・回転移動について、候補の角度を絞り込みたい。360度を1度ずつやったり、座標-10～10全探索したりと脳筋なWAを積み上げる。。
　結局Bのある座標はAのある座標の位置に来る訳で、Aから基準点以外の座標を適当に1つ(pa2とする)決め打って、
　Bの基準点以外の座標全部について(pb2とする)、pa2になるものとしてどのくらい回転するか角度を取る。
　これを適切にやると、やっと無限WA編が終わってTLEになった。ここまで大変だった。。
・さらに角度の候補を絞り込む。pa1,pa2に対応するものがpb1,pb2になるのであれば、それらの距離は同じになるはずである。
　(これ最初に考察で気付いてたのに、ガチャガチャ実装してる間になんか抜けちゃってたんだよね。。)
　よってそれぞれ距離を取っておいて、距離が一致するものだけ角度の候補に追加する。これで無事AC。
*/
...
void solve() {
    ...
    rep(i, N) {
        auto pa1 = A[i];
        auto pa2 = A[(i+1)%N];
        double dista = pa1.abs(pa2);
        rep(j, N) {
            // A[i]とB[j]が対応するものとして平行移動を決める
            auto diff = B[j]-A[i];
            auto pb1 = B[j]-diff;
            vector<double> degs;
            rep(k, N) {
                if (k == j) continue;
                // 辺(pa1,pa2)と(pb1,pb2)が対応するものとして回転を決める
                auto pb2 = B[k]-diff;
                double distb = pb1.abs(pb2);
                // (pa1,pa2)と(pb1,pb2)の長さが違えばそもそもありえない
                if (abs(dista-distb) < EPS) {
                    degs.eb(get_degree(pb2, pa1, pa2));
                }
            }
            for (double deg : degs) {
                vector<Point<double>> B2;
                rep(k, N) {
                    auto pb2 = B[k]-diff;
                    pb2 = rotate(pa1, pb2, deg);
                    B2.eb(pb2);
                }
                sort(ALL(B2));
                bool ok = 1;
