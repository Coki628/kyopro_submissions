/*
・なんとか自力AC！きつかった。。
・最初、実数っての見てなくて、整数だと思ってずっとバグらせてた。
　サンプルで、(2,3)とか区間なしだろーって思ってて、すげー悩んだ後気付いた。
・気付いた後も、しばらくはどうするか方針が立たなくて、
　なんとか2倍すれば整数のまま行けるっていう所に辿り着いてAC。。
・あとこれは自分のせいだけど、昔作った区間マージのやつが半開区間か閉区間か
　分からなくて困ってた。あとでライブラリ落ち着いて見てみたら、
　ちゃんとそっちに書いてる方は閉区間って書いてあった。。
　今回ライブラリから見つからなくて、適当に使ったとこから取ってきたんだよね。。
*/
...
void solve() {
    ...
    rep(i, N) {
        ...
        rep(j, i+1, N) {
            auto [l2, r2] = LR[j];
            ll len = distinct(LR[i], LR[j]);
            if (r1-l1+1+r2-l2+1 != len) ans++;
        }
    }
    print(ans);
}
