/*
参考：https://bakamono1357.hatenablog.com/entry/2021/03/29/234016
・自力ならず。後日upsolveした。
・公式解説の説明がよく分からず、shiroさんの解説記事がよく分かった。
・数え上げDP、部分和DP、ビット毎に見る、MOD数え上げ
・分かってしまえば典型知識で解ける問題。悔しいー。
・まず、ビット毎にN個の要素を見る。すると、全体の総和でxorを0にするには、
　どのビットもN個全体で0になっていないといけない。単体のビットで見れば、
　xorが0になるには、偶数個のビットが立っていればいい。
　逆にそうなっていれば、何だって条件を満たす。
　であれば、x=0,2,4,6..とNまで順に見ていって、N個からx個を選ぶ通り数、
　が毎回増えていくのが分かる。
　これを和が分かるように、M以下の和を状態に持ってDPしていけばいい。
　そのビットが全体でx個立った時、和がどのくらい増えるかは計算できる。
　最後のビットまで見終わった時、和の状態がMになっている通り数が答え。
*/
...
void solve() {
    ll N, M;
    cin >> N >> M;
    ll MXLOG = bit_length(M);

    ModTools mt(MOD, N+1);
    auto dp = list2d(MXLOG+1, M+1, (mint)0);
    dp[0][0] = 1;
    rep(i, MXLOG) {
        rep(j, M+1) {
            if (dp[i][j] == 0) continue;
            rep(k, 0, N+1, 2) {
                if (j+(1LL<<i)*k > M) break;
                dp[i+1][j+(1LL<<i)*k] += dp[i][j]*mt.nCr(N, k);
            }
        }
    }
    mint ans = dp[MXLOG][M];
