...
・なんとか自力AC。。
・部分和DP、復元
・やばい。使った要素数も持って部分和DPして、
　mod 200で2通り以上ある場所があれば頑張って2通り復元、
　ってやったんだけど、通され過ぎじゃない？
・やっぱりだった。想定解は鳩の巣原理で秒殺。
*/
...
void solve() {
    ...
    const ll M = 200;
    auto cnt = list3d(N+1, N+1, M, (mint)0);
    auto has = list3d(N+1, N+1, M, false);
    cnt[0][0][0] = 1;
    has[0][0][0] = 1;
    rep(i, N) {
        rep(j, N+1) {
            rep(k, M) {
                if (has[i][j][k]) {
                    ...
                    cnt[i+1][j+1][(k+A[i])%M] += cnt[i][j][k];
                    has[i+1][j][k] = has[i][j][k];
                    has[i+1][j+1][(k+A[i])%M] = has[i][j][k];
                }
            }
        }
    }
    ...
    rep(i, 1, N+1) {
        rep(j, M) {
            if (cnt[N][i][j] >= 1 or has[N][i][j] and cnt[N][i][j] == 0) {
                V[j].pb(i);
                if (cnt[N][i][j] >= 2) V[j].pb(i);
            }
        }
    }
    rep(x, M) {
        if (V[x].size() >= 2) {
            ...
            while (1) {
                if (has[i-1][j-1][(offset+k-A[i-1])%M]) {
                    ans1.pb(i);
                    k = (offset+k-A[i-1])%M;
                    i--;
                    j--;
