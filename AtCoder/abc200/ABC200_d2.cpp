/*
・部分和DP、復元
・終了後、ちょっと整理。鳩の巣気付かないにしても、
　このくらいのDP復元をさくっと書けるようになりたいね。。
*/
...
void solve() {
    ...
    const ll M = 200;
    // 1つも取らないケースだけ省きたいので、1つ以上取ったかを状態に持つ
    auto dp = list3d(2, N+1, M, 0LL);
    dp[0][0][0] = 1;
    rep(i, N) {
        rep(j, M) {
            dp[0][i+1][j] += dp[0][i][j];
            dp[1][i+1][j] += dp[1][i][j];
            dp[1][i+1][(j+A[i])%M] += dp[0][i][j];
            dp[1][i+1][(j+A[i])%M] += dp[1][i][j];
            // 2以上かどうかが分かればいいので2とchminする
            chmin(dp[0][i+1][j], 2LL);
            chmin(dp[1][i+1][j], 2LL);
            chmin(dp[1][i+1][(j+A[i])%M], 2LL);
        }
    }
    ...
    ll i = N;
    ll j = s;
    const ll offset = 20000000000;
    // なるべく後半で取ったような遷移を復元
    while (1) {
        if (dp[0][i-1][(offset+j-A[i-1])%M]) {
            ans1.pb(i);
            j = (offset+j-A[i-1])%M;
            i--;
            break;
        } elif (dp[1][i-1][(offset+j-A[i-1])%M]) {
            ans1.pb(i);
            j = (offset+j-A[i-1])%M;
            i--;
        } elif (dp[1][i-1][j]) {
    ...
    while (1) {
        if (dp[1][i-1][j]) {
            ...
            j = (offset+j-A[i-1])%M;
            i--;
        } elif (dp[0][i-1][(offset+j-A[i-1])%M]) {
            ans2.pb(i);
            j = (offset+j-A[i-1])%M;
            i--;
            break;
