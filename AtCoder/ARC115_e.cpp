/**
 * ・これはN^2の愚直解。これ書いて大きいサンプル合ったから、
 * 　俄然やる気が出てこの先のセグ木整備作業に踏み切った。
 */
...
// 区間和取得・区間加算
auto f = [](pair<mint, mint> a, pair<mint, mint> b) -> pair<mint, mint> { return { a.first+b.first, a.second+b.second }; };
auto g = [](pair<mint, mint> a, mint b) -> pair<mint, mint> { return { a.first + a.second*b, a.second }; };
...
void solve() {
    ...
    vector<pair<mint, mint>> B2(M);
    rep(i, M) B2[i] = {0, B[i]};

    auto dp = list2d(N+1, M, (mint)0);
    rep(i, 0, zp[A[1]]) dp[1][i] = 1*B[i];
    rep(i, 2, N+1) {
        mint sm = 0;
        rep(j, M) sm += dp[i-1][j];
        if (i%2 == 1) print(sm*-1);
        ...
        ll mi = zp[A[i]];
        rep(j, 0, mi) dp[i][j] = dp[i-1][j] + (sm*-1)*B[j];
        rep(j, mi, M) dp[i][j] = 0;
    }
    mint ans = sum(dp[N]);
    if (N%2 == 0) ans *= -1;
