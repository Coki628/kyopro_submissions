/*
参考：https://www.youtube.com/watch?v=TE_FUxWYfRA&t=1219s
　　　https://kotatsugame.hatenablog.com/entry/2021/07/20/041638
・自力無理。大変だった。考察過程はかつっぱさんの配信と、こたつがめさんのブログから。
・繰り上がりのある桁DP
・数列に使われる要素数が最大でもさほど多くないことが察せると、
　要素数を固定してDPする方針が見えてくる。
・繰り上げを状態管理したいので、下から桁DPする。
　ゼロが許されないので、要素数は広義単調減少で進めて、
　その桁で繰り上げる遷移としない遷移をする。
　その時の要素数Kから、値が全1=K〜全3=K*3まで範囲なら遷移させるとする。
・基本的には上記で進めればいいが、値が0に繰り上がってくると9になるとか、
　その時遷移できるなら+10してなくても繰り上げありの状態になるとか、
　最後の到達判定で最後の1つ手前に繰り上げありで到達できてて最後の桁が1なら、
　これは例外的に到達OKが判定にしたりとか、まあ色々考慮しなきゃな部分は多い。。
　ソース内コメントも参照。
*/
...
void solve() {
    ...
    ll K = 1;
    while (1) {
        // dp[i][j] := 下からi桁目まで見て、要素をj個使って到達可能かどうか
        auto dp0 = list2d(N+1, K+1, false);
        auto dp1 = list2d(N+1, K+1, false);
        dp0[0][K] = 1;
        rep(i, N) {
            ll a = toint(S[i]);
            rep(j, 1, K+1) {
        ...
        // 終了1つ手前に繰り上げで到達していて、最後が1なら例外的にOK(最後の0なので)
        if (sum(dp0[N]) or (S[N-1] == '1' and sum(dp1[N-1]))) {
            print(K);
            break;
        }
        K++;
