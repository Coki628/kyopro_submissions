/*
参考：https://www.youtube.com/watch?v=3U_N7zelnMM
　　　https://blog.hamayanhamayan.com/entry/2019/10/20/001048
・ABC高難度埋め
・解説PDF見ても意味分からんだったけど解説動画見たら理解できたので解いた。
　やっぱり数式で書かれるだけだと入って来ないんだよな。。
・式を考える、二分探索、調和級数
・まず最初の式変形？みたいなアプローチが出ないことには勝負できない。
　でもこれが思い浮かぶかと言うと、微妙だよね。。
・操作回数xを決め打つと、各要素についてどんなに多くてもx回以下しか使えないとなり、
　選べる要素の合計個数としては Σmin(A[i], x) 以下に限られる。
　で、操作回数がx回だと実際に選び出したい合計個数としては、
　Kを一度に選ぶ個数だとするとK*x個欲しくて、さっきのと合わせると、
　Σmin(A[i], x) >= K*x が満たされるなら達成可能だろうとなる。
　言われると確かにそうかもなんだけど、何か自力で導出できないよねーこれ。。
・で、これが分かると、後はΣmin(A[i], x)が何とかlogくらいの計算量で求まればいける状態になり、
　これは累積和を取っておくと、x未満の要素の和+x以上の要素の個数*x
　みたいなのが二分探索で求まるので、結局各Kについてlog^2*Nで答えが求まる。
・ちょっとだけ別解。はまやんさんの記事から。
　さっきは各kで最大値をにぶたんで探してたけど、実は各kについて最大値は絶対N/k以下なので、
　これを使って探索範囲を狭めれば、ここが調和級数のlogになるので、
　単調性を気にせず全探索でいける感じになる。
*/
...
void solve() {
    ...
    rep(k, 1, N+1) {
        rep(x, N/k+1) {
            ll i = bisect_left(C, x);
            ll cnt = acc.query(0, i)+x*(N-i);
            if (cnt >= x*k) {
                chmax(ans[k-1], x);
            }
        }
