...
・自力ならず。。惜しいとこまでやったけど。
・互いに素、構築
・最初、でかい素数1個使って、残りを2*3の倍数に、とかやろうとしたら
　2500個に全然足りなくて、色々考えて、2*3,3*5,5*2をベースにして
　そこに適当に掛けていけば何とか足りそう、ってやった。
　これが整ったのが終了7分前くらいで、通っても緑パフォくらいだったしno sub。
　終了後投げたらWAだったし。。で、何がダメだったか。
　結局TLの話とか見ちゃったんだけど、適当に掛けたから同じ数字が混ざってたんだね。
　出現済管理ちゃんとやるようにしたら無事AC。。
*/
...
void solve() {
    ...
    ll i = 3;
    ll x = 2;
    while (i < N and 6*x <= MX) {
        while (used.count(6*x)) x++;
        if (6*x > MX) break;
        ans[i] = 6*x;
        used.insert(6*x);
        i++;
    ...
    x = 2;
    while (i < N and 10*x <= MX) {
        while (used.count(10*x)) x++;
        if (10*x > MX) break;
        ans[i] = 10*x;
        used.insert(10*x);
        i++;
    ...
    x = 2;
    while (i < N and 15*x <= MX) {
        while (used.count(15*x)) x++;
        if (15*x > MX) break;
        ans[i] = 15*x;
        used.insert(15*x);
        i++;
