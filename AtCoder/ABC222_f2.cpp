/*
・終了後、20分くらいでAC。まじでDでもたつかなければ。。
・頂点拡張、全方位木DP
・到着地分のコストDが厄介。このままだと全方位に落とせないので工夫する。
　自分がやったのは、頂点拡張して、その街の先にコストDかけて別の街に行くと考える。
　これなら単にある街から最大距離の街を調べればいいだけになるので全方位に落とせる。
・ただしこれでダメなのがあって、ある街のコストDが極端に大きいと、他のどの街に行くよりそこが最大になってしまう。
　これをどうにかする。この場合は 最大コスト = D[i] になっているはずなのでそれで判定して、
　該当の拡張した辺だけ外してもう1回全方位する。こうなってる頂点は(多分)1個しかないはずなので、間に合う。
　(他にもDが大きいのがあれば、お互いにそこが最大値になって自分のところではなくなるはずなので)
・ちょっと別解。今回は大丈夫だけど、もし辺コストが0とかがあると破綻しそうな気がしたので。
　最大値と一緒に、2番目も持っておけば、最大値にダメなやつが入ってたら2番目を使えばいい。
*/
...
void solve() {
    ...
    auto rr = get_rerooting(N*2, f1, f2, mkp(0LL, 0LL), 0LL);
