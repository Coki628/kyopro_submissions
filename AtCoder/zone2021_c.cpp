...
・自力ならず。まあ時間もなかったんだけど。
・二分探索、最小値の最大化、ビットで管理
・実はこの問題の条件だと最大値がどこまで高いかは興味がなく、
　最終的なボトルネックとなりうる最小値に絡むかどうか、が論点になる。
・これは二分探索で「ある能力値がx以上かどうか」を決め打つといい。
　2値が5種類あるだけになるので、重複を消して大きく数が減って、
　3人選ぶ全探索を余裕を持ってできるようになる。
　チェックとしては、3人でorを取って誰か1人でも満たしてればOK、
　の形で、5要素全部を満たす場合があるかを見ればいい。
*/
...
void solve() {
    ...
    ll ans = bisearch_max(0, INF, [&](ll m) -> bool {
        set<ll> se;
        for (auto &li : A) {
            ll cur = 0;
            rep(i, 5) {
