/*
参考：https://blog.hamayanhamayan.com/entry/2020/06/27/224202
・ABC高難度埋め
・これは自力解きたかったけど、まあしょうがない。繰り上がる桁DPはまだちょっと慣れてない。
・繰り上がりも繰り下がりもある桁DP
・とりあえず何とかビット毎に処理していけないか考えたんだけど、ダメだった。。
　上からやろうとして、でも繰り上がりで決めた値変わっちゃう、、ってなった。
　下からも考えたけど、どっちにしろそのビットを動かした時に下位ビットも
　ガチャガチャ変わっちゃうように思えて、でも例えば3ビット目ならちょうど4とか、
　ちゃんとビットに合わせた値を渡すようにすれば下位ビット影響ないね。。
　繰り上がりと繰り下がりがあるけど、これは両方上位ビットの操作で解決できる。
　多少ゴチャゴチャするけども、10進数の桁DPでの繰り上がりよりはこっちのが分かりやすいね。
*/
    ...
    rep(i, M) {
        rep(down, 2) {
            rep(up, 2) {
                ...
                // このビットを使わない
                if ((b0^b1^b2) == 0) {
                    chmin(dp[i+1][down2][up2], dp[i][down][up]);
                }
                // このビットを使う
                b0--;
                if (b0 < 0) {
                ...
                if ((b0^b1^b2) == 0) {
                    chmin(dp[i+1][down2][up2], dp[i][down][up]+(1LL<<i));
                }
            }
        }
    }
    ll ans = (dp[M][0][0] < a0) ? dp[M][0][0] : -1;
    print(ans);
}
