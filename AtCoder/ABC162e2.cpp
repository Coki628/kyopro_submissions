/*
・主客転倒、約数系包除
・ABC206_eでやったのでこちらも復習。
　この時は全然包除の意識とかなかったけど、こうして見ると見通しがよくなるね。
・K^N種類の数列のGCDを求めるのは数が多すぎるので、
　GCDを先に決めて、そうなる数列がいくつあるかを数える方針を取る。
・GCDがある値の倍数ならOK、とすれば、使える数がGCDの倍数だけになって、
　その中なら好きに何でも使えるのでこれは冪乗ですぐ出せる。
　これを上から決めていけば、約数系包除で自身の倍数が決めた通り数を引いていけば、
　その数の通り数が決めていける。
*/
