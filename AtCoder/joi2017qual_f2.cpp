/*
・E8さん精選100+50問埋め：拡張ダイクストラ
・自力AC！けど思ったより時間かかりすぎ。。
・温度の状態も持ってダイクストラする。
　最初片側だけでやろうとしちゃって合わなかった。
　暑い側にいる時と寒い側にいる時は違う状態として持たなきゃダメだよね。。
　ということで、Xをニュートラルにして0〜X*2までで動かすようにした。
*/
...
template<typename T>
vector<vector<T>> dijkstra(vector<vector<pair<ll, T>>> &nodes, int src, vector<ll> &A, ll X) {

    int N = nodes.size();
    auto res = list2d(N, X*2+1, INF);
    priority_queue<tuple<T, ll, ll>, vector<tuple<T, ll, ll>>, greater<tuple<T, ll, ll>>> que;
    res[src][0] = 0;
    que.push({0, 0, src});
    ...
    while(!que.empty()) {
        ...
        for (auto p: nodes[u]) {
            ...
            if (A[u] == 1) {
                ...
                if (A[v] == 1) {
                    if (x <= X) {
                        if (dist+cost < res[v][min(x+cost, X)]) {
                            res[v][min(x+cost, X)] = dist+cost;
                            que.push({dist+cost, min(x+cost, X), v});
                        }
                    } else {
                        if (dist+cost < res[v][max(x-cost, X)]) {
                            res[v][max(x-cost, X)] = dist+cost;
                            que.push({dist+cost, max(x-cost, X), v});
                        }
                    }
                // 1 -> 0
                } elif (A[v] == 0) {
                    if (x-cost <= X) {
                        if (dist+cost < res[v][0]) {
                            res[v][0] = dist+cost;
                            que.push({dist+cost, 0, v});
                        }
                    }
                // 1 -> 2
                } else {
                    if (x+cost >= X) {
                        if (dist+cost < res[v][X*2]) {
                            res[v][X*2] = dist+cost;
                            que.push({dist+cost, X*2, v});
                        }
                    }
                }
            } elif (A[u] == 0) {
