/*
参考：https://babcs2035.hateblo.jp/entry/2019/05/03/163531
・その他埋めAC数上げ
・自力ならず。。これは悔しい。
・期待値DP、期待値の線形性
・期待値DPっぽいのも、後ろから決めてくのも、10冪でずらしてくのも分かってたのに詰め切れなかった。
　そこより前の桁数の添字が要らなかった。今見てる場所の桁をずらすのではなく、
　そこまでやってきた方の桁をずらす気持ちになると、今見てる場所の桁数でずらせばよくなるので要らない。
　できてみればこれはよく見る桁操作の形じゃんねー。。はぁー。。
*/
...
void solve() {
    ...
    auto rec = [&](auto&& f, ll i) -> mint {
        ...
        mint res = 0;
        rep(j, sz[i]) {
            res += f(f, i-1)*pow(10, tostr(A[i][j]).size(), MOD) + A[i][j];
        }
        res /= sz[i];
        memo[i] = res;
        visited[i] = true;
