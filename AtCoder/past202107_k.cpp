...
・ダイクストラ応用
・コスト2種類持ってダイクストラする。
　ライブラリの中身いじるとこでやたらバグらせて時間かけたのは良くない。。
*/
...
vector<T> dijkstra(vector<vector<pair<ll, T>>> &nodes, int src) {
    ...
    while(!que.empty()) {
        ...
        for (auto [v, cost] : nodes[u]) {
            if (dist.first+cost.first < res[v].first) {
                res[v] = {dist.first+cost.first, dist.second+cost.second};
                que.push({res[v], v});
            } elif (dist.first+cost.first == res[v].first and dist.second+cost.second < res[v].second) {
                res[v] = {dist.first+cost.first, dist.second+cost.second};
                que.push({res[v], v});
            }
        }
...
void solve() {
    ...
    cin >> N >> M;
    auto A = LIST(N);
    vector<vector<pair<ll, pll>>> nodes(N);
    rep(i, M) {
        ll u, v, c;
        cin >> u >> v >> c;
        u--; v--;
