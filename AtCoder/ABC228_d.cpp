...
・自力AC！
・クエリ系、セグ木にぶたん
・ちょっと問題が解釈しづらい。でもようはあるi以降で-1となるような最小indexが取れれば勝ち。
　これはセグ木にぶたんで取れる。円環なので2周作っとく。と思ったらなんかWA。。
　結局、操作をminにするところmaxにしてたのが原因。しょーもない。。
*/
...
void solve() {
    ...
    rep(_, Q) {
        ...
        if (op == 1) {
            ll i = x%N;
            ll j = seg.find_first(i, [&](ll a) { return a <= -1; });
            j %= N;
            seg.update(j, x);
            seg.update(j+N, x);
        } else {
