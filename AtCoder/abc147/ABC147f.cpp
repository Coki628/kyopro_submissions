/*
参考：https://blog.hamayanhamayan.com/entry/2019/12/13/214706
　　　https://noimin.hatenablog.com/entry/2019/12/25/234455
・ABC高難度埋め
・等差数列、MOD毎に分ける
・大変だった。はまやんさん記事とのいみんさん記事を合わせて読んでやっと汲み取れた感じ。
　MODで分けた後、ようは区間Setで重複なく数えようなんだってことが分かったら理解が進んだ。
・まず、総和が決まっているものを取り合うので、相手方の値は気にせず
　自分の値がとりうる通り数だけ気にすればいいと気付くと少し見通しが良くなる。
・続いて重複を除く方法を検討。公差Dの等差数列なので、mod Dで分けると、
　分けた別グループ間では重複がありえなくなり、しかもこれは/Dすることで
　Dを1とみなした世界での区間みたいにできるので、だいぶ扱いやすくなる。
　区間であれば、両端だけ分かれば区間Setに突っ込むと高速に重複削除で数えられる。
・そしてどういう時mod Dが一致するかと言えば、選び出した個数が同じなら
　どれを選んでも常にmodが一致する。modがずれるのは初項の部分だけで、
　初項部分が何個つくかは選んだ個数のみで決まるから。
・よって選んだ個数を決めて全探索する。この時大きい方からと小さい方からで全部選べば
　それが最大と最小で、間は適当に持ちかえれば全部達成できるのでこれが区間になる。
　これを区間Setに詰めながら、増加分を数えていけばOK。
*/
...
void solve() {
    ...
    rep(a, N+1) {
        ...
        ll mod = modulo(X*a, D);
        // 小さい方からa個取った場合
        ll mn = floor(arithmetic_sequence_sum(X, X+D*(a-1), a), D);
        // 大きい方からa個取った場合
        ll mx = floor(arithmetic_sequence_sum(X+D*(N-a), X+D*(N-1), a), D);
        ans += rs[mod].insert(mn, mx);
    }
    print(ans);
}
