/**
 * 参考：https://img.atcoder.jp/ddcc2020-qual/editorial.pdf
 * 　　　https://twitter.com/OxOmisoCyan/status/1258779482854318080
 * ・E8さん精選100+50問埋め：数学的な問題
 * ・自力ならず。。
 * ・まず、順番はどうくっつけても変わらない。これは色々試してたら思った。
 * 　で、どんな方法でもいいから全部を効率よくマージできるかというと、それはできなかった。
 * 　過去の自分が気合いで解いててびっくりした。今回は負けたわ…。
 * ・公式解はまじでエレガント。こういうのほんと出来る気がしないよね。。
 * 　桁が減る時のマージは値が変わらず、減らない時のマージは減る値が必ず9。気付かなかったなー。
 * 　で、桁和を/9の前に-1してるのがよく分からなかったけど、普通に割る9すると、
 * 　9ぴったりの時にもう1回操作できることになってしまうので、
 * 　今回は1桁になったら終了だからその調整をやってるんだね。
 */
