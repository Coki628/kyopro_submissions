...
・さらにkaichouさんが提示してくれた類題abc234_fとの関連を考えた。
　すると、j個ある状態からj+c個に遷移させる時にnCr(j+c, c)を掛けるといい、
　みたいな遷移の仕方が同じってことを言ってるのが分かった。
　実際、遷移の時階乗で割って最後に階乗掛けるってやらずに、
　上記のような二項係数を毎回の遷移で掛けても同様の答えになったので、なるほどーと思った。
　最後に階乗掛けるよりもこっちのが直感的な気がした。
*/
        ...
        rep(j, K+1) {
            rep(k, M+1) {
                ...
                rep(l, K+1) {
                    ...
                    dp[i+1][j+l][k+(l > 0)] += dp[i][j][k]*(W[i]/total).pow(l)*mt.nCr(j+l, l);
                }
            }
        }
    }
