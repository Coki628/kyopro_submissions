/*
参考：https://atcoder.jp/contests/abc130/editorial
・ABC高難度埋め
・むずい。
・まず各値がどう変化するのかを考えると、凸っぽくなる。
　(これがそもそも自力では見抜けず)
　で、この区間の状態が変化する区切りだけ見る。するとそのどれかが答えになる。
・区切り位置の判定は二分探索で見つける。
・これは小数のままやっててWA。誤差かなー？
　比較する差分を1から0.5にしたらサンプルは通ったんだけどね。。
*/
...
void solve() {
    ...
    ld x_static_mn = INF, x_static_mx = -INF, y_static_mn = INF, y_static_mx = -INF;
    ld x_toleft_mn = INF, x_toleft_mx = -INF, y_todown_mn = INF, y_todown_mx = -INF;
    ld x_toright_mn = INF, x_toright_mx = -INF, y_toup_mn = INF, y_toup_mx = -INF;
    rep(i, N) {
        ld x, y;
        char d;
        cin >> x >> y >> d;
    ...
    vector<ld> T;
    ld t;
    t = bisearch_max(0, INF, [&](ld m) {
        return get_x_mn(m) < get_x_mn(m+diff);
    }, 100);
    if (t < INF) T.pb(t), T.pb(t+diff);
    t = bisearch_min(0, INF, [&](ld m) {
        return get_x_mn(m) > get_x_mn(m+diff);
    }, 100);
    if (t < INF) T.pb(t), T.pb(t+diff);
    t = bisearch_max(0, INF, [&](ld m) {
        return get_x_mx(m) > get_x_mx(m+diff);
    }, 100);
    if (t < INF) T.pb(t), T.pb(t+diff);
    t = bisearch_min(0, INF, [&](ld m) {
        return get_x_mx(m) < get_x_mx(m+diff);
    ...
    if (t < INF) T.pb(t), T.pb(t+diff);
    t = bisearch_max(0, INF, [&](ld m) {
        return get_y_mn(m) < get_y_mn(m+diff);
    }, 100);
    if (t < INF) T.pb(t), T.pb(t+diff);
    t = bisearch_min(0, INF, [&](ld m) {
        return get_y_mn(m) > get_y_mn(m+diff);
    ...
    if (t < INF) T.pb(t), T.pb(t+diff);
    t = bisearch_max(0, INF, [&](ld m) {
        return get_y_mx(m) > get_y_mx(m+diff);
    }, 100);
    if (t < INF) T.pb(t), T.pb(t+diff);
    t = bisearch_min(0, INF, [&](ld m) {
        return get_y_mx(m) < get_y_mx(m+diff);
    ...
    ld ans = INF;
    for (ld t : T) {
        chmin(ans, (get_x_mx(t)-get_x_mn(t))*(get_y_mx(t)-get_y_mn(t)));
    }
    print(ans);
}
