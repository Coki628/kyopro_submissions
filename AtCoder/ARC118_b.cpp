/*
参考：https://atcoder.jp/contests/arc118/editorial
・自力ならず。。
・最大値の最小化、小数の二分探索、式変形
・差分として許せる上限xを決め打つと、答えの数列に使える範囲が定まる。
　最小は Ai/N-x <= Bi/M, 最大は Bi/M <= Ai/N+x となり、
　これをBiについて解いて、M*Ai/N-M*x <= Bi, Bi <= M*Ai/N+M*x となる。
・このBiの最小と最大を使って、まず最小を割り当てておいて、
　総和がM以下の範囲で、前から足せるだけ足していく、という貪欲を行い、
　最後までやってM個全部使い切れればOK。といった判定の二分探索になる。
・これでいいはずなんだけどWA。。誤差？EPS使ってみたけど変化なし。
　そもそも式変形ミスってる？分からん。
　上限小さい？10億くらいにしたけど変化なし。
・なんとかACした。最小を作った時に既に使った個数がM超になる場合があるっぽくて、
　これを即NGにしたら通った。。
*/
...
void solve() {
    ...
    ld x = bisearch_min(0, 1, [&](ld x) {
        vector<ll> mx(K), mn(K);
        rep(i, K) {
            mx[i] = floor(M*A[i]/(ld)N+M*x);
            mn[i] = ceil(M*A[i]/(ld)N-M*x);
        }
        ll cnt = M-sum(mn);
