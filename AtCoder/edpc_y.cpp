/*
・できそうな気がして取り組んでみたけど無理だった。。ひとまず撤退。
・座圧して、空白部分を二項係数でまとめて遷移させればいいと思ったんだけどうまくいかず。
・方針としては、斜め上から二項係数で遷移させて、重複する分を縦横から引くみたいにした。
　最後のサンプルが合わず。
*/
...
void solve() {
    ll H, W, N;
    cin >> H >> W >> N;
    vector<ll> rows = {0, 1, 2, H}, cols = {0, 1, 2, W};
    vector<pll> RC;
    rep(i, N) {
        ll r, c;
        cin >> r >> c;
    ...
    auto dp = list2d<mint>(H, W, 0);
    dp[1][1] = 1;
    rep(h, 1, H) {
        rep(w, 1, W) {
            if (h <= 1 and w <= 1 or blocked.count({h, w})) continue;
            if (blocked.count({h-1, w}) or
                blocked.count({h, w-1}) or
                blocked.count({h-1, w-1})
            ) {
                dp[h][w] += dp[h-1][w]+dp[h][w-1];
            } else {
                ll hlen = cpr.unzip(h)-cpr.unzip(h-1);
                ll wlen = cpc.unzip(w)-cpc.unzip(w-1);
                dp[h][w] += dp[h-1][w-1]*mt.nCr(hlen+wlen, hlen)+dp[h-1][w]+dp[h][w-1]-dp[h-1][w-1]*2;
            }
        }
    }
    mint ans = dp[H-1][W-1];
