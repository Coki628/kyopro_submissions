/*
参考：https://atcoder.jp/contests/abc196/editorial
・upsolveした。今日のこどふぉバチャで使ったからね。
・FFT、XOR、連続部分列
・自分ノートにも図を示したけど、FFTは片方反転させてから当てると、
　交差してたところが全部平行になるので、連続部分列でΣa*bしたみたいになる。
・この問題ではxorなので、xorを掛け算とするため、式変形して、
　a^b = a(1-b)+(1-a)b として、2つの項それぞれにFFTを当てる。
・ちなみにandなら掛け算そのまま、orなら両方反転してから(1-a)(1-b)ってやれば
　できると思う。多分。。
・で、これでFFTした2つの項を足し合わせれば答えになる。
　手前M-1個と後ろM-1個は半端な部分列を使ったやつなので、
　これらを除いたN-M+1個から答えを確認する。
*/
...
void solve() {
    ...
    reverse(ALL(T));
    vector<ll> A(N), B(M), fft(N+M-1);
    rep(i, N) A[i] = toint(S[i]);
    rep(i, M) B[i] = 1-toint(T[i]);
    auto res = convolution(A, B);
    rep(i, N+M-1) fft[i] += res[i];
    rep(i, N) A[i] = 1-toint(S[i]);
    rep(i, M) B[i] = toint(T[i]);
    res = convolution(A, B);
    rep(i, N+M-1) fft[i] += res[i];
