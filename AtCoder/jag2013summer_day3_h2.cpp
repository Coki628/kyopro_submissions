 ...
 * ・ハッシュの1文字更新メソッド作った。うまくいって嬉しい。
 * 　負数になるところをMODから引くようにしたらちゃんと動いた。
 */
...
struct RollingHash {
    ...
    // 長さlenのhashのx文字目(0-indexed)をaからbに変える
...
int main() {
    ...
    rep(i, 0, M) {
        rep(j, 0, L) {
            ...
            auto hash = rh.update(t_hash, M, i, T[i], ascii_letters[j]);
            if (C.count(hash)) {
                ans += C[hash];
            }
        }
