/*
参考：https://atcoder.jp/contests/zone2021/editorial
　　　https://www.youtube.com/watch?v=3ymJpfD1sPE
　　　https://drken1215.hatenablog.com/entry/2019/03/20/202800
・ABC系高難度埋め
・いやー、すげー大変だった。でも新しい知見が色々あって良かった。
・掃き出し法応用、線形独立
・初手の考察パートからして実は重い。。集合Bにある要素で任意にxorを取っていき、
　0~N-1までの全ての値を網羅できればいいのだが、このためには各ビットが最大ビットとして立ったようなものが必要で、
　これは掃き出し法が仕上がった時の行列の形にちょうど対応してる。(お気持ち。。)
・今まで掃き出し法は連立方程式の解のためにやると思ってたんだけど、そうじゃない時もあるのね。
　今回は行列の形としては一緒だけど、右端に右辺を作らない、元々1個の行列でやってる。
　こういう場合にextendedをfalseにするんだね。多分初めて使った。
・今回数列は10万あるので、3乗の計算量が印象にある掃き出し法は明らかに無理めに感じたんだけど、
　この3乗はO(RC^2)なので、行が大きくても、列が今回みたくビット数程度だったら使える。
・復元的なことが必要になるので、これまたライブラリそのままだとうまくいかない。
　行列の行は数列Bの要素と対応しているので、使った行とそれに対応するBの要素を覚えておく必要がある。
　この時使ったBの要素が、線形独立というものになるらしく、
　これは調べたら、他の要素を作るための最小部品たちみたいな感じっぽい。
　もっと高度なやり方もあるみたいだが、これらを使ってありうるものを全探索することでうまくいった。(証明AC)
　公式解説でN*n/2って言ってるのが多分このことなんだろう。適当な頂点を1つ取り、
　それに(選ばれた中で)適当なBの要素を1つ取ると、他方の頂点はxorで一意になる。
　この「頂点 xor 辺 = 他方の頂点」を全探索して、UFで連結判定した。
*/
...
void solve() {
    ...
    ll n = log2((ld)N);
    vector<ll> used;
    auto [rank, res] = gauss_jordan<BITLEN>(M, n, mat, B, used, false);
    if (rank != n) {
        print(-1);
        return;
    }
