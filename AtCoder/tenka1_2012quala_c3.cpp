/*
参考：https://anta1.hatenadiary.org/entry/20120803/1344008824
・試験管青diff残埋め
・自力ならず。。でもためになった。
・構文解析、補グラフ
・構文解析パートはおまけ程度で、グラフの考察がメイン。
・自力ではDFSっぽくしたり、UFで状態をまとめられないかとか考えて破滅した。
・dp[i][j] := i回辺を辿って、頂点jに到達できるか
　みたいなDPを作る。この時、遷移は辺ベースでやったけど、頂点ベースでやっても大丈夫だった。
・辺を張っていない側、いわゆる補グラフ側を遷移させたい時どうするかが問題。
　これには、遷移元としてありうる場所全てからの遷移を行って各頂点で遷移した数をカウントして、
　それが遷移元全てではないかどうかをチェックすればいい。
　1つでもその頂点に遷移してきてない遷移元があれば、補グラフではそこから遷移できるから。
・理解できたら納得だったけど、自分でいくら悩んでも出なかったかな。。
　補グラフのうまい扱い方を問うような問題はあまりやったことなかったぽい。
*/
...
void solve() {
    ...
    ll i = S.find('g')+5;
    ll j = i+1;
    while (j < sz and isdigit(S[j])) j++;
    ll u = toint(S.substr(i, j-i));
    u--;
    i = j;
    vector<ll> P;
    if (i < sz and S[i] == 'w') {
    ...
    rep(i, L) {
        if (P[i]) {
            ...
            rep(v, N) {
                // 遷移元全員からの辺がある訳ではない時は、補グラフでは必ずどこかから遷移できる
                if (C[v] < sm) {
                    dp[i+1][v] = 1;
                }
            }
