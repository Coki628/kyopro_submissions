/*
参考：https://drken1215.hatenablog.com/entry/2021/08/01/163600
　　　https://www.youtube.com/watch?v=hI8xC_1ZBf8&list=PLLeJZg4opYKb67CKDIVEDd7l-Qw1u_ln7&index=2
　　　https://manabitimes.jp/math/842
・むずい。まあ新履修だししょうがないけど。
・ABC高難度埋め
・原始根と位数、約数列挙、約数系包除
・まず、原始根と位数の説明は学んでくる必要がある。マスオさんとけんちょんさんとこ見てきた。
・つまりあるmod pの世界で冪乗を進めていくと、ちょうどp-1回で1周するものがあり、
　こうなるのがx乗ならxはmod pの原始根、ってなる。(多分)
　で、こういうxはどんな数にもあるようで、これは1~p-1までの数をちょうど1回ずつ通る。
　これを位数がp-1って言うんだけど、当然これより少ない場合もたくさんあって、
　今回の問題はこれらの位数の総和を求めるような感じになる。
　で、その他の位数はどうなっているかというと、視覚的には公式解説動画ですぬけさんが書いてくれた
　円環が分かりやすかったんだけど、p-1をフルに1ずつ円を回る感じとして、1つ飛ばしとか2つ飛ばしで
　回ったようなのが他の位数になる。この形はすごくgcdっぽいんだけど、やっぱりgcdになる。
　x=1~p-1について、辺を1~p-1本ずつ進んだものとして(順番は入れ替わるけど)1対1対応させられる。
　この時辺をx本進んだ場合の位数が、N=p-1として、N/gcd(N, x)で表せるので、これを使う。
　1~p-1まで全探索はできないが、このxはNの約数に限られ、これらをまとめられるのでそうする。
　約数列挙して、ある約数をdとして、N/gcd(N, d) * dとなるxの数、みたいにする。
　これで後はdとなるxの数が分かればいいが、これは大きい方から約数系包除で、
　N以下のdの倍数 => gcd(N, d)がdの倍数 というのを gcd(N, d)がちょうどd になるように
　自身の倍数から引いていくのをやればOK。全約数で足し合わせれば答え。
*/
...
template<typename T> void pint(const deque<T> &A) { vector<T> V(A.begin(), A.end()); print(V); }
...
void solve() {
    ...
    mint ans = 0;
    rep(i, M-1, -1, -1) {
        // N以下のdivs[i]の倍数 => gcd(N, divs[i])がdivs[i]の倍数
        C[i] = N/divs[i];
        // 自身の倍数分を引いて、gcd(N, divs[i])がちょうどdivs[i]の分だけにする
        rep(j, i+1, M) {
            if (divs[j]%divs[i] == 0) {
                C[i] -= C[j];
            }
        }
        ans += C[i] * N/gcd(N, divs[i]);
    }
    ans += 1;
    print(ans);
}
