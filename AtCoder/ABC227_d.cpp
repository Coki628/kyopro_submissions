/*
・きっちり自力AC！これは割とすんなりいけた。
・二分探索
・作りたいプロジェクト数を決め打てば、各部署から使える人数が定まる。
　それを全部足して、プロジェクト数*1つのプロジェクトの人数 に足りていればできるし、足りないなら無理。
　前に似たような考察の問題を多分解いたことがあって、適切に方針を思い出せたから良かった。
*/
...
void solve() {
    ...
    ll res = bisearch_max(0, INF, [&](ll m) {
        if (mul_overflow(m, K)) return false;
        vector<ll> B(N);
        rep(i, N) {
            B[i] = min(A[i], m);
        }
