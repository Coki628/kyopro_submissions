...
・ABC高難度埋め
・自力AC！！これは嬉しい。
・全方位木DP、主客転倒、寄与、期待値の線形性、余事象
・とりあえず部分木の期待値を求めながら木DPしてこれないかとか考えたけど全然方針立たずで、
　見方を変えて、各頂点が全体のうちどれだけ題意を満たすものに当てはまるか考えてみる。
　すると、これなら部分木のうち2つ以上で黒い頂点がある場合が該当するとみなせて、
　このままだと扱いづらいので余事象にすると、部分木に黒い頂点があるのが1つしかない場合とできて、
　これは2^部分木の頂点数から、全部白の1を引くとよく、自分は白い頂点でないといけないし、
　他の部分木も全て白い頂点である必要があるため、見てる部分木以外の状態は一意となり、
　各部分木で独立に数えていける。これは普通に木DPの遷移を書くと、各頂点を根として
　N回DFS回す感じのN^2を作ることができて、遷移の感じを見ても全方位できそう、となる。
・実際それで全方位できたんだけど、まあそれなりに大変ではあった。
　遷移の際、dp[u] -= xxx みたいにしていたが、ここが引き算だとうまくいかないので
　余事象っぽくひっくり返して足し算にしたり、DPの初期値で1とかpow(2, N-1, MOD)+1とかしていたのを、
　どうやら辺の遷移に乗せないとダメそうだとなり、初期値0で遷移側で対応するようにしたり、
　色々全方位に適用できるように整えて、仕上げた。
・全方位木DPの挙動チェックのため、f1,f2をちょっといじった。これも問題なくACする。
*/
...
template<typename F> pair<ld, ld> trisearch_min(ld lo, ld hi, const F &func, ll times) { ld m1 = lo; ld m2 = hi; rep(_, 0, times) { m1 = (lo*2+hi) / 3; m2 = (lo+hi*2) / 3; ld res1 = func(m1); ld res2 = func(m2); if (res1 <= res2) { hi = m2; } else { lo = m1; } } ld i = min(m1, m2); ld val = func(i); return {i, val}; }
template<typename F> pll trisearch_min(ll lo, ll hi, const F &func, ll offset=0) {  ll m1 = lo, l = lo; ll m2 = hi, r = hi; while (lo+2 < hi) { m1 = (lo*2+hi) / 3; m2 = (lo+hi*2) / 3; ll res1 = func(m1); ll res2 = func(m2); if (res1 <= res2) { hi = m2; } else { lo = m1; } } ll mn = INF; pll res; rep(i, max(m1-offset, l), min(m2+offset, r)) { ll val = func(i); if (val < mn) { mn = val; res = {i, val}; } } return res; }
...
vector<ll> bfs01(const vvpll &nodes, const vector<ll> &src) { ll N = nodes.size(); vector<ll> res(N, INF); deque<ll> que; for (ll s : src) { res[s] = 0; que.push_back(s); } while (!que.empty()) { ll u = que.front(); que.pop_front(); for (auto [v, c] : nodes[u]) { if (c == 0 and res[u] < res[v]) { res[v] = res[u]; que.push_front(v); } elif (c == 1 and res[u]+1 < res[v]) { res[v] = res[u]+1; que.push_back(v); } elif (c >= 2) { assert(c == 0 or c == 1); } } } return res; }
...
void solve() {
    ...
    cin >> N;
    // vvi nodes(N);
    auto rr = get_rerooting(N, f1, f2, Node({0, 0}), 0);
    rep(i, N-1) {
        ll u, v;
        cin >> u >> v;
        u--; v--;
    ...
    // cnt[i] := 頂点iが答えに寄与する通り数
    // (初期値は、自分が白頂点にある通り数-全頂点が白の1通り)
    vector<mint> cnt(N, pow(2, N-1, MOD)-1);
    // rep(i, N) {
    //     vector<ll> sz(N, 0);
    //     vector<mint> dp(N, 0);
