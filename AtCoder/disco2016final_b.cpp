/*
参考：https://www.slideshare.net/chokudai/ddpc2016final
・試験管青diff残埋め
・自力ならず。。これは解けるべきなやつだったな。悔しい。
・スケジューリング系、逆操作、プリキューで貪欲、二分探索
・プリキュー貪欲も二分探索も考えたんだけど、どうにも優先すべき要素が定まらず。
　逆操作の発想に行けなかった。確かに逆からやってけば、いわゆる見たことあるような
　プリキュー貪欲のスケジューリングに帰着できてる。。これならにぶたんもできる。
*/
...
void solve() {
    ...
    auto res = bisearch_min(0, MX, [&](ll m) {
        my_priority_queue<ll, vector<ll>> que;
        rep(i, m+1, MX) {
            for (ll a : adj[i]) {
                que.push(a);
            }
