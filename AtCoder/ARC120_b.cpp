/*
・すんなり自力AC！
・始点(終点)からの距離が同じマスは、どのルートでもどれかを通るので、
　同じになってないといけない。それが分かれば後は場合分け。
*/
...
void solve() {
    ...
    vector<set<char>> se(N*2);
    rep(h, H) {
        rep(w, W) {
            se[h+w].insert(grid[h][w]);
        }
    }
    ...
    rep(i, N*2) {
        if (!se[i].empty()) {
            if (se[i].size() == 3 or se[i].size() == 2 and not se[i].count('.')) {
                ans = 0;
                break;
            } elif (se[i].size() == 1 and (*se[i].begin()) == '.') {
                ans *= 2;
            } else {
                ans *= 1;
            }
