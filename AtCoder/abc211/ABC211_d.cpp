...
・きっちり自力AC！
・BFS応用
・BFSをちょっといじる。そこに来た通り数を持たせるようにする。
*/
...
// BFS
vector<pair<int, mint>> bfs(vvi &nodes, vector<int> src) {

    int N = nodes.size();
    vector<pair<int, mint>> res(N, {-1, 0});
    queue<int> que;
    for (int s : src) {
        res[s] = {0, 1};
        que.push(s);
    ...
    while(!que.empty()) {
        ...
        for (auto v: nodes[u]) {
            if (res[v].first == -1) {
                res[v] = {res[u].first+1, res[u].second};
                que.push(v);
            } elif (res[v].first == res[u].first+1) {
                res[v].second += res[u].second;
            }
        }
    }
    return res;
