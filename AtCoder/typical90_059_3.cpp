/*
参考：https://twitter.com/e869120/status/1401357950686429193/photo/2
・E8さん典型90問埋め
・SCC、メモ化再帰、DAG、bitset高速化、クエリ先読み
・bitsetの方針はそう遠くなかった。
　10万ビットを全部いっぺんに持つとMLEになるが、
　10万辺のDFSは線形なので結構回数を回せる。
　なので覚えてく情報を適度に区切って、その分の回数DFSを増やしても間に合う。
　いくつか試したところ、1000ビット区切り100回DFSくらいがちょうどよさそう。
　10万辺のDFSが100回回って計算量1000万くらいだけど、しっかりAC1.53秒。
*/
...
void solve() {
    ...
    vvi nodes(N2+1);
    rep(i, M) {
        nodes[cp[edges[i].first]].pb(cp[edges[i].second]);
    }
    rep(i, N2) {
        nodes[N2].pb(i);
    }
    ...
    rep(x, 0, N2, offset) {
        vector<bitset<offset>> dp(N2+1);
        vector<bool> visited(N2+1);
        auto dfs = [&](auto&& f, ll u) -> bitset<offset> {
            if (visited[u]) {
                return dp[u];
            }
            bitset<offset> res;
            if (u != N2 and x <= u and u < x+offset) res.set(u-x);
            for (ll v : nodes[u]) {
                res |= f(f, v);
            }

            for (auto [v, i] : qs[u]) {
                if (x <= v and v < x+offset and res[v-x]) {
                    ans[i] = 1;
                }
            }
            dp[u] = res;
