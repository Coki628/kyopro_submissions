/*
・自力ならず。数学おつかと思ったらにぶたんできるじゃん。。
・二分探索、式変形、誤差
・税抜価格をx、税込価格をf(x)とすると、これらの差分f(x)-xだけ、
　飛ばした数があることになる。
　そこでこの飛ばした数がちょうどNになるところを二分探索で見つければ、
　その時に飛ばした数、つまりf(x)-1が答えになる。
・なお、小数からfloorすると誤差で　3.99999みたいになってる時に死ぬので、
　整数型のまま計算するのがベター。(EPSでも対応できるとのこと)
*/
...
void solve() {
    ...
    ll x = bisearch_min(0, 1000000000000, [&](ll m) {
        return f(t, m)-m >= N;
    });
    ll ans = f(t, x)-1;
    print(ans); 
