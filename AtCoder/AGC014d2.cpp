...
・AGC青diff以下残埋め
・これ青じゃないけどね。前に蟻本演習で解けなくて保留にしたやつ。ついでにもう解説見ちゃった。
・木上のゲーム、木の完全マッチング
・葉が2つ以上分かれてると先手勝ちなのは分かるけど、両者置いたらその後、
　そこから先切って新たに葉になったと考えてよかったんだね。
・これは木の完全マッチングで、葉から貪欲にペアを決めていけば問題ないとのこと。
*/
...
void solve() {
    ...
    auto dfs = [&](auto&& f, ll u, ll prv) -> void {
        ...
        if (prv != -1 and P[prv] == -1 and P[u] == -1) {
            P[u] = prv;
            P[prv] = u;
        }
    };
    dfs(dfs, 0, -1);
    if (count(ALL(P), -1)) print("First");
    else print("Second");
}
