/*
参考：https://atcoder.jp/contests/past202010-open/editorial
・グラフに帰着、ダイクストラ
・グラフも考えたんだけど、この想定解は一生出る気がしないなぁ。
・まず、実際には開けない宝箱も、利得0円で開けるものとして、
　コストA[i]でiからi+1への辺を張る。こうすることで、
　例えばこの辺を全部通ってNに行きつくのは、コストがsum(A)ちょうど、
　つまり損益+-0となり、宝箱を何も開けずに終えたことに当たる。
　これに実際に鍵屋を使う区間を飛び越える辺と、
　重なった区間の分をコスト0で引き返せるようにする辺を追加して、
　あとはこのグラフにダイクストラして、
　出た最小コストを宝箱を全て取った利益sum(A)から引けば答え。
・列をグラフにしてダイクストラするとか、戻りのコスト0の辺を張るのとかは、
　nikkei2019-2-qual_d辺りが類題と言えそうだけど、
　利得ちょうどで1つ前に進む辺も張るっていう考察がある分、
　今回のこれのが難しいと思った。
*/
