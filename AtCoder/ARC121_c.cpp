...
参考：みんなのツイートとか
・乱択
・概ね、反転してて、できるところをやる。なければ適当にやる。でOK。
　なんだけど、小さいケースでダメなのがあって大変だった。。。
・N=4, [3,4,1,2] とか N=5, [4,5,2,3,1] とかで落ちてたぽい。
　このへんだと、選べる対象が少ないからか、同じところの繰り返しみたいな感じに、
　たまになってしまうようだ。
　で、しょうがないので個別にチェック関数作って、
　・基本的に2つ前と同じやつはやらない。
　・ただし選べるのが1つしかない場合はしょうがない。
　っていう判定にしたら通った。疲れたー。。。
*/
...
bool check(ll N, vector<ll> &ans, ll i) {
    if (i%2 == 1 and N <= 4 or i%2 == 0 and N <= 3) return true;
    bool ok = 1;
    ans.pb(i+1);
    if (ans.size() >= 3) {
        ll j = ans.size()-1;
...
void solve() {
    ...
    while (1) {
        ...
        if (ok) break;
            ...
            while (1) {
                i = randrange(0, N-1);
                if (i%2 == op and check(N, ans, i)) break;
            }
            swap(A[i], A[i+1]);
            ans.pb(i+1);
        }
