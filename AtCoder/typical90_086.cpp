/*
参考：https://twitter.com/e869120/status/1412904274900754441/photo/2
　　　https://twitter.com/e869120/status/1412904274900754441/photo/3
・E8さん典型90問埋め
・自力ならず。むずかった。
・ビット毎に見る、ビット全探索
・ビット毎に見ようとしたけど、ビット毎にしても結局内側の判定をうまく考えられなかった。
　余事象なら数えられないかとか、包除じゃないかとか疑ったけど、ことごとくハズレだったね。。
・正解は数列の長さNでビット全探索。それ自体は考えたけど、解法と結びつかなかった。
　今回の場合、条件に合うものを数えるのは難しくて、でも数列Nが12と小さいから、
　値の60ビットをビット毎に見ていれば、12個の01だからビット探索できる。
　で、このビットについてはこの2^12があり得る全てだから、
　1つずつチェックして妥当か判定すればいい。Q個全ての条件をクリアしたなら、
　答え候補の通り数に足す。各ビット同士は独立だから、最後に各ビットの通り数は掛け合わせる。
・ここまでの理屈を全部理解するのにも、解説見てからかなり時間かかったけど、
　(特に、条件に合うものを数えるんじゃなくて、全探索して、全部合うかどうかチェックする、
　で間に合うっていう発想に到らなかった。)
　そこそこ実装も大変。各値の扱いとしての60ビットと、数列Nの各位置の01を見る12ビットが
　両方出てくるので、2種類のビットが出てきていると分かっていても結構混乱した。
*/
...
void solve() {
    ...
    rep(k, MXLOG) {
        ...
        rep(bit, 1LL<<N) {
            ...
            bool ok = 1;
            rep(i, Q) {
                // 今チェックしている数列curのx,y,z番目の要素(のk番目のビット)が立っているかどうか
                ll x = cur>>X[i] & 1;
                ll y = cur>>Y[i] & 1;
                ll z = cur>>Z[i] & 1;
                // wのk番目のビットが立っているかどうか
                ll w = W[i]>>k & 1;
                // 条件を満たさないものが1つでもあればNG
                if (not ((x|y|z) == w)) {
                    ok = 0;
                    break;
                }
