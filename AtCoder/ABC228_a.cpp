/*
・うまく24足せば場合分けだけでもいけそうだったけど、
　変なコーナー踏みたくないので、ループ回して確実にした。
　ABCのAってこんな考えないとダメなやつでいいの？ってのが最近多いな。。
　って思ってたけど、どうも自動化対策らしい。
　簡単すぎると、問題文読み込ませるとコード書いちゃうみたいなやつで通っちゃうそうな。。
*/
...
void solve() {
    ...
    if (used.count(x) or used.count(x+24)) Yes();
    else No();
}
