/*
・最初に考えてた方針。遅延セグ木で殴る。区間最小値取得・区間加算で。
　適当に大きい値で初期化しておいて、取ったやつも大きい値に更新すればないことにできる。
・WAが取れなくて別方針考えて通したけど、終わってからよくチェックしたら、
　比較の不等号の向きが1箇所逆になっててバグってた。。そこ直したらAC。
・ちなみにプリキューよりは遅いけどmultisetよりは速かった。0.09秒。
*/
...
void solve() {
    ...
    cin >> Q;
    ll MX = 10000000000;
    auto seg = get_Lazy_segment_tree(Q, f, g, h, T, E);
    rep(i, Q) seg.set(i, MX);
    seg.build();
    ...
    rep(i, Q) {
        ...
        if (op == 1) {
            ...
            ll mn = seg.query(0, Q);
            print(mn);
            ll j = seg.find_first(0, [&](ll x) { return x <= mn; });
            seg.update(j, MX);
        }
    }
}
