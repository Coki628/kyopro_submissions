/*
・自力AC！なんだけどね。。
・愚直シミュした。でもこれ、DFSでオイラーツアーやる操作そのものなんだよね。
　全然気づかなかった。。これ何させたいんだろー？って思いながらシミュ書いてた。。
*/
...
void solve() {
    ...
    vector<ll> ans = {0}, prv(N, -1);
    vector<bool> visited(N);
    ll cur = 0;
    while (1) {
        visited[cur] = 1;
        while (nodes[cur].size() and visited[*nodes[cur].begin()]) {
            nodes[cur].erase(nodes[cur].begin());
        }
        if (nodes[cur].size()) {
            ll nxt = *nodes[cur].begin(); nodes[cur].erase(nodes[cur].begin());
            if (prv[nxt] == -1) {
                prv[nxt] = cur;
            }
            cur = nxt;
