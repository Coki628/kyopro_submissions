/**
 * ・なんとか自力AC。
 * ・冪乗の冪乗みたいなやつを求める。a^(b^c)はmod 10でいいとして、b^cをどうするかが問題。
 * 　普通にmod 10してもダメなんだろうなぁと思って、10*10=100でmod取ったらWA。。
 * 　じゃあ冪乗だし10^10ならどうだ、と思って10^10でmod取ったら通った。完全にマグレ…。
 */
...
void solve() {
    ...
    ll ans = pow(a, pow(b, c, 10000000000), 10);
    print(ans);
}
