/*
・試験管青diff残埋め
・きっちり自力AC！でも方針すぐ立った割にはもたついて良くなかったかな。
・Sparse Table上の二分探索
・セグ木にぶたんで行こうと思ったけど、ちょっと境界で迷ったので、Sparse Tableと自前のにぶたんでまとめた。
　やっぱ前後両方に探索しに行く時は逆に半開区間が頭壊れがち。こういう時は閉区間が見やすい。
・前後に番兵のINF置いて、後はまあやればいい。今の時代だと絶対青diffとかならなそう。。
*/
...
void solve() {
    ...
    H.eb(INF);
    auto st = get_sparse_table(H, [](ll a, ll b) { return max(a, b); });
    rep(i, 1, N+1) {
        ll res = 0;
        ll r = st.bisearch_fore(i, N+1, H[i], greater<ll>());
        res += r-i-1;
        ll l = st.bisearch_back(0, i, H[i], greater<ll>());
        res += i-l-1;
        print(res);
    }
}
