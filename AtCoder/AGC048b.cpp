/*
参考：https://atcoder.jp/contests/agc048/editorial
　　　https://drken1215.hatenablog.com/entry/2020/11/20/152323
・AGC青diff以下残埋め
・自力ならず。いやー、これぞAGCみたいなアドホック考察系。
・偶奇、ソートして貪欲
・DP高速化っぽいことをさせたそうな見た目をしていて、実はその方針だと全く無理なやつ。
　どうすれば見抜けるんだろうね。あとその先でソート貪欲でいいっていう考察も地味にそれでいいのかってなる。
・とりあえず注目すべきなのは、対になる括弧はいつでもindexの偶奇が異なるということ。
　ここからグループを2つに分けて、一番値が良くなりそうなペアから貪欲に取る。
　何が値が良くなりそうかというと、A,Bで正負逆にしたものを足し合わせれば、
　その位置がどっち側に強いかが分かるので、あるi,jペアを考える時も2つの位置でまた足し合わせれば、
　そのペアで組む時どっち側に倒した方が得か分かる。これは2つのグループを両方ソートしておいて、
　小さい方(大きい方からでもいい)から貪欲に取っていけば最適になる。
*/
...
void solve() {
    ...
    rep(i, 2) sort(ALL(BA[i]));

    ll ans = 0;
    rep(i, N/2) {
        // 2つのindexにあるA,Bの和を取って、値が強い方に決める
        ll sm = BA[0][i].first+BA[1][i].first;
        if (sm <= 0) {
            ans += A[BA[0][i].second]+A[BA[1][i].second];
        } else {
            ans += B[BA[0][i].second]+B[BA[1][i].second];
        }
    }
    print(ans);
}
