 ...
 * ・自力AC！
 * ・BFSとダイクストラ
 * ・さっきのやつ(joi2016qual_e)とだいたい同じ感じで、BFSで準備して、ダイクストラで決める感じ。
 * ・頂点5000あって、辺が最大でほぼ完全グラフまで張られるから計算量どうかなーと思ったらMLEした。
 * 　コストの最大がさほど大きくなかったので、ダイクストラの辺を全部llじゃなくintで構成したら
 * 　なんとかAC。時間は制限10秒に対する1.25秒だから余裕あるけど、メモリは256MB制限で247MB。
 * ・多分、辺を陽に張らないでダイクストラの中身魔改造する方針ならもっとメモリに優しいのかな。
 * ・はい、やってみた。やっぱりこっちのが全然軽い。AC0.88秒メモリ102MB。
 */
...
vector<T> dijkstra(int src) {
    ...
    while(!que.empty()) {
        ...
        rep(v, N) {
            if (u != v and G[u][v] <= R[u]) {
                if (dist+cost[u] < res[v]) {
                    res[v] = dist+cost[u];
                    que.push({dist+cost[u], v});
                }
            }
        }
    }
