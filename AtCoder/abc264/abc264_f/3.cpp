...
・さらに実験。生配列だとどうなるか。
　ちょっと速くなったのは予想通りなんだけど、
　添字の順序入れ替えても速度変わらず0.28秒くらいだった。(メモリも変わらず両方145MBくらい)
　つまり、vectorなら小さい方を浅い次元に持ってきた方がいいけど、
　生配列使うならどっちでも大丈夫、って思って良さそうだ。
　まあ結局、4次元以上くらいのDPやるなら生配列使えば？ってことなんだよね。
*/
...
void solve() {
    ...
    rep(i, H) rep(j, W) rep(k, 2) rep(l, 2) dp[i][j][k][l] = INF;
    dp[0][0][0][0] = 0;
    dp[0][0][1][0] = A[0];
    dp[0][0][0][1] = B[0];
    dp[0][0][1][1] = A[0] + B[0];
