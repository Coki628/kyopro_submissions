/*
参考：https://img.atcoder.jp/mujin-pc-2017/editorial.pdf
　　　https://tiramistercp.hatenablog.com/entry/mujin2017-a
・AGC系青diff以下残埋め
・自力ならず。むずい。
・ある位置にいるロボットについて、今残ってる中で最初にゴールさせられるか考えると、
　1,3,5,7...みたいに手前を置ければOKとなる。これは A[i] < cnt*2+1 みたいので判定できる。
　で、無理な所に来たら、1体ゴールさせてしまうとする。どれでもいい1つなので普通に掛け算すればOK。
　この時どれをゴールさせるかで色々変わってきてしまいそうな気になるが、その後の通り数には影響しない。
　1体減った状態で引き続き同じチェックを繰り返す。OKになったら次に進む。
　全部終わって最後まで残った分は自由に置けるので階乗する。
*/
...
void solve() {
    ...
    mint ans = 1;
    rep(i, N) {
        // この位置より後ろのロボットが通れるような置き方ができるかチェック
        while (A[i] < cnt*2+1) {
            // できないならここで1体ゴールさせるとする(この位置も使っていいので+1)
            ans *= cnt+1;
            cnt--;
        }
        cnt++;
