/*
・なんとか自力AC。予想以上に手間取った。
・3重ループを式変形で1つ削って何とか1万*1万まで落として通した。
　けどよく検討すると、各値最大が1万っていう前提自体、0が使える時点で崩れてる。。
・想定解は、厳しい方の条件(a+b+c)で先に候補を限定してループを回して、
　最後にもう1つの条件(a*b*c)をチェックするって感じだった。確かに。。
*/
...
void solve() {
    ...
    rep(a, 10001) {
        rep(b, 10001) {
            ll c1 = s-a-b;
            ll c2 = a and b ? t/a/b : INF;
            ans += max(min(c1, c2)+1, 0LL);
        }
    }
    print(ans);
}
