...
・E8さん典型90問埋め
・きっちり自力AC！
・Grundy数、複数山Nim、メモ化再帰
・まあこれもGrundy数の問題どっかで出たのはTLで知ってたから半分ネタバレだったけど。。
・Grundy数で決まるゲームが複数箇所で独立に展開される場合、
　全体でXORを取ると全体での勝敗が分かる。
・各盤面からの決着は、メモ化再帰で終了状態から決めていく感じに。
・なんか解説にはC++だと0.1秒くらいでACするって書いてあるけど、
　自分のこれ1.27秒もかかってるんだよね。。何が遅いのかな。
・試しにsetをunordered_setにしたら、ちょっと速くなってAC1.06秒。
　でももっと根本的に何かが遅そうだな…。
*/
...
void solve() {
    ...
    auto grundy = list2d(M+1, M*M+1, -1);
    auto rec = [&](auto&& f, ll w, ll b) -> ll {
        if (grundy[w][b] != -1) return grundy[w][b];
        if (w == 0 and b <= 1) return 0;
        set<ll> se;
        if (w >= 1) {
            se.insert(f(f, w-1, b+w));
        }
        if (b >= 2) {
            rep(k, 1, b/2+1) {
                se.insert(f(f, w, b-k));
