/*
・きっちり自力AC！(終了後投げたらちゃんと通った)
・明らかに無理なのは弾くとして、行ける場合、
　最初の移動だけ前に進むか後ろに行くかのコストがかかる。
　それ以降は隣りに行けばいいので、コスト1になる。
*/
...
void solve() {
    ...
    if (not count(ALL(A), 0) and count(ALL(B), 0)) {
        print(-1);
        return;
    }
    if (not count(ALL(A), 1) and count(ALL(B), 1)) {
        print(-1);
        return;
    }

    ll cost0 = INF, cost1 = INF;
    rep(i, N) {
        if (A[i] == 0) {
