/*
・E8さん精選100+50問埋め：偏角ソート
・なんとか自力AC。
・使った座標を基準にして180度以内、みたいにやったらWAで、
　誤差なのか、色々やってもうまくいかず。。
・基準にする向きを0.1度ずつ全探索してみたら通った。
　こっちのが誤差カバーできてなさそうなのにな。。
*/
...
void solve() {
    ...
    rep(i, N) {
        degs.pb({get_degree({1, 0}, {0, 0}, XY[i]), i});
    }
    rep(i, N) degs.pb({degs[i].first+360, degs[i].second});
    sort(ALL(degs));
    auto [degs2, ids] = zip(degs);
    ...
    rep(i, 3600) {
        ...
        Point cur = {0, 0};
        rep(j, N*2) {
            if (x < degs2[j]+EPS and degs2[j] < x+180-EPS) {
                cur = cur+XY[ids[j]];
            }
        }
        chmax(ans, cur.abs({0, 0}));
    }
    print(ans);
}
