/*
参考：https://ikatakos.com/pot/programming_algorithm/contest_history/atcoder/2021/0822_arc125
　　　https://kyopro-memorial.hatenadiary.jp/entry/2021/08/23/022354
・ARC青diff残埋め（青diffじゃないけどこれ本番でもちょっとやろうとしてたからupsolve）
・自力ならず。でもこれはCより全然好き。
・部分文字列DP応用、in-place DP
・数列の種類数ではないが、一意になるものの数なのでかなり近い感じ。
　各文字について、前回出現位置を覚えておいて、そこ以降から遷移させる。
　前回位置だった場所はそれ以降は使えないので値を0にしておく。
・0にするパートをやらない版。この問題をACする物ではないが、
　これだと本家部分列DPそのままになって、しかも文字種類数が多い場合でもこの形なら対応できる。
*/
...
void solve() {
    ...
    rep(i, N) {
        // dp.print(N+1);
        // 直前にその値を使った位置以降から遷移
        dp.update(i+1, dp.query((prv[i] != -1 ? prv[i] : 0), i+1));
    }
    // dp.print(N+1);
    mint ans = dp.query(1, N+1);
    print(ans);
}
