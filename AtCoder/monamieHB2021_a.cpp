...
・その他埋めAC数上げ
・なんとか自力AC！めっちゃ時間かかった。。
・8+8+10=26でカニ3匹をエビ1匹に置換できるので、とりあえず多すぎる分はほぼエビにできそう。
　N%26でできるだけエビにした余りをカニにできるといいけど、これだけだとWA。。
　さらに考えていくと、2,4,6,22とかが余った時はこれだとカニにできないけど、
　エビを1つこっちに持ってきて28,30,32,48とかにすればできる。
　これだとこの先も同じことを全部調べなきゃいけなくなりそうだけど、
　52以降はカニが全ての偶数をカバーできるので、52未満の範囲だけチェックすれば良さそう。
　よってまず0〜25の範囲でやってみて、ここが無理なら(かつNが足りるなら)+26して26〜51の範囲もチェックする、
　といった具合のことをやったら無事ACできた。
・公式解は式変形で賢く全探索の範囲を減らす、みたいなことやってた。
　こういうアプローチにできないからこの手のやついつも詰まるんだよな…。
*/
...
void solve() {
    ...
    const ll M = 26;
    // 必要な{足: カニ}を列挙
    map<ll, ll> ok = {{0, 0}, {8, 1}, {10, 1}, {16, 2}, {18, 2}, {20, 2}, {24, 3}, {26, 3}, {28, 3}, {30, 3}, {32, 4}, {38, 4}, {40, 4}, {48, 5}};
    // 全てエビにした残り0〜25でチェック
    ll res = N%M;
    if (ok.count(res)) {
        ll ans = ok[res];
