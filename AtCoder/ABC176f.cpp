/*
・ABC高難度埋め
・これは愚直3乗DPなのでTLE想定。でもこの方針までは自分で立てられたのでまあOK。
*/
...
void solve() {
    ...
    rep(i, N-1) {
        ...
        for (auto& [key, val] : dp[i]) {
            auto [a0, a1] = key;
            vector<ll> cur = {a0, a1, A[j+2], A[j+3], A[j+4]};
            rep(bit, 1LL<<K) {
                ...
                chmax(dp[i+1][{V2[0], V2[1]}], val+(V1[0] == V1[1] and V1[0] == V1[2]));
            }
        }
    }
    ll ans = 0;
    for (auto& [key, val] : dp[N-1]) {
        auto [a0, a1] = key;
        chmax(ans, val+(a0 == a1 and a0 == A[M-1]));
    }
    print(ans);
}
