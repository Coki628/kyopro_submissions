/*
・ABC高難度埋め
・1回解いたやつだけど、覚えてなかったしやってみた。自力AC？
　やる前にちらっと昔のやつ見ちゃったんだけど、まあDPっぽいってくらいしか見えなかったし。。ｗ
・3次元DP、座圧
・その時操作をする/しないの遷移で考えると、操作する時は前の列と同じに揃えて、
　値が下がる時に差分を一気に塗る感じにすると、状態として直前の高さと操作回数があれば良さそう。
　高さは10憶くらいあるけど、要素Nが300なので、座圧すれば300種類、これなら状態持てそう、となる。
　後は粛々とDPを書き上げればOK。想定解微妙に違ったけど、遷移の考え方はだいたい合ってたしOK。
*/
...
void solve() {
    ...
    auto dp = list3d(N+1, M, K+1, INF);
    dp[0][0][0] = 0;
    rep(i, N) {
        rep(j, M) {
            rep(k, K+1) {
                if (dp[i][j][k] == INF) continue;
                chmin(dp[i+1][A[i]][k], dp[i][j][k]+max(cp.unzip(j)-cp.unzip(A[i]), 0LL));
                if (k < K) {
