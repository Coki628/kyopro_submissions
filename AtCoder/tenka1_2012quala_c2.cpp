...
・試験管青diff残埋め
・これは自力WA。。
　UFで状態をまとめられないかとか思ったんだけど大嘘だった。。
　でも補グラフ側も考慮した頂点状態のマージとか結構頑張って考えたので一応取っておく。
　具体的には、隣接リストをもう1つ作っておいて、そっちから辺を削ることで、
　補グラフ側で到達可能かどうかを管理していく。
*/
...
void solve() {
    ll N, M;
    cin >> N >> M;
    vector<set<ll>> nodes(N), nodes2(N);
    rep(i, M) {
        ll u, v;
        cin >> u >> v;
        u--; v--;
    ...
    rep(i, L) {
        ...
        u = uf.find(V[0]);
        rep(j, (ll)V.size()-1) {
            ll v = uf.find(V[j]), w = uf.find(V[j+1]);
            if (not uf.same(v, w)) {
                ll r = uf.merge(v, w);
                if (r == v) {
                    for (ll x : nodes[w]) {
                    ...
                    for (ll x : nodes2[v]) {
                        x = uf.find(x);
                        if (not nodes2[w].count(x)) {
                            tmp.eb(x);
                        }
                    }
                    for (ll x : tmp) nodes2[v].erase(x);
                    nodes[w].clear();
                    nodes2[w].clear();
                    roots.erase(w);
                } else {
                    ...
                    for (ll x : nodes2[w]) {
                        x = uf.find(x);
                        if (not nodes2[v].count(x)) {
                            tmp.eb(x);
                        }
                    }
                    for (ll x : tmp) nodes2[w].erase(x);
                    nodes[v].clear();
                    nodes2[v].clear();
                    roots.erase(v);
                }
