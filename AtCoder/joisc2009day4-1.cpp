/*
・JOI埋め
・自力AC！やったね。2乗の木DPの自力ACは初だと思うので嬉しい。
・2乗の木DP
・頂点10000の木で、冊子の数が1000、計1000万なら状態が持てそうで、
　この2乗の状態はいかにも2乗の木DPに乗りそうではある。実際できた。
・普通にやったらMLEでメモリ870MBくらい使ってて、intにしたら445MBまで落ちたけど、
　256MB制約なのでまだダメ。で、今回内側の状態Mは最大1000なので、
　大きい部分木でMを超える部分を枝刈りしたらめちゃ速くなってすんなり通った。
*/
...
void solve() {
    ...
    auto dp = list2d(N, 2, 0);
    auto dfs = [&](auto&& f, ll u, ll prv) -> void {
        for (ll v : nodes[u]) {
            if (v == prv) continue;
            f(f, v, u);
            // 親の状態*子の状態を全通り見る
            ll n = dp[u].size(), m = dp[v].size();
            ll sz = min(n+m+1, M+1);
            vector<int> nxt(sz, 0);
            rep(i, n) {
                chmax(nxt[i], dp[u][i]);
                rep(j, m) {
                    // Mを超える状態は持たない
                    if (i+j >= sz) break;
                    chmax(nxt[i+j], dp[u][i]+dp[v][j]);
                }
            }
            // 親の状態を更新
            dp[u] = nxt;
        }
        // 1冊以上持っている状態なら自分の分も足せる
        rep(i, 1, dp[u].size()) {
            dp[u][i] += A[u];
        }
    };
