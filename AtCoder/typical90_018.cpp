...
・なんとか自力AC！
・幾何、三角関数、3次元での2点間距離
・与えられた情報から、頑張って色んなことを導くと答えが出る。疲れた。
　観覧車の高さ側を横軸にすると、割といつもの三角関数っぽい形になる。
　後は回り方が時計回りで普通の三角関数のと逆なので360-とかしてよしなに。
　詳細はソース内コメントとか自分ノートの図を参照。
・これで星3なんだから、界隈みんな数学強すぎて困る…。
*/
...
void solve() {
    ...
    rep(_, Q) {
        ...
        ld deg = 360-360*E/T;
        // 観覧車上の縦位置h
        ld h = L/2-L/2*cos(radians(deg));
        // 観覧車上の横位置w
        ld w = L/2*sin(radians(deg));
        // 観覧車上から像までの距離
        ld d1 = dist({0, w, h}, {X, Y, 0});
        // 観覧車上から像までの距離(高さなし)
        ld d2 = dist({0, w, 0}, {X, Y, 0});
        // 底辺/斜辺としてcosにして角度を出す
        ld ans = degrees(acos(d2/d1));
        print(ans);
    }
}
