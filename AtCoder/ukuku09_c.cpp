/*
・その他埋めAC数上げ
・自力AC！！やったね。時間かかったけど頑張った甲斐あった。
・グリッドBFS応用、SCC
・大変だった。公式解はもっと綺麗にやってた。
・矢印がサイクルになっている箇所は、位置とサイクル長が知りたいのでグラフにしてSCCしておく。
・矢印から壁に当たるとそこから先は動けないので、それは別の属性のマスとして設定する。
・BFSするが、「そこまでのどこかで止まることができるか」を状態として持っておく。
・これらの情報があれば、後は結果に対して頑張って場合分けで解ける。
　両点からBFSしておいて、中継地全探索する。
・場合分けは、矢印マスかどうか、サイクル内かどうか、止まれるか、両者どっちが先に着くか、
　あたりによって丁寧に作った。(詳細はソース内コメント)
・30*30=900マスグリッドのBFSを10万クエリ内で毎回回すので計算量はきつい。
　QCFiumしたり、BFS内で矢印判定をmapでしていたのを配列でできるようにしたら無事AC。
　その後、重いとこのllを全部intにしたりでさらにちょっと速くなって最終的にAC1.66秒。
*/
...
// グリッドBFS
vector<vector<vector<int>>> bfs(const vector<string> &grid, const vector<pii> &src, char invalid='#') {

    int H = grid.size();
    int W = grid[0].size();
    auto res = list3d(2, H, W, MOD);
    const vector<pii> directions = {{-1, 0}, {1, 0}, {0, -1}, {0, 1}};
    queue<tuple<int, int, int>> que;
    for (auto [h, w] : src) {
        int stop = (grid[h][w] == '.' or grid[h][w] == '*') ? 1 : 0;
        que.push({stop, h, w});
        res[stop][h][w] = 0;
    }
    while (!que.empty()) {
        auto [stop, h, w] = que.front(); que.pop();
        if (grid[h][w] == '*') continue;
        rep(d, 4) {
            auto [dh, dw] = directions[d];
            int nh = h + dh;
            int nw = w + dw;
            int nxstop = stop | (grid[nh][nw] == '.' or grid[nh][nw] == '*');
            if (nh < 0 or nw < 0 or nh >= H or nw >= W) continue;
            if (grid[nh][nw] == invalid) continue;
            if (grid[h][w] < 4 and grid[h][w] != d) continue;
            if (res[nxstop][nh][nw] == MOD) {
                res[nxstop][nh][nw] = res[stop][h][w] + 1;
                que.push({nxstop, nh, nw});
            }
        }
    }
    return res;
...
void solve() {
    ...
    map<char, ll> mp = {{'^', 0}, {'v', 1}, {'<', 2}, {'>', 3}};
    map<char, ll> mprev = {{'^', 1}, {'v', 0}, {'<', 3}, {'>', 2}};

    // サイクル矢印マスの設定
    auto cycle = list2d(H, W, -1);
    scc_graph scc(H*W);
    rep(h, H) {
        rep(w, W) {
            ll i = gridtoid(h, w, W);
            rep(d, 4) {
                ...
                int nw = w + dw;
                if (nh < 0 or nw < 0 or nh >= H or nw >= W) continue;
                if (mp.count(grid[h][w]) and mp[grid[h][w]] == d) {
        ...
        rep(w, W) {
            if (grid[h][w] == '#') {
                rep(d, 4) {
                    ...
                    int nw = w + dw;
                    if (nh < 0 or nw < 0 or nh >= H or nw >= W) continue;
                    if (mprev.count(grid[nh][nw]) and mprev[grid[nh][nw]] == d) {
                        grid[nh][nw] = '*';
                    }
                }
            }
    ...
    // mapを使わない高速化のため、矢印を0~3の数値に変換しておく
    rep(i, H) {
        rep(j, W) {
    ...
    rep(_, Q) {
        ...
        auto dist1 = bfs(grid, {{sh, sw}});
        auto dist2 = bfs(grid, {{gh, gw}});
        int res = MOD;
        rep(i, 1, H-1) {
            rep(j, 1, W-1) {
                chmin(res, max(dist1[1][i][j], dist2[1][i][j]));
                if (grid[i][j] < 4) {
                    // サイクルじゃない矢印マス
                    if (cycle[i][j] == -1) {
                        // aが先に着いて待つ
                        if (dist1[1][i][j] <= dist2[0][i][j]) {
                            chmin(res, dist2[0][i][j]);
                        }
                        // bが先に着いて待つ
                        if (dist1[0][i][j] >= dist2[1][i][j]) {
                            chmin(res, dist1[0][i][j]);
                        }
                        // 同時に着く
                        if (dist1[0][i][j] == dist2[0][i][j]) {
                            chmin(res, dist1[0][i][j]);
                        }
                    // サイクル矢印マス
                    } else {
                        ...
                        if (dist1[1][i][j] < MOD) {
                            ...
                            // bが回ってる所にaが合わせる
                                ...
                                int ma = dist1[1][i][j]%mod;
                                int mb = dist2[0][i][j]%mod;
                                if (ma > mb) mb += mod;
                                int wait = mb-ma;
                                chmin(res, dist1[1][i][j]+wait);
                            }
                        }
                        if (dist2[1][i][j] < MOD) {
                            // bが先に着いて待つ
                            ...
                            // aが回ってる所にbが合わせる
                                ...
                                int ma = dist1[0][i][j]%mod;
                                int mb = dist2[1][i][j]%mod;
                                if (mb > ma) ma += mod;
                                int wait = ma-mb;
                                chmin(res, dist2[1][i][j]+wait);
                            }
                        }
                        // 両方止まれない時は周期が一致している場合だけ
                        if (dist1[0][i][j]%mod == dist2[0][i][j]%mod) {
                            chmin(res, max(dist1[0][i][j], dist2[0][i][j]));
                        }
                    }
