/*
参考：https://atcoder.jp/contests/abc217/editorial
・自力ならず。
・スターリング数
・問題見てすぐ、スターリング数のDPをちょっといじればいいのではと思ったけど、
　中身分かってないDPの中身なんてちょっといじれるはずもなく。。
・さて、DPの中身の説明は公式解説でも言及してくれていた。
　ここでk個のグループのいずれかに、今回増えるi人目が合流する方の遷移で、
　mod Mでiと同じ人が所属するグループにはこのi人目は入れない。
　modで同じ人は(i-1)/M人いるはずなので、k - (i-1)/M個のグループのどれかにこの人が入ることになる。
　場合によってはここで引きすぎてマイナスにならないかとか心配になったが、
　1グループに1人以上いる条件があるので、グループ数kより人数i-1が大きくなることはなさそうなので、
　問題はなさそう。また、この(i-1)/M人はそれぞれ別のグループに所属しないといけないので、
　必ず1つのグループには最大1人しかおらず、同じ所に2人入って数がズレることもない。
・時間がたっぷりあってスターリング数のDPをしっかりググって調べたら解けただろうか。うーん。。
*/
...
vector<vector<mint>> stirling(int N, int K, int M) {
...
void solve() {
    ...
    auto res = stirling(N, N, M);
    rep(i, 1, N+1) print(res[N][i]);
}
