/*
参考：https://twitter.com/laycrs/status/1434149269557026822
　　　https://atcoder.jp/contests/abc217/submissions/25594787
・自力ならず。あとちょっとだと思ってたのに、upsolveするのにだいぶ時間かかった。。
・3乗の区間DP、nCr系数え上げ
・やることはtypical90_019,past202012_lあたりととても近い。でもちょっと工夫しないと通らない。
　基本的には問題設定がかなり似ているtypical90_019の遷移をベースに作っていたのだが、まあ合わない。
　多分だけど、最適化である程度適当なやつを見てしまっても最小値を取らない限り問題のなかったものが、
　今回は数え上げなので、余計なのを見てしまうとダブルカウントでバグるとかそんなんだと思ってるけど、
　詳細は分からない。もしかしたらどこか直せばそっちでも通るのかもしれない。
・とりあえず通ったのは、遷移のやり方を少し変えて、
　その時の(l,m)を組ませるために、[l+1,m) と [m+1,r) から遷移させてくる、というもの。
　でもこれもバグ取り大変だったんだけど、この時(l,m)は左区間を囲んでいるから、
　このペアは左区間に含まれるものとして左右をマージさせる通り数を数えるんだよね。。
　つまり左区間のペア数に囲むやつの分で+1する。
・ちなみに自力だとここでnCr(左右全体, 左(か右))が通り数になるってのもそもそも出てなくて、
　自由に並べるから順列で階乗かー！とか言って迷走してた。ほんと数え上げ弱い。。
　各集合内での並び順は既に決まっているはずで、ここではあくまでもマージする時の操作順序の通り数なので、
　nCr(5+3, 3)みたいにして、5+3個の席から3つを選び出す通り数みたいなやつになる。
・遷移とその数え上げパートができれば後はだいたいやったことある区間DPと同じ方針で大丈夫。
*/
...
void solve() {
    ...
    auto rec = [&](auto&& f, ll l, ll r) -> mint {
        ...
        mint res = 0;
        // 今回(l,m)を組ませるために、[l+1,m) と [m+1,r) から遷移させてくる
        rep(m, l+1, r) {
            if (se.count({l, m})) {
                // (l,m)は左区間を囲んでいるので、左区間に含める
                ll cnt1 = (m-(l+1))/2 + 1;
                ll cnt2 = (r-(m+1))/2;
                res += f(f, l+1, m)*f(f, m+1, r)*mt.nCr(cnt1+cnt2, cnt1);
            }
        }
        visited[l][r] = 1;
        memo[l][r] = res;
