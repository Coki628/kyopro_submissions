/*
参考：https://atcoder.jp/contests/abc208/editorial
　　　https://www.youtube.com/watch?v=FhQVDtJyM6o
・ABC高難度埋め
・冪乗和、ラグランジュ補間
・制約が小さければグリッドDPみたいのをやればいいんだけど、でかい。
　うまいこと遷移工夫して行列累乗にでも持っていけるのかと思ったら違った。
・行と列をN,Mとして、M=0の列に注目すると、ここはiのK乗になっていて、次の列はその累積和になっている。
　この冪乗の和は、K乗和であればK+1次の多項式となる。これは知らなきゃ知らないだろう。
　でもK乗というのはf(x)=x^KなのでK次式で、だいたい累積和取ると次数が1上がるようなお気持ちがあるので、
　じゃあK+1次式になるのかなって気持ちにはなる。
　で、その先の列はどうかと言うと、同様に次数が増えていって(0-indexedで)M列目はK+M次式になるとのこと。
　これも値の変化を見ると、次の列に行く度に累積和を重ねるような感じになっているので、
　まあやっぱり累積和取ると次数1つ増えるのかなみたいな気持ちになる。一般にそうかは知らん。。ｗ (多分違う)
・とりあえずこれが分かると、N次の多項式でN+1個の値が分かってるとラグランジュ補間で
　任意の値が分かるってな話があるので解ける。
*/
...
void solve() {
    ...
    // M列目はK+M次式になるので、補間のためにK+M+1項を用意する
    ll S = K+M+1;
    auto dp = list2d<mint>(S, M+1, 0);
    rep(i, S) {
        dp[i][0] = pow(i, K, MOD);
    }
    rep(i, 1, S) {
    ...
    auto res = lagrange_polynomial(F, N);
    print(res);
}
