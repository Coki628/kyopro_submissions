...
・その他埋めAC数上げ
・きっちり自力AC！
・クエリ系だけど、特別なデータ構造とかは使わない。個々の範囲が0~100であることと、
　それぞれの分かってる情報を元に話を整理すれば答えは出せる。PAST前半～中盤とかで出そうかも？
*/
...
void solve() {
    ...
    ll total = sum(A);
    auto known = list2d(N, N, false);
    vector<ll> sm(N), cnt(N, N-1);
    rep(i, N) {
        sm[i] = total-A[i];
        known[i][i] = true;
    }
    rep(_, M) {
        ...
        if (op == 0) {
            ...
            if (known[b][c]) {
                ...
                ll lo = max(sm[b]-100*(cnt[b]-1), 0LL);
                ll hi = min(sm[b], 100LL);
                print(mkp(lo, hi));
            }
        }
    }
