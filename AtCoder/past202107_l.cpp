...
・indexセグ木
・範囲を狭めながら何回もやればいい。
　制約でその回数が20万を超えないことは保証されてる。
・最初セグ木にぶたんでやろうとしたらなんかWAになったのでindexセグ木で書き直した。
*/
...
void solve() {
    ...
    auto seg = get_segment_tree_index([](ll a, ll b) { return min(a, b); }, INF);
    seg.build(A);
    ...
    rep(i, Q) {
        ...
        if (op == 1) {
            ...
            vector<ll> ans;
            auto [mn, i] = seg.query(x, y);
            while (1) {
                auto [cur, j] = seg.query(i, y);
                if (cur > mn or j >= y) break;
                ans.pb(j);
                i = j+1;
            }
            for (auto &a : ans) a++;
