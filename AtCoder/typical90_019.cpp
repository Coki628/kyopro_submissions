/*
・E8さん典型90問埋め
・自力AC！けどちょい時間かかったのは良くない。
・3乗の区間DP
・大きく分けて、位置l,rを使わない遷移と使う遷移の2つのループを回す。
・ちょっと閉区間でやってみようかなとか言ってたら無限バグらせ編になった。
　区間DPいつも添字バグらせて時間食ってる気がして困る…。
・結局半開区間に直して、過去の区間DPと動き方を合わせながら調整してやっとAC。
*/
...
void solve() {
    ...
    auto memo = list2d(N, N+1, -1LL);
    auto rec = [&](auto&& f, ll l, ll r) -> ll {
        if (memo[l][r] != -1) return memo[l][r];
        if (r-l == 0) return 0;
        if ((r-l)%2 == 1) return INF;
        if (r-l == 2) return abs(A[l]-A[r-1]);
        ...
        // 位置l,rを使う遷移
        rep(m, l+1, r-1) {
            chmin(res, f(f, l+1, m)+f(f, m, r-1)+abs(A[l]-A[r-1]));
        }
        memo[l][r] = res;
        return res;
    };
