...
・ダイクストラ、頂点拡張
・JOIお家芸って感じの拡張ダイクストラ。
　「道を反転させる」はそういう操作を含む別の道としてグラフに組み込む。
　最初コスト2種類にするやつかと思ったけど、作ってるうちに添字側で持つといいじゃんとなった。
　それだと2乗が効くこの制約にも納得がいくし。
*/
...
// ダイクストラ
vvl dijkstra(const vector<vector<tuple<ll, ll, ll>>> &nodes, int src, int M, int goal=-1) {

    int N = nodes.size();
    auto res = list2d(M+1, N, INF);
    priority_queue<tuple<ll, ll, ll>, vector<tuple<ll, ll, ll>>, greater<tuple<ll, ll, ll>>> que;
    res[0][src] = 0;
    que.push({0, 0, src});

    while (!que.empty()) {
        auto [dist, rev, u] = que.top(); que.pop();
        // if (u == goal) return res;
        if (res[rev][u] < dist) continue;
        for (auto [v, r, cost] : nodes[u]) {
            if (rev+r > M) continue;
            if (dist+cost < res[rev+r][v]) {
                res[rev+r][v] = dist+cost;
                que.push({dist+cost, rev+r, v});
            }
        }
    }
    return res;
