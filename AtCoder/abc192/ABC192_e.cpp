...
template<typename T>
vector<T> dijkstra(vector<vector<tuple<ll, T, ll>>> &nodes, int src) {
    ...
    while(!que.empty()) {
        ...
        for (auto p: nodes[u]) {
            tie(v, cost, k) = p;
            ll cost2 = (k-dist%k)%k;
            if (dist+cost+cost2 < res[v]) {
                res[v] = dist+cost+cost2;
                que.push({dist+cost+cost2, v});
            }
        }
    }
    return res;
