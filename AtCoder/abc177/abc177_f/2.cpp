...
void solve() {
    ...
    auto seg = get_lazy_segment_tree(W+1, f, g, h, T, E);
    rep(w, 1, W+1) {
        seg.set(w, {0, w});
    }
    seg.build();
    rep(h, 1, H+1) {
        ...
        cin >> l >> r;
        r++;
        // 障害物より手前でまだ残っている最右位置を探す
        l = seg.find_last(l, [](const Node &a) { return a < MOD; });
        if (l != -1) {
            // あればそこから障害物の右端まで進むものとして更新
            seg.update(l, r, seg[l]-l);
        } else {
            // なければ障害物右端までを通行不可に
            seg.update(l, r, MOD);
        }
        ll res = seg.query(1, W+1);
        res = res < MOD ? res+h : -1;
        print(res);
    }
}
