...
　　　https://kanpurin.hatenablog.com/entry/2020/08/31/171732
・遅延セグ木、区間等差数列更新
・前に高難度埋めで抽象化無理そうだったから保留にしたやつ。optさんが抽象化に成功してた。
・ようは更新の左端として配列のindexを持っておく。こうすると、
　その左端位置を更新の値から引くことで、その左端が0スタートの等差数列みたいにできる。
...
void solve() {
    ...
    rep(h, 1, H+1) {
        ...
        cin >> l >> r;
        l--;
        ll i = seg.find_last(l, [](const Node &a) { return a < MOD; });
        if (i != -1) {
            // 位置iを初項seg[i]とした公差dの等差数列を[l,r)に更新
            seg.update(i, r, seg[i]-i*d);
        } else {
            seg.update(l, r, MOD);
        }
        // seg.print(W);
