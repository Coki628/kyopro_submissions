/*
参考：https://atcoder.jp/contests/abc177/editorial
・ABC高難度埋め
・いけると思ったらいけてなかった。やっぱむずいや。
・setとmultiset
・setで区間の端点、multisetで区間長を管理するやつ。昔こどふぉで類題はやったことがある。(cf1418d)
・まあ今回は実装してみたらset<pll>よりmap<ll, ll>のが扱いやすく感じたのでそっちでやった。
　右端がb+1にまとまる時、合わせてmultisetからも不要分を消すんだけど、
　この時ちゃんと整合性を取るために丁寧にやった。(多分、存在しないやつ消そうとしたり、
　本来それじゃないやつ消しちゃったりしたらバグるし。。)
*/
...
void solve() {
    ...
    rep(h, 1, H+1) {
        ...
        while (prev(mp.upper_bound(b))->first >= a) {
            ...
            mp.erase(p.first);
            ms.erase(ms.find(p.first-p.second));
            if (b < W) {
                if (mp.count(b+1)) {
                    // 左端が最大なら更新
                    if (mp[b+1] < p.second) {
                        ms.erase(ms.find(b+1-mp[b+1]));
                        mp[b+1] = p.second;
                        ms.insert(b+1-mp[b+1]);
                    }
                } else {
                    mp[b+1] = p.second;
                    ms.insert(b+1-mp[b+1]);
