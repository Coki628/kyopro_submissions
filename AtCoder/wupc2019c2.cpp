/*
・自力AC！！通せて嬉しい。
・その他埋めAC数上げ。というか、過去へのリベンジ。
・最初通せなかった時から2年半くらい経ってた。久々に見たら、なんか解けそうな方針が浮かんだのでやった。
・全域木の構築、DFS
・まず、適当な全域木を構築して、それをDFSで葉から決めていくとする。
　基本方針は、隣接したもの同士でグループにしていく感じ。
　未確定な兄弟が2以上あればそこで組み、自分だけなら親と組む。
　これでほぼ出来るが、根だけ余る場合があるのでそこは別途処理。
　子頂点同士がサイクルになっていればそこに混ぜてもらう感じにして、
　そうでないなら子と孫で組んでるはずなのでそこに入って縦3段みたいに組む。
*/
...
void solve() {
    ...
    auto dfs = [&](auto&& f, ll u, ll prv) -> void {
        ...
        // 根が余った場合
        if (prv == -1 and not used[u]) {
            // 子頂点サイクルができているならそこに繋ぐ
            if (tmp.size() >= 2) {
                ans[tmp.back()] = u;
                ans[u] = tmp[0];
            // そうでなければ子-孫間を繋ぎ変える
