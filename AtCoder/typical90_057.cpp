...
・E8さん典型90問埋め
・きっちり自力AC！
・連立方程式の解(ガウス・ジョルダン法)の通り数
・C++でまだこれ整備してなかったからちょっと時間かかった。
　今回は通り数なので、2^(N-rank)が答え。
　(aoj2530_uecpc2013より。けんちょんさんの記事にも言及がある。)
・多分前やった時も思ったんだけど、行列が直感と逆っぽくなるので注意。
　数列Aの各要素A[i]はi列目、その各ビットがj行目みたいになる。
　で、最後の列に右辺にしたい、求めたいビットのパターンを置く。
*/
...
// constexpr int MOD = 1000000007;d
...
void solve() {
    ...
    auto [rank, res] = gauss_jordan<BITLEN>(M, N+1, mat, true);
    if (rank == -1) {
        print(0);
    } else {
        ll ans = pow(2, N-rank, MOD);
        print(ans);
    }
}
