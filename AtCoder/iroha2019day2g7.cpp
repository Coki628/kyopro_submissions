...
・これは割とさくっと自力AC！
・まあ多分拡張ダイクストラっぽいのを初めてやった問題だしね。よく覚えてる。
*/
...
template<typename T>
vector<vector<T>> dijkstra(vector<vector<pair<ll, T>>> &nodes, int src, vector<pll> &XY, ll K) {

    int N = nodes.size();
    auto res = list2d(N, K+1, INF);
    ...
    while(!que.empty()) {
        ...
        auto [x, y] = XY[u];
        if (k < K and dist+y < res[u][min(k+x, K)]) {
            res[u][min(k+x, K)] = dist+y;
            que.push({dist+y, min(k+x, K), u});
        }
        for (auto [v, cost] : nodes[u]) {
            if (dist+cost < res[v][k]) {
                res[v][k] = dist+cost;
                que.push({dist+cost, k, v});
            }
        }
    }
    return res;
...
void solve() {
    ...
    auto res = dijkstra(nodes, 0, XY, K);
    ll ans = res[N-1][K];
    if (ans < INF) {
        print(ans);
    } else {
