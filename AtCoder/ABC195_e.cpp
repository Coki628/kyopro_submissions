 ...
 * ・きっちり自力AC！これは早かった。
 * ・ゲームDP、メモ化再帰
 * ・実は状態は少なくて、iの位置とmod 7での値が分かればいいので、DPできる。
 * 　ここからは教科書通りにゲームDPすればよくて、
 * 　先手番ならtrueが1つでも返ればtrue、後手番ならfalseが1つでも返ればfalse、とやる。
 */
...
void solve() {
    ...
    auto rec = [&](auto&& f, ll i, ll num) -> bool {
        if (memo[i][num] != -1) {
            return memo[i][num];
        }
        if (i == N) {
        ...
        bool res;
        if (T[i] == 'T') {
            res = f(f, i+1, num*10%7) | f(f, i+1, (num*10+toint(S[i]))%7);
        } else {
            res = f(f, i+1, num*10%7) & f(f, i+1, (num*10+toint(S[i]))%7);
        }
        memo[i][num] = res;
