/*
・E8さん精選100+50問埋め：Range Minimum Query (RMQ)
・すんなり自力AC！
・テーマがRMQの問題なのにセグ木使わないし、diff的にも絶対嘘解法っぽい方針だなー、
　って思いつつも心のままに投げたら普通に通った。。
・ずらす回数を全探索。ずらした時にchminする。ただしそのまま同じ配列使うと、
　最後の1個が更新後の値を使ってしまうので、別の配列を用意する。
　この全体合計にずらす回数*コスト足すのから最小を取る。
*/
...
void solve() {
    ...
    rep(i, 1, N+1) {
        vector<ll> nxt(N);
        rep(j, N) {
            nxt[(j+1)%N] = min(A[(j+1)%N], A[j]);
        }
        chmin(ans, sum(nxt)+K*i);
        A = nxt;
    }
    print(ans);
}
