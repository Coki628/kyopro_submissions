/*
・なんとか自力AC！
・制約10億の数え上げだし、明らかに苦手系っぽかったけど何とかなってよかった。
　今日これまで落としたら多分灰パフォだったからな。。
・とりあえず、状態P-1個でN回進むDP遷移を考えてみる。
　P-1以下の各要素への各遷移を考えると、和がPちょうどになる
　1つだけは遷移できないことが分かる。で、このP-2個の遷移は、
　どの要素においても同じように行われることも分かる。
　そこで、P-1個のうち要素を1つだけを考えると、
　数列N個を1つ進める毎に、*(P-2)するだろうなあとなる。
　で、そんな要素が(P-1)個ある訳だから、最終的には(P-2)^(N-1)*(P-1)が答え。
*/
