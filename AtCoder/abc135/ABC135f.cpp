 ...
 * ・自力ならず。。さすがに黄diffむずい。
 * ・ローリングハッシュ、周期、文字列をグラフに帰着、有向グラフの閉路検出、DAGのDP、メモ化再帰
 * ・解説読んだ。でも公式解説だけで理解できてよかった。
 * ・無限に続く文字列Sを周期Nと考え、各文字を頂点、
 * 　部分文字列が一致する箇所について辺を張る有向グラフと考える。
 * ・部分文字列の一致は文字列アルゴ各種で色々できる。
 * ・グラフが出来上がったら、やることは最長パスor閉路を見つけること。
 * 　閉路が見つかれば無限に続いてしまうので即終了。
 * 　そうでなければ、DAGのDPみたいにそこより先の最大値を確認しながらメモ化再帰。
 */
...
void solve() {
    ...
    rep(i, N) {
        // 部分文字列の一致で辺を張る
        if (rh.query(t_table, 0, M) == rh.query(s_table, i, i+M)) {
            nodes[i].pb((i+M)%N);
        }
    }

    vector<ll> memo(N), visited(N);
    auto rec = [&](auto&& f, ll u) -> bool {
        if (visited[u] == 2) {
            return 0;
        }
        // 閉路検出
