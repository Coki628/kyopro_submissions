 ...
 * ・自力AC。遅かったけど。
 * ・半分の部分を桁毎に調べる。
 * 　文字列と数値の変換をゴチャゴチャやった。C++だとやっぱ多少めんどい。
 * ・大きい桁のためににぶたんしたけど、最大で10^12の半分で6桁見れればいいので、
 * 　多分普通にループで全探索回してもよかった…。
 */
...
void solve() {
    ...
    rep(d, 2, 13, 2) {
        ...
        ll mn = toint("1"+string(d/2-1, '0'));
        ll mx = toint(string(d/2, '9'));
        ll res = bisearch_max(mn-1, mx+1, [&](ll m) {
            ll x = toint(to_string(m)+to_string(m));
            return x <= N;
        });
        ans += res-mn+1;
    }
