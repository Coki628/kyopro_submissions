...
・きっちり自力AC！
・実は状態が少ない
・振り分けた後の数列A,Bは累積GCDして1に近づいていくので、
　実はとりうる状態数は少なそう、となる。
・これが当たりで、「i個目まで見て、j=gcd(A),k=gcd(B)の状態に到達可」
　みたいなDPを2次元目mapのペアでやれば通る。
*/
...
void solve() {
    ...
    rep(i, N) {
        auto [a, b] = AB[i];
        for (auto [j, k] : dp[i]) {
            dp[i+1].insert({gcd(j, a), gcd(k, b)});
            dp[i+1].insert({gcd(j, b), gcd(k, a)});
        }
    }
