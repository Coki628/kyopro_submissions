/*
・きっちり自力AC！
・setのにぶたんでシミュる。C++が恩恵を受ける問題だった。
　まあ自分が今もPython使いだったら、先読み+座圧+BITで押し込むと思う。
　UFで解く賢い解法があったそうだけど。
*/
...
void solve() {
    ...
    rep(_, Q) {
        ...
        cin >> op >> x;
        if (op == 1) {
            auto cur = *prev(se.upper_bound({x, INF}));
            se.erase(cur);
            pll nxt1 = {cur.first, x};
            pll nxt2 = {x, cur.second};
            se.insert(nxt1);
            se.insert(nxt2);
        } else {
            auto cur = *prev(se.upper_bound({x, INF}));
            ll res = cur.second-cur.first;
            print(res);
        }
    }
}
