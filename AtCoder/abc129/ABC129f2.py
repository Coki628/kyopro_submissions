...
・行列累乗の練習になると聞いていたやつ。自力ならず。。
・行列累乗
・まず遷移をまとめられず。log10したいのに/10してた。。
　それとi*b+aを行列に落とし込みたいけど、iとか付いてると定数項にはできないし、
　どーしよーってなって終わった。
・やり方としては、こっちも同じiと連動して動くんだから、DPにしてしまえばいいだけだった。
　答えを求めるdp0と、この等差数列を保持するdp1を作って、両方一緒に進めればいい。
・あと、その時のdp1の値の桁数によって、10の何乗するかが変わってしまう。
　これもそのままだと行列に落とせないので、何桁動くか毎に処理を分ける。
　各桁に何項あるかは、多分計算O(1)でできるんだろうけど、式変形して二分探索した。
・桁毎に項数をまとめられたら、式が固まるので行列に落とせばいい。
　dp2つと定数項で3*3を作る。行列と漸化式の対応はソース内コメント参照。
・ある桁の計算が終わったら、その結果を次の処理の初期値として置く。
・ちょっと考えて、にぶたんのとこ計算O(1)にした。できる人はこんなんすぐ出るんだろうな。。
"""
