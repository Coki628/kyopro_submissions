...
・自力ならず。。これは悔しい。
・セグ木
・ずっとDP方針で悩んで、今何本持ってるかの添字が要るなぁ。。から進めず。
・DPっぽい感じがするけど実はDPすらいらない。見方を、その町で何本買うかでなく、
　その町で飲むドリンクはどの町で買うか、とすると、K個前までの町のうち最小の所で購入するだけになる。
　これはセグ木なりで適当に取れる。
*/
...
void solve() {
    ...
    ll ans = 0;
    rep(i, N) {
        ans += seg.query(max(i-K+1, 0LL), i+1);
    }
    print(ans);
}
