/*
参考：https://blog.hamayanhamayan.com/entry/2020/08/23/035153
　　　https://atcoder.jp/contests/abc176/editorial
・ABC高難度埋め
・むずい。けどやっぱり高難度は得られる知見もでかい。
・インラインDP、in-place DP、実家？呼び方統一してほしい。
・普通に考えられるのは3乗のDP。まあこれも多少DP慣れしてないと思いつかなそうだけど、
　ここまでは何とか自力で進めた。
・で、これを2乗に落とすためにin-place DPする。よくよく精査すると、
　更新されるのがその時見ている3つに関連するものに限定されるためこれができる。
　各遷移はソース内コメントやはまやんさん、公式解説を参照。
　この遷移を全部正確に自力で詰めるのは、やはりまだ到底無理そうだと分からされる。
・解説見ながら進めてもなかなかWAが取れなくて、特に最後まで引っかかってたのが、
　片側の情報毎のmaxが必要になる部分。自分のコードではcol[any]みたいなやつ。
　遷移元が片側しか固定されない場合、反対側は何でもいいので、その列に限定したmaxが必要になる。
　これの使い所のイメージがなかなか頭に入ってこなくて大変だった。
*/
...
void solve() {
    ...
    auto dp = list2d(N, N, -INF);
    dp[A[0]][A[1]] = dp[A[1]][A[0]] = 0;
    ll prevmx = 0, alladd = 0;
    vector<ll> col(N, -INF);
    col[A[0]] = col[A[1]] = 0;
    rep(i, N-1) {
        ll j = i*3;
        vector<ll> cur = {A[j+2], A[j+3], A[j+4]};
        sort(ALL(cur));
        ...
        if (cur[0] == cur[1] or cur[1] == cur[2]) {
            ll a = cur[1];
            ll b = cur[0] == cur[1] ? cur[2] : cur[0];
            rep(any, N) {
                // 今回2つ出たaと元々あったaを使って、元々あったanyと今回出たbを残す
                // chmax(dp[i+1][b][any], dp[i][a][any]+1);
                tmp.pb({b, any, dp[a][any]+1});
            }
        }
        // others
        rep(k, K) {
            ...
            ll b = cur[(k+1)%K];
            ll c = cur[(k+2)%K];
            // 今回出たaと元々あった2つのaを使って、今回出たb,cを残す
            // chmax(dp[i+1][b][c], dp[i][a][a]+1);
            tmp.pb({b, c, dp[a][a]+1});
            rep(any, N) {
                // 今回出たaと元々あったanyを残す
                // (ここで遷移元のanyじゃない側はどこでもいいので、各列での最大値が必要になる)
                // chmax(dp[i+1][a][any], col[any]);
                tmp.pb({a, any, col[any]});
            }
        }
        for (auto &comb : combinations(cur, 2)) {
            ll a = comb[0], b = comb[1];
            // 今回出たa,bを残す
            // chmax(dp[i+1][a][b], prevmx);
            tmp.pb({a, b, prevmx});
        }
        // 遷移元の情報を潰さないように、更新は最後にまとめてやる
        for (auto& [a, b, val] : tmp) {
            chmax(dp[a][b], val);
            chmax(dp[b][a], val);
            chmax(col[a], val);
            chmax(col[b], val);
    ...
    rep(a, N) {
        rep(b, N) {
            chmax(ans, dp[a][b]+alladd+(a == b and a == A[M-1]));
        }
    }
    print(ans);
}
