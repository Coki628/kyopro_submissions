...
・これは部分点2だけ。
・トポロジカルソート
・とりあえずSCCでやった。でもこれだと復元1個しかできない。。
・ランダムに制限を1つずつ追加していく方針。まあダメだけど。。
・ちょっと変えて、1回変えてすぐ戻す感じに。まあダメだけど。。
・そもそも追加する辺を完全にランダムに。まあダメだけど。。
・方針を変えてメモ化再帰。入りの頂点をランダムにしたり、
　途中の辺の並びをランダムにしたりしたけど、これもダメ。
　さっきよりちょっとAC増えたけどね。。
*/
...
void solve() {
    ...
    while (ans_all.size() < K) {
        ...
        auto hash = rh.get_hash(ans, 0, ans.size());
        if (not se.count(hash)) {
            ans_all.pb(ans);
            se.insert(hash);
        }

        clock_t cur = clock();
        if (cur-start > 2900000) {
            print(-1);
            return;
        }
    }
    for (auto &ans : ans_all) print(ans);
}
