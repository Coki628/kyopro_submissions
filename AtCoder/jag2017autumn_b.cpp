...
・自力AC！
・構文解析、スタック、括弧列
・括弧列の問題みたいに、閉じ括弧が来た時点でその手前を決める、みたいに進めるとうまくいく。
　対戦の度に勝利数を減らしていって、矛盾する場所がないか見ていく。
・全部終わった後、勝利数を全員使い切ってるかのチェックをしてなくて1ペナ。
*/
...
void solve() {
    ...
    ll N = 0;
    rep(i, L) {
        if (ascii_lowercase.find(S[i]) != -1) N++;
    }
