...
・自力AC！！やったね。
・実装、グリッド、2次元ロリハ
・まあまあ苦戦したけど粘って良かった。最後に工夫が必要だった。
・基本的には、ありうるサイズ、回転の型を全部ハッシュで用意して、
　愚直に元のグリッドを全チェックする。でもまともにやったらTLE。
　計算量はグリッド*型サイズの種類数で、これは1億4000万くらいにあるので確かにきつめ。
・1000*1000グリッドで、型の最小サイズが7なので、全部見るとざっと1000/7=142種類くらいのサイズを
　チェックする必要があるのだが、当然グリッドのサイズがあるので、これら全てが存在することはない。
　元の型をよく観察すると、2*2四方が塗られた領域はいずれの型にも存在しないので、
　2*2四方が塗られていたらそれはサイズが2、同様に3*3が塗られていたらサイズ3だと判断できる。
　この方法でグリッドを前もってチェックして、ありえるサイズを列挙しておき、
　それらのサイズについてだけロリハやってどれが何個あるかチェックしたら間に合った。
　AC1.40秒。絶対想定解じゃなさそー。。
*/
...
vector<string> scale(const vector<string> &grid, ll K) {
    int H = grid.size();
    int W = grid[0].size();
    vector<string> res(W*K, string(H*K, '*'));
    rep(i, H*K) {
        rep(j, W*K) {
...
void solve() {
    ...
    for (ll n : V) {
        ...
        rep(i, 3) {
            ...
            rep(_, 4) {
                vector<uint64_t> ptn_hashs(R);
                rep(i, R) {
                    ptn_hashs[i] = rh1.get_hash(scaled[i], 0, C);
                }
                auto target = rh2.get_hash(ptn_hashs, 0, R);
                se[i].insert(target);
                scaled = rot90(scaled);
            }
        }
    ...
    for (ll n : V) {
        ll R = n*7, C = n*7;
        auto rowhashs = list2d<uint64_t>(H, W-C+1, 0);
        rep(i, H) {
            auto table = rh1.build(grid[i]);
            rep(j, W-C+1) {
                ll k = j+C;
        ...
        auto colhashs = list2d<uint64_t>(W-C+1, H-R+1, 0);
        rep(i, W-C+1) {
