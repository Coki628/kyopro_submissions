...
def bfs(x):
    ...
    while len(que) != 0:
        ...
        for direction in directions:
            ...
            # 壁はスキップ
            if field[nxt[1]][nxt[2]] == '*':
                continue
            # 現在位置+1の距離を移動先に記録
            if field[nxt[1]][nxt[2]] == '.':
                dist = min(memo[cur[1]][cur[2]] + 1, memo[nxt[1]][nxt[2]])
                # 距離がT以下で訪問済ではない時だけ
                if dist <= T and not visited[nxt[1]][nxt[2]]:
                    memo[nxt[1]][nxt[2]] = dist
                    # 現在の経過時間をキューの優先度として、早い方から先に処理するようにする
                    nxt[0] = dist
                    heappush(que, nxt)
            # 現在位置+xの距離を移動先に記録
            elif field[nxt[1]][nxt[2]] == '#':
                dist = min(memo[cur[1]][cur[2]] + x, memo[nxt[1]][nxt[2]])
                if dist <= T and not visited[nxt[1]][nxt[2]]:
                    memo[nxt[1]][nxt[2]] = dist
                    nxt[0] = dist
                    heappush(que, nxt)
