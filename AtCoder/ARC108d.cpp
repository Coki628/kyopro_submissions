...
・ARC青diff残埋め
・これは自力WA。。
・隣り合う状態4パターンが何個ずつあるかを状態に持ったんだけど、これだとダメなんかなー。。
*/
...
void solve() {
    ...
    map<string, ll> stoid = {{"AA", 0}, {"AB", 1}, {"BA", 2}, {"BB", 3}};
    ...
    stoc["BB"] = bb;
    vvi nxt(4);
    for (string from : {"AA", "AB", "BA", "BB"}) {
        char c = stoc[from];
        string to1 = {from[0], c};
        string to2 = {c, from[1]};
        nxt[stoid[from]].eb(stoid[to1]);
        nxt[stoid[from]].eb(stoid[to2]);
    }

    N -= 2;
    vector<set<vector<ll>>> dp(N+1);
    dp[0].insert({0, 1, 0, 0});
    rep(i, N) {
        for (auto& v : dp[i]) {
            rep(j, 4) {
