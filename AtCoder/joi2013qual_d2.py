...
・numpy実装もやろうとしたけど、とりあえずうまく行かなかったので保留。
"""
...
dp = np.full((D+1, mxC+1), -INF, dtype=np.int64)
...
prevcs = np.array(list(range(mxC+1)), dtype=np.int64)
for i in range(1, D):
    t = T[i+1]
    for j in range(N):
        a, b, c = ABC[j]
        if a <= t <= b:
            # ここ、うまく遷移してない
            dp[i+1] = np.maximum(dp[i+1], dp[i]+np.abs(c-prevcs))
