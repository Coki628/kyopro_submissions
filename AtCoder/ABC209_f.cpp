/*
参考：https://blog.hamayanhamayan.com/entry/2021/07/11/154241
・ABC高難度埋め
・挿入DP、累積和で遷移高速化
・ついこの前edpc_tで挿入DPらしきものを初めてやった時、どの辺が挿入なのか釈然としなかったけど、これで分かった。
　edpc_tは残っている要素の並びに対して、何番目を選ぶ、みたいな遷移をしていくのに対して、
　今回のこれは置き終わった並びに対して、何番目に挿し込む、みたいな遷移をする。確かに挿入っぽい。
　実際にしている遷移の形はほぼ同じ。
・配る方が考えやすいから先に配る遷移で作って、貰う遷移にして、累積和で高速化、っていう流れも一緒。
　多分先にこっちやって後からedpc_tやる方が順序としてはいいのかもね。
*/
...
void solve() {
    ...
    // dp[i][j] := i番目まで決めて、直前の木の操作順が(ここまでに決めた中で)j番目の場合の通り数
    auto dp = list2d<mint>(N, N, 0);
    dp[0][0] = 1;
    rep(i, 1, N) {
        ll done = i;
