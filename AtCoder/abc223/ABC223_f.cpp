...
・終了後10分くらいで自力AC。
・括弧列、クエリ系、遅延セグ木
・括弧列を+-の累積で表すと、スワップは+2か-2の区間加算、
　クエリは区間最小が端以上かつ左右端が同値か？て判定できるので、遅延セグ木やる。
・区間加算の範囲を勘違いして1長くしてたのが敗因。考察時に手計算ミスって。まじでしょうもない。。
*/
...
void solve() {
    ...
    rep(i, Q) {
        ...
        if (op == 1) {
            if (S[l] == '(' and S[r] == ')') {
                seg.update(l+1, r+1, -2);
            } elif (S[l] == ')' and S[r] == '(') {
                seg.update(l+1, r+1, 2);
            }
            swap(S[l], S[r]);
        } else {
            if (seg[l] == seg[r+1] and seg.query(l, r+2) >= seg[l]) {
                Yes();
            } else {
                No();
            }
