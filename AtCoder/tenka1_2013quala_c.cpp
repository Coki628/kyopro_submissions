...
・試験管青diff残埋め
・時間かかったけど自力AC！！
・グリッド、場合分け、再帰全探索、行列累乗
・大変。グリッドの大きさによって3つくらい、別の問題みたいに違う解法で解く。
・HもWも大きい時は、考察と手元実験する。すると、中に入る数字はほぼ固定であることが分かる。
　一部の例外を除いて、1行の数字は312131213121...の4つで繰り返す。
　グリッド全体で見ると、特に制約の厳しい3の置き方は限定的で、
　左下に連続するか右下に連続するかのいずれかになる。
　最初に一部の例外と言ったのは、四隅だけは配置によって2通りにできる場合があるから。
　これは3が左下の時には左上と右下、逆の場合は右上と左下について確認する必要がある。
　H,Wが両方4以上ある時は、3の都合上これ以上は動かせないのでここに該当する。
・HもWも小さい時は3^Nの再帰全探索ができるので、実際に全てのマスに順に置いていく。
・H,Wの片方だけが大きい時は、DPからの行列累乗に乗せる。
　ABC199_fとかABC204_fみたいな感じでDPの遷移から、遷移行列を作る。
　てか、ここまで書いてて思ったんだけど、再帰全探索いらなくて全部こっちで行けそうだな。。
・ちなみに3行前までの状態を持ってDPしたらTLEした。3列の場合での有効な3マスパターンが7つあって、
　計算量は100万*7^3(遷移も入れたら7^4か)くらいになり、これだと億超えだからきついみたい。。
　で、もう少し考えたらこの遷移なら行列累乗に乗りそうだと思って、やってみたら無事うまくいった。
*/
...
void solve() {
    ...
    if (H == 1 and W == 1) {
        ...
        auto rec = [&](auto&& f, ll i) -> void {
            ...
            if ((h < 1 or grid[h-1][w] != 1) and (w < 1 or grid[h][w-1] != 1)) {
                grid[h][w] = 1;
                f(f, i+1);
                grid[h][w] = 0;
            }
            if (
                (h < 1 or grid[h-1][w] != 2) and (w < 1 or grid[h][w-1] != 2)
                and (h < 2 or grid[h-2][w] != 2) and (w < 2 or grid[h][w-2] != 2)
            ) {
                grid[h][w] = 2;
                f(f, i+1);
                grid[h][w] = 0;
            ...
            if (
                (h < 1 or grid[h-1][w] != 3) and (w < 1 or grid[h][w-1] != 3)
                and (h < 2 or grid[h-2][w] != 3) and (w < 2 or grid[h][w-2] != 3)
                and (h < 3 or grid[h-3][w] != 3) and (w < 3 or grid[h][w-3] != 3)
            ) {
                grid[h][w] = 3;
                f(f, i+1);
                grid[h][w] = 0;
        ...
        auto rec = [&](auto&& f, ll i) {
            ...
            if (cur.size() < 1 or cur[cur.size()-1] != 1) {
                cur.eb(1);
                f(f, i+1);
                cur.pop_back();
            }
            if (
                (cur.size() < 1 or cur[cur.size()-1] != 2)
                and (cur.size() < 2 or cur[cur.size()-2] != 2)
            ) {
                cur.eb(2);
                f(f, i+1);
                cur.pop_back();
            ...
            if (
                (cur.size() < 1 or cur[cur.size()-1] != 3)
                and (cur.size() < 2 or cur[cur.size()-2] != 3)
                and (cur.size() < 3 or cur[cur.size()-3] != 3)
            ) {
                cur.eb(3);
                f(f, i+1);
                cur.pop_back();
        ...
        rep(j, L) {
            rep(k, L) {
                rep(l, L) {
                    vvl cur = { patterns[j], patterns[k], patterns[l] };
                    rep(m, L-1) {
                        auto nxt = patterns[m];
                        bool ok = true;
                        rep(h, H) {
                            if (nxt[h] == 1 and cur[2][h] == 1) {
                                ok = false;
                                break;
                            }
                            if (nxt[h] == 2 and (cur[2][h] == 2 or cur[1][h] == 2)) {
                                ok = false;
                                break;
                            }
                            if (nxt[h] == 3 and (cur[2][h] == 3 or cur[1][h] == 3 or cur[0][h] == 3)) {
                                ok = false;
                                break;
                            }
                        }
                        if (ok) {
                            mat[k*L*L+l*L+m][j*L*L+k*L+l] = 1;
                        }
                    }   
                }
            }
        ...
        MatPow<ll> mp;
        auto res = mp.solve(mat, init, W);
        ll ans = sum(res);
        print(ans);

        // auto dp = list4d(W+1, L, L, L, 0LL);
        // dp[0][L-1][L-1][L-1] = 1;
        // rep(i, W) {
        //     rep(j, L) {
        //         rep(k, L) {
        ...
        rep(x, 4) {
            ...
            ll a = x;
            ll b = x+(H-1)+(W-1);
            if (modulo(a, 4) == 2 or modulo(a, 4) == 3) {
                cnt *= 2;
            }
            if (modulo(b, 4) == 1 or modulo(b, 4) == 2) {
                cnt *= 2;
            }
            // 3が右下
            ans += cnt;
