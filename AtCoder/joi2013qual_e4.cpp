 ...
 * ・きっちり自力AC！
 * ・3次元座圧
 * ・計算量的にいもすしなくてもいいのは覚えてた。よって愚直に座圧を書くのみ。
 */
...
void solve() {
    ...
    auto [zpd, unzpd] = compress(S3);
    ll M1 = zpx.size();
    ll M2 = zpy.size();
    ll M3 = zpd.size();

    auto cnt = list3d(M1, M2, M3, 0);
    rep(i, N) {
        rep(j, zpx[X1[i]], zpx[X2[i]]) {
            rep(k, zpy[Y1[i]], zpy[Y2[i]]) {
                rep(l, zpd[D1[i]], zpd[D2[i]]) {
                    cnt[j][k][l]++;
                }
            }
        }
    ...
    rep(i, M1) {
        rep(j, M2) {
            rep(k, M3) {
                if (cnt[i][j][k] >= K) {
                    ll s = (unzpx[i+1]-unzpx[i])*(unzpy[j+1]-unzpy[j])*(unzpd[k+1]-unzpd[k]);
                    ans += s;
                }
            }
        }
