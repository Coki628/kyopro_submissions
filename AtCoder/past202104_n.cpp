...
・自力ならず。。
・ソートしてDP、式変形
・ソートの優先順位の付け方に工夫が必要。A/Bの降順にする。
　これの正当性は2つの要素を順番逆で付けたものと式変形すると分かる。
・その式変形は出なかったけど、このソート自分でも試してた。
　割った時の型がllのままだったから切り捨てになってダメだったんだ。
　ケアレスミスじゃん。しょーもな。。
*/
...
void solve() {
    ...
    sort(ALL(AB), [](pll a, pll b) -> ld {
        return a.first/(ld)a.second > b.first/(ld)b.second;
    });

    auto dp = list2d(N+1, H+1, -INF);
    dp[0][H] = 0;
    rep(i, N) {
        auto [a, b] = AB[i];
        rep(j, H+1) {
            ...
            if (j) {
                chmax(dp[i+1][max(j-b, 0LL)], dp[i][j]+a*j);
            }
        }
    }
    ll ans = max(dp[N]);
