/*
参考：https://img.atcoder.jp/dwango2015-prelims/editorial.pdf
　　　https://atcoder.jp/contests/dwango2015-prelims/submissions/26711157
・試験管青diff残埋め
・自力AC！！時間かかったけどこれは嬉しい。
・確率DP、期待値DP
・いわゆる自己ループのある期待値DP。そのパートは割と素直だと思うんだけど、
　各遷移先への確率を求めるパートがだいぶ悩んだ。
・GCPそれぞれの手を何人出すかの確率が求まれば、それに期待値DP内ではそれに基づいて遷移すればいい。
　通り数はnCrで求まりそうだと思ったけど、今回はmod取らないで実数計算なので、
　通り数を陽に求めるとnCr(100, 50)とかやったらllが余裕でオーバーフローすると思ったのでやめた。
　もうしばらく悩んで、確率DPっぽいことをすれば求まると気付いてやった。
・遷移先をどうするかは問題に書いてあるルールに基づいて丁寧にやればOK。
　2グループ同人数でどっちかを選ぶ所は、同人数じゃない時と同一視しちゃって大丈夫。
　(どっち選んでも結果的に人数は同じだし。)
・期待値DPはあいこ扱いで自己ループのパターンがあるけど、これは期待値DPの論点で頻出のやつ。
　ただし今回は遷移が「5通りの遷移のうち2つが自己ループ」とかみたいな通り数ベースのじゃなくて、
　確率ベースの遷移だったからちょっと今までと違った。でも考え方は同じで、
　通り数全体だった所を確率全体とみなして100%=1として、自己ループじゃない通り数を数えるのは、
　回数じゃなくてそうなる確率を素直に足していけばよくて、これらを *= total/cnt の所に使えばいい。
・前計算を確率DPじゃなくてnCrでやる版。実数計算できるnCr作った。
*/
...
void solve() {
    ...
    auto calc = [](ll n, ll a, ll b) -> ld {
        ll c = n-a-b;
        return nCr<ld>(n, a)*nCr<ld>(n-a, b)/pow((ld)3, (ld)n);
    };

    // P[i][j] := ゲームの結果、i人からj人になる確率
    auto P = list2d(N+1, N+1, (ld)0);
    rep(n, 1, N+1) {
        rep(i, n+1) {
            ll a = i;
            rep(j, i, n+1) {
    ...
    auto rec = [&](auto&& f, ll n) -> ld {
        if (n == 1) return 0;
        if (memo[n] != -1) return memo[n];

        ld total = 1, cnt = 0, exp = 1;
        rep(i, 1, n) {
            exp += f(f, i) * P[n][i];
            cnt += P[n][i];
        }
