/**
 * ・自力AC！これは嬉しい。
 * ・グリッド、行と列を頂点とみなしてH+W頂点のグラフにするやつ、UnionFind
 * ・動き方の雰囲気から、行列を頂点にするやつっぽいのは割とすぐ気付いた。
 * 　で、止まれる点が増えたら、そこの行列がマージみたいなUFかなってなる。
 * 　でもちょっと色々やって合わなくて、考えたら、全部の行か全部の列の、
 * 　どっちかが全部繋がったらOKなんじゃないかと思う。
 * 　これを踏まえた連結確認をやったらサンプル合って、ケアレスミスで1WAしたけど無事AC。
 */
...
int main() {
    ...
    // 行・列がそれぞれいくつのグループにまとまってるか見る
    set<ll> seh, sew;
    rep(i, H) {
        seh.insert(uf.find(i));
    }
    rep(i, W) {
        sew.insert(uf.find(H+i));
    }
    ll ans = min(seh.size()-1, sew.size()-1);
    print(ans);
    return 0;
}
