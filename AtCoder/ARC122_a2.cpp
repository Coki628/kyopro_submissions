/*
・最初にやろうとしたDPの方針。
　回文判定みたいに前と後ろを突き合わせると、
　最後の通り数を基準にしたその時の通り数みたいなやつ？が出てくる。
　これ前にこどふぉでも見たことあるんだよな。(cf1467_d)
　その時も前後掛け合わせて何で合うのか分からなくて、
　釈然としなかったのは覚えてる。
*/
...
void solve() {
    ...
    mint ans = (dp[N-1][0]+dp[N-1][1])*A[0];
    rep(i, 1, N) {
        ans += dp[i][0]*dp[N-i][0]*A[i];
        ans -= dp[i][1]*dp[N-i][1]*A[i];
    }
    print(ans);
}
