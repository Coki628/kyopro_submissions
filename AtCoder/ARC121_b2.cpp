/*
参考：https://atcoder.jp/contests/arc121/editorial
・実は同じBを取っちゃう可能性は気にしなくてよくて、
　それを考慮した簡潔な版。
・理由としては、同じやつを取り合っちゃうような状況の場合、
　GB,BR型の組が1つの方が絶対同じか得する感じになるから。だと思う。
・実際、やってみるとこれでちゃんと通る。。
*/
...
void solve() {
    ...
    if (odd.size() == 0) {
        ...
        ll N0 = adj[odd[0]].size();
        ll N1 = adj[odd[1]].size();
        ll even = 3-odd[0]-odd[1];
        ll ans = INF;
        // odd0:odd1
        for (ll a : adj[odd[0]]) {
            ll i = bisect_right(adj[odd[1]], a);
            if (i) chmin(ans, abs(a-adj[odd[1]][i-1]));
            if (i < N1) chmin(ans, abs(a-adj[odd[1]][i]));
        }
        // odd0:even, odd1:even
        ll N2 = adj[even].size();
        ll mn1 = INF, mn2 = INF;
        for (ll a : adj[odd[0]]) {
            ll i = bisect_right(adj[even], a);
            if (i) chmin(mn1, abs(a-adj[even][i-1]));
            if (i < N2) chmin(mn1, abs(a-adj[even][i]));
        }
        for (ll a : adj[odd[1]]) {
            ll i = bisect_right(adj[even], a);
            if (i) chmin(mn2, abs(a-adj[even][i-1]));
            if (i < N2) chmin(mn2, abs(a-adj[even][i]));
        }
        chmin(ans, mn1+mn2);
        print(ans);
    }
