/*
参考：https://sen-comp.hatenablog.com/entry/2021/02/20/184839
　　　https://kmjp.hatenablog.jp/entry/2021/02/07/0900
・自力ならず。
・gcd系、添字入れ替え、約数列挙
・まず、確実に言えることとして、min(A)より大きいのは無理。
　逆にmin(A)以下で生成可能な値は全てOK。minからは新しい値はできないので、
　任意の組でgcdすることによってどんな値が作られるか考える。
・ここで、適当にペアにしてガチャガチャやると嘘貪欲だし、
　部分和DPのgcd版みたいのをmapで進めると状態が多すぎて無理。
　でもこのDP方針が実は正解のヒントになっていたりする。
・このDPの添字に使っていたgcdを値に使うことを考えてみる。
　値は元々存在チェックで、無かったようなものなので、
　じゃあ添字は何にすればいいんだってなって、まあ自分で考え至るのはここまでだと思う。
　dp[g] := gを目指した時に実際に作れるgcd
　みたいな感じにする。gcdをgとしたい時に、元の値はgの倍数であることが必須なので、
　逆に倍数にしてってA[i]を通るような、つまりA[i]の約数に絞って値を更新する。
　するとgとしては、使えるA[i]全てからgcdをかけてもらえる感じになるので、
　これで実際に値の方がgになっていればそのgcdは達成可能だし、そうでないなら無理。
・ここにもう1つの条件であるmin(A)以下ってことも加えれば、答えを数えられる。
*/
