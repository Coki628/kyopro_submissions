/*
・終了後自力AC？TLの話はちょっと見ちゃったけど、自分で考えてたこと以上はほぼ頭に入ってきてない。
・数え上げ、主客転倒、式変形
・N-1箇所に+を入れるか否かの2^(N-1)通りの値の総和。当然、これ系は主客転倒で各S[i]が何回使われるか、の方針を考える。
・各S[i]は、例えばS[i]=3なら3,30,300,...と後ろにいくつ0が付くかで使われ方が異なる。
　これは、自身以降どこに最初に+が入るかによる。なので、自由に選べる所を2冪、決まってる所を固定にして、
　j-i個の0が続くとすると、2重ループでなら、「2^手前で自由な場所*S[i]*10^(j-i)*2^後ろで自由な場所」として
　ΣΣ2^i*S[i]*10^(j-i)*2^(N-j-2) といった形に表せる。(最後だけちょっと不規則になるので別途足すけど)
　ここで、10^(j-i) = 10^j*10^(-i) と式変形すれば、i,jの添字が独立にできるので、
　j側を予め累積和で前計算しておけば、各iについて一気に計算できる。
　この指数添字の式変形のアプローチはARC116_bで悔しい思いをした時のことを覚えてたのが役に立った。
・真剣に考察始めてからそこまで時間かからずに解けたので、Eじゃなくてこっち取り組んでたら通せてたかもなぁ。。
　まあでもこればっかりは見極めむずいのでしょうがない。
*/
...
void solve() {
    ...
    Accumulate<mint> acc(N-1);
    rep(i, N-1) {
        acc.set(i, mint(10).pow(i)*mint(2).pow(N-i-2));
    }
    acc.build();

    mint ans = 0;
    rep(i, N) {
        ans += mint(2).pow(i)*A[i]/mint(10).pow(i)*acc.query(i, N-1);
        // rep(j, i, N-1) {
        //     // ans += mint(2).pow(i)*A[i]*mint(10).pow(j-i)*mint(2).pow(N-j-2);
        //     ans += mint(2).pow(i)*A[i]*mint(10).pow(j)/mint(10).pow(i)*mint(2).pow(N-j-2);
        // }
        ans += mint(2).pow(i)*A[i]*mint(10).pow(N-1-i);
    }
    print(ans);
}
