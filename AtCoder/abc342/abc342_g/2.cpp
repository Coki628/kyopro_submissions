...
struct Func {
    ...
    bool operator==(const Func &f) const {
        return add == f.add and sub == f.sub;
    }
};
auto g = [](ll val, const Func &func) -> ll {
    assert(func.sub.empty());
    if (func.add.size()) {
        chmax(val, (ll)*--func.add.end());
    }
    return val;
};
auto h = [](Func &f1, const Func &f2) -> Func {
    for (auto &a : f2.add) {
        f1.add.insert(a);
    }
    for (auto &a : f2.sub) {
        auto itr = f1.add.find(a);
        if (itr != f1.add.end()) {
            f1.add.erase(a);
        } else {
            f1.sub.insert(a);
...
void solve() {
    ...
    auto seg = get_dual_segment_tree(N, g, h, T, E);
    rep(i, N) {
        Func func;
        func.add.insert(A[i]);
        seg.update(i, func);
