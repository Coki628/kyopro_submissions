/*
参考：https://twitter.com/e869120/status/1386138990361726978/photo/4
　　　https://github.com/E869120/kyopro_educational_90/blob/main/sol/023-04b.cpp
・E8さん典型90問、残りの高難度埋め
・速いhashmap
・大変だった。高速化が。色々やったけどダメで、最後はシンプルなやつが効いた。
・結局中身ほとんどいじってなくて、速いhashmap使って普通にDPしたら通った。。
　普通のmapとunordered_mapだとダメだったから、これが初めて役立った。昔見つけた時一応キープしといてよかった。
・制約8秒でギリギリのAC7.13秒。
*/
...
void solve() {
    ...
    ll N = H*W;
    vector<gp_hash_table<int, mint, custom_hash>> dp(2);
    dp[0][0] = 1;
    int msk = (1LL<<(W+1))-1;
    rep(i, N) {
        auto [h, w] = idtogrid(i, W);
        for (auto [bit, val] : dp[i%2]) {
            dp[(i+1)%2][msk&(bit<<1)] += val;
            ...
            dp[(i+1)%2][msk&(bit<<1)|1] += val;
        }
        dp[i%2].clear();
    }
    mint ans = 0;
    for (auto [k, v] : dp[N%2]) ans += v;
    print(ans);
}
