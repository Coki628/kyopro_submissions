/*
参考：https://blog.hamayanhamayan.com/entry/2019/07/21/002537
・ABC高難度埋め
・当然自力ならず。未履修典型だったししょうがない。
・箱根DP
・順列の並べ替えを、並べ替え前と後の列でペアして決めていく感じ。
　基本はその本家箱根DPの考え方がベースになって、そこに今回は、
　「奇妙さ」という状態が1つ増える。
・5種類の遷移の詳細はソース内コメント参照。
　遷移時の奇妙さの増え方については、未確定のものが残った状態で
　見ている位置が1つ進むと、未確定要素1つにつき1ずつ奇妙さが増える感じ。
　(確定させる移動先のindexが確実に1つ遠くなるから。)
　遷移時の通り数の増え方については、本家箱根DPと考え方はだいたい同じ。
・あと、未確定がj個、って言い方したけど、未確定がjペアあるって
　考える方が分かりやすいかもしれない。個で言うなら常に2個ずつ決まってく感じ。
*/
...
void solve() {
    ...
    // dp[i][j][k] := i番目まで見て、未確定がj個、奇妙さがkの時の通り数
    auto dp = list3d<mint>(N+1, N+1, K+1, 0);
    dp[0][0][0] = 1;
    rep(i, N) {
        rep(j, N+1) {
            rep(k, K+1) {
                // ここでは何も決めない
                if (j < N and k+(j+1)*2 <= K) dp[i+1][j+1][k+(j+1)*2] += dp[i][j][k];
                // 並び替え前にiだったものを未確定だったj個から決めて
                // 並び替え後にiになるものは決めない
                if (k+j*2 <= K) dp[i+1][j][k+j*2] += dp[i][j][k]*j;
                // 並び替え前にiだったものは決めないで
                // 並び替え後にiになるものを未確定だったj個から決める
                if (k+j*2 <= K) dp[i+1][j][k+j*2] += dp[i][j][k]*j;
                // 並び替え前にiだったものを未確定だったj個から決めて
                // 並び替え後にiになるものも未確定だったj個から決める
                if (j and k+(j-1)*2 <= K)  dp[i+1][j-1][k+(j-1)*2] += dp[i][j][k]*j*j;
                // 並び替え前にiだったものを並び替え後にもiとする
                if (k+j*2 <= K) dp[i+1][j][k+j*2] += dp[i][j][k];
            }
        }
    }
    mint ans = dp[N][0][K];
