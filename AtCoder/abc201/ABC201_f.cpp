/*
参考：https://kanpurin.hatenablog.com/entry/2021/05/27/064905
　　　https://kmjp.hatenablog.jp/entry/2021/05/16/0900
・ABC高難度埋め
・遷移の作り方が難しいDP、DP高速化
・主にかんプリンさんの解説で理解を進めた。
・動かさない人の集合を決めると、残りの人達に行うべき操作が一意になる。
　これをDPで並びが前の人から確定していく。この辺の理屈は難しい。
　とりあえず3種類の操作を、最初の要素を置く前と、遷移中、最後の要素を置いた後で見るんだけど、
　まあ詳細は先人たちの解説を参照して欲しい。自分がうまく説明できるレベルじゃない。
・確定は並びが前の人からしていくが、DP配列上の並びは人の番号順となっているのでややこしい。
　つまりDP配列としては前から決まっていく訳ではない。でもここでDP配列上の自分より手前から貰う遷移をすることで、
　並びが自分より前でかつ番号順も自分より前な人だけを遷移元の対象とできる。
・この貰う遷移もひとクセあって、遷移元の添字にも遷移先の添字にも依存した遷移なので、
　そのままだとセグ木に乗せづらい。ここで、これが累積和なことを利用して、
　セグ木に乗せる時には最後までの区間和を足して、貰ってくる時は逆にその位置から最後までの区間和を引くことで、
　足し引き合計としては、遷移元から遷移先までの区間和とすることができている。
*/
...
void solve() {
    ...
    Accumulate<ll> acca(A), accb(B), accc(C);
    auto seg = get_segment_tree(N, [](ll a, ll b) { return min(a, b); }, INF);
    vector<ll> dp(N, INF);
    rep(i, N) {
        // chmin(dp[P[i]], accb.query(0, P[i]));
        // rep(j, P[i]) {
        //     chmin(dp[P[i]], dp[j]+acca.query(j+1, P[i]));
        // }
        dp[P[i]] = min(seg.query(0, P[i])-acca.query(P[i], N), accb.query(0, P[i]));
        seg.update(P[i], dp[P[i]]+acca.query(P[i]+1, N));
    }
    ll ans = INF;
    rep(i, N) {
        chmin(ans, dp[i]+accc.query(i+1, N));
    }
    print(ans);
}
