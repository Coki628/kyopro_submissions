/**
 * 参考：https://blog.hamayanhamayan.com/entry/2019/02/18/215807
 * ・ライブラリ整備
 * ・遅延セグ木、区間和取得・区間加算・区間更新
 * ・区間加算と区間更新を両方やる。それだけならBeats貼るだけなんだけど、
 * 　別件(ARC115_e)で普通の遅延セグ木にそれを乗せたいことがあったので、作った。
 * ・基本的にははまやんさんが記事でやってることをほぼ自分のセグ木の関数に載せ替えてる。
 * 　ただ、そっちの実際にやりたい方がmint数え上げで負数をフラグに使えないので、
 * 　updateかどうかをboolで別に持って、作用素を3つ構成にした。
 * ・うまく同じ挙動にできるか心配だったけど、無事AC確認。
 */
...
void solve() {
    ...
    LazySegTree<pll, Func> seg(f, g, h, {0, 0}, {0, 0, 0});
    ...
    rep(_, M) {
        ...
        seg.update(0, N, {t-cur, 0, 0});
        ans += seg.query(l, r).first;
        seg.update(l, r, {0, 0, 1});
        cur = t;
    }
    print(ans);
}
