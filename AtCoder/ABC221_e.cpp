...
void solve() {
    ...
    vector<ll> accmn(N+1, INF), accmx(N+1, -INF);
    rep(i, N-1, -1, -1) {
        accmn[i] = min(A[i], accmn[i+1]);
        accmx[i] = max(A[i], accmx[i+1]);
    }
    ...
    Accumulate<mint> acc(pow2);

    mint ans = 0; 
    rep(i, N-1) {
        rep(j, i+1, N) {
            if (accmx[j] < A[i]) break;
            if (accmn[j] >= A[i]) {
                ans += pow2[j-i-1] * acc.query(0, N-j);
                break;
            }
            if (A[i] <= A[j]) {
                ans += pow2[j-i-1];
