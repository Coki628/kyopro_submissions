...
・セグ木でDP
・セグ木26本生やしてDPした。
　DP[c][i] := i番目の前まで見て、次に使う先頭文字がcの時の最小値
　みたいに。書く添字で遷移見るのは文字数26個分、
　今回末尾にする文字が、cと同じの時だけ遷移させない、みたいにやる。
・終了後、出してみたらWAで、添字バグってた。。
　末尾チェックに使うのはi-1番目だけど、次の先頭文字になるのはi番目だね。
　同じにしちゃってた。。
・セグ木がテンプレート引数にしてから、vectorに入れられなくなってたのを
　何とかしてみた。結局、テンプレート引数にfucntion当てたらうまく動いた。
*/
...
void solve() {
    ...
    vector<SegmentTree<ll, function<ll(ll, ll)>>> seg(M+1, {N+1, [](ll a, ll b) { return min(a, b); }, INF});
    rep(i, M) seg[i].build();
    seg[A[0]].update(0, 0);
    rep(i, 2, N+1) {
        rep(j, M) {
            if (A[i-1] == j) continue;
            ll mn = seg[j].query(0, i-1)+1;
            if (mn < seg[A[i]][i]) {
                seg[A[i]].update(i, mn);
            }
        }
