/*
参考：https://twitter.com/e869120/status/1409177497833406469/photo/1
・AGC青diff以下残埋め
・自力ならず。。これは解けるべきだっただろうな。
・順列、部分和DP応用
・E8さんが噛み砕いた解説を上げてくれていたので、それで理解。
・部分和DPをベースに解いていくであろうことは予想できる。だが順列が問われている以上、
　並び順を考慮した通り数にしないといけないので、普通にはやれない。どこかで順序に関する情報をまとめる必要がある。
・さて、ここで自分が気付くべきであった事実が2つある。
　1つは、2人が取る要素の集合と取る順序が決まれば、順列は一意に定まること。
　これは実際に適当に集合を決めてやってみると分かるが、ほんとになるようにしかならない。
　ここまで来ればこれに気付けたかもしれないけど、この前段階がある。
　2つ目、言及されてないけど自分的にはこっちが決め手。
　2人が取る要素の集合(ただし和が等しくなるもの)を決めると、それを達成するような順列は必ず存在するということ。
　今回は操作がgreedyに決まってしまうので、要素の組み合わせによっては達成できたりできなかったり
　するんじゃないかと思ってしまったんだけど、これがそもそもいけなかった。
　よくよく考えると確かに、今回のように2人の和が等しく終わる場合ということなら、
　どっちかが余ってかつ自分のターンにならないってことはありえない。
　これに気付けないと先に進めなかった。
・ここまでの考察が整えば、後は普通の部分和DP+何個取ったかも持たせて、
　取った個数に応じて両者の並び順の順列を掛ければいい。
*/
