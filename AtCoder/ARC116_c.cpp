/*
参考：https://atcoder.jp/contests/arc116/editorial
・自力ならず。。
・MOD数え上げ、 nCr系、重複組み合わせ、素因数分解
・2次元DPっぽいところから、全く考察が進まなかった。。
・終了状態に注目する。すると、その値になるためには、
　必ずどこかで必要な素因数を全て取ってくる必要があることが分かる。
　そしてこのどこか、はN回の遷移のうちどこでもいいことも分かる。
　こうなると、各素因数について、N個のグループに対して、
　その素因数の個数分を振り分ける通り数なので、重複組み合わせがハマる。
　これを各終了状態M個について求めて和にすればOK。
・ちなみに素因数分解は√Nかかる愚直なやつを使ったけど、
　これで20万にN√Nでも余裕で間に合った。AC0.21秒。
*/
