/*
・メビウス関数の履修、約数系包除
・難しかったー。とりあえず今の理解としては、
　・最終的に「gcd=1での通り数」が欲しい
　・gcd=1~Nでの「gcdがx以上となる通り数」なら求まる
　って状況の時に、求まった各xでの値にメビウス関数を掛ければ、
　gcd=1での通り数が求まる、って感じなんだと思う。
*/
...
void solve() {
    ...
    auto mobius = eratosthenes_sieve(R);

    // GCDが1になる通り数
    ll res = 0;
    rep(g, 1, R+1) {
        // GCDがgの倍数になる通り数
        ll cnt = pow(R/g-(L-1)/g, 2);
        // メビウス関数を係数として足し引きする
        res += cnt*mobius[g];
    }
    // 互いに素ではない(gcd!=1)数 = 全体 - 互いに素な数(gcd=1)
    ll ans = pow(R-L+1, 2)-res;
