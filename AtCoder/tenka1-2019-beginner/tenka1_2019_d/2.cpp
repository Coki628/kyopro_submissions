/*
参考：https://www.youtube.com/watch?v=uQvc7k2xamY
・ABC系高難度埋め
・自力ならず。。やっぱ黄diffは甘くない。
・三角形の性質、数え上げ、余事象、部分和DP応用、包除原理
・三角形が成立する条件として、other1+other2 > max って感じで覚えてたんだけど、
　そこまで分かってて 全体の総和/2 > max だってことに結びついてなかった。言われると確かに。。
・で、これの余事象を使う。このままだと、maxにしたい集合1つ決めようとしても、
　それがmaxなのかどうかが他方2つの集合がないと分からない。それに対して、
　全体の総和/2 <= max の形なら、ある集合が全体の総和/2以上であれば、
　それは(同率1位はあるにせよ)確実に最大値を取ってると言える。これなら、
　dp[i][j] := i個見て、集合3つのうち、ある1つに入る和がjとなる通り数
　みたいなDPから全体の総和/2の部分を取り出せる。
・で、これだと同率1位の辺が2つで残り1つが0みたいなケースが被る。
　これを包除で取り除く。解説PDF見ただけだと、何でここが被るって気付けるんだって思ったけど、
　解説放送見て3つの集合のベン図見たら分かった。確かにああいう考察をしたら気付けそうだ。
　同様の場所が計3箇所あるってことも視覚的にはっきり見て取れる。
　これに該当する場所は、似たようなDPで求まる。むしろこっちのが集合2個で考えるので、普通の部分和DPと言える。
・その部分和DPパートについて注意として、最初にやる方は集合3つで考えるので、
　今見ている(最大値として数えたい)集合以外に行くような、つまり空振りする遷移が2回ある。
　この遷移自体はabc169_fで見た時のことをよく覚えていたので何とか自力で気付けた。
　(abc169_fの方が遷移があからさまに3個ある感じには見えないのでそこは難しく感じる。)
・ここまで整えば、数え上げた通り数を適切に包除原理で足し引きすれば答えが出る。
・余談だけど解説放送がこの頃まだりんごさんで、今見たらなんか懐かしかったな。
*/
...
void solve() {
    ...
    ll M = sum(A);
    // dp[i][j] := i個見て、集合3つのうち、ある1つに入る和がjとなる通り数
    auto dp = list2d<mint>(N+1, M+1, 0);
    dp[0][0] = 1;
    rep(i, N) {
        rep(j, M+1) {
            // ある1つではない所に行く遷移2回
            dp[i+1][j] += dp[i][j]*2;
            if (j+A[i] <= M) dp[i+1][j+A[i]] += dp[i][j];
        }
    }
    // 半分以上になる部分を使う
    mint sub = 0;
    ...
    dp = list2d<mint>(N+1, M+1, 0);
    dp[0][0] = 1;
    rep(i, N) {
        rep(j, M+1) {
            // 今度は絶対使わない集合が1つあるので、ここの遷移が1回
            dp[i+1][j] += dp[i][j];
            if (j+A[i] <= M) dp[i+1][j+A[i]] += dp[i][j];
        }
    }
    // 半分ちょうどだけ使う(総和が奇数ならそもそも無理なのでそこは場合分け)
    mint add = M%2 == 0 ? dp[N][ceil(M, 2)] : 0;

    mint ans = mint(3).pow(N) - sub*3 + add*3;
    print(ans);
}
