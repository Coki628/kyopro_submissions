/*
参考：https://www.youtube.com/watch?v=KMsEXLe_8go
・自力ならず。。これは悔しい。
・DAGのDP応用
・メモ化再帰にしないで、辺を陽に張らないで、各行列のそれまでの最大値だけを持っていれば、
　各マスでの遷移が一発で済む。
・行列の最大値更新は、aが同値のものを全て済ませてからやりたいので、
　一時配列みたいので更新したい行列と値を持っておく。
*/
...
void solve() {
    ...
    for (auto& [_, li] : A) {
        ...
        for (auto i : li) {
            auto [r, c] = RC[i];
            dp[i] = max(rmx[r], cmx[c])+1;
            tmp.eb(r, c, dp[i]);
        }
        for (auto [r, c, val] : tmp) {
            chmax(rmx[r], val);
            chmax(cmx[c], val);
        }
    }
