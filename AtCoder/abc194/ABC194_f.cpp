 ...
 * ・leading zeroの扱いが面倒な桁DP。もうちょいすんなり解きたかったんだけど、そこでだいぶ手こずった。
...
void solve() {
    ...
    auto dp0 = list2d<mint>(N+1, K+1, 0);
    auto dp1 = list2d<mint>(N+1, K+1, 0);
    dp0[0][0] = 1;
    rep(i, N) {
        ...
        // ここまでで出現済の種類数
        ll nxtsm = sum(appeared[i+1]);
        // まだ開始しない
        dp0[i+1][0] = 1;
        // dp0へこの桁から開始(0以外に遷移)
        if (i == 0) dp0[i+1][1] = 1;
        // dp1へこの桁から開始(0以外に遷移)
        if (i == 0) dp1[i+1][1] += max(A[i]-1, 0LL);
        else dp1[i+1][1] += M-1;
        // dp0 <- dp0
        if (nxtsm <= K) dp0[i+1][nxtsm] = 1;
        // dp1 <- dp0 (出現済のいずれかへ)
        if (i > 0 and sm <= K) dp1[i+1][sm] += losm;
        // dp1 <- dp0 (未出現のいずれかへ)
        if (i > 0 and sm+1 <= K) dp1[i+1][sm+1] += max(A[i]-losm, 0LL);
        rep(j, 1, K+1) {
            // dp1 <- dp1 (出現済のいずれかへ)
            dp1[i+1][j] += dp1[i][j]*j;
            // dp1 <- dp1 (未出現のいずれかへ)
            if (j+1 <= K) dp1[i+1][j+1] += dp1[i][j]*(M-j);
        }
    }
    mint ans = dp0[N][K]+dp1[N][K];
    print(ans);
}
