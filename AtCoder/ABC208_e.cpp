...
・終了後15分くらいで自力AC！悔しい。。
・桁DP
・いわゆるsmaller flagを持つタイプの、狭義桁DPって感じ。
　で、数値はでかくなるけど、掛ける数が0~9だけなのもあって状態数自体は少ない。(と信じて進む。)
・0が開始前か途中かで扱いが違うからうまく場合分け作る。
・K超は無視してよさそうだけど実は*0で戻ってくる可能性があるので、
　K+1のところでその他全部みたいなのをまとめて持っておく。
・この最後のポイントがなかなか気付かなくて間に合わなかった。。
　でも本当の敗因はDで時間取られ過ぎたことだよね。。
*/
...
void solve() {
    ...
    vector<map<ll, ll>> dp1(M+1), dp2(M+1);
    rep(i, M) {
        ll a = toint(S[i]);
        // この桁から始める遷移
        rep(d, 1, 10) {
        ...
        for (auto [k, v] : dp1[i]) {
            rep(d, 10) {
                if (d < a) {
                    dp2[i+1][min(k*d, K+1)] += v;
                } elif (d == a) {
                    dp1[i+1][min(k*d, K+1)] += v;
                }
            }
        }
        for (auto [k, v] : dp2[i]) {
