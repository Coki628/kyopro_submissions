...
・ライブラリ整備：2次元BIT
・ついでにBITも作っておいた。やっぱBIT速いね。
　累積和の代わりに使っても、log2つ乗ってるはずなのに速度が2.4秒→3.8秒にしかならなかった。
*/
...
void solve() {
    ...
    BIT2D<ll> bit(H, W);
    rep(i, H) rep(j, W) bit.add(i, j, grid[i][j]);

    ll H1 = H-h1;
    ll W1 = W-w1;
    ll H2 = H-h2;
    ll W2 = W-w2;
    auto sm = list2d(H2+1, W2+1, 0LL);
    rep(i, H2+1) {
        rep(j, W2+1) {
            ll ni = i+h2;
            ll nj = j+w2;
            sm[i][j] = bit.query(i, j, ni, nj);
        }
    }
    ...
    ll dw = w1-w2+1;
    ll ans = 0;
    auto seg = get_segment_tree_2d(sm, [](ll a, ll b) { return max(a, b); }, -INF);
    rep(i, H1+1) {
        rep(j, W1+1) {
            chmax(ans, bit.query(i, j, i+h1, j+w1) - seg.query(i, j, i+dh, j+dw));
            // ll mx = 0;
            // rep(h, i, i+dh) {
            //     rep(w, j, j+dw) {
            //         chmax(mx, sm2[h][w]);
