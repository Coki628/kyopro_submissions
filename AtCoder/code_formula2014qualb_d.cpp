/*
参考：https://www.slideshare.net/chokudai/code-formula2014-qualb
　　　https://kmjp.hatenablog.jp/entry/2014/08/28/1000
　　　https://atcoder.jp/contests/code-formula-2014-qualb/submissions/19060313
　　　https://twitter.com/_10AC/status/1242896679893733376
・試験管青diff残埋め
・自力ならず。
・桁DP系、繰り上がり、種類数
・繰り上がりのある、下から桁DPで解けそうな見た目をしている。
　実際できたんだけど、求めるのが通り数じゃなくて種類数なので、重複なく数えるために遷移の工夫が必要。
・情報が少なくて、内容を解釈するのが大変だった。。
　断片的な情報を繋ぎ合わせて、何とか自分の解釈を作って、実装したらちゃんと答えが合ってよかった。
・普通に通り数を求めるように桁DPすると、例えば1円13枚使った場合と10円1枚1円3枚使った場合みたいのが
　思いっきり重複して数えられそうである。さてどうするか。
・ここで自分が使ったのが、今見ている桁を0~9のどれにするかを固定して遷移させるというもの。
　ここを固定すれば、他の桁がどんな値を取ってもそれは遷移毎に別の値になる。(はず)
　0~9どれを使うかも状態に持ちたくなったが、この遷移は今の桁の状態によらないので、ここは持つ必要がない。
　遷移できるかどうかは、今の桁が元々持つ枚数A[i]と、繰り上がってきた値によるので、
　状態としては、今見ている桁と繰り上がりの値があればいい。
・今まで見た繰り上がり処理は1桁上に行くか行かないかくらいのものだったので、
　今回みたいに大きな繰り上がりがあって、それを丸ごと状態として添字に持つのは新しいアプローチだったな。
*/
...
void solve() {
    ...
    // 繰り上がりで増えうる桁を上に増やしておく
    rep(_, 5) A.eb(0);
    N += 5;
    const ll M = 6000;

    // dp[i][j] := 下からi桁目まで見て、繰り上がりがjの時の種類数
