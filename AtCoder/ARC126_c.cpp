/*
参考：https://atcoder.jp/contests/arc126/editorial
　　　https://atcoder.jp/contests/arc126/submissions/25985187
・自力ならず。
・GCD系、調和級数、累積和応用
・Kがでかいのを例外処理してmax(A)-1以下のgcdを全探索って方針は合ってた。その先が詰めれず。
・gcdの倍数毎に処理する。あるgcdの倍数gを目指す値は決まっていて、(g-1,g]の区間になる。
　ここに含まれる値について、個数と和をBIT2本でやるやつみたいに累積和で前計算しておくと、
　g*個数 - 和 で、余事象っぽくその区間の全要素をgにするために必要な量が分かる。
　gcdを固定してその倍数だけ見ると、全体としては調和級数でlogになるので間に合う。
*/
...
void solve() {
    ...
    ll M = max(A);
    vector<ll> C(M*2+1), sm(M*2+1);
    rep(i, N) {
        C[A[i]]++;
        sm[A[i]] += A[i];
    }
    Accumulate<ll> acc(C), accsm(sm);
    ...
    rep(g, M-1, 1, -1) {
        ...
        rep(l, 0, M+1, g) {
            ll r = l+g;
            cur += r*acc.query(l+1, r+1) - accsm.query(l+1, r+1);
        }
        // rep(i, N) {
        //     cur += (g-A[i]%g)%g;
        // }
