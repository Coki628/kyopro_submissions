 ...
 * ・さくっと自力AC！
 * ・さっき解いたaoj1193_icpc2014prelimをちょっとだけ複雑にした感じ。
 * 　ほぼ同じパーツを流用して使える。
 */
...
ll erase(vvl &grid, ll combo) {
    ll res = 0;
    rep(i, H) {
        ll cnt = 1, cur = grid[i][0];
        rep(j, 1, W) {
...
void drop(vvl &grid) {
    rep(i, H-1, -1, -1) {
        rep(j, W) {
            if (grid[i][j] == -1) {
                ll k = i;
                while (k >= 0 and grid[k][j] == -1) k--;
                if (k != -1) swap(grid[k][j], grid[i][j]);
            }
        }
    }
}

ll check(vvl grid, ll h, ll w) {
    grid[h][w] = -1;
    ll ret = 0, combo = 0;
    while (1) {
        drop(grid);
        ll res = erase(grid, combo);
        if (res == 0) break;
        combo++;
        ret += res;
    }
