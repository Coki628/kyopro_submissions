/*
参考：https://atcoder.jp/contests/abc175/editorial
・ABC高難度埋め
・むずいけど、自力考察で方針だいたい合ってたし、橙diffにしてはやりやすかったかな。
・回文、メモ化再帰
・方針としては、元の文字列を反転させたものを右側で使う用として用意しておいて、
　左右で一致していない、はみ出した部分について、そこにプレフィックスが一致するものを当てはめて、
　またはみ出した部分で同じことをして、はみ出した部分が回文になったらそこで成功としてコストを返す、
　みたいな感じで作る。自力で書いたやつはここで遷移候補をプレフィックス毎に分けたりして
　ゴチャゴチャしちゃったんだけど、Nが50しかないから、普通に全部見ればそれでよかった。
・状態としては、
　memo[i][j][k] := 左/右を見ていて(i)、j個目の文字列のk文字目以降がはみ出した状態
　みたいな3つを持てばOK。
・状態が2*50*20で遷移がNだとして、明らかに間に合いそうなのにTLE取れなくて悩んだ。
　結局一番の原因だったのは、閉路があって無限ループしてたこと。
　普段は閉路がない前提でDPやるから意識してなかったけど、確かにこの問題だとあるね。。
　なのでグラフのサイクル判定みたいな処理を挟んだら、無事ちゃんと動いた。
　他にも色々細かいとこバグらせてて、実装力もまだまだだなーと分からされる。
*/
...
void solve() {
    ...
    auto memo = list3d(2, N, 20, -2LL);
    auto rec = [&](auto&& f, bool pt, ll i, ll x) -> ll {
        if (memo[pt][i][x] >= 0) return memo[pt][i][x];
        if (memo[pt][i][x] == -1) return INF;
        memo[pt][i][x] = -1;

        ll res = INF;
        string cur = S[1-pt][i].substr(x, S[1-pt][i].size()-x);
        ...
        rep(j, N) {
            auto &s = S[pt][j];
            ll preflen = min(s.size(), cur.size());
            if (s.substr(0, preflen) == cur.substr(0, preflen)) {
                if (s.size() <= cur.size()) {
                    chmin(res, f(f, pt, i, x+s.size())+cost[j]);
                } else {
                    chmin(res, f(f, 1-pt, j, cur.size())+cost[j]);
                }
            }
        }
        memo[pt][i][x] = res;
    ...
    ll ans = INF;
    rep(i, N) {
        chmin(ans, rec(rec, 1, i, 0)+cost[i]);
    }
    if (ans >= INF) ans = -1;
    print(ans);
}
