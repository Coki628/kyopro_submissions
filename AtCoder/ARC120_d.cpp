/*
参考：https://atcoder.jp/contests/arc120/editorial
　　　https://kmjp.hatenablog.jp/entry/2021/06/21/0930
・ARC青diff残埋め
・自力ならず。。スタック苦手なんだよなー
・括弧列、スタック
・まず考察からして厳しい。大小半分ずつで分けて、ペアが真ん中を跨げば最大が保証されるみたいなんだけど、
　本当か？？ってなる。。解説で明らかにって言ってるけど明らかじゃないだろこれ。。
・で、これが分かれば、ソートして元indexでどこが半分以上以下かを持っておいて、
　スタック末尾と違うのが来たらペア組む、同じのが来る限り保留なので詰める、ってやってくとうまくいく。
・大小半分で区切ればOKについて考えてたけど、昇順になってるとして、
　前半分の位置を固定したら、後ろ半分のどれとペア組ませても距離変わらないよね、
　って考えたらまあちょっと納得できた気がする。
*/
...
void solve() {
    ...
    my_deque<pll> st;
    rep(i, N) {
        if (st.empty() or st.back().first == lower[i]) {
            st.eb(lower[i], i);
        } else {
            auto [_, j] = st.pop_back();
            ans[j] = '(';
            ans[i] = ')';
        }
    }
