/*
・JOI埋め
・自力AC！やったね。難易度7で2問自力ダメで悔しかったから難易度8を1問通した。
・グリッド、UnionFind、二分探索
・それぞれのグリッドで、ある認証レベルの時に始点から到達できる最大マス数みたいのを考える。
　これは認証レベルが低い方から、UFで順次連結していけば、高速にチェックを網羅できる。
　ここで得た情報から、マス数でソートして認証レベルの値も持たせた配列を用意しておいて、
　両方合わせた時にちょうどマス数がR以上となるような位置を、片方を軸にして
　他方をにぶたんで見ていく。(半分全列挙の後半のマージのパートみたいな感じ)
　これで組ませたそれぞれの最適なペアの合計認証レベルから最小のものを取ればそれが答え。
*/
...
void solve() {
    ...
    rep(k, 2) {
        int sw, sh;
        cin >> W[k] >> H[k] >> sw >> sh;
        auto grid = list2d(H[k], W[k], 0);
        map<int, vector<pii>> cells;
        rep(i, H[k]) {
            rep(j, W[k]) {
                cin >> grid[i][j];
                cells[grid[i][j]].eb(i, j);
            }
        }
        sh--; sw--;
        int s = gridtoid(sh, sw, W[k]);
        UnionFind uf(H[k]*W[k]);
        A[k].eb(0, 0);
        for (auto& [a, li] : cells) {
            for (auto [h, w] : li) {
                int i = gridtoid(h, w, W[k]);
                for (auto [dh, dw] : directions) {
                    int nh = h+dh;
                    int nw = w+dw;
                    if (nh < 0 or nw < 0 or nh >= H[k] or nw >= W[k]) continue;
                    if (grid[nh][nw] <= a) {
                        int j = gridtoid(nh, nw, W[k]);
                        uf.merge(i, j);
                    }
                }
            }
