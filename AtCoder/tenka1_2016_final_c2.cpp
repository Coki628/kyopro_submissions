/**
 * 参考：https://tsutaj.hatenablog.com/entry/2017/09/06/144337
 * ・蟻本演習4-7-2
 * ・Trie木でDP
 * ・前に初めてTrie木使った問題(AGC047b)は考察パートがむずくてしっかり使えた感がなかったけど、
 * 　今回は考察が素直な感じっぽかったので、うまく使い方が入ってきた。
 * ・各部分文字列に合わせて、Trie木上を走査していくんだけど、
 * 　毎回最初から探し直したらもちろんTLEするので、1回の走査で途中も全部見る感じにする。
 * ・ループの構造は同じでも、ロリハ解よりかなり速くて、AC0.36秒。
 */
...
int main() {
    ...
    rep(i, 0, N) {
        ...
        rep(j, 1, L+1) {
            if (i+j > N) break;
            // iから始まる部分文字列に沿ってTrie木上で頂点を進めていく
            v = trie.next(v, S[i+j-1]);
            // この先を使う単語がないので終了
            if (v == -1) break;
            // ここで終わる単語があったら、IDを確認してその重みを使って遷移
            // (今回は単語に重複はないようなので常に添字0でOK)
            if (!trie.nodes[v].accept.empty()) {
                chmax(dp[i+j], dp[i]+W[trie.nodes[v].accept[0]]);
            }
        }
    }
    ll ans = dp[N];
