...
・公式解の方針
・幾何、平行移動、回転移動、重心
・点集合を回転させても重心の位置は変わらないので、重心を取っておけば、どこに平行移動したかは特定できる。
　こうすると平行移動のために全ペア試すループを回さなくて済むので、
　回転移動のために雑に全ペア試してチェックするのでも通せる。
*/
...
void solve() {
    ...
    // 重心を求める
    Point<double> centroida, centroidb;
    rep(i, N) {
        centroida += A[i];
        centroidb += B[i];
    }
    ...
    rep(i, N) {
        rep(j, N) {
            // A[i]とB[j]が対応するものとして回転を決める
            double deg = get_degree(B[j], centroida, A[i]);
            vector<Point<double>> B2(N);
            rep(k, N) {
                B2[k] = rotate(centroida, B[k], deg);
            }
            sort(ALL(B2));
            bool ok = 1;
            rep(k, N) {
