...
・なんとか自力AC！
・実装
・なんとか通ったから良かったものの、結構な闇実装になってしまった。絶対もっと簡単にやる方法ある。
・あるi,jを考えると、その位置を進出確定させられるかどうかは
　K - 既に決まってる人数 - 未確定で自分より上位になる可能性がある人数
　みたいになる。前者はsetとかで適当に持っておくとして、後者もi,jが決まれば
　そこから見て左下の長方形領域みたいになるので、(N-i-1)*j と表せる。
・ここで確定できるようならする。優先度の高い方から条件を満たす限り決めるようにするが、
　1本のプリキューとかでうまく持てなかったので、K本のキューの先に入れた方から順に取る感じにした。
　これならKがその位置より大きいようなのは選ばないで済む。
・愚直にK本のキューを毎回チェックするので、計算量心配だったけど全然大丈夫だった。
・ちょっと整えた。プリキュー1本でできるようにまとめた。ちゃんと条件作ればこれで大丈夫だった。
・余談だけど、この問題公式解説リンクはないけど、ググると公式解説スライドが出てくる。
*/
...
void solve() {
    ...
    set<ll> done;
    my_priority_queue<pll, vector<pll>, greater<pll>> que;
    rep(i, N) {
        vector<ll> res;
        rep(j, K) {
            que.push({j, i});
            ll unknown = (N-i-1)*j;
            // この位置で確定できる条件を満たしてかつ、キューの先頭のjがこの位置以下
            while (K-(ll)done.size()-unknown > 0 and que.size() and que.top().first <= j) {
                auto [j2, i2] = que.pop();
                if (not done.count(mat[i2][j2])) {
                    done.insert(mat[i2][j2]);
                    res.eb(mat[i2][j2]);
                }
            }
        }
