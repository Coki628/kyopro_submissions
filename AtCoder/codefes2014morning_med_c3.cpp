/*
参考：https://jupiro.hatenablog.com/entry/2020/06/25/164316
・試験管青diff残埋め
・行列累乗、誤差対策、正規化
・じゅぴろさんの記事でC++でも通る誤差対策が載ってたので。
　具体的には「スイッチが押される確率+ スイッチが押されない確率」
　は必ず1なのでそこが1になるように正規化する。
・コード上は行列累乗の2乗法パートに2行追加しただけ。これで通った。
*/
...
void solve() {
    ...
    // vector<ld> dp0(N+1), dp1(N+1);
    // dp0[0] = 1;
    // rep(i, N) {
    //     dp0[i+1] = dp0[i]*(1-p)+dp1[i]*p;
    //     dp1[i+1] = dp0[i]*p+dp1[i]*(1-p);
    // }
    // ld ans = dp1[N];
    // print(ans);
