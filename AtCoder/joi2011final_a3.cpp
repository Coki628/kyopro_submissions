 ...
 * ・E8さん精選100+50問埋め：累積和
 * ・きっちり自力AC！
 * ・2次元累積和を3つ作る。実装頑張れなJOIっぽい問題だよね。
 * ・1回サンプル合わなくて、ん？ってなった。
 * 　累積する時の外側ループで+1まで回してなかった。まあ割とすぐ気付いてよかった。
 */
...
int main() {
    ...
    rep(_, M) {
        ...
        vector<ll> res(3);
        rep(k, 3) {
            res[k] = acc[k][th][tw]+acc[k][sh][sw]-acc[k][th][sw]-acc[k][sh][tw];
        }
        print(res);
    }
    return 0;
