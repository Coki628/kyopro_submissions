/*
参考：https://betrue12.hateblo.jp/entry/2019/04/30/214443
　　　https://kmjp.hatenablog.jp/entry/2019/05/01/0930
・その他埋めAC数上げ
・自力ならず。。悔しい。
・K番目の値の二分探索、尺取り法、区間or
・これも解けてみれば典型詰め合わせじゃん、、悔しいー。。
・まずちょっと問題読み違えてたね。全ペアのorかと思ったら違って、全区間のorだね。
　で、こういうK番目を求めるやつは、うまく単調性を見つければ、にぶたんが効く。
　今回はorで区間を伸ばすほど値が大きくなるのでそれを使う。
　ある値以上になる区間がK個以上ある最大値を求める。
・区間がどこまで有効かは尺取りで条件を満たさない間右端を伸ばして、
　条件を満たすようになった以降、Nまでの右端は全て使えるとした。
・区間orを求めるところはセグ木に乗せた。にぶたんと合わせてlog2つ乗るからどうかなーと思ったけど、
　問題なくAC、0.58秒。Sparse Tableとか使ったらもちょい速くなりそうだけどまあこれならセグ木で大丈夫ね。
*/
...
void solve() {
    ...
    seg.build(A);
    ll res = bisearch_max(0, 1LL<<M, [&](ll x) {
        ll r = 1, cnt = 0;
        rep(l, N) {
