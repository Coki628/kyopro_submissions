/*
参考：https://note.com/syamashi/n/nf54dc95647a5
　　　https://merom686.hatenablog.com/entry/2021/10/17/010655
・ARC青diff残埋め
・自力ならず。ARCは無理めなやつ多いね。。
・こういう2重ループOKで、DPじゃなくて賢く2つ決めて全探索みたいのは本当に読めない。
　何を決めたらうまくいくか見えてこないんだよねー。。
・平均が一番高くなる位置を見つけて、それで平均M以下に抑えらえるならそこに全部使うし、
　無理ならとりあえずMでやって、もっと手前を見ていく。
・なんかここの説明書いてたらもっと簡潔にできるじゃん、ってなったので書き直し版。
・今回は後ろから見ていって、Mずつ使う区間、残るSを均等割りする区間、使わない区間、
　みたいになるので、この3区間の区切り位置2つを全探索すればそれで最適解が出る。
*/
...
void solve() {
    ...
    rep(i, N+1) {
        rep(j, i, N+1) {
            ...
            // Mの区間が長すぎてSが足りない
            if (val > S) continue;
            ld avg = (j-i) ? (S-val)/(j-i) : 0;
            // Sの区間が長すぎて平均がMを超える
            if (avg > M) continue;
            // 両方OKなら答えとして使えるので使う
            chmax(ans, avg*acc.query(i, j)+M*acc.query(j, N));
        }
    }
    print(ans);
}
