/**
 * 参考：https://atarimae.biz/archives/24942#_2
 * ・E8さん精選100+50問埋め：その他のテクニック：幾何計算
 * ・自力AC！でもめっちゃ時間かかった…。
 * ・三角関数、二分探索
 * ・角度調べて、2辺と角度からの面積の求め方調べて、他にも色々する。
 * ・C++、radians,degrees変換の関数ないのね。作った。
 * 　先に作ったのに、sin,cosに入れる時に当てるの忘れててサンプル合わねー、ってやってた。
 */
...
int main() {
    ...
    ld res = bisearch_max(0, 90, [&](ld m) {
        // コップの底
        ld h1 = a*sin(radians(m));
        // コップの先端
        ld h2 = b*sin(radians(90-m));
        if (h1 <= h2) {
            // 水面
            ld c = a/cos(radians(m));
            // 下の三角形と上の平行四辺形
            ld T = a*c*sin(radians(m))/2 + c*(h2-h1);
            return T >= S;
        } else {
            ld c = b/cos(radians(90-m));
            // 三角形だけ
            ld T = c*h2/2;
            return T >= S;
        }
