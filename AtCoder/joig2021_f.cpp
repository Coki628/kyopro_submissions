/*
・これは部分点のみ。
　一応バチャしてたので、考えうる最大限の部分点は取った。
　全探索2つとビット全探索で3パターン書いてる。
*/
...
void solve() {
    ...
    assert(S == 1 or H <= 10 and W <= 10 or M <= 15);
    ...
    if (S == 1) {
        ...
        rep(h, H) {
            rep(w, W) {
                se.insert(A[h][w]);
                if (w <= L[A[h][w]] and w >= R[A[h][w]] and h <= U[A[h][w]] and h >= D[A[h][w]]) {
                    removed = true;
                }
            }
        }
        ll ans = se.size()-removed;
        print(ans);
    } elif (H <= 10 and W <= 10) {
        ll ans = INF;
        rep(uh, H) {
                ...
                rep(dh, uh+1, H+1) {
                    rep(rw, lw+1, W+1) {
                        if ((dh-uh)*(rw-lw) <= S) {
                            ll removed = 0;
                            rep(i, M) {
                                if (lw <= L[i] and rw > R[i] and uh <= U[i] and dh > D[i]) {
                                    removed++;
                                }
                            }
                            chmin(ans, M-removed);
        ...
        ll ans = INF;
        rep(bit, 1LL<<M) {
            ll lmn = INF, rmx = -INF, umn = INF, dmx = -INF;
            rep(i, M) {
                if (bit>>i & 1) {
                    chmin(lmn, L[i]);
                    chmax(rmx, R[i]);
            ...
            ll s = max(rmx-lmn+1, 0LL)*max(dmx-umn+1, 0LL);
            if (s <= S) {
                chmin(ans, M-popcount(bit));
            }
        }
        print(ans);
    }
