...
・終了後自力AC。。
・状態を見つけてDP
・クーポン獲得が決まった時点でそれは答えに加味できるので、
　答えとは別に持たなければいけない状態は実は、
　クーポンになっていない端数部分の0~99だけ。
　これが分かると、N*100=1000万のDPにできるので解ける。
・しっかり集中して考察したら解けたなぁ。。
　今回解けない問題多すぎて思考が分散されててダメだったわ…。
*/
...
void solve() {
    ...
    auto dp = list2d(N+1, 100, -INF);
    dp[0][0] = 0;
    rep(i, N) {
        auto [p, u] = PU[i];
        rep(j, 100) {
            chmax(dp[i+1][j], dp[i][j]);
            auto [d, m] = divmod(j+p, 100);
            chmax(dp[i+1][m], dp[i][j]+u-p+d*20);
        }
    }
    ll ans = max(dp[N]);
    print(ans);
