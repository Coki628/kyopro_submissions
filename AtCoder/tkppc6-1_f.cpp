/*
・なんとか自力AC！時間切れだったけど、自力通せたのは嬉しい。
・貰うDP高速化、遅延セグ木、区間和取得・区間乗算・区間更新
・もちろんまともに2冪通りのパターンを試すのは無理なので、主客転倒で寄与を考えていく。
・dp[i] := i番目のマスまでの状態を全て決めた時の通り数
　みたいにすると、K個前～直前までを遷移させてくればいけそうではある。
　どう考えればいいかしばらく悩んだけど、結局遷移で飛ばすことになるマスは
　白黒どっちに塗ってもいいみたいな扱いになるので、その数だけ2冪掛ければいいと分かる。
　これでサンプルも合ったが、まだN^2なので遷移高速化を検討する。
　遷移元によって掛ける数が*2ずつ変わってしまうので、まともに区間和取得とかでは無理。
　でもこの増え方が規則的なので、何とか扱いやすい形にできないか考えていると、
　毎回自分より手前に区間乗算で*2みたいなことができると、
　ちょうど左の方ほど*2される回数が多くなり、実際やってみると合う。
・後はこの、区間和取得、1点更新、区間乗算、が全て実施できれば通せそうだ。
　乗算試したことないけど、遅延セグ木に乗りそう。前に更新と加算を同時にやるの作ったやつをベースに、
　色々ガチャガチャしてみると、まー大変だったけど何とか作れた。
　やってみて分かったのは、
　・乗算は単位元が1で、0が来た瞬間即死するので、そこの初期化は全部1
　・加算と違って区間長で扱う値が変化したりしないので、取得側が和でも区間長は持たなくていい
　とかそんな感じ。
*/
...
void solve() {
    ...
    // vector<mint> dp(N);
    // dp[0] = 1;
    auto dp = get_Lazy_segment_tree(N, f, g, h, T, E);
    dp.update(0, {1, 1, 1});
    rep(r, 1, N) {
        ll l = max(0LL, r-K);
        dp.update(r, {1, dp.query(l, r), 1});
        dp.update(0, r, {2, 0, 0});
        // rep(i, l, r) {
        //     dp[r] += dp[i];
        // }
