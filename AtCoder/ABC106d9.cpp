/**
 * ・E8さん精選100+50問埋め：累積和
 * ・前にpythonでTLEした、bitsetで累積和みたいなやつ。試しにやってみたらC++はさすがのAC1.47秒。
 * 　Moよりちょっと遅いけど、これでも通るんだなー。
 */
...
int main() {
    ...
    // S1[i] := 時刻iでの出発前の列車の集合
    // S2[i] := 時刻iでの終着後の列車の集合
    vector<bitset<200007>> S1(N+1), S2(N+1);
    rep(i, N) rep(j, M) S1[i].set(j);
    rep(i, M) {
        ll l, r;
        cin >> l >> r;
        r--;
