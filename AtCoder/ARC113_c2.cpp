...
struct LazySegTree {
    ...
    LazySegTree(int N, F f, G g, H h, T ti, E ei):
            f(f), g(g), h(h), ti(ti), ei(ei) {
        build(N);
    }
...
void solve() {
    ...
    vector<LazySegTree<pll, ll>> seg(M, {f, g, h, {0, 0}, INF});
    rep(c, M) seg[c].build(vector<pll>(N, {0, 1}));
    rep(i, N) {
        seg[A[i]].update(i, 1);
    }
    ...
    rep(i, N-3, -1, -1) {
        if (A[i] == A[i+1] and A[i+1] != A[i+2]) {
            ans += N-i - seg[A[i]].query(i, N).first;
            rep(c, M) {
                if (c == A[i]) {
                    seg[c].update(i, N, 1);
                } else {
