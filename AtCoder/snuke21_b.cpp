...
・自力AC！いいね！
・辞書順最小、貪欲
・題意の操作で起こることを考えると、後ろが1つ前にずれてくる。
　こうなった時に、辞書順が小さくなるか大きくなるかが重要で、
　sの1つ後ろ(sが連続するならそれが終わった直後)の大小を確認するといい。
　これが分かると各sについて、取り除いて得するか損するかで大別できる。
　後は得するやつを前からできるだけ除いて、
　損するやつはできるだけ前を使いたくないので後ろから優先して使う、で無事AC。
・最初WAになって悩んだけど、sが連続する時の優先度を正しく考察してなかったからだった。
*/
...
void solve() {
    ...
    rep(i, N) {
        if (K and T[i] == 's' and good[i]) {
            removed[i] = 1;
            K--;
        }
    }
    rep(i, N-1, -1, -1) {
        if (K and T[i] == 's' and not removed[i]) {
            removed[i] = 1;
            K--;
        }
    }
