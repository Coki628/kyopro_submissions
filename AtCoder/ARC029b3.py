...
・ちょっと編集版、どうせ今回軸にするの(0,0)だけだから、
　それに合わせて回転させる式を簡単にしておいた。
"""
...
def rotate(s, dig):
    x1, y1 = s
    # (0,0)を軸として(x1,y1)を左回りにdig度回転させた座標(x2,y2)
    x2 = x1 * cos(radians(dig)) - y1 * sin(radians(dig))
    y2 = x1 * sin(radians(dig)) + y1 * cos(radians(dig))
    return (x2, y2)
