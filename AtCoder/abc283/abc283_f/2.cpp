...
・下処理して座圧のlog外した。下駄履かせればいいんだけど、
　どこまで作ればいいか結構悩んだし、使う時もはみ出すから調整が必要だった。
・これでAC2.0秒ギリギリ。
*/
...
void solve() {
    ...
    // 下処理で座圧のlogを外す
    vector<int> mpu(800007), mpv(800007);
    int offset = 400000;
    rep(i, -offset, 400007) {
    ...
    rep(i, N) {
        int u = i - P[i];
        int v = i + P[i];
        ans[i] = bisearch_min(-1, 400000, [&](int m) {
            // (u,v)を中心とした1辺が2mの正方形領域の中にある点の個数
            int lu = max(u - m + offset, 0);
            int ru = min(u + m + 1 + offset, 800006);
            int lv = max(v - m + offset, 0);
            int rv = min(v + m + 1 + offset, 800006);
