/*
・きっちり自力AC！
・重み付き二部マッチング → フローやる。負辺にならないように適切にoffsetを入れる。
・マッチングさせない時の利得を予め基準として取っておいて、
　そのペアをマッチングさせた時に、させない場合と比べてどのくらい得するか、
　をコストとして辺を張る。
*/
...
void solve() {
    ...
    mcf_graph<ll, ll> mcf(N+M+2);
    ll s = N+M;
    ll t = N+M+1;
    rep(i, N) {
        mcf.add_edge(s, i, 1, 0);
        mcf.add_edge(i, t, 1, offset);
        rep(j, M) {
            if (mat[i][j] == '1') {
                mcf.add_edge(i, N+j, 1, offset-(A[i]-B[i])-(C[j]-D[j]));
            }
        }
    }
    rep(j, M) {
