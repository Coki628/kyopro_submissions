/*
参考：https://atcoder.jp/contests/past202109-open/editorial
・二分探索、K番目の数、実はソートできる
・絶対値ペアは前後が入れ替わっても値は変わらないので、順番が関係ない。
　つまりソートしても問題ない。これ前どっかでも同じテーマで詰まったことあるな。。
・これが分かれば、内側の数え上げる処理はにぶたんとか尺取りにできるので、通る。
　この時、前後逆も数えてしまうので*2することと、自分の分で1引くこと。
・尺取りサボってにぶたんでやったらACしたけど1.00秒かかった。。
*/
...
void solve() {
    ...
    ll res = bisearch_min(-1, MX, [&](ll m) {
        ll cnt = 0;
        rep(i, N+1) {
            ll l = bisect_left(acc, acc[i]-m);
            ll r = bisect_left(acc, acc[i]+m+1);
            cnt += r-l-1;
        }
        return cnt >= K*2;
    });
