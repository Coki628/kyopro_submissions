...
・自力ならず。。悔しい。
・二分探索、K番目の数
・Gと同じテーマがここでも出てた。当然こっちのが内側がむずいけど。
　累積和を取れば、全ペアから絶対値の差が小さいものを集める感じになって、
　そのままだと2乗かかるから高速に取得する。
・Wavelet Matrixのrange_freqで対象の範囲内にある値の数を数えたけど、
　にぶたんのlogとWMのlogが両方かかる30万のNはTLE。。。
*/
...
void solve() {
    ...
    ll offset = -min(acc);
    rep(i, N+1) acc[i] += offset;
    const ll MXLOG = 20;
    CompressedWaveletMatrix<ll, MXLOG> wm(acc);
    const ll MX = 1000000000000000;

    ll res = bisearch_min(-1, MX, [&](ll m) {
        ll cnt = 0;
        rep(i, N) {
            cnt += wm.range_freq(i+1, N+1, max(acc[i]-m, 0LL), min(acc[i]+m+1, MX));
        }
        return cnt >= K;
    });
    print(res);
