/*
・きっちり自力AC！
・smallerもgreaterもある桁DP
・絶対天才解法があるんだろうなと思いながら桁DPした。でも練習にはなって良かった。
・L,Rの条件はもちろん、Nの条件も桁毎の01で状態管理できる。
　遷移とかの書き方はABC138fを参考に。
　この複雑度はまだ慣れてないから結構見ながら書いたけど、
　このくらいをソラで一気に書けるようになるといいね。
・なお、やっぱり天才解法が想定解だった模様。(あんまちゃんと見てもいない)
*/
    ...
    rep(i, M) {
        rep(a, 2) {
            rep(b, 2) {
                rep(c, 2) {
                    rep(x, 2) {
                        // Lが1でLのgreater確定前なら1じゃないと無理
                        if (A[i] == 1 and a == 0 and x != 1) continue;
                        // Rが0でRのsmaller確定前なら0じゃないと無理
                        if (B[i] == 0 and b == 0 and x != 0) continue;
                        // Nが0でNのsmaller確定前なら0じゃないと無理
                        if (C[i] == 0 and c == 0 and x != 0) continue;
                        // 遷移先
                        ll aa = (A[i] == 0 and x == 1) ? 1 : a;
                        ll bb = (B[i] == 1 and x == 0) ? 1 : b;
                        ll cc = (C[i] == 1 and x == 1) ? 1 : c;
