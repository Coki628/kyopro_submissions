...
struct Clock {
    ...
    string to_str() {
        return zfill(tostr(hour), 2)+":"+zfill(tostr(minute), 2)+":"+zfill(tostr(second), 2);
    }

    void shift(int diff_second) {
        ll res = hour*60*60+minute*60+second;
        res = (res+86400+diff_second)%86400;
        ll hm = res/60;
        second = res%60;
        tie(hour, minute) = divmod(hm, 60);
    }
};
