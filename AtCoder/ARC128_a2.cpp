/*
参考：https://twitter.com/shiro537/status/1449374721120759809
・元々こっちでやろうとしてWAになってた。
・DP復元
・logにして乗除を加減にしてやる。でないと値がでかくなりすぎて、long doubleでもやられる。
*/
...
void solve() {
    ...
    auto dp = list2d(2, N+1, mkp((ld)-INF, -1));
    dp[0][0] = {1, -1};
    rep(i, N) {
        if (dp[0][i].first >= dp[1][i].first-A[i]) {
            dp[0][i+1] = {dp[0][i].first, 0};
        } else {
            dp[0][i+1] = {dp[1][i].first-A[i], 1};
        }
        if (dp[1][i].first >= dp[0][i].first+A[i]) {
            dp[1][i+1] = {dp[1][i].first, 1};
        } else {
            dp[1][i+1] = {dp[0][i].first+A[i], 0};
        }
    }
