...
・自力ならず。むずいなー。。
・隣接スワップ、転倒数、種類数の数え上げDP
・実は愚直いけるんじゃあとか思ったけど無理だよね。。
・状態は各文字使った数と現在の転倒数合計を持つ。
　何文字目まで見たか、は3文字とも使った数持ってるならそこから一意なので持たなくてもいい。
　(逆にどこまで見たか持って、文字は2文字しか持たないのもいいんだと思う。)
　ここまで言われても実装見て精査するまで遷移がどうなるのか分からなかった。。
・さて、遷移は3つ、その状態から、次にどの文字を置くか、だ。
　これなら排反に分かれるはずで、重複なく種類数が数えれる感じはする。
　自分でもそうは思ったんだけど、じゃあそのある文字を置くにあたって、
　どういった遷移を作ればいいの？って話だ。
　ここで考えるのは、その文字を今置いた時、いくつ転倒数が増えるか？になる。
　これを数えるには既に置いた文字について、1つずつ全部見ていく。
　予め各文字が出現する元位置のindexを取っておけば、例えば今Kが3回出現済だとして、
　左から3つのKが元々どこの位置にいたかは分かる。
　これと今置こうとしているやつの元位置(どの文字の何回目かが分かれば一意)を比べればいい。
　既に置いたKのが右から来ていた場合は、この2者でスワップが発生しているはずなのでカウントする。
　これで遷移先となる転倒数がK以内なら妥当な操作なので遷移さえればいい。
*/
...
void solve() {
    ...
    vector<ll> sz(3);
    rep(i, 3) sz[i] = pos[i].size();
    auto dp = list4d(sz[0]+1, sz[1]+1, sz[2]+1, K+1, 0LL);
    dp[0][0][0][0] = 1;
    rep(i, sz[0]+1) {
        rep(j, sz[1]+1) {
            rep(k, sz[2]+1) {
                rep(x, K+1) {
                    ...
                    if (i < sz[0]) {
                        ll add = 0;
                        // 今置こうとしているものより後ろにいたやつが今前にいたら、転倒数が増える
                        rep(jj, j) {
                            if (pos[1][jj] > pos[0][i]) add++;
                        }
                        rep(kk, k) {
                            if (pos[2][kk] > pos[0][i]) add++;
                        }
                        if (x+add <= K) dp[i+1][j][k][x+add] += dp[i][j][k][x];
                    }
                    // Eを置く
                    if (j < sz[1]) {
                        ll add = 0;
                        rep(ii, i) {
                            if (pos[0][ii] > pos[1][j]) add++;
                        }
                        rep(kk, k) {
                            if (pos[2][kk] > pos[1][j]) add++;
                        }
                        if (x+add <= K) dp[i][j+1][k][x+add] += dp[i][j][k][x];
                    }
                    // Yを置く
                    if (k < sz[2]) {
                        ll add = 0;
                        rep(jj, j) {
                            if (pos[1][jj] > pos[2][k]) add++;
                        }
                        rep(ii, i) {
                            if (pos[0][ii] > pos[2][k]) add++;
                        }
                        if (x+add <= K) dp[i][j][k+1][x+add] += dp[i][j][k][x];
                    }
                }
            }
        }
    }
    ll ans = sum(dp[sz[0]][sz[1]][sz[2]]);
    print(ans);
}
