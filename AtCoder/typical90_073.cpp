/*
参考：https://twitter.com/gojira_kyopro/status/1407727893317263367
・E8さん典型90問埋め
・自力ならず。。
・木DP、数え上げ、余事象
・木DPであること、状態3つ持つことまではできたんだけど、
　適切な遷移を詰められなかった。
・遷移が難しい。公式スライドのだと両方あるとこ(dp[2][u])への遷移が理解できなくて、
　上記ゴジラさんのツイートでやっと理解が進んだ。
・dp[2][u]への遷移について、先に全通りを計算しておいて、例えばA[u]が'a'なら、
　dp[0][u]の通り数はdp[0][v]で切らないかdp[2][v]で切るかなので計算できて、
　dp[1][u]の通り数は自明に0なので、後は全体が分かれば余事象で出せる、
　と考えるのが自分の中では一番納得できた。
・dp[2][u]は直接計算できないけど、全通りであれば、
　dp[0][v],dp[1][v]で切らない + dp[2][v]で切る/切らない なので、これは計算できる。
・dp[2][u]がどうなるかを直接考えるから難しくなる訳で、
　こうやって余事象で考えれば解けなくはなさそう、となった。
*/
...
void solve() {
    ...
    auto dfs = [&](auto&& f, ll u, ll prv) -> void {
        ...
        for (ll v : nodes[u]) {
            if (v == prv) continue;
            f(f, v, u);
            total *= dp[0][v]+dp[1][v]+dp[2][v]*2;
            if (A[u] == 'a') {
                dp[0][u] *= dp[0][v]+dp[2][v];
            } else {
                dp[1][u] *= dp[1][v]+dp[2][v];
            }
        }
        dp[2][u] = total-dp[0][u]-dp[1][u];
    };
    dfs(dfs, 0, -1);
    mint ans = dp[2][0];
    print(ans);
