...
・きっちり自力AC！
・グリッド、最小カット、行列を頂点でマスを辺にするやつ
・これもよく覚えてた。マスを辺にするやつこれで初めて見たんじゃないかな。
・最初WAが出てちょっと悩んだけど、流量がINFより大きくなる時もあるっぽくて、
　INFぴったりじゃなくてINF以上にしたらAC。
・あとこれも無向辺普通に張ってるけどちゃんと最小カット出るね。
*/
...
void solve() {
    ...
    rep(i, H) {
        rep(j, W) {
            if (grid[i][j] == 'S') {
                mf.add_edge(s, i, INF);
                mf.add_edge(s, H+j, INF);
            } elif (grid[i][j] == 'T') {
                mf.add_edge(i, t, INF);
                mf.add_edge(H+j, t, INF);
            } elif (grid[i][j] == 'o') {
                mf.add_edge(i, H+j, 1);
                mf.add_edge(H+j, i, 1);
            }
