/*
参考：https://atcoder.jp/contests/agc055/editorial
　　　https://twitter.com/tomerun/status/1454826111775379456
・自力ならず。これは全然思いつく気がしないな。。
・ABCの順列6個を順番に試す。それぞれの文字を前半、真ん中、後半から、
　どれかが足りなくなるまで取る。これが全部終わると全部取りきれてる。
*/
...
void solve() {
    ...
    auto C = list2d(3, 3, my_queue<ll>());
    rep(i, 3) {
        auto s = S.substr(N*i, N);
        rep(j, N) {
            C[i][s[j]-'A'].push(j);
        }
    }
