/*
参考：https://img.atcoder.jp/yahoo-procon2018-final-open/editorial.pdf
・その他埋めAC数上げ
・調和級数、素因数分解、包除原理、ビット全探索
・ふんわりとした方針は近かったものの、実際色々違ってるし足りないしダメだった。
　素因数分解して素因数毎に数えるのは近い筋だったんだけど、数列A側じゃなく1~M側で回す。
　数列A側は前計算で各iの倍数にいくつ該当するか数えておく。
　これは各A[i]を予め埋め込んでおいて、1~Mで調和級数のループ回せば、A[i]をいくつ踏んだかで数えられる。
・素因数の種類数はかなり少ないのは分かってたけど、この制約10万の範囲で言えば、ビット全探索できるくらい少ない。
　これが重要で、ビット全探索できるなら、素因数の使う/使わないの組み合わせを全部見れるので、
　奇数個重なったパターンと偶数個重なったパターンを包除原理で足し引きすれば、重複のないものを求められる。
*/
