...
・自力ならず。
・チェビシェフ距離、縦横独立、工夫して全探索
・結局、最大から2番目になりうるのは、最大距離ペアからどっちか1つずらしたやつ。
　これを縦横で見るとして、必要になるのはそれぞれの軸で4つずつ計8つだけ。
　この8つだけで全探索してソートすればOK。
・言われると分かりそうに思えるんだけど、自分では解けなかった。。。
*/
...
void solve() {
    ...
    set<ll> se;
    rep(i, 2) {
        se.insert(XI[i].second);
        se.insert(XI[N-i-1].second);
        se.insert(YI[i].second);
        se.insert(YI[N-i-1].second);
    }
    ...
    for (auto i : se) {
        for (auto j : se) {
            if (i >= j) continue;
            dist.pb(max(abs(XY[i].first-XY[j].first), abs(XY[i].second-XY[j].second)));
        }
    }
