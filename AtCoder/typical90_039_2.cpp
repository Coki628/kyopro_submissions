/*
参考：https://twitter.com/monkukui2/status/1393043692399722496
・E8さん典型90問埋め
・全方位木DP
・最初自分もこの方針で行こうとして、うまく遷移を作れず方針変えたんだけど、
　monkukuiさんが出来るって言ってたのでこれはこれで書けた方がいいと思い復習。
・全頂点について、
　dp[root] := rootを端点とするパスの重みの総和
　が分かればよく、これは部分木のサイズと、
　部分木の頂点がそこまでに通った辺数の和を持てば求まる。
*/
...
void solve() {
    ...
    ll N;
    cin >> N;
    ReRooting<Node, ll> rr(N, f1, f2, {0, 0});
    rep(_, N-1) {
        ll u, v;
        cin >> u >> v;
        u--; v--;
    ...
    auto res = rr.build();
    ll ans = 0;
    // sm[i] := 頂点iを端点とするパスの重みの総和
    for (auto a : res) ans += a.sm;
    ans /= 2;
    print(ans);
}
