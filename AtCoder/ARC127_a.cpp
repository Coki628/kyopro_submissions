...
・なんとか自力AC！
・頑張る桁DP
・うまい貪欲がきっとあるんだろうと思いつつも詰めれず、桁DP。
　dp[i][j][k] := i桁目まで見て、先頭に1が続いた回数がj回、
　k=0: 開始前で0, k=1:先頭1が連続中, k=2: 1以外の数字が出た
　+ smallerフラグでさらに0,1、っていう添字4つの大作。
*/
...
void solve() {
    ...
    ll N = S.size();
    auto dp0 = list3d(N+1, N+2, 3, 0LL);
    auto dp1 = list3d(N+1, N+2, 3, 0LL);
    dp0[0][0][0] = 1;
    rep(i, N) {
        rep(j, N+1) {
            rep(k, 3) {
                rep(d, 10) {
                    ll nxtj = (d == 1 and k != 2) ? j+1 : j;
                    ll nxtk = k;
                    if (k == 0 and d >= 2) {
                        nxtk = 2;
                    }
                    ...
                    if (toint(S[i]) > d) {
                        dp1[i+1][nxtj][nxtk] += dp0[i][j][k];
                        dp1[i+1][nxtj][nxtk] += dp1[i][j][k];
                    } elif (toint(S[i]) == d) {
                        dp0[i+1][nxtj][nxtk] += dp0[i][j][k];
                        dp1[i+1][nxtj][nxtk] += dp1[i][j][k];
                    } else {
                        dp1[i+1][nxtj][nxtk] += dp1[i][j][k];
                    }
