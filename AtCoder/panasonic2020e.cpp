/*
参考：https://youtu.be/bHzohtVsG0Q
　　　https://blog.hamayanhamayan.com/entry/2020/03/15/002311
・ABC系高難度埋め
・まーじで大変。これ系は生涯自力ACできるか分からない。
・何かを決めて全探索
・とりあえず方針が見えない。2乗できるしDPか？ってなるくらいしかできないけど違う。
・3つの文字列について、開始位置をどのくらいずらしたかを全探索することを考えると、
　文字列長のループ2重で済むので間に合う。その位置で問題ないかの判定は、
　2つの文字列間で問題ないかを前計算しておけば、3つ合わせた時のチェックはO(1)で出来る。
・文字列の長さがバラバラで様々な状況が考えられるので、はみ出た分は全部OKになるように、
　'?'埋めで後ろに十分な番兵を仕込んでおく。
・それでもなお結構気付きにくいコーナーケースみたいのもあって大変。
　これ本番中に通すのとかまじで凄いなぁって思う。
*/
...
void solve() {
    ...
    cin >> S[0] >> S[1] >> S[2];
    const ll L = 6007;
    rep(i, N) S[i] += string(L, '?');

    // 部分文字列s,tを一致させられるか
    auto check = [&](const string &s, const string &t) -> bool {
        ll len = s.size();
        rep(i, len) {
            if (s[i] != t[i] and s[i] != '?' and t[i] != '?') return false;
        }
        return true;
    };

    // ok[i][j][k] := 文字列をS[i],S[j]の順に並べて、S[j]をk後ろにずらした位置で、一致させられるか
    auto ok = list3d(N, N, L, true);
    rep(i, N) {
        rep(j, N) {
            if (i == j) continue;
            ll n1 = S[i].size()-L;
            ...
            ll len = max(n1, n2);
            rep(k, n1+1) {
                ok[i][j][k] = check(S[i].substr(k, len), S[j].substr(0, len));
            }
        }
    }

    ll ans = INF;
    for (auto& perm : permutations(vector<ll>({0, 1, 2}))) {
        ll i1 = perm[0], i2 = perm[1], i3 = perm[2];
        string& s1 = S[i1], s2 = S[i2], s3 = S[i3];
        ll n1 = s1.size()-L, n2 = s2.size()-L, n3 = s3.size()-L;
        rep(j, n1+1) {
            // ここでn1ともmax取らないと、s1が長くてs2が短い時を網羅できない
            rep(k, max(n1, n2)+1) {
                if (ok[i1][i2][j] and ok[i2][i3][k] and ok[i1][i3][j+k]) {
                    chmin(ans, max({n1, j+n2, j+k+n3}));
                }
            }
        }
    }
