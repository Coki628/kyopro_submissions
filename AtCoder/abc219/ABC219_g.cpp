/*
・時間内自力AC！！これは嬉しい。ほぼ既出だとしても。
・グラフ、平方分割
・ほぼ典型90の83。平方分割する。でもそのままだとなんか1つTLEで、区切りのsqrt(M)を固定値300にしたら通った。。
　なんか1つだけ異常メモリでTLEしてるので、絶対なんかバグってる。。
・多分原因分かった。辺0本頂点20万個みたいなグラフで死んでるぽい。そういうのが来た場合、
　次数が多い頂点だけ情報持たせる準備パートで、次数 >= D ってやってると、
　ceil(sqrt(M))でもsqrt(M)でもM=0ならD=0になってしまうので、全ての頂点について対象に入ってしまい、
　結果N*Nの2次元リストを構築しようとするので死ぬ。これならメモリ爆食いも説明がつく。
　次数 > D って条件に変えたら、無事ACした。典型83の方は連結グラフだったからこういうのがなかったんだね。。
*/
...
void solve() {
    ...
    vector<int> color_updated(N, 1), last_updated(N, -1), color_used(L, -1), last_used(L, -1);
    rep(i, N) {
        color_updated[i] = i+1;
    }
    rep(i, Q) {
        ...
        rep(d, L) {
            if (connected[d][x]) {
                if (last_used[d] > last_updated[x]) {
                    last_updated[x] = last_used[d];
                    color_updated[x] = color_used[d];
                }
            }
        }
