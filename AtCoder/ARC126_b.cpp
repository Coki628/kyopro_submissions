...
・LISのDP
・これがLISになるとか、気付かねぇ～。。。
　ABC038dだって問題なんとなく覚えてるくらいには印象に残ってたけど、
　この問題と同じに帰着するとは思わない。視点が違うだけでこうも違く見えるんだよな。。
・さて解法だけど、ほぼABC038dと同じことになる。という、丸々コピペでも通るらしい。。
　普通のLISは数列のindexが1,2,3..に対して、値A[1],A[2],A[3]..がどこまで単調増加になるかって話だけど、
　これは2つの数列A,Bについても、片側でソートしてindex側とみなせば同じことが言える。
　ただしこの時、配列Aはindexと違って同値を含む可能性があるので、そこの処理に注意が必要。
　具体的には、ソートの条件でAが同値だったらBの降順としないといけない。
　なぜかって、それはABC038dをやった時にも散々不思議に思って確認したと思うけど頭には入ってない。。
　でもDPの更新の仕方を見る限り、後ろからやらないと更新済のとこ参照しちゃうからだったと思う。
　ちょうどナップザックを配列1次元1つでやる時は後ろからやらないとダメなのと同じ感じで。
*/
...
void solve() {
    ...
    // dp[i] := 最後に使ったペアのb位置がiの時の最大ペア数
    auto dp = get_segment_tree(N, [](ll a, ll b) { return max(a, b); }, 0LL);
    rep(i, M) {
        auto [a, b] = AB[i];
        dp.update(b, max(dp[b], dp.query(0, b)+1));
    }
    ll ans = dp.all();
    print(ans);
}
