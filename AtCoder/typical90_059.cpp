/*
・E8さん典型90問埋め
・SCC、メモ化再帰、DAG、マージテク、クエリ先読み
・色々頑張ったけど、TLE2件取れず。まあそうだよね。。
*/
...
void solve() {
    ...
    vector<set<ll>> visited(N2+1);
    vector<bool> ans(Q);
    auto dfs = [&](auto&& f, ll u) -> set<ll> {
        if (visited[u].size()) {
            in[u]--;
            if (in[u]) return visited[u];
            else {
                auto tmp = visited[u];
                visited[u].clear();
                return tmp;
