/**
 * ・なんとか自力AC！
 * ・最初'0'*N+'1'*1+'0'*NみたいにやってWAで、そこから色々考えて試して粘った。
 * 　やっぱりN個連続させるのは有力なんじゃないかと思って、
 * 　真ん中じゃなく、'0'*N+'1'*Nの前後に0か1を1個付けてみてうまくいった方、としたら通った。
 * 　TL見る限り、結局前後どっちでもうまくいったみたいだけどね。
 */
...
void solve() {
    ...
    string ans = string(N, '0')+string(N, '1')+string(1, '0');
    ll j = 0, k = 0, l = 0;
    bool ok = 1;
    rep(i, N*2+1) {
        while (j < N*4 and ans[i] != S1[j]) j++;
        while (k < N*4 and ans[i] != S2[k]) k++;
        while (l < N*4 and ans[i] != S3[l]) l++;
        if (j == N*4 or k == N*4 or l == N*4) {
            ok = 0;
            break;
        }
        j++;
    ...
    ans = string(1, '1')+string(N, '0')+string(N, '1');
    j = 0, k = 0, l = 0;
    ok = 1;
    rep(i, N*2+1) {
        while (j < N*4 and ans[i] != S1[j]) j++;
