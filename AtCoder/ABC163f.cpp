/*
参考：https://betrue12.hateblo.jp/entry/2020/04/20/002624
・ABC高難度埋め
・むずい。でもやっぱりグラフ楽しい。アルメリアさんの記事が一番入ってきた。
・余事象、木DP、マージテク
・ある色が1つも使われていない頂点集合であれば、ある色の頂点で囲われた1つの連結成分みたいにできる。
　これを愚直に全色試すと当然N^2でTLEなので、1回のDFSにまとめる方法を考える。
・ある部分木以下で各色が使われた所を管理して、その色が使われたより先の頂点数を持っておくと、
　祖先側にその色が来た時、部分木全体の頂点数 - その色が使われたより先の頂点数、として
　ある色の頂点で囲われた1つの連結成分の頂点数が分かるので、そこの全ペア計算で答えにできる。
　これを木DPしながら順に見ていって、色の管理をマージテクを使ったmapとすればいける。
　また、最後に根からの分を別途全色について追加する。
　これが余事象となるので、全体の全ペアから引けばそれが答え。
*/
...
void solve() {
    ...
    vector<ll> sz(N), ans(N);
    auto dfs = [&](auto&& f, ll u, ll prv) -> map<ll, ll> {
        // mp[c] := 今見ている部分木以下で、色cで閉じられたより先にある頂点数
        map<ll, ll> mp;
        for (ll v : nodes[u]) {
            if (v == prv) continue;
            auto res = f(f, v, u);
            sz[u] += sz[v];
            // 色C[u]で閉じられたより手前にある頂点数
            ll cnt = sz[v] - res[C[u]];
            // ここを1つの連結成分として答えに追加
            ans[C[u]] += cnt*(cnt+1)/2;
            if (res.size() > mp.size()) {
