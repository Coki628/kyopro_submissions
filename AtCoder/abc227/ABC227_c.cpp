/*
・これはTLE。この後にぶたんしてたパートを式変形割り算にしたら通った。
*/
...
void solve() {
    ...
    ll ans = 0;
    rep(a, 1, M) {
        ll bc = bisearch_max(a-1, INF, [&](ll m) {
            if (mul_overflow(a, m)) return false;
            return a*m <= N;
        });
        if (bc == a-1) break;
        rep(b, a, ceil(sqrt(bc))+1) {
            if (mul_overflow(a, b) or a*b > N) break;
            ll c = bisearch_max(b-1, INF, [&](ll m) {
                if (mul_overflow(a*b, m)) return false;
                return a*b*m <= N;
            });
            if (c == b-1) break;
            ans += c-b+1;
