...
・自力AC！
・ビット全探索、グリッドBFS、実装
・結構面倒だけど、こういうのは数学強い人に勝てるがちだから結構好き。
・ビット全探索とBFSする。島1個判定はともかく、穴判定どうするかちょい悩んだ。
　穴側でもBFSして、外周に出れないやつがあったらNGってやった。
*/
...
void solve() {
    ...
    rep(bit, 1LL<<NN) {
        ...
        // このビットに対応するグリッドcurの構築
        auto cur = list2d(N, N, -1);
        rep(i, NN) {
            if (bit>>i & 1) {
                auto [h, w] = idtogrid(i, N);
                cur[h][w] = 1;
            }
        }
        auto used = list2d(N, N, false);
        ll cnt = 0;
        rep(h, N) {
            rep(w, N) {
                ll i = gridtoid(h, w, N);
                // 島が1つの連結成分になってるか確認する
                if ((bit>>i & 1) and not used[h][w]) {
                    int H = cur.size();
                    int W = cur[0].size();
                    const vector<pii> directions = {{-1, 0}, {1, 0}, {0, -1}, {0, 1}};
                    queue<pii> que;
