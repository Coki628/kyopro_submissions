/*
参考：https://img.atcoder.jp/yahoo-procon2019-qual/editorial.pdf
・ARC系青diff残埋め
・いやー、有名問題。ついに本家を履修した。むずかった。自力ならず。。
・耳DP
・同じ所でひたすら往復させると、ほぼ偶奇だけの問題にはできそう、
　さらに考察を進めると、偶数区間 → 奇数区間 → 偶数区間 ってなりそう、
　始まる前後と合わせて、状態5つ持ってDPすればいけるんじゃないか、
　まで行ったのに詰め切れなかった。。
　まあでもこれそもそも耳DPってヒントなしだったら、制約20万なのに、
　区切り位置4箇所もあったらループ絶対無理じゃん－、で終わってた気がするので、やっぱりむずい。
・正解は、偶数区間は0でない偶数でないといけないので、A[i]=0,1を適切に場合分けする、だった。
　これを加味してやる。あと、すぬけくんが減らせる最大数でDPしてAの総和から引いてたんだけど、
　これだと前述のA[i]=0,1の時に最大化なのにマイナスを入れないといけなくなってバグる。。
　なので、素直に、りんごさんが達成できる最小数でDPすると通る。
*/
...
void solve() {
    ...
    const ll M = 5;
    auto dp = list2d(N+1, M, INF);
    dp[0][0] = 0;
    rep(i, N) {
        rep(j, M) {
            rep(k, j, M) {
                if (k == 0 or k == 4) {
                    chmin(dp[i+1][k], dp[i][j]+A[i]);
                }
                if (k == 1 or k == 3) {
                    chmin(dp[i+1][k], dp[i][j]+(A[i] < 2 ? 2-A[i] : (A[i]%2 == 0 ? 0 : 1))); 
                }
                if (k == 2) {
                    chmin(dp[i+1][k], dp[i][j]+(A[i]%2 == 0 ? 1 : 0));
                }
            }
        }
    }
