/*
・きっちり自力AC！こういう実装枠、久々に来た気がする。
・グリッド、実装
・実装頑張る。周り削ったやつを回転させて4つ見た。rot90はC++に移植してなかったから今回初で持ってきた。
・終了後、ちょっと整えた。特に、2次元vectorの一致判定はそのままイコールするだけで大丈夫だった。
*/
...
void solve() {
    ...
    ll u1 = INF, d1 = 0, l1 = INF, r1 = 0;
    ll u2 = INF, d2 = 0, l2 = INF, r2 = 0;
    rep(i, N) {
        rep(j, N) {
            if (grid1[i][j] == '#') {
                chmax(d1, i);
    ...
    rep(i, N) {
        if (u1 <= i and i <= d1) {
            shape1.pb("");
            rep(j, l1, r1+1) {
                shape1.back() += grid1[i][j];
            }
        }
        if (u2 <= i and i <= d2) {
            shape2.pb("");
            rep(j, l2, r2+1) {
                shape2.back() += grid2[i][j];
            }
        }
    }

    ll H1 = shape1.size(), W1 = shape1[0].size();
    ll H2 = shape2.size(), W2 = shape2[0].size();
    rep(_, 4) {
        if (shape1 == shape2) {
            Yes();
            return;
