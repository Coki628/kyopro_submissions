...
void solve() {
    ll N, M;
    cin >> N >> M;
    vector<vector<pair<int, ll>>> nodes(N);
    vector<pii> edges;
    rep(i, M) {
        ll u, v;
        cin >> u >> v;
    ...
    auto route = dk.get_route(0, N-1);
    set<pll> used;
    rep(i, route.size()-1) {
        used.insert({route[i], route[i+1]});
    }    

    rep(i, M) {
        auto [u, v] = edges[i];
        if (not used.count({u, v})) {
            print(mn);
        } else {
            nodes[u].erase(find(ALL(nodes[u]), mkp(v, 1LL)));
            auto dk = get_dijkstra(nodes);
            auto dist = dk.solve(0);
            ll res = dist[N-1] < INF ? dist[N-1] : -1;
            print(res);
            nodes[u].eb(v, 1);
        }
    }
