...
・自力AC！ここまで解き切れてよかった。
・最短経路、BFS
・制約とかの見た目から、ワーシャルフロイドをいじりたくなったけど、途中で切り替えられてよかった。
・なんとなく、最短経路に影響する辺は少ないように思える。
　最初に普通の最短経路と別に、逆辺を張ったグラフで終点から最短経路も取っておくと、
　ある辺を消した時に、始点～u と v～終点 のコストが両方分かるようになるので、
　これ+1が元と最短コストと一致するかどうかで、最短経路に係る辺かどうかが分かる。
　それで最短経路に係る辺だった場合だけ辺取って愚直に再度BFSをやる、としたら通った。
・本番中は多分そういう辺は少ないだろうくらいの出たとこ勝負をしてしまったけど、
　公式解説によると高々N-1本だそう。なんとなく最短パスが複数あったりはするだろうから
　そういう時に結構辺数増えたりしないかなーって心配だったんだけど、
　その時はどうやってもパス長が短くなってしまうし、結局合計でN本以上にはならないんだね。。
*/
...
void solve() {
    ...
    auto distrev = bfs(nodesrev, {(int)N-1});
    ll mn = dist[N-1];
    if (mn == MOD) {
        rep(i, M) print(-1);
        return;
    ...
    rep(i, M) {
        auto [u, v] = edges[i];
        if (dist[u]+distrev[v]+1 > mn) {
            print(mn);
        } else {
            nodes[u].erase(find(ALL(nodes[u]), v));
            auto cur = bfs(nodes, {0});
            ll res = cur[N-1] < MOD ? cur[N-1] : -1;
            print(res);
            nodes[u].eb(v);
        }
    }
