/*
参考：https://atcoder.jp/contests/abc218/editorial
・ABC高難度埋め
・自力ならず。。いけそうな気もしたんだけどね。
・中央値、座圧してBITにぶたん、木DP、ゲームDP
・方針は近いことをやろうとしてたんだけど、途中状態でも中央値取ろうとしたりしてたし、
　これ状態多すぎて無理じゃね？って詰んでたし。。そもそも木なので、
　1個の集合だけ持っておいて、走査の進行に合わせて値を出し入れしていけばよかった。。
　今回は途中の経過に依らず、最終的な経路によってのみスコアが決まるので、
　終了状態でのスコアさえ全部調べておけば、途中の分岐はそれらから優先すべきものを取るだけ。
　動的に集合を動かして中央値判定するのは座圧してBITにぶたんして、
　途中の経路は木のDFSでターン交互のゲームDPみたいにすればOK。DAGじゃなくて木なのでメモ化は要らない。
*/
...
void solve() {
    ...
    auto get_median = [&]() -> ll {
        ll cnt = bit.query(0, M);
        if (cnt%2 == 1) {
            ll a = bit.bisearch_fore(0, M-1, ceil(cnt, 2));
            return cp.unzip(a);
        } else {
            ll a = bit.bisearch_fore(0, M-1, cnt/2);
            ll b = bit.bisearch_fore(0, M-1, cnt/2+1);
            return (cp.unzip(a)+cp.unzip(b))/2;
        }
    };

    vector<ll> dp(N);
    auto dfs = [&](auto&& f, ll u, ll prv, ll depth) -> void {
        bit.add(A[u], 1);
