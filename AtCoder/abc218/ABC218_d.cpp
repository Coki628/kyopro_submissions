/*
・きっちり自力AC！
・2重ループが回る制約なので全ペア見る方針を考える。
　すると、ペアを対角線に置けば(x1,y2),(x2,y1)となる座標が存在するかの判定でいける。
　ただしこれだと逆側から見た時も数えてしまうので最後に割る2する。
・と思ったらこれでも合わなくて、もう少し考えたら、ペアのx,y座標どちらかが同じ位置だと
　ぺったんこになってしまって長方形を作れないので、これを条件で省く。これでAC。
*/
...
void solve() {
    ...
    rep(i, N) {
        ...
        rep(j, i+1, N) {
            ...
            if (se.count({x1, y2}) and se.count({x2, y1})) {
                ans++;
            }
        }
    }
