...
・2次元累積和、二分探索
・ある値xを中央値として使いたいとすると、
　K*K四方の中にx以下の値がceil(K*K, 2)個以上ないといけない。
　こういう場所があるかどうかで二分探索する。
　K番目の値を求める時の二分探索みたいな感じで。
・判定処理内では、2次元累積和を作っておくことで、
　1つのK*K領域に対する判定をO(1)でできるようにしておく。
・あとは問題文の中央値の説明が、大きい方からx番目みたいな言い方だから、
　よく使う小さい方からx番目ってことにするとどうなるかでちょっと悩む。
*/
...
void solve() {
    ...
    ll mid = ceil(K*K, 2);
    auto res = bisearch_min(-1, MX+1, [&](ll m) {
        auto acc = list2d(N+1, N+1, 0LL);
        rep(i, N) {
            rep(j, N) {
                if (grid[i][j] <= m) {
                    acc[i+1][j+1]++;
        ...
        rep(i, N-K+1) {
            rep(j, N-K+1) {
                ll sm = acc[i+K][j+K]+acc[i][j]-acc[i][j+K]-acc[i+K][j];
                chmax(mx, sm);
            }
        }
        return mx >= mid;
