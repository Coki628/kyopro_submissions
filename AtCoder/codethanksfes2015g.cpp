/*
参考：https://n-knuu.hatenablog.jp/entry/2015/12/20/023508
・その他埋めAC数上げ
・自力ならず。。ちょっとしたことだった。
・頂点拡張、ダイクストラ応用
・各頂点で取りうる色は限定的なので、それを2次元目にmapで持つ。
　でもこれだけだと、全然関係ない頂点も全部網羅しようとしてしまって時間が厳しいようで、
　頂点Nに到達した時即終了するようにすると無事AC。これで結構変わることあるんだなー。
*/
...
// ダイクストラ
template<typename T>
vector<defaultdict<ll, T>> dijkstra(const vector<vector<pair<ll, T>>> &nodes, int src, int goal, vector<ll> &C, vector<ll> &B) {

    int N = nodes.size();
    vector<defaultdict<ll, T>> res(N, {INF});
    ...
    while (!que.empty()) {
        ...
        for (auto [v, e] : nodes[u]) {
            if (dist+B[e]+abs(c-C[e]) < res[v][C[e]]) {
                res[v][C[e]] = dist+B[e]+abs(c-C[e]);
                que.push({dist+B[e]+abs(c-C[e]), C[e], v});
            }
        }
    }
    return res;
...
void solve() {
    ...
    auto res = dijkstra(nodes, 0, N-1, C, T);
    ll ans = INF;
    for (auto [k, v] : res[N-1]) {
        chmin(ans, v);
    }
    print(ans);
}
