...
・HLDで部分木クエリすれば目的の範囲は取れる。
　深さDちょうどはHLD番号で振り直した深さをWavelet Matrixに乗せて、
　range_freqした。
*/
...
void solve() {
    ...
    // 深さをHLD番号にエンコード
    vector<int> depth(N);
    rep(i, N) depth[hld.in[i]] = hld.dep[i];
    WaveletMatrix<int, 18> wm(depth);
    ...
    rep(_, Q) {
        ...
        ll l = hld.in[u];
        ll r = hld.out[u];
        auto res = wm.range_freq(l, r, d, d+1);
        print(res);
    }
}
