/*
参考：https://atcoder.jp/contests/abc202/submissions/22810787
・うしさんの覗いたらたまたま気になる解法やってたので。
・DFS、dequeでマージテク、クエリ先読み
・実はこのdequeの先頭に0付けて位置1つずらすのは、自分で最初やろうとした。
　結局うまくまとまらなくて、一直線グラフとかでN^2にならね？
　か思って却下になったんだけど、ちゃんとできる解法だった。
・具体的には、マージテクすればよかった。
　これだと大きいのを何回も操作しなくて済む。
　setでマージテクDFSはやったことあったけど、dequeでもできるんだね。
・あと頂点毎に答えが出るので、クエリ先読みでデータ持ち替えてからやる。
*/
...
void solve() {
    ...
    vector<ll> ans(Q);
    auto dfs = [&](auto&& f, ll u, ll prv, ll depth) -> deque<ll> {
        // que[i] := ここを根とした部分木から見て深さiにある頂点数
        deque<ll> que = {1};
        for (ll v : nodes[u]) {
            if (v == prv) continue;
            auto res = f(f, v, u, depth+1);
            // 深さを1つずらす
            res.push_front(0);
            // マージテク
        ...
        for (auto [i, d] : qs[u]) {
            if (0 <= d-depth and d-depth < que.size()) {
                ans[i] = que[d-depth];
            }
        }
        return que;
