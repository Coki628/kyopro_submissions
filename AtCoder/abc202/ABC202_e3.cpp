/*
参考：https://atcoder.jp/contests/abc202/editorial
・公式解の方針
・オイラーツアー、二分探索
・オイラーツアーで行きがけと帰りがけの時間は頂点毎に持っておく。
　それと深さ毎に行きがけ時間を持っておけば、
　各深さで、ある頂点の部分木に含まれるものがいくつあるかは
　二分探索でチェックできる。
*/
...
void solve() {
    ...
    rep(i, Q) {
        ...
        cin >> u >> d;
        u--;
        ll res = bisect_left(adj[d], out[u])-bisect_left(adj[d], in[u]);
        print(res);
    }
}
