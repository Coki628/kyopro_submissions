/*
参考：https://atcoder.jp/contests/arc106/editorial
　　　https://maspypy.com/atcoder-%e5%8f%82%e5%8a%a0%e6%84%9f%e6%83%b3-2020-10-24arc106#toc4
　　　https://math.nakaken88.com/textbook/basic-n-th-power-expansion-and-binomial-theorem/
・ARC青diff残埋め
・二項定理、シグマの式変形
・冪乗の式からいきなりnCrが出てきて最初意味分からなかったが、ググったりして理解した。
　これが二項定理だ。今までいまいちピンと来なかったんだけど、今日は分かったと思う。
　ようは (x+y)^2 = x^2+xy+y^2 みたいな式変形って中学の頃からずっとやって来てたけどこれなんだ。
　これって3乗以降になってもやたら規則的で対称的な形になるけど、これを一般化したのが二項定理なんだね。
　式を書くなら (x+y)^k = ΣnCr(k, i)*x^i*y^(k-i) みたいな感じになる。
　これだけ見るとなんでこんなゴチャゴチャにするのか意味不明だけど、x,yが独立になるから、
　競プロ的にはΣを動かせるようになって大きな意味を持ってくる。Σが動かせると、独立に計算できるようになるから。
　今回はこれでi,jの添え字が独立したΣにできるから、それで答えが出せるようになってる。
・ここが分かれば、後は本当はループ半分だから引く所とか、独立させたシグマは前計算しておくとか、
　その辺は何度もやってる観点だから大丈夫。数学強いとむしろ二項定理のが空気なのかもしれないけど、
　自分的にはここで初めて何がおいしいのか理解してしっくり入ってきた感じ。
・前計算が素直にやると冪乗のlogが乗ってO(NKlogK)なので、6000万にlog乗るくらいになってきつい。
　まあそれでも制約3秒で間に合ったけど、これは多分C++殴り。(AC2.23秒)
・ちゃんと速くしてみた。前計算のループ順入れ替えて、logかからないようにしてる。これだとAC0.27秒。
*/
...
void solve() {
    ...
    // ΣA[i]^k と Σ(A[i]+A[i])^k を前計算
    vector<mint> S(K+1), sub(K+1);
    // rep(k, K+1) {
    //     rep(i, N) {
    //         S[k] += mint(A[i]).pow(k);
    //         sub[k] += mint(2*A[i]).pow(k);
    //     }
    // }
    rep(i, N) {
        mint mul = 1, mul2 = 1;
    ...
    ModTools<mint> mt(K+1);
    rep(x, 1, K+1) {
        mint res = 0;
        rep(k, x+1) {
            // ΣA[i]^k*ΣA[i]^(x-k)*nCr(x, k)
            res += S[k]*S[x-k]*mt.nCr(x, k);
        }
        // 半分は余計なので減らす
        res -= sub[x];
        res /= 2;
