...
void solve() {
    ...
    multiset<int> init1, init2;
    init1.insert(Y[0]);
    rep(i, 1, N) init2.insert(Y[i]);
    int res = bisearch_max(0, MOD, [&](int m) {
        ...
        rep(l, N) {
            while (r < N and X[r]-X[l] < m) {
                se2.erase(se2.find(Y[r]));
                r++;
            }
            if (r == N) break;
            if (
                X[r]-X[l] >= m and se1.size() and se2.size() and
                (se1.back()-se2.front() >= m or se2.back()-se1.front() >= m)
            ) {
                return true;
            }
            if (l+1 < N) se1.insert(Y[l+1]);
        }
        return false;
    });
    print(res);
