/*
参考：https://www.youtube.com/watch?v=NxuL8oOwIvM
・ABC高難度埋め
・むずい。包除とか疑ってたけど違ったし。。
・期待値の線形性、Σ入れ替え(主客転倒)、余事象、種類数の種類数
・前半の数え上げの考察パートがまず解けない。でもこのくらいはできたい。。
　期待値なんだけど、やっぱり全部足してって最後に割るみたいな方針を取る。これはまあよくある。
　でも足し合わせるのは通り数ではなく「種類数の合計」みたいなものになる。
　これもまあサンプルからも見て取れる。ここからが正念場。
　この足し合わせを式にすると、あるKについて「K個取る全パターンのΣ(全色見るΣ(その色があるかどうか))」
　みたいになるんだけど、これを入れ替えて、「全色見るΣ(K個取る全パターンのΣ(その色があるかどうか))」
　と解釈すると「全色見るΣ(ある色が出現するパターン数)」みたいにできて見通しが良くなる。
　まあでもこのままだとまだダメで、さらにもう1つ、余事象にして、
　「全色見るΣ(ある色が出現するパターン数 = K個取る全パターン数 - ある色が出現しないパターン数)」
　であれば、全パターンとある色が出現しないのはすぐ出る。全パターンはいいとして、出現しないのは、
　その色以外からK個取る通り数なので、nCr(N - その色の個数, K) みたいな感じ。
・さて、ここまで落とし込めてやっと次が本質。色毎に見ると最悪Nくらい内ループにかかるので、
　これを高速化する。でもこっちのが自分的には思いつくかもしれない。
　各色について、色の個数が同じやつはやる計算が同じなので、これをまとめる。
　これは種類数の種類数みたいな、Counter2回やるやつでできる。
　こうすると内側の計算量が√Nになるそう。種類数の種類数はたまに見たことあったけど、
　√Nにできるのは知らなかったなー。
*/
