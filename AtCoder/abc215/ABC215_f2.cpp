/*
参考：https://atcoder.jp/contests/abc215/editorial
・自力ならず。
・二分探索、尺取り
・決め打ちにぶたんはちょっと考えたんだけど、うまく基準を作れなかった。
・最大値K以上を達成できるか？みたいなにぶたんをしようとすると、
　abs(xi-xj) >= K and abs(yi-yj) >= K を満たすペアがあるかみたいな判定にできて、
　X軸をソートして尺取りしながらY軸をうまく管理すると平面走査っぽくできる。
・のはずなんだけどWAが取れなくて悩んだ。結局この形だと、
　真ん中をYの集合から外して、尺取った外側を集合に入れていく感じになるので、
　よく考えると条件を満たすのは集合全体のabs(max-min)じゃなくて、
　abs(前集合のmax-後ろ集合のmin)かその逆、みたいになるんだった。
・これを踏まえて実装を直すと、WAはなくなってTLEになった。multisetのlogは重い。。
　20万にlog2つなら、普通のC++なら全然間に合う感じがするのにこれは全然ダメ。。
・multisetをmap管理に変えてみた。全体的にはこっちのが速くなってそうなんだけど、
　たまにこっちのが遅いケースもあるみたい。
　多分eraseの回数が多くなると重くなるのかなぁ。。
*/
...
struct my_map : public map<_Key, _Tp, _Compare, _Alloc> {
    pair<_Key, _Tp> front() {
        return *this->begin();
    }
    pair<_Key, _Tp> pop_front() {
        auto res = this->front();
...
void solve() {
    ...
    my_map<int, int> init1, init2;
    init1[Y[0]]++;
    rep(i, 1, N) init2[Y[i]]++;
    int res = bisearch_max(0, MOD, [&](int m) {
        auto mp1 = init1;
        auto mp2 = init2;
        int r = 1;
        rep(l, N) {
            ...
            if (r == N) break;
            if (
                X[r]-X[l] >= m and mp1.size() and mp2.size() and
                (
                    mp1.back().first-mp2.front().first >= m or
                    mp2.back().first-mp1.front().first >= m
                )
            ) {
                return true;
            }
