...
・なんとか自力AC！危なかった。。
・式変形、枝刈り全探索
・Nがループ1周も回せないくらいにでかいので、どうするか考える。
　aを決めたとして、あるaに対して条件を満たす最大のb*cは、a*b*c <= N より、
　式変形して b*c <= N/a の最大となるb*cで、これは整数の範囲だと b*c = floor(N, a) とできる。
　次にこのb*cまでを範囲としてbを決め打って、同様に今度は最大のcを
　c <= N/(a*b) から c = floor(N, a*b) で求める。これでa,bを決めた時の最大のcが定まるので、
　cが取りうる範囲である[b,c]を答えに追加する。
　これに bc < a　や c > b といった答えに適さない値で適切な枝刈りをしておく。無事AC。。
・これCに置くの結構きつくない？って思うけど茶diffなんだよな～。。
*/
...
void solve() {
    ...
    rep(a, 1, M) {
        ll bc = N/a;
        if (bc < a) break;
        rep(b, a, ceil(sqrt(bc))+1) {
            if (mul_overflow(a, b) or a*b > N) break;
            ll c = N/(a*b);
            if (c < b) break;
            ans += c-b+1;
        }
