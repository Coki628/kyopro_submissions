/*
参考：https://atcoder.jp/contests/abc211/editorial
・自力ならず。。
・再帰全探索
・見るからに全探索通りそうな見た目をしていたのに、詰めきれず。。
　再帰の内側で四方を見に行くような感じに作ったんだけど、それじゃ足りなくて、
　再帰の内側で全マス見に行く、でよかったんだね。間に合うんだなこれで。
*/
...
void solve() {
    ...
    set<ll> se;
    auto rec = [&](auto&& f, ll cur) {
        if (se.count(cur)) return;
        se.insert(cur);
        if (popcount(cur) == K) return;
        vector<pll> nxt;
        rep(h, N) {
            rep(w, N) {
