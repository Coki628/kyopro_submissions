...
・自力AC！時間かかったけど嬉しい。
・グラフ、平方分割
・past202012_oと同じような方針でいけそうなのはすぐ見えたのに時間かかった。
・結局周囲の更新に使う色と時間(color_used,last_used)と、
　実際に自分が更新された色と時間(color_updated,last_updated)がごっちゃになってたのが
　良くなかった。ここを変数きっちり分けて管理したら無事AC。
*/
...
void solve() {
    ...
    ll Q;
    cin >> Q;
    vector<ll> color_updated(N, 1), last_updated(N, -1), color_used(L, -1), last_used(L, -1);
    rep(i, Q) {
        ll x, y;
        cin >> x >> y;
        x--;
