...
・これは自力TLE。。
・DAG、メモ化再帰
・全ての関係に辺を張るのは多すぎるので、値aが隣接する頂点同士だけに張った。
　でもこれでも、多分同値の頂点が多いケースとかで辺が増えすぎるんだと思う。。
*/
...
void solve() {
    ll H, W, N;
    cin >> H >> W >> N;
    vector<map<ll, vector<ll>>> rows(H), cols(W);
    rep(i, N) {
        ll r, c, a;
        cin >> r >> c >> a;
        r--; c--;
