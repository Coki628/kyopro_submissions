...
void solve() {
    ...
    iota(ALL(goal), 0);
    goal[N-1] = -1;
    ll s = find(ALL(V), -1) - V.begin();

    map<vector<ll>, ll> memo;
    queue<tuple<ll, ll, vector<ll>>> que;
    memo[V] = 0;
    que.push({0, s, V});
    while (que.size()) {
        auto [dist, u, V] = que.front(); que.pop();
        if (V == goal) break;
        for (ll v : nodes[u]) {
            swap(V[u], V[v]);
            if (not memo.count(V)) {
                memo[V] = dist+1;
                que.emplace(dist+1, v, V);
            }
            swap(V[u], V[v]);
        }
    }
    ll ans = memo.count(goal) ? memo[goal] : -1;
    print(ans);
}
