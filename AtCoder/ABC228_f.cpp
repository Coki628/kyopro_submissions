...
・自力AC？2Dセグ木使うは聞いてしまったけど、これは頑張れば自分でも導けた気がする。
・2次元累積和、2Dセグ木
・2Dセグ木は初めて使った。ライブラリはTLで流れてきたやつをベースに、
　うしさんセグ木の抽象化とかを入れてちょっとカスタマイズした。
・解法としては、先手後手それぞれで必要な2次元区間和が取れるように累積和を準備して、
　先手の区間和に対して、対応する範囲での後手の区間和から最大値が取れればいい。
　これは累積和配列を2Dセグ木に突っ込むとできる。
・100万グリッドにlog2つ乗るからまあまあきついけど、ちゃんと間に合って無事AC0.59秒。
*/
...
struct SegmentTree2D {
    ...
    int H, W;
    vector<vector<Monoid>> dat;

    const F f;
    const Monoid M1;

    SegmentTree2D(const F f, const Monoid &M1) : f(f), M1(M1) {}
    SegmentTree2D(vector<vector<Monoid>> &grid, const F f, const Monoid &M1) : f(f), M1(M1) {
        H = W = 1;
        while(H < (int)grid.size()) H <<= 1;
        while(W < (int)grid[0].size()) W <<= 1;
        dat.assign(2*H-1,vector<Monoid>(2*W-1, M1));
        init(grid);
    }
    void init(vector<vector<Monoid>> &grid) {
        for (int i = 0; i < (int)grid.size(); i++)
            for (int j = 0; j < (int)grid[0].size(); j++)
                dat[i+H-1][j+W-1] = grid[i][j];
        for (int i = 2*H-2; i > H-2; i--)
            for (int j = W-2; j >= 0; j--)
                dat[i][j] = f(dat[i][2*j+1], dat[i][2*j+2]);
        for (int i = H-2; i >= 0; i--)
            for (int j = 0; j < 2*W-1; j++)
                dat[i][j] = f(dat[2*i+1][j], dat[2*i+2][j]);
    }
    Monoid query(int li, int lj, int ri, int rj) { return query_h(li,lj,ri,rj,0,H,0); }
    Monoid query_h(int li, int lj, int ri, int rj, int si, int ti, int k) {
        if(ri <= si or ti <= li) return M1;
        if(li <= si and ti <= ri) return query_w(lj,rj,0,W,k,0);
        const int mi = (si+ti)/2;
        return f(query_h(li,lj,ri,rj,si,mi,2*k+1), query_h(li,lj,ri,rj,mi,ti,2*k+2));
    }
    Monoid query_w(int lj, int rj, int sj, int tj, int i, int k) {
        if(rj <= sj or tj <= lj) return M1;
        if(lj <= sj and tj <= rj) return dat[i][k];
        const int mj = (sj+tj)/2;
        return f(query_w(lj,rj,sj,mj,i,2*k+1),query_w(lj,rj,mj,tj,i,2*k+2));
    }
};

template<typename Monoid, typename F>
SegmentTree2D<Monoid, F> get_segment_tree_2d(vector<vector<Monoid>> &grid, const F& f, const Monoid& M1) {
    return {grid, f, M1};
}

template<typename Monoid, typename F>
SegmentTree2D<Monoid, F> get_segment_tree_2d(const F& f, const Monoid& M1) {
    return {f, M1};
}
...
void solve() {
    ...
    auto acc = list2d(H+1, W+1, 0LL);
    rep(i, H) rep(j, W) acc[i+1][j+1] = grid[i][j];
    rep(i, H+1) {
        rep(j, W) {
            acc[i][j+1] += acc[i][j];
        }
    ...
    ll H1 = H-h1;
    ll W1 = W-w1;
    auto sm1 = list2d(H1+1, W1+1, 0LL);
    rep(i, H1+1) {
        rep(j, W1+1) {
            ll ni = i+h1;
            ll nj = j+w1;
            sm1[i][j] = acc[ni][nj]+acc[i][j]-acc[ni][j]-acc[i][nj];
        }
    }
    ll H2 = H-h2;
    ll W2 = W-w2;
    auto sm2 = list2d(H2+1, W2+1, 0LL);
    rep(i, H2+1) {
        rep(j, W2+1) {
            ll ni = i+h2;
            ll nj = j+w2;
            sm2[i][j] = acc[ni][nj]+acc[i][j]-acc[ni][j]-acc[i][nj];
        }
    }
    ...
    ll dw = w1-w2+1;
    ll ans = 0;
    auto seg = get_segment_tree_2d(sm2, [](ll a, ll b) { return max(a, b); }, -INF);
    rep(i, H1+1) {
        rep(j, W1+1) {
            chmax(ans, sm1[i][j]-seg.query(i, j, i+dh, j+dw));
            // ll mx = 0;
            // rep(h, i, i+dh) {
            //     rep(w, j, j+dw) {
            //         chmax(mx, sm2[h][w]);
