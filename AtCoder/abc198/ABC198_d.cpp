/*
・なんとか自力AC！でもこれくらいで時間食ってちゃいけないね。
・順列全探索
・問題のルール上、文字の種類数が10超だと0~9で割り当てができないので即NG。
　10以内だったら、どの文字と数字を紐付けるかは順列全探索できる。
　10!が結構きつめだけど、363万くらいなので、
　内側で文字数(10)くらいを回してもまあなんとかなる。制約5秒だし。
*/
...
void solve() {
    ...
    set<char> se;
    for (char s : S) se.insert(s);
    for (char s : T) se.insert(s);
    for (char s : U) se.insert(s);
    if (se.size() > 10) {
        print("UNSOLVABLE");
        return;
    }
    ...
    vector<ll> P;
    rep(i, 10) P.pb(i);
    for (auto perm : permutations(P, N)) {
        vector<ll> A, B, C;
        for (auto s : S) {
            A.pb(perm[mp[ord(s)-97]]);
        }
        for (auto s : T) {
            B.pb(perm[mp[ord(s)-97]]);
        }
        for (auto s : U) {
            C.pb(perm[mp[ord(s)-97]]);
        }
