/*
参考：https://github.com/E869120/kyopro_educational_90/blob/main/editorial/080.jpg
・E8さん典型90問埋め
・自力ならず。悔しいなぁ。。
・数え上げ、包除原理、ビット
・包除いつも解けないなぁ。今回も余事象を数えるとこまでは行ったのに。
　(まあそこまで行くのもかなり時間かかっててダメだったんだけど)
・こういう、条件を満たす集合を探すけど、集合同士だいぶ被るよなぁみたいな時は、
　包除を疑うのが良いのかもしれない。
*/
...
void solve() {
    ...
    rep(bit, 1, 1LL<<N) {
        ...
        // 全体Dから引いて使えるビットにする
        ll use = D-popcount(cur);
        // この条件を満たす通り数を、要素数の偶奇によって+-する
        ans += pow(2, use)*pow(-1, popcount(bit));
    }
    print(ans);
}
