...
// 区間01反転・区間01最大連長取得
struct Node {
    ll mx0, mx1, lval, rval, llen, rlen, sz;
    operator ll() const {
        return mx1;
    }
};
...
auto g = [](Node a, bool b) -> Node {
    ...
    if (!b) {
        ...
        // 0と1の関係が入れ替わる
        return {a.mx1, a.mx0, 1 - a.lval, 1 - a.rval, a.llen, a.rlen, a.sz};
    }
};
auto h = [](bool a, bool b) -> bool { return a ^ b; };
const Node T = {0, 0, -1, -1, 1, 1, 1};
const bool E = false;
...
void solve() {
    ...
    rep(i, N) {
        if (S[i] == '0') {
            seg.set(i, {1, 0, 0, 0, 1, 1, 1});
        } else {
            seg.set(i, {0, 1, 1, 1, 1, 1, 1});
        }
    }
    seg.build();
