/**
 * 参考：https://www.youtube.com/watch?v=N20qoG42U1I
 * ・自力ならず。問題の意味が汲み取れず。。
 * ・functional graph
 * ・「SからSへの関数」って言い回しがよく意味分からなかったんだけど、
 * 　ようは行き先を指してる有向グラフになるやつらしい。functional graphって言うんだって。
 * ・で、それを踏まえて今回の条件を整理すると、
 * 　・ある頂点を選んだら、その行き先も選ぶ
 * 　・行き先が同じになる2頂点は同時に選ばない
 * 　の2つを守って部分集合を作るって話になる。
 * ・そうすると、できることは、
 * 　・サイクルに属する頂点は全て選ぶor全て選ばない
 * 　・サイクルに属さない頂点(どっかのサイクルに入っていく頂点)は常に選べない
 * 　になって、今回のグラフの特性(出次数がどの頂点も1)から、
 * 　同じ連結成分には必ずサイクルが1個になるので、ようは連結成分を数えればいい。
 * 　作れる部分集合の数を答えるので、選ぶor選ばないで2^(連結成分数)、これを空集合の分-1する。
 */
