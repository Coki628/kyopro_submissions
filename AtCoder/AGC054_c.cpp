/*
参考：https://ks2m.hatenablog.com/entry/2021/06/28/012537
　　　https://emtubasa.hateblo.jp/entry/2021/06/22/224211#AGC054
・AGC青diff以下残埋め
・自力ならず。AGCの青diffむずいね。
・BIT、転倒数
・なんかそもそも題意の操作を掴むのにすごく時間がかかった。。
　ようは転倒してる所を直していく感じなんだけど…。
・逆操作っぽいものを考えるんだけど、最小回数の操作にしないといけないので、
　転倒してる所を何でも直していい訳ではない。
　各iについて、条件を満たすjがK個以下になっているが、ここで既にK未満となっている場所(にある要素)は
　もし直前に動かしていたとしたら、最小ではない余計な操作になってしまう(既に操作前でK以下を達成しているため)ので、
　これは操作対象から外す。つまり現状で条件を満たすjがK個の場所だけを考える。
・それでどこまで動かせるかと言うと、i以降どこに動かしてもいい。
　しかもこれは各i独立に考えてよく、単純に各iでの結果を掛け合わせれば答えになる。
　言われればまあそうかもと思えるけど、自分で導くのは険しいよなぁ。。
・実は制約が2乗OKになってるのでBIT使わないでも通る。紛らわしい。。
*/
