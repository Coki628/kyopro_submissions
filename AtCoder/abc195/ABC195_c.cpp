/**
 * ・きっちり自力AC！
 * ・1000以上の数には一番右のコンマは絶対あるし、
 * 　100万以上の数には次のコンマが絶対ある、って考えるとすぐ。
 * ・外れ方針引くとちょっと詰まるやつだったみたい。すんなりいってよかった。
 */
...
void solve() {
    ...
    ll ans = 0;
    ans += max(N-999, 0LL);
    ans += max(N-999999, 0LL);
    ans += max(N-999999999, 0LL);
    ans += max(N-999999999999, 0LL);
    ans += max(N-999999999999999, 0LL);
    ans += max(N-999999999999999999, 0LL);
    print(ans);
}
