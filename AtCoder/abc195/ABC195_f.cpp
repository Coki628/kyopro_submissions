/**
 * 参考：https://blog.hamayanhamayan.com/entry/2021/03/14/002322
 * ・自力ならず。
 * ・素数列挙、bitDP応用
 * ・どうにか素因数とかに絞り込んで状態減らしたbitDPだろう、
 * 　とは思ってたものの、正解に辿り着く詰め方はできなかった。
 * ・公式解説とかTLで、必要な素因数がB-Aの72まででいい理由が釈然としなかったんだけど、
 * 　はまやんさんの説明から、値の差が最大で72しかないから、73以上ある素因数だと
 * 　AからBの範囲内の値にそれを掛けた時に、絶対Bより大きくなってしまう。
 * 　よって考慮しなくていい。ってのが一番しっくり来た。
 * ・これで考慮しないといけない素数が20個だけになったので、bitDPで状態が持てる。
 * 　ある値A[i]を使うor使わないの遷移を、ナップザックみたく進めていく。
 * 　この時、使う遷移は今の状態と重ねた時素因数が被らない場合のみできる。
 */
...
void solve() {
    ...
    auto primes = eratosthenes_sieve(MX);
    ll M = primes.size();
    vector<ll> B(N);
    rep(i, N) {
        rep(j, M) {
            // A[i]がこの素因数を持つならビットを立てる
            if (A[i]%primes[j] == 0) {
                B[i] |= 1<<j;
            }
        }
    ...
    auto dp = list2d(N+1, 1<<M, 0LL);
    dp[0][0] = 1;
    rep(i, N) {
        rep(bit, 1<<M) {
            // A[i]を使わない遷移
            dp[i+1][bit] += dp[i][bit];
            // 重複する素因数がなかったら、A[i]を使う遷移
            if ((bit&B[i]) == 0) {
