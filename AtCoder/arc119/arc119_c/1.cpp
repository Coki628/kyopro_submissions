/*
参考：https://atcoder.jp/contests/arc119/editorial
・自力ならず。。初手から何も出ず。
・「奇数番目の和 = 偶数番目の和」になると条件を達成できる。
　これの思いつき方だけど、公式解の式を立てる方法が凡人にはまだ使えそう。。
・で、条件が分かったら後は実装方針だけど、これも自分でできたか微妙。。
　片側を負の向きにして、総和が0になるとOKみたいな形にすると、
　累積和取って同じ値になるペア数ってやれる。Zero Sum Ranges系みたいに。
・こういう、整えてみれば典型っぽい考え方でいけるんだけど考察要る、
　みたいなやつ、いつまで経っても安定して解けないなー。。
*/
