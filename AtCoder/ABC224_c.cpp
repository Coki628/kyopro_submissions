...
・自力AC！
・幾何、三角形の性質
・傾き(切片)が3頂点それぞれのペアで等しかったらぺったんこで三角形は作れない。それ以外はOK。
・最初、3辺からヘロンの公式で面積0をチェックしようとしたら、なんかサンプルからして合わなかった。。
　誤差かな、そもそも間違ってるんかな、うーん。。。
*/
...
ld check(ld x1, ld y1, ld x2, ld y2) {
    if (x1 == x2) return INF;
    return (y2-y1)/(x2-x1);
}
...
void solve() {
    ...
    rep(i, N) {
        rep(j, i+1, N) {
            rep(k, j+1, N) {
                ld a = check(X[i], Y[i], X[j], Y[j]);
                ld b = check(X[i], Y[i], X[k], Y[k]);
                ld c = check(X[k], Y[k], X[j], Y[j]);
                if (not (abs(a-b) < EPS and abs(b-c) < EPS and abs(c-a) < EPS)) {
                    ans++;
                }
            }
        }
