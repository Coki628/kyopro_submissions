/*
参考：https://kusano-prog.hatenablog.com/entry/20120818/1345260509
　　　https://competitive12.blogspot.com/2020/04/2012-b-c.html
・試験管青diff残埋め
・自力ならず。。これも解けるべきだよなぁ。
・bitDP、部分集合の列挙
・自分では方針立たず。なんかいもすとかUFとかやって死んでた。
　制約がbitDPって言ってんだから察するべきだったよね。。
　部分集合列挙のやり方取りにABC187_f見に行ったらDP部分全体がほぼそのままだった。。
・こういうの、補グラフの彩色数って言うらしい。
*/
...
void solve() {
    ...
    rep(i, N) {
        ...
        auto shm = split(s, ':');
        auto thm = split(t, ':');
        LR[i] = {shm[0]*60+shm[1], thm[0]*60+thm[1]};
        LR[N+i] = {(shm[0]+24)*60+shm[1], (thm[0]+24)*60+thm[1]};
    }
