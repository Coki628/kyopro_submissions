/*
参考：https://atcoder.jp/contests/abc210/editorial
・自力ならず。悔しいな。
・グリッドDP
・45度回転とか考えてて、DPの方針に全然行かなかった。。
・左上→右下と右上→左下で分けて考えればDPできる。まずこれが出なかったね。。
・で、遷移はどうするかと言うと、状態を
　0: ここから始める
　1: 移動中
　2: ここで終わる
　みたいに持っておけば、それぞれのコストを足しながら遷移していける。
・逆の斜めは、行リバースしたら同じことやるだけですんなりできた。
・状態3つにしちゃったけど、状態0いらなそうだね。。
*/
...
void solve() {
    ...
    auto dp0 = list2d(H+1, W+1, INF);
    auto dp1 = list2d(H+1, W+1, INF);
    auto dp2 = list2d(H+1, W+1, INF);
    rep(i, 1, H+1) {
        rep(j, 1, W+1) {
            dp0[i][j] = grid[i][j];
            dp1[i][j] = min({dp0[i-1][j], dp0[i][j-1], dp1[i-1][j], dp1[i][j-1]})+C;
            dp2[i][j] = min({min({dp0[i-1][j], dp0[i][j-1], dp1[i-1][j], dp1[i][j-1]})+C+grid[i][j], dp2[i-1][j], dp2[i][j-1]});
        }
    }
    ll ans = dp2[H][W];

    rep(i, 1, H+1) {
        reverse(grid[i].begin()+1, grid[i].end());
    }
