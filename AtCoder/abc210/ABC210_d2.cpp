/*
参考：https://kotatsugame.hatenablog.com/entry/2021/07/20/041638
　　　https://atcoder.jp/contests/abc210/submissions/24297500
・別解。こたつがめさんのブログから。やっぱり強い人はデータ構造の使い方もうまい。
・遅延セグ木、区間最小値取得・区間加算
・縦移動の最小コストは累積minで集めておく。これで、残るは他の列からあるマスに
　どう移動して終わるのが最善かを決める横移動だけの問題になる。
・横移動のコストは横位置が動けば変化してしまうが、1マスにCずつの規則的な変化なので、
　これは区間加算で手前を減らして先を増やしてみたいにすれば動的に管理できる。
　具体的なものはソース内コメントも参照。
*/
...
void solve() {
    ...
    ll ans = INF;
    auto accmn = list2d(H, W, INF);
    accmn[0] = grid[0];
    rep(i, 1, H) {
        rep(j, W) {
            // 縦移動の最小コストを累積minしておく
            accmn[i][j] = min(grid[i][j], accmn[i-1][j]+C);
            // 縦移動のみで終えるパターン
            chmin(ans, accmn[i-1][j]+C+grid[i][j]);
        }
    }

    rep(i, H) {
        auto seg = get_Lazy_segment_tree(W, f, g, h, T, E);
        rep(j, W) {
            // 列0にいる状態の他列からの移動コストで初期化
            seg.set(j, accmn[i][j]+C*j);
        }
        seg.build();
        rep(j, W) {
            // 別の列からマス[i][j]に移動して終わる最小を取る
            chmin(ans, min(seg.query(0, j), seg.query(j+1, W))+grid[i][j]);
            // 1列動いた分の移動コストを加減する
            seg.update(0, j+1, C);
            seg.update(j+1, W, -C);
        }
