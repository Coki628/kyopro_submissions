...
struct Dijkstra {
    ...
    vector<T> solve(const vector<int> &src, T init=T()) {
        vector<T> res(N, inf);
    ...
    vector<T> solve(int s, T init=T()) {
        return solve(vector<int>({s}), init);
    }
};
...
void solve() {
    ll N, M;
    cin >> N >> M;
    vector<vector<pair<int, pll>>> nodes(N);
    vector<ll> C(M), D(M);
    rep(i, M) {
        ll u, v, c, d;
        cin >> u >> v >> c >> d;
    ...
    auto calc = [](ll t, ll c, ll d) {
        auto [i, val] = trisearch_min(t, INF, [&](ll m) {
            return (m-t)+d/(m+1);
        }, 500);
        return c+val;
    };

    auto dk = get_dijkstra(nodes, INF, [&](ll a, pll b) {
        return a + calc(a, b.first, b.second);
    });
    auto res = dk.solve(0);
