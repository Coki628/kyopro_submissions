/*
・ABC高難度埋め
・なんとか自力AC？前ざっくりした方針は見たけど具体的な実装方針が立たなくて放置してたやつ通せた。
・スライドbitDP、行列累乗
・制約から、スライドbitDPができそうな見た目はしていて、実際できる。
　そこから先W個のマスを状態に持って、今の場所に半畳を置く、一畳を縦に置く、一畳を横に置く、
　を適切な条件で遷移させると、Hが10万くらいまでは解けそうだ。(コメントアウト1個目参照)
　でも今回は10^12なので無理。行列累乗に持っていく方針を検討する。
・作ったDPをそのまま行列累乗に乗せようとすると無理。
　添字iに依存した w != -1 という条件が横置きの時に必要になるから。
　これをi側の添字ではなくすために、列の動きの分をj側の状態に含める。(コメントアウト2個目参照)
・これでiに依存しない遷移が作れたので、1回分の遷移を各状態から試して、
　そこから行きつく状態に応じて遷移行列を埋めていく。これで通るかと思ったら、TLE。。ここからも結構大変だった。
・計算量を検討すると、列の情報を状態に含めてしまったので、状態数が最大で2^6*6=384になって、
　行列累乗はこれの3乗で約5660万、これにHの最大10^12のlogが乗るので確かにこれはきつい。。
・高速化をした。まず行列積の計算で添字j,kの順序を逆にすると速くなるという情報があったため、
　これを試すと実際速くなって、最大ケースで6秒以上かかっていたのが4秒台になった。
　さらに、今回おそらく行列がだいぶ疎だろうと思って、何とかそれを利用できないか考えると、
　行列積の3重ループで、2重になった所でmat[i][k]が0だった場合、その回の3重ループは全て0で
　回しても意味がないことに気付く。ここをcontinueしてみる。するとこれがかなり効いて、無事AC0.96秒。
・苦労はしたものの、おかげで自分の行列累乗ライブラリが速度改善できたのでOK。
・解説を確認すると、やっぱり列情報持たずに2^6=64の3乗を想定してるっぽいので、検討してみる。
　DP遷移を確認していくと、最終的に0行0列から1行0列までに至る状態の通り数が分かればいいので、
　1回の遷移の最中には列情報を持つけど、遷移行列に詰める時にはそこは無くしていいのでは、となる。
　実際これが当たり。遷移確認のDPでは列情報を持って添字3つでやるけど、
　遷移行列には列の添字は使わず列=0の所からだけ持ってくる。これで行列累乗に乗せると、無事通ったし、速い。
　まあ多分計算量が*6^3=216くらい変わってるので、それはそうか。
*/

// #pragma GCC target("avx2")
// #pragma GCC optimize("O3")
// #pragma GCC optimize("unroll-loops")

#define _USE_MATH_DEFINES
#include <bits/stdc++.h>
using namespace std;

using ll = long long;
using ull = unsigned long long;
using ld = long double;
using pll = pair<ll, ll>;
using pii = pair<int, int>;
using vvl = vector<vector<ll>>;
using vvi = vector<vector<int>>;
using vvpll = vector<vector<pll>>;
#define name4(i, a, b, c, d, e, ...) e
#define rep(...) name4(__VA_ARGS__, rep4, rep3, rep2, rep1)(__VA_ARGS__)
#define rep1(i, a) for(ll i = 0, _aa = a; i < _aa; i++)
#define rep2(i, a, b) for(ll i = a, _bb = b; i < _bb; i++)
#define rep3(i, a, b, c) for(ll i = a, _bb = b; (a <= i && i < _bb) or (a >= i && i > _bb); i += c)
#define rrep(i, a, b) for (ll i=(a); i>(b); i--)
#define pb push_back
#define eb emplace_back
#define mkp make_pair
#define ALL(A) A.begin(), A.end()
#define elif else if
#define tostr to_string
constexpr ll INF = 1e18;
// constexpr ll INF = LONG_LONG_MAX;
// constexpr int MOD = 1000000007;
constexpr int MOD = 998244353;
constexpr ld EPS = 1e-10;
constexpr ld PI = M_PI;

const string digits = "0123456789";
const string ascii_lowercase = "abcdefghijklmnopqrstuvwxyz";
const string ascii_uppercase = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
const string ascii_letters = ascii_lowercase + ascii_uppercase;

template<typename T> vector<vector<T>> list2d(int N, int M, T init) { return vector<vector<T>>(N, vector<T>(M, init)); }
template<typename T> vector<vector<vector<T>>> list3d(int N, int M, int L, T init) { return vector<vector<vector<T>>>(N, vector<vector<T>>(M, vector<T>(L, init))); }
template<typename T> vector<vector<vector<vector<T>>>> list4d(int N, int M, int L, int O, T init) { return vector<vector<vector<vector<T>>>>(N, vector<vector<vector<T>>>(M, vector<vector<T>>(L, vector<T>(O, init)))); }

template<typename T=ll> vector<T> LIST(ll N) { vector<T> A(N); rep(i, 0, N) cin >> A[i]; return A; }

void print() { cout << '\n'; }
void print(ld out) { cout << fixed << setprecision(15) << out << '\n'; }
void print(double out) { cout << fixed << setprecision(15) << out << '\n'; }
template<typename T> void print(T out) { cout << out << '\n'; }
template<typename T1, typename T2> void print(pair<T1, T2> out) { cout << out.first << ' ' << out.second << '\n'; }
template<typename T> void print(vector<T> A) { rep(i, 0, A.size()) { cout << A[i]; if (i != A.size()-1) cout << ' '; } cout << '\n'; }
template<typename T> void print(deque<T> A) { rep(i, 0, A.size()) { cout << A[i]; if (i != A.size()-1) cout << ' '; } cout << '\n'; }
template<typename T> void print(set<T> S) { vector<T> A(S.begin(), S.end()); print(A); }
#define debug(x) (cout << #x << ": ", print(x));

void Yes() { print("Yes"); }
void No() { print("No"); }
void YES() { print("YES"); }
void NO() { print("NO"); }

ll floor(ll a, ll b) { if (a < 0) { return (a-b+1) / b; } else { return a / b; } }
ll ceil(ll a, ll b) { if (a >= 0) { return (a+b-1) / b; } else { return a / b; } }
template<typename T> pll divmod(ll a, T b) { ll d = a / b; ll m = a % b; return {d, m}; }
template<typename T> bool chmax(T &x, T y) { return (y > x) ? x = y, true : false; }
template<typename T> bool chmin(T &x, T y) { return (y < x) ? x = y, true : false; }

template<typename T> T sum(vector<T> &A) { T res = 0; for (T a: A) res += a; return res; }
template<typename T> T max(vector<T> &A) { return *max_element(ALL(A)); }
template<typename T> T min(vector<T> &A) { return *min_element(ALL(A)); }

ll toint(string s) { ll res = 0; for (char c : s) { res *= 10; res += (c - '0'); } return res; }
int toint(char num) { return num - '0'; }
char tochar(int num) { return '0' + num; }
int ord(char c) { return (int)c; }
char chr(int a) { return (char)a; }

ll pow(int x, int n) { ll res = 1; rep(_, 0, n) res *= x; return res; }
ll pow(int x, ll n) { ll res = 1; rep(_, 0, n) res *= x; return res; }
ll pow(ll x, int n) { ll res = 1; rep(_, 0, n) res *= x; return res; }
ll pow(ll x, ll n) { ll res = 1; rep(_, 0, n) res *= x; return res; }
ll pow(ll x, ll n, int mod) { ll res = 1; while (n > 0) { if (n & 1) { res = (res * x) % mod; } x = (x * x) % mod; n >>= 1; } return res; }

int popcount(ll S) { return __builtin_popcountll(S); }
int bit_length(ll x) { return x != 0 ? floor(log2(x))+1 : 0; }
string bin(ll x) { string res; while (x) { if (x & 1) res += '1'; else res += '0'; x >>= 1; } reverse(ALL(res)); if(res == "") res += '0'; return res; }

ll gcd(ll a, ll b) { return __gcd(a, b); }
ll lcm(ll x, ll y) { return (x * y) / gcd(x, y); }

ld degrees(ld radians) { return radians * 180.0 / PI; }
ld radians(ld degrees) { return degrees * PI / 180.0; }

template<typename T> int bisect_left(vector<T> &A, T val) { return lower_bound(ALL(A), val) - A.begin(); }
template<typename T> int bisect_right(vector<T> &A, T val) { return upper_bound(ALL(A), val) - A.begin(); }
template<typename F> ll bisearch_min(ll mn, ll mx, const F &func) { ll ok = mx, ng = mn; while (ng+1 < ok) { ll mid = (ok+ng) / 2; if (func(mid)) ok = mid; else ng = mid; } return ok; }
template<typename F> ll bisearch_max(ll mn, ll mx, const F &func) { ll ok = mn, ng = mx; while (ok+1 < ng) { ll mid = (ok+ng) / 2; if (func(mid)) ok = mid; else ng = mid; } return ok; }

template<typename T> map<T, ll> Counter(vector<T> &A) { map<T, ll> res; for (T a : A) res[a]++; return res; }
map<char, ll> Counter(string &S) { map<char, ll> res; for (char c : S) res[c]++; return res; }
template<typename T1, typename T2> pair<vector<T1>, vector<T2>> zip(vector<pair<T1, T2>> &A) { ll N = A.size(); pair<vector<T1>, vector<T2>> res = {vector<T1>(N), vector<T2>(N)}; rep(i, N) { res.first[i] = A[i].first; res.second[i] = A[i].second; } return res; }

template<typename T> struct Accumulate {
    vector<T> acc; int N;
    Accumulate(vector<T> &A) { N = A.size(); acc = A; rep(i, N-1) acc[i+1] += acc[i]; acc.insert(acc.begin(), 0); }
    T query(int l, int r) { assert(0 <= l and l <= N and 0 <= r and r <= N); return acc[r]-acc[l]; }
    T get(int i) { return query(i, i+1); }
    T operator[](int i) { return query(i, i+1); }
};

template<int mod> struct ModInt {
    int x;
    ModInt() : x(0) {}
    ModInt(int64_t y) : x(y >= 0 ? y % mod : (mod - (-y) % mod) % mod) {}
    ModInt &operator+=(const ModInt &p) { if((x += p.x) >= mod) x -= mod; return *this; }
    ModInt &operator-=(const ModInt &p) { if((x += mod - p.x) >= mod) x -= mod; return *this; }
    ModInt &operator*=(const ModInt &p) { x = (int) (1LL * x * p.x % mod); return *this; }
    ModInt &operator/=(const ModInt &p) { *this *= p.inverse(); return *this; }
    ModInt operator-() const { return ModInt(-x); }
    ModInt operator+(const ModInt &p) const { return ModInt(*this) += p; }
    ModInt operator-(const ModInt &p) const { return ModInt(*this) -= p; }
    ModInt operator*(const ModInt &p) const { return ModInt(*this) *= p; }
    ModInt operator/(const ModInt &p) const { return ModInt(*this) /= p; }
    bool operator==(const ModInt &p) const { return x == p.x; }
    bool operator!=(const ModInt &p) const { return x != p.x; }
    bool operator<(const ModInt &p) const { return x < p.x; }
    ModInt inverse() const { int a = x, b = mod, u = 1, v = 0, t; while(b > 0) { t = a / b; swap(a -= t * b, b); swap(u -= t * v, v); } return ModInt(u); }
    ModInt pow(int64_t n) const { ModInt ret(1), mul(x); while(n > 0) { if(n & 1) ret *= mul; mul *= mul; n >>= 1; } return ret; }
    friend ostream &operator<<(ostream &os, const ModInt &p) { return os << p.x; }
    friend istream &operator>>(istream &is, ModInt &a) { int64_t t; is >> t; a = ModInt< mod >(t); return (is); }
    static int get_mod() { return mod; }
    // operator int() const { return x; }
};
using mint = ModInt<MOD>;

// 行列累乗
template<typename T>
struct MatPow {

    MatPow() {}

    vector<vector<T>> mat_dot(const vector<vector<T>> &A, const vector<vector<T>> &B) {
        int n1 = A.size();
        int n2 = A[0].size();
        int m2 = B[0].size();
        auto res = list2d(n1, m2, (T)0);
        rep(i, 0, n1) {
            rep(k, 0, n2) {
                if (A[i][k] == 0) continue;
                rep(j, 0, m2) {
                    res[i][j] += A[i][k]*B[k][j];
                }
            }
        }
        return res;
    }

    vector<vector<T>> mat_pow(vector<vector<T>> mat, ll k) {
        int n = mat.size();
        auto res = list2d(n, n, (T)0);
        rep(i, 0, n) {
            res[i][i] = 1;
        }
        while (k > 0) {
            if (k & 1) {
                res = mat_dot(res, mat);
            }
            mat = mat_dot(mat, mat);
            k >>= 1;
        }
        return res;
    }

    vector<T> solve(vector<vector<T>> mat, const vector<T> &_init, ll K) {
        int n = mat.size();
        auto init = list2d(n, 1, (T)0);
        rep(i, 0, n) init[i][0] = _init[i];
        auto res = mat_pow(mat, K);
        res = mat_dot(res, init);
        vector<T> ret(n, 0);
        rep(i, 0, n) ret[i] = res[i][0];
        return ret;
    }
};

void solve() {
    ll H, W;
    cin >> H >> W;
    swap(H, W);

    // ll N = H*W;
    // auto dp = list2d(N+1, 1LL<<W, (mint)0);
    // dp[0][0] = 1;
    // rep(i, N) {
    //     auto [h, w] = divmod(i, W);
    //     rep(bit, 1LL<<W) {
    //         dp[i+1][bit>>1] += dp[i][bit];
    //         if (!(bit>>0 & 1)) {
    //             dp[i+1][(bit>>1)|(1LL<<(W-1))] += dp[i][bit];
    //             if (w != W-1 and !(bit>>1 & 1)) {
    //                 dp[i+1][(bit>>1)|(1LL<<0)] += dp[i][bit];
    //             }
    //         }
    //     }
    // }
    // mint ans = dp[N][0];
    // print(ans);

    // ll N = W*(1LL<<W);
    // auto dp = list2d(H+1, N, (mint)0);
    // dp[0][0] = 1;
    // rep(h, H) {
    //     rep(w, W) {
    //         rep(bit, 1LL<<W) {
    //             dp[h+(w == W-1)][(w+1)%W+(bit>>1)*W] += dp[h][w+bit*W];
    //             if (!(bit>>0 & 1)) {
    //                 dp[h+(w == W-1)][(w+1)%W+((bit>>1)|(1LL<<(W-1)))*W] += dp[h][w+bit*W];
    //                 if (w != W-1 and !(bit>>1 & 1)) {
    //                     dp[h+(w == W-1)][(w+1)%W+((bit>>1)|(1LL<<0))*W] += dp[h][w+bit*W];
    //                 }
    //             }
    //         }
    //     }
    // }
    // mint ans = dp[H][0];
    // print(ans);

    ll N = 1LL<<W;
    auto mat = list2d(N, N, (mint)0);
    rep(from, N) {
        auto dp = list3d(2, W, N, (mint)0);
        dp[0][0][from] = 1;
        rep(h, 1) {
            rep(w, W) {
                rep(bit, 1LL<<W) {
                    dp[h+(w == W-1)][(w+1)%W][bit>>1] += dp[h][w][bit];
                    if (!(bit>>0 & 1)) {
                        dp[h+(w == W-1)][(w+1)%W][(bit>>1)|(1LL<<(W-1))] += dp[h][w][bit];
                        if (w != W-1 and !(bit>>1 & 1)) {
                            dp[h+(w == W-1)][(w+1)%W][(bit>>1)|(1LL<<0)] += dp[h][w][bit];
                        }
                    }
                }
            }
        }
        rep(to, N) {
            mat[to][from] = dp[1][0][to];
        }
    }
    vector<mint> init(N);
    init[0] = 1;
    MatPow<mint> mp;
    auto res = mp.solve(mat, init, H);
    print(res[0]);
}

int main() {
    cin.tie(0);
    ios::sync_with_stdio(false);
    // cout << fixed << setprecision(15);

    // single test case
    solve();

    // multi test cases
    // int T;
    // cin >> T;
    // while (T--) solve();

    return 0;
}
