/*
参考：https://blog.hamayanhamayan.com/entry/2021/06/27/005805
・区切るDP、MOD毎にまとめる、遷移が難しいDP
・難しい。MOD毎にまとめて状態を持っておく。普通にやると、途中の区間和は持てないように思えるけど、
　先頭からの区間和のmod iが同じ場所は、同じ場所同士で遷移するということをうまく使う。
　つまり各mod iで直前の総和だけ持っておけば、それが次の遷移で使える。
・sm[j][k] := 直前の状態まででj回区切った時、先頭からここまでの区間和のmod (j+1)がkの時の通り数
　みたいなものを持つ。この時ゼロ除算避けと、次に遷移する時にはその時区切った回数より1回多いものが必要になることから、
　区切った回数とmodは1個ずらして持った方がかえって混乱が少ないと、やってるうちに思うようになった。
・で、これがあればDP配列への遷移は1回で済んで、計算したDP配列を、メモ配列の次の遷移に必要な場所に配っていく感じ。
*/
...
void solve() {
    ...
    // dp[i][j] := i番目まで見て、j回区切った時の通り数
    auto dp = list2d<mint>(N+1, N+1, 0);
    // sm[j][k] := 直前の状態まででj回区切った時、先頭からここまでの区間和のmod (j+1)がkの時の通り数
    auto sm = list2d<mint>(N+1, N+1, 0);
    dp[0][0] = 1;
    sm[0][0] = 1;
    rep(i, 1, N+1) {
        rep(j, 1, N+1) {
            // j回目に区切る遷移
            dp[i][j] = sm[j-1][acc.query(0, i)%j];
        }
        rep(j, 1, N) {
            // 今の状態をメモに配る。j回区切った時、先頭からここまでの区間和のmod (j+1)が同じものをまとめる。
            sm[j][acc.query(0, i)%(j+1)] += dp[i][j];
        }

        // rep(j, N) {
        //     rep(k, i) {
        //         if (acc.query(k, i)%(j+1) == 0) {
        //             dp[i][j+1] += dp[k][j];
        //         }
        //     }
        // }
