...
・きっちり自力AC！
・座圧、in place DP
・自分より小さいor大きい値からだけ遷移させてくるとすると、
　値を座圧してBITとかに詰めておけば高速に遷移できる。
　次が山側か谷側かで遷移は真逆からになるので、状態2つ持ってやる。
・つまり座圧してBIT2本持ってin place DP。長さ1はダメなので、最後にN*2引く。
*/
...
void solve() {
    ...
    A = cp.zip(A);
    ll M = cp.size();
    BIT<mint> bit0(M), bit1(M);
    rep(i, N) {
        bit1.add(A[i], bit0.query(0, A[i])+1);
        bit0.add(A[i], bit1.query(A[i]+1, M)+1);
    }
    mint ans = bit0.query(0, M)+bit1.query(0, M)-N*2;
    print(ans);
}
