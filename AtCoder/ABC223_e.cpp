...
・自力ならず。。
・全探索、三分探索
・なんか面積がA,B,Cぴったりじゃないといけないと思って詰んだ。
　A以上とかでいいのね。確かにそうじゃなきゃそもそも長方形にできるとは限らないもんね。。
・まず、A,B,Cの立ち位置と縦横は全探索する。これで大分場合分け減らせる。
・本番中も、こんなん横並びかT字っぽいやつ以外ないんじゃないかって思ったけど実際そうだった。
　落ち着いて考えれば、面積から片側めいっぱいにして他方出すのも割り算切り上げするだけだし、そんな難しい話じゃなかった。
・T字にする方がやや厄介だけど、2つで縦目一杯使った時の横の最小は凸っぽいのでさぶたんした。
　前後調べる回数が10だとWAが1つだけあって、雑に100にしたら通った。。
*/
...
void solve() {
    ...
    rep(_, 2) {
        for (auto perm : permutations(A)) {
            ...
            // T字っぽいやつ
            auto [_, w] = trisearch_min(1, H, [&](ll h1) {
                ll h2 = H-h1;
                if (h1 == 0 or h2 == 0) return INF;
                ll w1 = ceil(a, h1);
                ll w2 = ceil(b, h2);
                return max(w1, w2);
            }, 100);
