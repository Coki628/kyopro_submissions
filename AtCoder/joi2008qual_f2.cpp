 ...
 * ・しっかり自力AC！
 * ・辺の追加毎に毎回全頂点ダイクストラはさすがに無理だろう。さあどうするか。
 * 　更新された辺と繋がる2本の頂点からだけ更新。これだとWA。
 * 　さらにその2頂点を経由するルートを、全頂点ペアについてchminする。これでAC。
 * ・昔自分で解いたやつ見たら、辺の追加の時には再計算しなくて、
 * 　照会クエリが来た時だけその求めたい頂点についてのダイクストラしてた。確かにそれもあるな。
 */
...
int main() {
    ...
    rep(i, K) {
        ...
        if (op == 0) {
            ...
            nodes[u].pb({v, c});
            nodes[v].pb({u, c});
            G[u] = dijkstra(nodes, u);
            G[v] = dijkstra(nodes, v);
            rep(i, N) {
                rep(j, N) {
                    chmin(G[i][j], G[u][i]+G[u][j]);
                    chmin(G[i][j], G[v][i]+G[v][j]);
                }
            }
        }
    }
