/*
参考：https://img.atcoder.jp/abc144/editorial.pdf
・ABC高難度埋め
・メモ化再帰、DAGのDP、期待値DP
・全ての辺について、1本外すのを試すとE^2になり間に合わない。
　ここで頂点に注目して、各頂点から一番コストの高い遷移先への辺だけを試すようにする。
　これだと計算量がV*Eとなり間に合う。
・最初この方針を聞いた時、これだと一番コスト高の遷移先が複数あった時、
　そのうち1つを試すだけは不十分なのではと考えた。
　でも別の見方で、「辺を削ることで、その頂点に集めてくる最終的な合計コストを減らせるか？」
　っていう考え方にすると、同じコストの遷移先ならどれを削っても結果は同じなはずで、
　最大値のどれか1つを削る、っていうことに納得ができた。
・さて、後は実装なんだけど、最初に基準となるDPをやっておいて、
　辺を1つ削った状態で同じDPを試して、そのうちのdp[0]の最小値を取る、
　って感じでいいんだけど、計算量問題ないくらいのはずなのになぜかTLEが取れない。
　結構原因見つけるのに時間かかった。
・結果的にはsetのイテレーションが遅かったぽい。辺を削る時に毎回線形かかると気持ち悪いので、
　logで済むように隣接リストをsetのやつにしてたんだけど、これがいけなかった。
　いつものvectorのやつにしたら普通に通ってAC0.54秒。(これでも結構遅いけど。。)
・一番深い所では特にinsertやeraseみたいなlogかかる操作はしてないのに遅かった。
　今ちゃんと計算量書いて気付いたけど、600*約18万=約1億ちょいあったんだね。。
　1000万くらいの気がしてたけど、これならsetが怪しいのもまあ納得か。。
　それにしても線形の操作でもsetかvectorかでこんなに差が出るんだね。覚えとこう。。
*/
...
void solve() {
    ...
    rep(i, N) {
        if (memo[i] != -1 and nodes[i].size() >= 2) {
            ld mx = -1;
            ll k = -1, x = -1;
            rep(j, nodes[i].size()) {
                if (memo[nodes[i][j]] > mx) {
                    mx = memo[nodes[i][j]];
                    k = j;
                    x = nodes[i][j];
                }
            }
            nodes[i].erase(nodes[i].begin()+k);
            vector<ld> memo2(N, -1);
            auto dfs2 = [&](auto&& f, ll u) -> ld {
