/*
参考：https://www.youtube.com/watch?v=UHfTqvaD0pk
・AGC青diff以下残埋め
・自力ならず。やはりAGCの考察はむずいね。
・グリッドBFS、多点スタート
・気付かなければならない重要な事実が1つある。それはK箇所のマスを開いた後、K個先まで進めると考えると、
　塞がったマスというのはあってないようなものだと言うこと。つまり、自分が次に進みたいKマスについて開いて、
　その方向に進むとすれば、K個先のいずれのマスにも到達できるからだ。
　気付いてしまえばびっくりするくらい当たり前なんだけど、この問題で先にK個進んでその後K箇所開くって書き方をされてるのもあり、
　なんとなく(進行方向に対して)横に1個ずつ的なイメージでしかマスを解放できないように感じてしまった。
　確かに自分の進みたい方向に縦に連続K個マスを開けて道を作れば、1回の操作でどんな方向にもK個先に進める。
・それが分かれば、最初の1回とその先で1回毎に進める分とで、2回BFSすればいいだけ。
　多点スタートとかあるけど、それは別に本質じゃない。
*/
...
void solve() {
    ...
    auto res2 = bfs(grid2, src);
    ll ans = INF;
    rep(i, H) {
        chmin(ans, ceil(res2[i][0], K));
        chmin(ans, ceil(res2[i][W-1], K));
    }
    rep(j, W) {
        chmin(ans, ceil(res2[0][j], K));
        chmin(ans, ceil(res2[H-1][j], K));
    }
    print(ans+1);
}
