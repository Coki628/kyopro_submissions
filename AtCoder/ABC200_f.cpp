/*
参考：https://atcoder.jp/contests/abc200/editorial
・ABC高難度埋め
・むずい。でもやっぱりためになる。
・主客転倒、期待値、MODの切り上げ除算
・ある文字列Tの操作回数は、ceil(異なる0,1が隣り合う箇所の数, 2) と表せる。
　これは分かったんだけど、この個数を高速に求める方法は分からず。。
・まず、お決まりの主客転倒で1箇所毎の値を求めていって、最後に通り数を掛ける方針になる。
　なんだけど、この1箇所毎の値は状況によって異なってしまうし、まとめられないように思えた。
・やり方があった。期待値にする。元々0,1の場合は一意だし、?になってる所も、
　それぞれになる期待値を1/2として、各箇所の期待値を求めていける。
　この期待値を全部足せば、文字列T全体での期待値が求まる。こうなればもう通り数掛けるだけ。
　K個くっつける扱いがどうかと思ったが、同じ文字列がK繋がってるので、
　それぞれの隣接箇所はK回ずつ出てくる。最後と最初の所はK-1回。これを掛ける。
・さてここまでで隣り合う個数は求まったが、これを割る2切り上げしないといけない。
　MOD数え上げなので、普通にやると切り上げはできない。これもやり方があった。
　奇数だった所を全て+1するとすれば、割り切れる除算で同じ答えぴったりになるので、
　MODの世界ではこれで同じ値を求めることができる。
　この奇数になる通り数は、先頭と末尾の0,1が異なるかどうかで決まるので、
　先程同様に隣接箇所での期待値を求めて、通り数全体を掛ければいい。
　この数だけ足してから割る2すれば、求めていた答えを得られる。
*/
...
void solve() {
    ...
    ll q = count(ALL(S), '?');
    mint inv2 = mint(1)/2;
    // 文字a,bが隣り合った時、異なる0,1となる期待値(ここでは確率と同値)を返す
    auto check = [&](char a, char b) -> mint {
        if (a == '?' or b == '?') return inv2;
        elif (a == b) return 0;
        else return 1;
    };

    // SをK個つなげたT1つについての、異なる0,1が隣り合う箇所の数の期待値を求める
    mint exp = 0;
    rep(i, N-1) {
        // TにはS[i]とS[i+1]が隣り合う箇所がK個ある
        exp += check(S[i], S[i+1])*K;
    }
    // S[N-1]とS[0]が隣り合う箇所がK-1個ある
    exp += check(S[N-1], S[0])*(K-1);

    // このようなTが2^(K*q)通りある
    mint sm = exp*pow(2, K*q, MOD);

    // 異なる0,1が隣り合う箇所の数が奇数になるTの通り数は、
    // そうなる確率(先頭と末尾が異なるか)に、全通りを掛ける
    mint oddcnt = check(S[N-1], S[0])*pow(2, K*q, MOD);

    // MOD除算はそのままではceilできないので、奇数になった箇所を全て+1して調整
    sm += oddcnt;

    // この時点で和の中に奇数のものがなくなっているので、普通に割る2で答えになる
    mint ans = sm*inv2;
    print(ans);
}
