/**
 * ・なんとか自力AC！
 * ・貪欲でWAになって、最後に操作で使った文字を状態に持ってDPした。
 * 　結局貪欲でいけたらしい…。
 */
...
void solve() {
    ...
    S += '*';
    vector<ll> R(26, N);
    auto dp = list2d(26, N+1, -INF);
    rep(c, 26) dp[c][N] = 0;
    rep(l, N-1, -1, -1) {
        rep(c, 26) {
            chmax(dp[c][l], dp[c][l+1]);
            if (l+1 < N and S[l] != 'a'+c and S[l] == S[l+1] and S[l+1] != S[l+2]) {
                chmax(dp[S[l]-'a'][l], dp[c][l+1] + (R[c]-l) - (acc[S[l]-'a'][R[c]]-acc[S[l]-'a'][l]) + (N-R[c]));
                R[S[l]-'a'] = l;
            }
        }
    }
    ll ans = 0;
    rep(c, 26) chmax(ans, dp[c][0]);
    print(ans);
}
