...
・E8さん典型90問、残りの高難度埋め
・これはmap使わないで色々頑張った方。
・まず最初に遷移に使うビット集合だけを集める。これは実際の遷移を3行目くらいまでシミュすると全部取れるみたい。
　この時に、集めてる全体の集合allsetと、その都度の遷移に使うcurは分けて管理しないと無理で、
　毎回allsetの方を遷移に使うとめちゃくちゃ遅かった。
・で、必要なビットが集まったら、メイン処理でlogを乗せたくないので座圧する。
　遷移先ビットについても、座圧後の番号が分かるようにしておく。
・ここまで作れれば、後は座圧後の番号で普通にDPしていけばOK。AC4.16秒でmapのよりはだいぶ速くなった。
・ちなみに全体集合の要素数が最大ケースっぽいやつ(24*24の障害物なし)で69万個くらいだったから、
　最後のDPパートの計算量が24*24*69万=4億くらいで、確かにこれはlog乗せたくない。
　前半のビット集めるパートは、最大を回すのが1回だとして24*69万=1660万くらいで、
　これは確かにsetのlog乗ったらきつい。逆によく間に合ったなって感じか。
　あと、setの変わりにunordered_set使ったり、速いhashmap使ったりも試してみたけど、
　どっちも7秒くらいかかったりで結局普通のsetが今回は一番速かった。
*/
...
template<typename key, typename val> val sum(const unordered_map<key, val> &mp) { val res = 0; for (auto [k, v] : mp) res += v; return res; }
...
void solve() {
    ...
    rep(i, N) {
        ...
        set<int> nxt;
        for (auto bit : cur) {
            nxt.insert(msk&(bit<<1));
            // 左
            if (w > 0 and bit>>0 & 1) continue;
            // 右上
            if (w < W-1 and bit>>(W-2) & 1) continue;
            // 上
            if (bit>>(W-1) & 1) continue;
            // 左上
            if (w > 0 and bit>>W & 1) continue;
            nxt.insert(msk&(bit<<1)|1);
        }
        for (auto bit : nxt) allset.insert(bit);
        swap(nxt, cur);
    }
    vector<int> need;
    for (auto bit : allset) need.eb(bit);
    Compress<int> cp(need);
    ll M = cp.size();
    vector<pll> to(M);
    rep(i, M) {
        int bit = need[i];
        to[i].first = cp[msk&(bit<<1)];
        to[i].second = cp[msk&(bit<<1)|1];
    }

    auto dp = list2d(2, M, (mint)0);
    dp[0][0] = 1;
    rep(i, N) {
        auto [h, w] = idtogrid(i, W);
        rep(j, M) {
            int bit = need[j];
            dp[(i+1)%2][to[j].first] += dp[i%2][j];
