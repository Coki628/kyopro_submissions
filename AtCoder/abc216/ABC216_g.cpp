/*
・自力AC？本番中は見ることもできず。
　今やったらすんなり解けた。ネタバレはあったけど、これは自力で行けたんじゃなかろうか。
・区間スケジューリング、BITにぶたん
・右端ソートして区間スケジューリングぽいものをやる。1が足りなければ右から貪欲に敷き詰める。
　場所はBITにぶたんで探す。手持ちのBITにぶたんが最初の1を探すことはできても、
　最初の0の位置を探すことができないので、01反転させた状態で考えた。
　具体的には、最初に全部敷き詰めておいて、そこから必要なだけ抜いていく感じ。
*/
...
void solve() {
    ...
    rep(i, N) bit.add(i, 1);
    for (auto [r, l, x] : RLX) {
        while (bit.query(l, r) > x) {
            ll i = bit.bisearch_back(l, r-1, 1);
            bit.add(i, -1);
        }
    }
