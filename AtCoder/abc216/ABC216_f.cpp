/*
・自力AC？終了後ネタバレはしてたけど、まあ近い方針で解こうとはしてた。時間なかった。。
・ソートして部分和DP
・Aの昇順にソートしたものでBの部分和DPをやると、
　その要素B[i]を追加する時、常にそのA[i]がその時のmaxとなる。
　つまりその...B[i]という集合が全て答えに使える。
　しかもここでいつも新しいA[i]を入れることになるので重複もない。
・ただし条件として、B[i]を足した時点でA[i]以下である必要はある。
　普通にDPしながら、この条件を満たすものを答えに足していけばOK。
*/
...
void solve() {
    ...
    const ll MX = 5000;
    auto dp = list2d<mint>(N+1, MX+1, 0);
    dp[0][0] = 1;
    mint ans = 0;
    rep(i, N) {
        rep(j, MX+1) {
            ...
            // このA[i]をmaxとして最後に加えたものが、ここに至るまでの部分集合全てで答えに使える
            if (j+B[i] <= A[i]) {
                ans += dp[i][j];
            }
        }
