...
・自力ならず。。悔しい。
・キューで賢くシミュレーション
・使えるようになったものからキューに詰めていく。
　考えがまとまればそれだけなんだよね。。
・もうちょっと説明すると、最初はどこから始めてもいいから、
　とりあえずM箇所全部チェックして、次に使えそうになったところはキューに詰める。
　以降は順番に取り出して、同じことしていけばいいだけ。
　1回目に出現したところでuseにそのindexを保持して、
　2回目に出現したところで、1,2回目両方のindexをキューに。
　リストが空になったところはdoneをtrueに。
　キューが空になるまでやって、全てのリストが空になっていればOK。
*/
