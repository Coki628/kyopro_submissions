/*
・PAST未ACのupsolve
・自力ならず。これ貪欲しか頭になかったのは反省。。
　解説でDPだよって話ちら見して、そういう目で見たら普通に解けたわ。。
・部分和DPっぽいことをやる。
　dp[i][j][k] := i番目まで見て、そこまでの総和がjで、直前の値がkの時の最小値
　みたいにすれば欲しい情報は全て揃う。
　これだと添字だけでN^4=1億、遷移にもNで計100億くらいの計算量になるので、
　厳しいように思えるが、ダメだったら高速化考えよう、ってとりあえず投げると通る。
　AC0.25秒で余裕もある。多分、枝刈りが効いて定数倍高速化してるんだと思うけど、
　さすがに/100もしてないと思うしなぁ。
　でもこういう感じの添字多めのDPは確かに定数倍効きやすいイメージはある。
・追記。これ違ったわ。制約が総和の上限100だった。各要素100ずつじゃない。
　それなら遷移も合わせて全部でN^4でそれはまあ通るよねって感じだ。
*/
...
void solve() {
    ...
    auto dp = list3d(N, sm+1, MX+1, (ld)INF);
    dp[0][0][0] = 0;
    rep(i, N-1) {
        rep(j, sm+1) {
            rep(k, MX+1) {
                ...
                if (i == N-1) {
                    chmin(dp[i+1][j][0], dp[i][j][k]+hypot(1, k));
                } else {
                    rep(l, MX+1) {
                        if (j+l > sm) break;
                        chmin(dp[i+1][j+l][l], dp[i][j][k]+hypot(1, abs(k-l)));
                    }
                }
            }
        }
    }
    auto ans = dp[N-1][sm][0];
    print(ans);
}
