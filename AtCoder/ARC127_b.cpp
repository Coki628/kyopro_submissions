/*
参考：https://twitter.com/SSRS_cp/status/1441749369137418250
　　　https://atcoder.jp/contests/arc127/editorial
・自力ならず。。
・被りなく作れるかはひとまず置いておいて、作る3N個の文字列の辞書順最大のものを
　最小化するにはどうするか考えてみる。するとまず確実に言えるのは、先頭が2になるものがN個必要ということ。
　そしてその中で理想を言えば、2桁目以降は0000..からN番目までとしたい。
　これで0,1から始まるものも重複なくN個作れるか。結論としては、できる。
　全部の値について、1ずらすか2ずらすかすると、個々の文字列の相対的な位置関係は変わらないので、
　元にした文字列が異なる以上、異なるものを作れる。
　また、このように各値がローテーションすると、0が1と2になり、1が0と2になり、2が0と1になるので、
　各値の出現回数もちょうど均等にできる。
・よくできてるなぁと思うけど、自分で導ける気がしないなぁ。。
*/
