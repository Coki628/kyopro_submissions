...
・なんとか自力AC！！
・連立方程式、二部グラフ
・公式解はもっと鮮やかに解いてたけど、なんとか通せて良かった。
・考察過程は長い。まず自明っぽい所を詰めようと思う。式がM本で変数がN個ある連立方程式みたいに見れそう、となり、
　であれば式がN本以上あれば一意なので成り立つかどうかで0or1、木の場合だけ分けて考える、かと思ったけど、少し違った。
・頂点4つ辺4本のサイクルで試すと、なんと一意にならず、何やっても合う。。
　なんとなく式に思いを馳せると、偶数個の式を変形してもおそらく全部0になって無だと気付く。
　これが奇数個の式なら交互に足し引きしていけば2a残るので値を一箇所特定できる。
・ここから、奇数長サイクルがある=二部グラフではない場合は答えが一意になり、そうでない時は値が自由に動く、と予測。
　で、取りうる範囲は二部の片方をギリギリまで下げて、他方の最小を見てどこまで動かせるか。
　この範囲を決める考察はARC115_bから持ってきた。
・これで考察は概ね合ってたぽくて、最終的にこの方針で粘って通したんだけどまあ大変だった。
　二部グラフになってる場合でもそもそも値が合わない所があったらNGとか、
　奇数長サイクルを拾いたいのに戻り時に頂点0まで取っちゃってたり、
　矛盾チェック時に一意になった値の最小が0以下になってないかチェックし忘れてたり。。
*/
...
void solve() {
    ...
    auto dfs = [&](auto&& f, ll u, ll p) -> bool {
        ...
        P[u] = p;
        for (ll v : nodes[u]) {
            if (!f(f, v, 1-p)) return false;
        }
        return true;
    };
    if (dfs(dfs, 0, 0)) {
        // 適当に一点を決めてまず辻褄が合うかチェック
        vector<ll> A(N);
        vector<bool> visited(N);
        auto dfs2 = [&](auto&& f, ll u, ll cur) -> bool {
            if (visited[u]) {
                return cur == A[u];
            }
            visited[u] = true;
            A[u] = cur;
            for (ll v : nodes[u]) {
                if (!f(f, v, C[{u, v}]-cur)) return false;
            }
            return true;
        };
        bool ok = dfs2(dfs2, 0, 1);
        if (not ok) {
            print(0);
            return;
        }
        ...
        auto dfs3 = [&](auto&& f, ll u, ll cur) -> void {
            if (visited[u]) return;
            visited[u] = true;
            B[P[u]].eb(cur);
            for (ll v : nodes[u]) {
        ...
        dfs3(dfs3, 0, 1);
        // B[0]側を正数にするために必要な調整量
        ll need = 1LL - min(B[0]);
        for (ll &a : B[1]) {
            a -= need;
        }
        ll ans = max(min(B[1]), 0LL);
        print(ans);
    } else {
        ll a = 0;
        bool p2 = true;
        vector<bool> visited(N);
        auto dfs2 = [&](auto&& f, ll u, ll depth) -> bool {
            if (visited[u]) {
                return u == x and depth%2 == 1;
            }
            visited[u] = true;
            for (ll v : nodes[u]) {
                if (f(f, v, depth+1)) {
        ...
        vector<ll> A(N);
        visited.assign(N, false);
        auto dfs3 = [&](auto&& f, ll u, ll cur) -> bool {
            if (visited[u]) {
                return cur == A[u];
            }
            visited[u] = true;
        ...
        ll ans = dfs3(dfs3, x, a) and min(A) > 0;
        print(ans);
    }
}
