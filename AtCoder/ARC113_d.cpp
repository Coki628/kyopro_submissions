/**
 * 参考：https://atcoder.jp/contests/arc113/editorial
 * 　　　https://www.youtube.com/watch?v=i8FOx68E2g4&feature=youtu.be
 * ・自力ならず。。
 * ・数え上げ、場合分け、重複削除
 * ・なんか取っ掛かりも掴めないで終わった。数え上げむずいや。
 * ・とりあえず、行列どっちか1の時は、1の方は選ぶ余地がなくて、1でない方を自由に選ぶ感じ。
 * ・そうでない時、基本的にはN側のmax<=M側のminを満たすようなものを数える。
 * 　これは境界を固定すると数えられるが、それだけだと重複が生じる。
 * 　そこでN側はmaxの値を境界に固定すると、うまくいく。
 * 　これをやるには、境界^N - (境界-1)^N とすれば、最大値が境界ちょうどのものだけになる。
 * ・これ水diffだもんなぁ。全人類数え上げ強い。。
 */
...
void solve() {
    ...
    if (N == 1) print(pow(K, M, MOD));
    elif (M == 1) print(pow(K, N, MOD));
    else {
        mint ans = 0;
        rep(i, 1, K+1) {
