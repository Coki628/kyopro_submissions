...
・きっちり自力AC！
・二分探索、オーバーフロー対策
・数列全体でにぶたんして、1つ戻して途中どこかは全探索で見る。
　にぶたん中に数値が大きくなる可能性があるので、オーバーフロー対策処理を入れておく。
・これでいいと思ったら1WA。INFが小さかった。。
・こんなにぶたん+αでもCになるかぁとか思ってたら、後でTL見たらみんな割り算してた。確かに。。
　この割り算で済むやつにぶたんしちゃうの、定期的にやっちゃうんだよね。。
*/
...
void solve() {
    ...
    ll sm = sum(A);
    ll res = bisearch_min(0, INF, [&](ll m) {
        if (mul_overflow(sm, m)) return true;
        ll res = sm*m;
        return res > X;
    });
    ll sm2 = sm*(res-1);
