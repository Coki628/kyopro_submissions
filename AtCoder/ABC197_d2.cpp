...
ld degrees(ld radian) { return radian * 180.0 / PI; }
ld radians(ld degree) { return degree * PI / 180.0; }
...
void solve() {
    ...
    ld center_x = (x1+x2)/2.0;
    ld center_y = (y1+y2)/2.0;
    ld ansx = (x1-center_x)*cos(radians(360.0/N)) - (y1-center_y)*sin(radians(360.0/N)) + center_x;
    ld ansy = (x1-center_x)*sin(radians(360.0/N)) + (y1-center_y)*cos(radians(360.0/N)) + center_y;
    print(mkp(ansx, ansy));
}
