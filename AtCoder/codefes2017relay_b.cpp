/*
・その他埋めAC数上げ
・自力AC！！過去に何回も詰まってスキップしてたやつだから嬉しい。
・完全N分木、LCA応用
・まずN=1の自明場合分けはしておく。これだと木が深くなり過ぎるので。
・で、とりあえずLCAっぽく親に遡っていけないか考えてみる。
　すると、各階層で一番左の要素の頂点番号を前もって調べておけば、
　クエリで受けた頂点がその階層で何番目なのかが分かり、
　これを使うと親が階層で何番目なのかも分かるので、親の頂点番号も分かる、となる。
・親に遡る手段さえ整えば、後はLCAの要領で、深い頂点を同じ深さまで持ってきて、
　そこからは同時に動かして一致するまでやる、とすればOK。
・公式解説見たら、親の頂点、floor(v+N-2, N)で常に求まるらしい。まじか。。
*/
...
void solve() {
    ...
    auto get_parent = [&](ll u, ll depth) {
        assert(depth > 0);
        // 頂点uがその階層で何番目か(0-indexed)
        ll i = u-head[depth];
        // 親が階層で何番目か(0-indexed)
        ll j = i/N;
        return head[depth-1]+j;
    };
    ...
    rep(_, Q) {
        ...
        ll udepth = get_depth(u);
        ll vdepth = get_depth(v);
        if (udepth > vdepth) {
            swap(udepth, vdepth);
            swap(u, v);
        }
        while (udepth < vdepth) {
            v = get_parent(v, get_depth(v));
            vdepth--;
        }
        while (u != v) {
            u = get_parent(u, get_depth(u));
            v = get_parent(v, get_depth(v));
        }
        print(u);
    }
