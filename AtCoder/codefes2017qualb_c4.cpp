/*
・E8さん精選100+50問埋め：二部グラフ
・自力AC！まあ二部グラフっていうヒントあってのものぽいけど。。
　それでも蟻本演習の時は解けなかったみたいだから嬉しい。
・二部グラフ判定
・結局、奇数長の閉路がある(二部グラフではない)と、どうとでも繋げてしまうので、
　完全グラフ-今ある辺が答え、逆に二部グラフだと、自分と同じ側には
　どう頑張っても張れないので、完全二部グラフ-今ある辺が答え。
*/
...
void solve() {
    ...
    vector<ll> memo(N, -1);
    auto rec = [&](auto&& f, ll u, bool pt) -> bool {
        if (memo[u] != -1) {
            return memo[u] == pt;
        }
        memo[u] = pt;
