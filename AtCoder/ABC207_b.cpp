/*
・終了後、落ち着いて考えて自力AC。Bこれむずいよ。
・場合分け、二分探索
・操作0はありえなくて、まず1回必要。
　その後は、やる度に答えに使う値が単調減少か単調増加になる(はず…)。
　なので1回やった時点でOKなら即OKとして、
　単調増加ならこれ以後OKになることはないし、
　単調減少ならどこかでOKになる境界位置がある。
・にぶたんの右端INFにしてて(多分)オーバーフローで1WA。
　それ直してもWAで飛ばしたんだけど、
　その時一緒に追加した b > c なら即NGみたいなIF文消したら通った。。
　確かにaの値によってはbのが大きくても単調減少に(多分)なるんだよね。
*/
...
void solve() {
    ...
    const ll MX = 100000000000000;
    ll res = bisearch_min(-1, MX, [&](ll m) {
        return cyan+b*m <= (red+c*m)*d;
    });
    if (res == MX) {
        print(-1);
    } else {
