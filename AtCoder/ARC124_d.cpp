/*
参考：https://twitter.com/laycrs/status/1419297239017484288
　　　https://twitter.com/terry_u16/status/1419313356272324614
・自力ならず。
・任意位置のスワップ、サイクル数
・スワップ位置に制限がないと、各サイクル(連結成分)毎に要素数-1回の操作で完了、
　ここをベースに考えていく。
・片側に片寄った成分の場合、そこ単体で移動を完了できないので、
　他方から1回借りてくるみたいな操作が要る。この時、余分に2回操作が必要になる。
・ただし、両側にこういった状況がある場合、この借りてくるパートは反対側にとっても必要な操作なので、
　多い方のついでで少ない方も完了できる。
　しばらくうまくイメージできなかったけど、2個2個くらいの少ない連結成分で
　手元で試してみると、確かに同時に済ませられることが分かる。(自分ノートも参照)
*/
