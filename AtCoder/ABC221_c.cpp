...
・きっちり自力AC！
・ビット全探索と順列全探索
・愚直に分け方全通りと並び全通りをやっても間に合う。何とか。。(AC1.41秒)
・後でTL見てたら、並びは全部やらなくても、数字降順が最善とあった。確かに。。頭ついてなかった。
*/
...
void solve() {
    ...
    rep(bit, 1<<N) {
        ...
        for (auto &perm1 : permutations(A)) {
            if (perm1[0] == 0) continue;
            string s = join(perm1);
            for (auto &perm2 : permutations(B)) {
                if (perm2[0] == 0) continue;
                string t = join(perm2);
                ll a = toint(s);
                ll b = toint(t);
                chmax(ans, a*b);
