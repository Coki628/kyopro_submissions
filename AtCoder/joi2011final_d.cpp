/*
参考：https://physics0523.hatenablog.com/entry/2019/04/06/072342
・JOI埋め
・自力ならず。。方針絶対近いはずなのにって思ったけど詰められず。
・XY独立、中央値
・こんなん、おそらくXY独立でソートして中央値だろう、って判断は間違ってなかった。
　けど、要素が偶数個で中央値が複数ある時(この問題では2次元なので長方形領域になる)、
　その中の適当にどれかではダメで、4点のどれかを全探索、だった。
　一応解説見て、あんまりスッキリはしてないんだけど、とりあえず図を見て、
　その長方形領域の四隅のどれかが最適になるってのは分かった。。
*/
...
void solve() {
    ...
    vector<pll> bases;
    if (N%2 == 1) {
        bases.eb(X[N/2], Y[N/2]);
    } else {
        bases.eb(X[N/2-1], Y[N/2-1]);
        bases.eb(X[N/2-1], Y[N/2]);
        bases.eb(X[N/2], Y[N/2-1]);
        bases.eb(X[N/2], Y[N/2]);
    }
    ll ans1 = INF;
    pll ans2;
