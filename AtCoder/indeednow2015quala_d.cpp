...
・自力AC！やったね。
・両側探索、BFS
・かなり久しぶりに使った手法だけど、思い出せてよかった。
　手数24の全探索は無理だけど、12なら何とかなるので、
　始点と終点両方から12手まで調べて、合流してる場所を全部チェックする。
・制約4秒だからいいけど、AC2.97秒でまあかかってる。雑にグリッドのままmapに持ってるしね。
　1次元にエンコードとかすればもう少し速くなるんだろうけど。
・両側探索じゃなくてA*(エースター)でも良くて、そっちのが速いらしい。
　これまた蟻本以来みたいな論点だけど。
*/
...
void solve() {
    ...
    while (que.size()) {
        ...
        for (auto [di, dj] : directions) {
            ...
            if (not memo.count(cur)) {
                memo[cur] = dist+1;
                if (memo[cur] <= 12) que.push({cur, {ni, nj}});
            }
            swap(cur[i][j], cur[ni][nj]);
        }
    }
    ...
    map<vvi, ll> memo2;
    my_queue<pair<vvi, pii>> que2;
    que2.push({goal, {H-1, W-1}});
    memo2[goal] = 0;
    while (que2.size()) {
        auto [cur, ij] = que2.pop();
        ll dist = memo2[cur];
        if (cur == grid) break;
        auto [i, j] = ij;
        for (auto [di, dj] : directions) {
            ...
            if (ni < 0 or nj < 0 or ni >= H or nj >= W) continue;
            swap(cur[i][j], cur[ni][nj]);
            if (not memo2.count(cur)) {
                memo2[cur] = dist+1;
                if (memo2[cur] <= 12) que2.push({cur, {ni, nj}});
            }
            swap(cur[i][j], cur[ni][nj]);
        }
    }
    ...
    ll ans = INF;
    for (auto& [cur, dist] : memo) {
        if (memo2.count(cur)) {
            chmin(ans, dist+memo2[cur]);
        }
    }
    print(ans);
}
