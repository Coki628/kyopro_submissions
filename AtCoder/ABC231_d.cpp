/*
・自力AC！けど今日はここのペナがなければ、、って感じ。
・無向グラフの閉路検出
・次数3以上は自明にNGだとして、1ペナしたあと、閉路あってもダメじゃんとなる。
　で、ここで有向グラフの閉路チェック用DFSを持ってきてしまい迷走。
　気付くのに2ペナかかって、これは良くなかった。
　無向グラフだとUFでつないでくだけでチェックできる。
*/
...
void solve() {
    ...
    bool ok = true;
    rep(i, N) {
        if (nodes[i].size() > 2 or not uf.is_tree(i)) {
            ok = false;
            break;
        }
    }
