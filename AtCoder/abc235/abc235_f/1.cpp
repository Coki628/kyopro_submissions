...
・状態の持ち方はまあまあ面倒で、
　dp[i][j][k][l] := i桁見て、使った桁集合j、smallerフラグk=0/1、leading zero終わったかフラグl=0/1
　ってやった。なんかもっと省略できそう。
　今ぱっと思ったのは、leading zeroはビットが1つでも立ってるかで判定できそうだね。
...
const int L = 1LL<<10;
mint dp0[10007][L][2][2], dp1[10007][L][2][2];
            ...
            rep(k, 2) {
                rep(l, 2) {
                    rep(d, 10) {
                        ...
                        if (d > 0) nxl = 1;
                        if (nxl == 0) {
                            dp0[i+1][j][nxk][nxl] += dp0[i][j][k][l];
                            dp1[i+1][j][nxk][nxl] += dp1[i][j][k][l]*10+dp0[i][j][k][l]*d;
                        } else {
                            dp0[i+1][j|1LL<<d][nxk][nxl] += dp0[i][j][k][l];
                            dp1[i+1][j|1LL<<d][nxk][nxl] += dp1[i][j][k][l]*10+dp0[i][j][k][l]*d;
                        }
                    }
                }
            }
