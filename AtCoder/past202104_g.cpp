/*
・終了後なんとか自力AC。長かった。。
・辺ベースで考える。各辺1回使えば十分で、使えるかどうかのチェックがしたいと考えると、
　優先度付きキューにコスト順に辺を詰めて、その日使えるコストまで使う、
　ここで増やせた頂点に紐づく辺を次回用に新たに追加、
　プリキューなのでここは動的にコスト順になる、
　使った辺は破棄すれば何回も通ることもない。と全て整い、無事ACした。
　でも時間0.49秒もかかってるし、解法もこの位置の問題にしては手が込みすぎなので、
　絶対もっと簡単に済む想定解があるんだと思う。。。
　→なかった。想定解がこれで、強い人はセグ木にぶたんとか駆使して通してた…。
*/
...
void solve() {
    ...
    priority_queue<P, vector<P>, greater<P>> edges;
    vvpll nodes(N);
    rep(i, M) {
        ll u, v, c;
        cin >> u >> v >> c;
    ...
    rep(_, Q) {
        ...
        set<ll> nxse;
        while (not edges.empty() and get<0>(edges.top()) <= x) {
            auto [c, u, v] = edges.top(); edges.pop();
            if (not se.count(u)) {
                nxse.insert(u);
            }
            if (not se.count(v)) {
