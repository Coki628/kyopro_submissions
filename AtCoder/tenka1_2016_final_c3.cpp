 ...
 * ・別のロリハの問題見てた時に「もしかして文字列長毎に分ける意味なくない？」
 * 　って思ってやってみた。これでもやっぱりちゃんとAC。ちょっと速くなって1.16秒。
 */
...
int main() {
    ...
    unordered_map<uint64_t, ll> p_hashs(L+1);
    rep(i, 0, M) {
        auto p_hash_table = rh.build(P[i]);
        p_hashs[rh.query(p_hash_table, 0, P[i].size())] = W[i];
    }
    ...
    rep(i, 0, N) {
        ...
        rep(j, 1, L+1) {
            ...
            if (p_hashs.count(s_hash)) {
                chmax(dp[i+j], dp[i]+p_hashs[s_hash]);
            }
        }
    }
    ll ans = dp[N];
