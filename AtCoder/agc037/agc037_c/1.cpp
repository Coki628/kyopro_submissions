...
・AGC青diff以下残埋め
・自力AC！やったね。
・逆操作、indexセグ木、式変形、シミュレーション
・そのままだとどこから操作するかが見えづらいが、逆操作にすると、
　自身が左右より大きくないと操作できないので、かなり考えうる範囲を狭められる。
　ここで、常に値が最大のものから優先して操作していけば、自身が左右以上であることが保証できるのでそうする。
　やってて話が合わなくなったらNG判定する。これを愚直シミュするとTLE。何を速くできるか考ええる。
・ある位置の値が、極端に左右より大きく目標値が遠い場合、かなり多い回数を費やさないと終わらないことが分かる。
　この時、この値が左右と同値以下にならない限り、左右の値が変更されることはないので、そうなる所までは一気にやっていい。
　思わずにぶたんしそうになってけどこれは割り算でできる。左右の値をa,b、A[i]にするまでに必要な値をneed、
　操作回数をcntとすると、cnt*(a+b) <= need となる最大のcntまでは操作していい。
　これは式変形すると cnt <= need/(a+b) となり、正数の範囲であれば cnt = floor(need, a+b) と表せる。
　これで操作回数を一気に進めるようにすると無事AC。
*/
...
void solve() {
    ...
    auto seg = get_segment_tree_index([](ll a, ll b) { return max(a, b); }, -INF);
    seg.build(B);
    ll ans = 0, ok = 0;
    while (1) {
        auto [mx, i] = seg.query(0, N);
        if (mx == A[i]) {
            ok++;
            seg.update(i, 0);
            if (ok == N) break;
