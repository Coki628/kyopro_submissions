/**
 * ・E8さん精選100+50問埋め：半分全列挙
 * ・きっちり自力AC！
 * ・半分全列挙、二分探索
 * ・条件がK個ちょうど使いたいなので、隣接リストに個数毎で総和を入れて管理した。
 * 　これをソートしておけば、各個数毎ににぶたんで条件に合う数を確認できる。
 * ・2^20=約100万にlogが乗るけど、余裕持ってAC0.28秒。(制約3秒)
 */
...
void solve() {
    ...
    rep(bit, 1<<N2) {
        ...
        adj2[popcount(bit)].pb(sm);
    }

    rep(i, N2) sort(ALL(adj2[i]));
    ll ans = 0;
    rep(i, N1+1) {
        if (K-i > N2) continue;
        if (i > K) break;
        for (ll x : adj1[i]) {
            ll l = bisect_left(adj2[K-i], L-x);
            ll r = bisect_right(adj2[K-i], R-x);
            ans += r-l;
        }
    }
    print(ans);
