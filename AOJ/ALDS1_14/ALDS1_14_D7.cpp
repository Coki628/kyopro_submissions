/*
参考：https://onlinejudge.u-aizu.ac.jp/solutions/problem/ALDS1_14_D/review/5302147/hitonanode/C++17
・AOJコース埋め
・Aho-Corasick法
・hitonanodeさんがアホコラでも解いていたのでやってみた。
　2758MBの揺るぎないMLEだった。。
　まあそれはしょうがないとしてTLEもするっぽいし、やり方違うのかな。。
*/
...
void solve() {
    ...
    AhoCorasick<75, '0'> aho;
    ll Q;
    cin >> Q;
    rep(i, Q) {
        string T;
        cin >> T;
        // 先にクエリの方の文字列を単語として全て読み込む
        aho.add(T);
    }
    aho.build();

    // 文字列Sを1文字ずつ進めて、各位置で終わる単語を確認する
    vector<ll> ans(Q);
    ll v = 0;
    rep(i, N) {
        // i文字目まで終わった時のアホコラ上での位置v
        v = aho.move(S[i], v).second;
        // この位置で終わる単語を確認
