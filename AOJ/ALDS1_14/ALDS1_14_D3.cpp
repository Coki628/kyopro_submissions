...
void solve() {
    ...
    vector<int> rsa(M);
    rep(i, M) rsa[sa[i]] = i;
    auto sast = get_sparse_table(sa, [](int a, int b) { return min(a, b); });
    auto lcpst = get_sparse_table(lcp, [](int a, int b) { return min(a, b); });

    ll i = 0, j = N+1;
    while (j < M) {
        ll res = 0;
        ll l = rsa[j];
        ll r = lcpst.bisearch_fore(l, M-2, A[i], less<int>())-1;
        if (r == -2) r = M-2;
        if (l <= r) {
            if (sast.query(l+1, r+2) < N) res = 1;
        }
        // ll k = rsa[j];
        ...
        // }
        r = rsa[j]-1;
        l = lcpst.bisearch_back(0, r, A[i], less<int>())+1;
        if (l <= r) {
            if (sast.query(l, r+1) < N) res = 1;
        }
        // k = rsa[j]-1;
