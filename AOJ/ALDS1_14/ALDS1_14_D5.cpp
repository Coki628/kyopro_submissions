...
・AOJコース埋め
・Suffix Array、LCP
・文字列を全部繋げて、SAとLCPを用意する。
　各クエリ文字列の開始位置を見て、SA上での位置から前後に動いてみて、
　LCPが文字列長以上のところで、元indexが最初の文字列SのやつがあればOK、
　ってやった。
・同じ文字が続くと、前後に動く量が多いっぽくてTLE。。
・SA上のOKな範囲を取るのに、1ずつ前後に動くんじゃなくて、にぶたんでやった。
　区間最小が必要なのでSparse Tableでやったけど、盛大にMLE。
　262MB制限に対して300ちょっとかと思ったけどよく見たら3350MBだった。。
・Sparse Tableをセグ木に変更。それでも399MBでMLE。。
・なんとか通った！長かったー。。以下、メモリに対する工夫を。
・効いたもの
　・.shrink_to_fit()する。.clear()しただけだとメモリ開放されない。
　・処理を分ける。今回のセグ木の操作は同時にやらなくても、
　　lcp上の範囲チェックが全部済んでから、もう1回ループ回して、
　　改めてsa上の所定の範囲での最小indexを探す、としていい。
　　最初のが終わって、lcpのセグ木配列を消してから、saのセグ木を作る。
　　結局これが決め手だった。
・効かなかったもの
　・セグ木クラスを捨てて配列と関数だけにしてみた。
　　pythonだとこれが結構効いたはずだけど、C++ではメモリ消費はほぼ変化なかった。。
・最終的にメモリ259MB、時間1.07秒でAC。メモリがギリギリなのはしょうがないとして、
　おそらく1000万くらいの文字列にSAしてセグ木作ってるんだけど、
　それでも1秒ちょっとでACするんだなー。
　そっか、セグ木って構築だけなら線形だから1000万にlogは乗らないんだね。
　で、今回クエリは1万だから、ここにセグ木のlogが乗ってもボトルネックにはならない訳だ。
*/
...
void solve() {
    ...
    vector<int> rsa(M);
    rep(i, M) rsa[sa[i]] = i;
    auto lcpseg = get_segment_tree([](int a, int b) { return min(a, b); }, MOD);
    lcpseg.build(lcp);
    lcp.clear();
    lcp.shrink_to_fit();
    ...
    while (j < M) {
        ll res = 0;
        ll l = rsa[j];
        ll r = lcpseg.find_first(l, [&](ll x) { return x < A[i]; })-1;
        if (r == -2) r = M-2;
        LR.pb({l+1, r+2});
        r = rsa[j]-1;
        l = lcpseg.find_last(r+1, [&](ll x) { return x < A[i]; })+1;
        LR.pb({l, r+1});
        j += A[i];  
        i++;
    }
    ...
    auto saseg = get_segment_tree([](int a, int b) { return min(a, b); }, MOD);
    saseg.build(sa);
    sa.clear();
    sa.shrink_to_fit();
    ll l, r;
