/*
参考：https://onlinejudge.u-aizu.ac.jp/solutions/problem/ALDS1_14_D/review/4883493/Luzhiled/C++17
・AOJコース埋め
・Suffix Array、SA上の二分探索
・jag2014summer_day4_fでやったのと同じ方針。
　SAには部分文字列が辞書順で並んでいるので、
　比較対象の文字列より大きいかや小さいかを二分探索で探せば、
　境界となった位置で一致してるかチェックして同じものがあったかどうか確認できる。
　部分文字列の切り出しはlog回くらいしか行われないため、速度は問題なし。
　てかすげー速い。AC0.07秒。頑張ったのが何だったんだってなるやつ。
*/
...
void solve() {
    ...
    rep(i, Q) {
        string T;
        cin >> T;
        ll j = bisearch_min(-1, N-1, [&](ll x) {
            return T <= S.substr(sa[x], T.size());
        });
        if (T == S.substr(sa[j], T.size())) print(1);
        else print(0);
    }
}
