/*
参考：https://cpprefjp.github.io/reference/list/list/splice.html
・AOJコース埋め
・splice：ようはリストの連結
・どうやら連結リストに該当のメソッドが存在していたので使ってみた。
　こちらはリストの長さに関わらずO(1)でいけるっぽい。
*/
...
template<typename T> void print(list<T> li) { vector<T> A(li.begin(), li.end()); print(A); }
