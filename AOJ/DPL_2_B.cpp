/*
参考：https://rustforbeginners.hatenablog.com/entry/chinese-postman-dpl-2-b
・AOJコース埋め
・中国人郵便配達問題
・ワーシャルフロイド、一筆書き、オイラーグラフ、bitDP
・やっとこれACできた。昔着手して放置してたやつ。
・まず、一筆書きで戻ってこれるパス(オイラーグラフと言う)であれば、
　題意(全ての辺を通る)を満たせる。辺を通る回数には制限がないので、
　いくつかの辺を複数回通ることによってこの形にすることを考える。
・オイラーグラフは全ての頂点の次数が偶数になるので、次数が奇数の頂点を列挙して、
　それら全てをペアにすることで、全ての頂点の次数が偶数である状況を作る。
　この時、奇数の頂点が奇数個になることはない。(グラフ中の次数の合計は偶数だから)
　また、離れた頂点同士でパスを繋ぐ場合も、ペアにするとした2頂点は
　端点となるため次数が1増えるが、途中で通った頂点は次数が2増えて偶奇が変化しないので、
　この点でも問題がない。
・奇数の頂点のみに絞ったグラフでの辺のコストは、
　予めワーシャルフロイドで、全頂点間の最短距離を計算しておく。
・辺のペアをどう作るかはbitDPでマッチング済の頂点集合を管理することでできる。
　この時行うbitDPは、2頂点ずつ集合に追加されるもので、
　TSPのものと形は似ているが全く同じことをしている訳ではないので注意。
*/
        ...
        rep(bit, 1LL<<L) {
            rep(u, L) {
                ...
                rep(v, u+1, L) {
                    if (bit>>v & 1) continue;
                    // 頂点u,vでペアにする
                    chmin(dp[bit|(1LL<<u)|(1LL<<v)], dp[bit] + dist[V[u]][V[v]]);
                }
            }
        }
        // 全ての辺 + L個の頂点でペアを組ませる最小コスト
        ans += dp[(1LL<<L)-1];
    }
    print(ans);
}
