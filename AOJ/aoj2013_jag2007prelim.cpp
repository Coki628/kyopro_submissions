 ...
 * ・速攻自力AC！
 * ・累積和、いもす
 * ・いもすは知ってればやるだけ。どっちかと言えば時刻のエンコードが本質かも。
 */
...
int main() {
    ...
    while (1) {
        ...
        rep(i, N) {
            ...
            cin >> s >> t;
            ll hh, mm, ss;
            hh = toint(s.substr(0, 2));
            mm = toint(s.substr(3, 2));
            ss = toint(s.substr(6, 2));
            ll l = hh*60*60 + mm*60 + ss;
            imos[l]++;
            hh = toint(t.substr(0, 2));
            mm = toint(t.substr(3, 2));
            ss = toint(t.substr(6, 2));
            ll r = hh*60*60 + mm*60 + ss;
            imos[r]--;
        }
        rep(i, MX-1) imos[i+1] += imos[i];
        ll ans = max(imos);
        print(ans);
    }
    return 0;
