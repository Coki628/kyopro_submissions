...
・AOJコース埋め
・最大長方形
・スタックでよしなにやるやつ。何回も見てるけど、このスタックの挙動全然頭に入ってこないんだよね。。
・降順ソートしてUFで隣接要素を連結させる方針で通した。
*/
...
void solve() {
    ...
    for (auto [a, i] : AI) {
        used[i] = 1;
        if (i and used[i-1]) uf.merge(i-1, i);
        if (i != N-1 and used[i+1]) uf.merge(i, i+1);
        chmax(ans, a*uf.get_size(i));
    }
    print(ans);
}
