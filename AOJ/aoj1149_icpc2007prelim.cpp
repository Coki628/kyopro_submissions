/**
 * ・E8さん精選100+50問埋め：実装問題
 * ・自力AC？WAが出てテストケース見たけど手がかりは得られずよくコードと問題見た。
 * 　push_backでいいのにinsertで元の位置に要素戻してた。。
 * ・長方形の縁に沿って回るのとかちょっと実装あるけど、制約小さいしまあ愚直にやればいい。
 * ・分裂する要素の管理も、制約小さいのでvectorで愚直に操作すればいい。
 */
...
pair<pll, pll> cut(pll cur, ll s) {
    auto [h1, w1] = cur;
    pll pos = {0, 0};
    while (s) {
        while (s and pos.second < w1) {
            s--;
            pos.second++;
        }
        while (s and pos.first < h1) {
            s--;
            pos.first++;
        }
        while (s and pos.second > 0) {
            s--;
            pos.second--;
        }
        while (s and pos.first > 0) {
            s--;
            pos.first--;
        }
    }
    pll res1, res2;
    if (pos.first == 0 or pos.first == h1) {
        ll w2 = pos.second;
        res1 = {h1, w2};
        res2 = {h1, w1-w2};
    } else {
    ...
    if (res1.first*res1.second > res2.first*res2.second) {
        swap(res1, res2);
    }
    return {res1, res2};
}
