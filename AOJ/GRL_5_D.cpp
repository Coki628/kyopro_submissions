...
・AOJコース埋め
・さくっと自力AC！
・HL分解、パスクエリ、辺属性、1点加算区間和取得
・なんとなくHLDは頂点属性で使うことが多かった気がして、
　辺にコスト割り当てる時、親子どっちだろうとか考えたけど、
　どの頂点も親に向かう辺は一意なので、辺から見れば、
　子側の頂点にその辺のコストを割り当てるようにすればうまくいく。
*/
...
void solve() {
    ...
    rep(_, Q) {
        ...
        if (op == 0) {
            ...
            ll v;
            cin >> v;
            ll res = hld.query(0, v, 0LL, [&](ll a, ll b) { return bit.query(a, b); }, [](ll a, ll b) { return a+b; }, true);
            print(res);
        }
    }
}
