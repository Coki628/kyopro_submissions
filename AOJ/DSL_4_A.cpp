 ...
 * ・きっちり自力AC！添字も1つもバグらせなかった。
 * ・座標圧縮、2次元いもす
 * ・半開区間をしっかり意識してやればバグらせない。いい感じ。
 */
...
void solve() {
    ...
    auto [zpx, unzpx] = compress(X);
    auto [zpy, unzpy] = compress(Y);
    ll M = zpx.size();
    ll L = zpy.size();
    auto imos = list2d(M, L, 0);
    for (auto [xy1, xy2] : XY) {
        auto [x1, y1] = xy1;
        auto [x2, y2] = xy2;
        imos[zpx[x1]][zpy[y1]]++;
    ...
    rep(i, M) rep(j, L-1) imos[i][j+1] += imos[i][j];
    rep(j, L) rep(i, M-1) imos[i+1][j] += imos[i][j];

    ll ans = 0;
    rep(i, M-1) {
        rep(j, L-1) {
            if (imos[i][j]) {
                ll w = unzpx[i+1]-unzpx[i];
                ll h = unzpy[j+1]-unzpy[j];
                ans += h*w;
            }
        }
    }
