 ...
 * ・蟻本演習4-7-3
 * ・文字列検索、Aho-Corasick法
 * ・長かったー。さすがにICPC系は重実装多いね。
 * ・とりあえず色々込み入ったDP。解説よく読んで丁寧に実装したけど、
 * 　こういう状態の持ち方とかちゃんと自力で導けるようにしたいね。
 * 　色々やってることはソース内コメントも参照。
 * ・ある位置からある単語を始めた時、予め読み込み済のパターンにどのくらいマッチするか、
 * 　をアホコラが賢く管理してくれる。この問題の解説スライドの説明は分かりやすかった。
 * 　ライブラリはいつもながら、うしさんありがとうです。
 * ・計算量が余裕で億超えのハードな感じなんだけど、メモリのがきつい。
 * 　256MB制限で、DP配列llでやったら2000M超えてたからね。intにしても1000M超。
 * 　解説にもあった通りかなりスパースな状態なはずなので、3次元目をmapにして、
 * 　余計な要素作らないようにしたら無事AC3.50秒。(制約8秒)
 * 　mapをunorderedにしたらもちょい速くなるかと思ったのに、ちょっと遅くなって4.08秒。。
 */
...
int main() {
    ...
    while (1) {
        ...
        //                   季語を使っていない/使った状態の通り数
        auto dp0 = list2d(M+1, id+1, unordered_map<int, int>());
        auto dp1 = list2d(M+1, id+1, unordered_map<int, int>());
        dp0[0][id][0] = 1;
        rep(i, 0, M) {
            rep(u, 0, id+1) {
                rep(a, 0, L) {
                    if (!dp0[i][u].count(a) and !dp1[i][u].count(a)) continue;
                    for (ll v : nodes[u]) {
                        string word = S[v];
                        ll n = word.size();
                        // 詩の長さオーバー
