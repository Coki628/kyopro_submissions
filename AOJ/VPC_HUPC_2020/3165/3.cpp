/*
参考：https://twitter.com/stoq_/status/1445027689421631498
・遅延セグ木、区間等差数列加算
・もう1問、stoqさんが例題出してくれたからやってみた。
・思った以上にやるだけじゃなかった。さっきは更新だったけど今度は加算。
　加算だと、更新の左端で帳尻合わせるための差し引きを先にやってしまうと、
　差し引きした値が0になった時に(多分)変化なしとみなされて、作用素の更新が行ってもらえない。
　更新だと単位元INFとかならそれと被る心配はなかったけど、加算は単位元0だからこれがある。
　なので作用素には操作した事実と差し引きしたい値を別に持たせることにした。
　で、最終的に {その位置を更新した回数cnt, 差し引きしたい量sub} を持ったらうまくいった。
　これを使って関数gで要素とマージする時に、まとめて処理する。
・今回は1点取得だけど、最小値じゃなくて和でやってみた。これでも大丈夫。
　いくつか区間和も試してみたけど、そっちも大丈夫そう。
・より操作を一般化した。公差1じゃなくてもいいし、更新時も-1とかしなくていいように直観的に。
*/
