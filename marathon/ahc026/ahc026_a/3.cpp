/*
・ビームサーチ
・これは作ったけど、評価を現在までのコストでやったら全然伸びなかった。
　(貪欲より下がった。。)
*/
...
auto solve(int testcase = -1) {
    ...
    using state = tuple<int, vector<pii>, vvi>;
    ...
    while (1) {
        ...
        rep(_, BEAM_WIDTH) {
            ...
            auto [cost, ops, curbox] = cur_beam.top();
            cur_beam.pop();
            // 最後に回収した箱
            ...
            if (k < curbox[m].size() - 1) {
                rep(nxt, M) {
                    ...
                    op2(m, i, ops, curbox);
                    nxt_beam.push({cost + nxcost, ops, curbox});
                    // 状態戻し
                    curbox = tmp;
                    ops.pop_back();
                    ops.pop_back();
        ...
        // 今一番いい状態を確認
        auto [cost, ops, curbox] = cur_beam.top();
        int i = ops.size() ? ops.back().first : -1;
        if (i == N and cost < mncost) {
            mncost = cost;
            mxops = ops;
        }
    }
