...
auto solve(int testcase = -1) {
    ...
    auto respond = [&](int op, const vector<pii> &cells, int qcnt) -> pair<double, double> {
        if (op == 1) {
            auto [h, w] = cells[0];
            return {grid[h][w], 1};
        } else {
...
#endif
    auto ask = [&](int h, int w, int qcnt) -> pair<double, double> {
#ifdef __LOCAL
        ofs << "q 1 " << h << " " << w << endl;
        auto [v, c] = respond(1, {{h, w}}, qcnt);
#else
        cout << "q 1 " << h << " " << w << endl;
        int v;
        cin >> v;
