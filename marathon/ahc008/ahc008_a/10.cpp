/*
・行数を3にした。あと入口を1手で閉めれるようにしたり。
　あと盤面をその場で評価してるせいで後続の人の行動がバグってたので、
　全員分まとめて後から適用させるように修正。
・ちょっと伸びて、7億4400万くらい。
*/
    ...
    rep(turn, TURN_COUNT, 0, -1) {
        ...
        rep(i, M) {
            ...
            // 巡回しながら埋める行を決める
                ...
                // この行を進むと決める
                if (h%3 == 1 and not prog[d01][h+1]) {
                    prog[d01][h+1] = true;
                    // 片側やってれば終わる所
                    if (h == 5 and not prog[d01][2]) {
                        prog[d01][2] = true;
                        progcnt++;
                    }
                    progcnt++;
            ...
            // ペットがいる行を探す
                ...
                if (done[d01][h] and not used[d01][h]) {
                    ...
                    if (can_block(h, w+dw)) {
                        blocked[h][w+dw] = true;
                        auto [cellcnt, petcnt, mancnt, dcnt] = bfs(h, w+dw*2, P);
                        dogcnt -= dcnt;
                        blocked[h][w+dw] = false;
                        if (
