...
struct Human {
    ll h, w, phase, targetrow, targetrow2;
    bool down;
};
...
ld solve(int testcase=-1) {
    ...
    rep(i, M) {
        ...
        // 探索時の基準行
        H[i].targetrow2 = i%2 == 0 ? H[i].targetrow : H[i-1].targetrow;
        if (H[i].w == 1) {
            H[i].phase++;
        }
    }
            ...
            // 巡回
                ...
                if (M%2 == 1 and i == M-1) {
                    ...
                    if (h%3 == 1) {
                        ...
                        if (is_saferow(h)) {
                            // 左チェック
                            if (action[i] == '.' and done[0][h] and not used[0][h]) {
                                if (can_block(h, targetcol[tci]-1)) {
                                    blocked[h][targetcol[tci]-1] = true;
                                    auto [cellcnt, petcnt, mancnt, dcnt] = bfs(h, targetcol[tci]-2);
                                    blocked[h][targetcol[tci]-1] = false;
                            ...
                            // 右チェック
                            if (action[i+1] == '.' and done[2][h] and not used[2][h]) {
                                if (can_block(h, targetcol[1-tci]+1)) {
                                    blocked[h][targetcol[1-tci]+1] = true;
                                    auto [cellcnt, petcnt, mancnt, dcnt] = bfs(h, targetcol[1-tci]+2);
                                    blocked[h][targetcol[1-tci]+1] = false;
                            ...
                            // 中央チェック
                            if (action[i] == '.' and action[i+1] == '.' and done[1][h] and not used[1][h]) {
