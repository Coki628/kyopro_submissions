/*
・一本道をたくさん作っておいて、ペットがいなくなった所でそこに入って閉じる方針。
　ちょっとだけ伸びて、2億7500万くらい。
*/
        ...
        rep(i, M) {
            ...
            // ペットがいない行を探す
                ...
                if (done[h] and not used[h]) {
                    blocked[h][w] = true;
                    auto [cellcnt, petcnt] = bfs(h, w+1);
                    blocked[h][w] = false;
                    // ペットがいない行or残りターンが少ないなら入る
                    if (petcnt == 0 or TURN_COUNT-turn < 5) {
                        used[h] = true;
                        action[i] = 'R';
                        move_human(H[i], i, action[i]);
                        phase[i]++;
