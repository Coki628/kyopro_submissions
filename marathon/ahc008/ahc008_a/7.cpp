/*
・真ん中を起点にして、両側に閉じ込めていく方針。
　でもそれだけだと犬が多い時とかにすげー点数悪くて結局3億9000万くらいで、
　これはその対策として犬の囮を1人用意してるんだけど、これで5億ちょい。
*/
        ...
        rep(i, M) {
            ...
            // ペットがいる行を探す
                ...
                if (done[d01][h] and not used[d01][h]) {
                    ...
                    blocked[h][w] = true;
                    blocked[h+1][w] = true;
                    auto [cellcnt, petcnt, mancnt] = bfs(h, w+dw);
                    blocked[h-1][w] = false;
                    blocked[h][w] = false;
                    blocked[h+1][w] = false;
                    if (
