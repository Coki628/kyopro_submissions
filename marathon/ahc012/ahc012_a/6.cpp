/*
・イチゴが多い所から切る貪欲。あんま良くなくて6100万くらい。
*/
...
ld solve(int testcase=-1) {
    ...
    // from lib.rs
    auto compute_score = [&](const vector<array<int, 4>>& ans) -> tuple<ld, vector<int>, vvi> {
        vvi pieces;
        vector<int> piece(N);
        iota(ALL(piece), 0);
        pieces.eb(piece);
        ...
        return {round((ld)1e6 * num / den), B, pieces};
    };
    ...
    vector<array<int, 4>> ans;
    rep(_, K) {
        auto [score, B, pieces] = compute_score(ans);
        sort(ALL(pieces), [](const vector<int>& a, const vector<int>& b) { return a.size() > b.size(); });
        if (pieces[0].size() <= 10) break;
        int i = pieces[0][0];
        int px = X[i] + 1;
        int py = Y[i];
        int qx = randrange(xmn, xmx+1);
