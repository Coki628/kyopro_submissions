/*
・1点更新・1点削除・1点追加山登り。ちょっと伸びて、7400万点くらい。
・終了後記。結局これが最終提出。
　乱択で適当な線K本引いて初期解、1点更新/追加/削除で山登り。
　スコア計算はヴィジュアライザのRustそのままだから毎回全計算。
　コードテストで300回くらいしか回らず。
　差分計算したいけどどうやんだよこれって思って終わったけど、
　そうかX軸かY軸に平行なのしか引かなければグリッドみたくなるからまだ管理しやすかったんだな…。
　あと線が最大100本しかないから、100ビットの状態で管理するってやつは
　なるほどなぁと思った。(わどさんのツイートより)
*/
...
ld solve(int testcase=-1) {
    ...
    while (1) {
        ...
        while (op == -1) {
            op = randrange(0, 3);
            if (op == 1 and ans.size() < 10) op = -1;
            if (op == 2 and ans.size() == K) op = -1;
        }
        // 1点更新
        if (op == 0) {
            int i = randrange(0, ans.size());
            int px = randrange(xmn, xmx+1);
            int py = randrange(ymn, ymx+1);
            int qx = randrange(xmn, xmx+1);
            int qy = randrange(ymn, ymx+1);
