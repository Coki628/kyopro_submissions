...
int calc_score(int K2, vvi field, const vector<array<ll, 4>> &cas, const vvi &mp) {
...
ld solve(int testcase=-1) {
    ...
    int score = calc_score(K2, grid, cas, mp);
    ...
    while (1) {
        ...
        if (cas.size() + mas.size() <= K2) {
            int nxscore = calc_score(K2, grid, cas, mp);
            if (nxscore > score) {
                score = nxscore;
                updated = true;
            }
    ...
    score = calc_score(K2, grid, cas, mp);
    return score;
}
