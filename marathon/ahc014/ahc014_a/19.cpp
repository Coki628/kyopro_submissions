...
・優先度付けのソートをやめてループ回して線形にした。これで3946万点！
・最深部のp1p3取ってくる所いじって最適化した。
　後半の乱択の試行回数も増えたし、前半も速くなった。
　ビーム幅も10から12に増やせたけど、ちょっと下がったので戻した…。
　一番いいやつで3961万点。
*/
                        ...
                        if (p4 != NG and p2.x <= p4.x) {
                            ...
                            if (p1 != NG) {
                                ...
                                if (ok) {
                                    ...
                                    auto [dx4, dy4] = dir8[d4];
                                    int cx4 = p1.x, cy4 = p1.y;
                                    if (usede[d4][cx4][cy4]) {
                                        continue;
                                    }
                                    // p1 -> p2
                                    while (cx4 != p2.x or cy4 != p2.y) {
                                        cx4 += dx4, cy4 += dy4;
                                        // p2到達前に使用済頂点にぶつかった
                                        if ((cx4 != p2.x or cy4 != p2.y) and usedv[cx4][cy4]) {
                                            ok = false;
                                            break;
                                        }
                                    }
