/*
・場合分け、乱択
・新しく取り入れた条件パターンを調べる。いったん補正は外してある。
　なにげに完全乱択が疎のケースでかなり有力っぽい。
C: 92 46 25 6 2 1 0 28 0 0
C: 91 43 25 9 1 0 0 31 0 0
*/
...
auto solve(int testcase=-1) {
    ...
    while (elapsed < TL) {
        int optype = randrange(0, 8);
        // 0,1は多いので割合を増やす
        // if (optype == 4) optype = 0;
        // elif (optype == 5) optype = 1;
        // 傾向として0はオールラウンド、1,2は密に強く、3,4,5,6,7は疎に強い
        ...
        // if (optype == 5 and dens >= 0.05) optype = 0;
        // if (optype == 6 and dens >= 0.05) optype = 0;
