/*
参考：https://twitter.com/inani_waon/status/1575008675713798144
・chokudaiサーチ
・改修してMLEしないようにした。多すぎる分を捨てればいいそうなので、そうした。
　ちゃんと動くようにはなったけど、乱択*貪欲より回数は動かせないし、
　条件も1種類をずっと使っているので、さほど伸びない。3586万点だった。
*/
...
auto solve(int testcase=-1) {
    ...
    using state = tuple<ll, ll, ll, vector<array<Point<int>, 4>>>;
    // 溜まりすぎたビームを減らす
    auto refresh_beam = [&](priority_queue<state, vector<state>, greater<state>>& cur_beam) -> void {
        priority_queue<state, vector<state>, greater<state>> new_beam;
        rep(_, 10) {
            new_beam.push(cur_beam.top()); cur_beam.pop();
        }
        swap(cur_beam, new_beam);
    };

    vector<array<Point<int>, 4>> ans;
    ll mxscore = calc();
    const int BEAM_WIDTH = 1;
    vector<priority_queue<state, vector<state>, greater<state>>> beam(1);
    beam[0].push({0, 0, 0, {}});
    vector<int> done(1);
    ...
    while (elapsed < TL) {
        ...
        while (elapsed < TL and i < beam.size()) {
            ...
            rep(_, BEAM_WIDTH) {
                ...
                if (cur_beam.size() > 30) {
                    refresh_beam(cur_beam);
                }
                auto [__, ___, ____, curans] = cur_beam.top(); cur_beam.pop();
                // 状態作成
                for (auto& arr : curans) {
                    put(arr);
                }
