/*
・場合分け、乱択
・4パターンに絞り込んだ。これで最大4111万点。
C: 105 49 14 32 0 0 0 0 0 0
C: 118 40 11 31 0 0 0 0 0 0
C: 109 43 20 28 0 0 0 0 0 0
・結局これが最終提出。一応青パフォで耐えてくれた。
　何回も投げてた割には、特にそんな下振れすることもなくシステム終了。
・後からTL見てた限り、ここからさほど大きな変更なくできた工夫としては、
　1点(とそれに依存する全部)を壊して、そこから更新できるなくなるまで再構築、
　みたいのをやれば良かったっぽい。途中ちょっとそれ考えたんだよなー。
　終盤ほぼ忘れてたけどやれば良かったか。依存関係管理しないといけなくなるから、
　まあまあ実装とかめんどいと思うけどね。でも多分やればできた。
*/
...
auto solve(int testcase=-1) {
    ...
    auto get_cost = [&](const Point<int>& p) -> int {
    ...
    // XY -> UV変換
    auto xy_to_uv = [](const Point<int>& p) -> Point<int> {
        return {p.x-p.y, p.x+p.y};
    };
    // UV -> XY変換
    auto uv_to_xy = [&](const Point<int>& uv) -> Point<int> {
        // 目指す位置が整数座標でない
        if ((uv.y+uv.x)%2 != 0) {
            return NG;
        }
    ...
    while (elapsed < TL) {
        int optype = randrange(0, 6);
        // 0,1は多いので割合を増やす
        if (optype == 4) optype = 0;
        elif (optype == 5) optype = 1;
        // 傾向として0はオールラウンド、1,2は密に強く、3は疎に強い
...
int main() {
    ...
    #ifdef __LOCAL
        // テスト番号指定
        // auto T = gen_cases1({192,192,192,192,192,});
        // sからn件実施
        // auto T = gen_cases2(0, 20);
        // [s,t)を実施
        auto T = gen_cases3(0, 200);
        // [s,t)の範囲でランダムにn件実施
