...
ld solve(int testcase=-1) {
    ...
    // validは新規の辺設置で、こちらは既存の辺の置き換え時のチェック用
    auto valid2 = [&](
        vvi& target, int ch, int cw, int prvc, int c
    ) -> bool {
        int edgecnt1 = 0, edgecnt2 = 0;
        rep(d, 4) {
            auto [dh, dw] = dir4[d];
            int nh = ch + dh;
            int nw = cw + dw;
            ...
            if (target[nh][nw] != -1) {
                ...
                // 辺がつく場合、連結成分を確認
                if ((prvc>>d & 1) and (target[nh][nw]>>revd & 1)) {
                    edgecnt1++;
                }
                if ((c>>d & 1) and (target[nh][nw]>>revd & 1)) {
                    edgecnt2++;
    ...
    // ここにパーツcを置く時の辺を列挙
    auto check_edges = [&](
        const vvi& target, int ch, int cw, int c
    ) -> vector<int> {
        vector<int> res;
        rep(d, 4) {
            auto [dh, dw] = dir4[d];
    ...
    auto swap_edges = [&](vvi&target, int ch, int cw, int nh, int nw) {
        auto edges1 = check_edges(target, ch, cw, target[ch][cw]);
        auto edges2 = check_edges(target, nh, nw, target[nh][nw]);
        bool ok1 = valid2(target, ch, cw, target[ch][cw], target[nh][nw]);
        bool ok2 = valid2(target, nh, nw, target[nh][nw], target[ch][cw]);
        swap(target[ch][cw], target[nh][nw]);
        auto edges1new = check_edges(target, ch, cw, target[ch][cw]);
        auto edges2new = check_edges(target, nh, nw, target[nh][nw]);
        bool okok = ok1 and ok2 and edges1 == edges1new and edges2 == edges2new;
        if (okok) {
            return true;
        } else {
            swap(target[ch][cw], target[nh][nw]);
    ...
    while (rest > 0) {
        ...
        int ch = randrange(0, H);
        int cw = randrange(0, W);
        if (ch == H-1 and cw == W-1) continue;

        if (target[ch][cw] == -1) {
            rep(c, L-1, -1, -1) {
                if (C[c] == 0) continue;
                if (valid(target, ch, cw, c)) {
                    target[ch][cw] = c;
                    C[c]--;
                    break;
                }
            ...
            int nh = H - 1, nw = W - 1;
            while (not (nh == H-1 and nw == W-1) and not nh == ch and nw == cw) {
                nh = randrange(0, H);
                nw = randrange(0, W);
            }
            if (target[nh][nw] == -1) {
                if (valid(target, nh, nw, target[ch][cw])) {
                    bool changed = false;
                    target[nh][nw] = target[ch][cw];
                    rep(c, L-1, -1, -1) {
                        if (C[c] == 0) continue;
                        if (valid2(target, ch, cw, target[ch][cw], c)) {
                            target[ch][cw] = c;
                            C[c]--;
                            changed = true;
                            break;
                ...
                int dist = abs(ch-nh) + abs(cw-nw);
                // 隣接してると対岸が変わっちゃって面倒なのでパス
                if (dist > 1) {
                    swap_edges(target, ch, cw, nh, nw);
                }
            }
        }
    }
    finalize(target, C, sh, sw);
    auto res = construct(
        H, W, N, T, sh, sw, key,
        nodes, target, hwtod
    );
