/*
・とりあえずいい盤面探すパートと、スライドパズルを解くパートを作った。
　課題はまだ色々ある。
*/
...
ld solve(int testcase=-1) {
    ...
    // 現状の盤面nodesでのbittonodes2を達成させるためのコスト
    ...
    // fixされていない数字cのピースを見つける
    auto fixed = list2d(H, W, false);
    auto find = [&](int c) -> pii {
        rep(h, H) {
            rep(w, W) {
                if (not fixed[h][w] and nodes[h][w] == c) {
                    return {h, w};
                }
            }
        }
    ...
    rep(h, H-2) {
        rep(w, W-2) {
            assert(nodes2[h][w] != -1);
            auto [h2, w2] = find(nodes2[h][w]);
            // 目当ての数字をここに持ってくるルート
            bring2(nodes2[h][w], h, w);
        }
        // 端2マスをまとめて決める
        // (ch,cw),(h3,w3),(h2,w2)と並べる
        bring2(nodes2[h][W-1], h, W-2);
        bring2(nodes2[h][W-2], h+1, W-2);
        bring(ch, cw, h, W-1);
        // (h3,w3),(h2,w2)を正しい位置に
        move(h, W-1, h, W-2);
        move(h, W-2, h+1, W-2);
    ...
    rep(w, W-2) {
        // 端2マスをまとめて決める
        // (ch,cw),(h3,w3),(h2,w2)と並べる
        bring2(nodes2[H-1][w], H-2, w);
        bring2(nodes2[H-2][w], H-2, w+1);
        bring(ch, cw, H-1, w);
        // (h3,w3),(h2,w2)を正しい位置に
        move(H-1, w, H-2, w);
        move(H-2, w, H-2, w+1);
