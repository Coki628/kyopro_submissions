/*
・ダメなケースを少しでもマシにするための対応をした。
　まだ全然不十分だけど、元々1万点乗らないようなやつだけとりあえず。
　これで718M！この時点でのギリ黄パフォ乗った。
・ちょっとN決めの集計方法変えた。最大値じゃなくて和で見るように。
　こっちのが少ない回数で平均にできそうだったから。
　これでなんかちょい伸びて721M！
*/
...
auto solve(int testcase=-1) {
    ...
    -> tuple<vector<string>, vector<vvi>, vvi> {
        ...
        // int each = L / M * 3;
        // rep(i, M) {
        //     G[i] = string(min(each*i, (ll)L), '1') + string(max(L-each*i, 0LL), '0');
        //     graphs[i] = strtograph(N, G[i]);
        //     keys[i] = get_key(N, graphs[i]);
        // }
        return {G, graphs, keys};
