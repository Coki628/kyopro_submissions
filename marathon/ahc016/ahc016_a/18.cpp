/*
・ダメケース判定が最大値だとブレが大きい気がしたので、平均を見るようにしたやつ。
　まあ悪くもなってなさそうだけど、良くもなってない。
*/
...
auto solve(int testcase=-1) {
    ...
    double mxavg = vec[0].first / (double)opcnt;

    // どうにもなんないやつを個別対応
    bool bad = false;
    if (mxavg < 10000 or N == 4) {
        bad = true;
        N = 4;
    }
