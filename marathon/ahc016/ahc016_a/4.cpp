...
auto solve(int testcase=-1) {
    ...
    // 渡されるグラフHの生成シミュレーション
    auto generate_input_graph = [&](const vvi& G) -> pair<string, vvi> {
        vvi H(N);
        rep(u, N) {
            vector<int> vec(N);
            for (auto v : G[u]) {
    ...
    // Hの生成を予めシミュしておく
    const int PREPCNT = 100;
    auto simuH = listnd(M, N, 0.0);
    rep(i, M) {
        rep(_, PREPCNT) {
            auto nodes = generate_input_graph(graphs[i]).second;
    ...
    rep(k, Q) {
        string H;
            ...
            int s;
            cin >> s;
            H = generate_input_graph(graphs[s]).first;
        #else
            cin >> H;
        #endif
