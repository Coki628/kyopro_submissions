...
void solve(ll t=-1) {
    ...
    auto query = [&](ll q, ll sh, ll sw, ll th, ll tw, ll prev_result) -> pair<string, vector<int>> {
        if (q < K) {
            // 進行方向
            char hc = sh <= th ? 'D' : 'U';
            char wc = sw <= tw ? 'R' : 'L';
            ...
            ll ch = sh, cw = sw;
            string res;
            vector<int> path {(int)gridtoid(ch, cw, W)};
            while (hcnt or wcnt) {
                // 縦横どっちか
    ...
    rep(q, Q) {
        ...
        auto [s, path] = query(q, sh, sw, th, tw, prev_result);
        ...
        rep(i, m-1) {
            ...
            cost[u][v] += prev_result/(ld)m;
            cost[v][u] += prev_result/(ld)m;
        }
    }
    #ifdef __LOCAL
