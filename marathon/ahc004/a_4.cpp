...
・マラソン精進
・山登り、高速化
・変更マスがある該当の行列だけチェックするようにした。
　操作回数は1200回くらいまで増えた。
　これは結構結果が出て、24億6700万くらいになった。
・mapをunordered_mapにしたら操作回数が3400回くらいに増えた。
　スコアも伸びて26億9300万くらい。
・unordered_mapをreserveするようにすると、回数が5000回くらいに。
・出力を見ると、結構なマスが.のまま残っているのに気付く。
　これはスコアが伸びた時しか更新を適用しないためで、今回は1マス変えても
　スコア変化がない場合がかなりあって、これを埋めに行った方がよさそうと思う。
　がしかしそうすることで、更新適用回数が大きく増えるので、
　後半でmapの要素数がかなり大きくなり、速度が大きく低下するらしいことを見つける。
　焼きなましが妙に遅くなってしまっていたのはこのためだった。
　更新回数を増やして色々試す方がメリットが大きいと判断し、TLを2.9->2.5秒まで短くする。
　これでTLEを回避。ここでスコアは大きく伸びていて36億2200万くらい。
・ちょっとTL余裕あったので2.7秒まで戻して36億7000万点。自力解はこれが最高点。
*/
