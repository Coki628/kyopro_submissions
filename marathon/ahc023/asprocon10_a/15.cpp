...
・高速化のため、雑に書いてた所を色々整備。がしかしスコアはほぼ変化なし…。
・山登りの処理を12でやってた方に戻した。3656万に戻った。。
・結局これが最終提出になった。
*/
...
auto solve(int testcase = -1) {
...
#endif
    const int T = 100, H = 20, W = 20;
    int dummy, sh, sw = 0;
    cin >> dummy >> dummy >> dummy >> sh;
    array<array<array<int, 4>, W>, H> blocked = {};
    // 下の境界
    rep(h, H - 1) {
        rep(w, W) {
            char tmp;
    ...
    // 作物の優先度
    vector<array<int, 3>> agris;
    rep(k, K) {
        agris.pb({S[k], -D[k], (int)k});
    }
    // 左端ソート、降順右端ソート
    sort(ALL(agris));
    const int N = H * W;
    ...
    // スコア計算
    auto calc = [&](vector<array<int, 4>>& ans) {
        ll sm = 0;
    ...
    while (timer.get_elapse() < TL) {
        ...
        auto bfs = [&](const vector<pii> &src, bool struct_tree = false) {
            array<array<int, W>, H> res;
            rep(h, H) rep(w, W) res[h][w] = MOD;
            queue<pii> que;
            for (auto [h, w] : src) {
                que.push({h, w});
                res[h][w] = 0;
        ...
        auto seg = get_segment_tree(N, [](int a, int b) { return min(a, b); }, MOD);
        BIT<int> bit(N);
        // 未使用セル
        priority_queue<array<int, 3>> que_unused;

        auto add_agri = [&](int h, int w, int k) {
            int u = gridtoid(h, w, W);
            seg.update(curhld.in[u], D[k]);
            assert(bit[curhld.in[u]] == 0);
            bit.add(curhld.in[u], 1);
        };
        auto remove_agri = [&](int h, int w) {
            int u = gridtoid(h, w, W);
            seg.update(curhld.in[u], MOD);
            assert(bit[curhld.in[u]] == 1);
            bit.add(curhld.in[u], -1);
        };
        // 周囲の状況によって重み付け
        ...
        auto is_applicable = [&](int h, int w, int c) {
            ...
            int mn = seg.query(curhld.in[u], curhld.out[u]);
            // ここより手前で取れる作物の最大収穫時(1つもあってはいけない)
            int sm = curhld.query(u, root, 0, [&](int a, int b) { return bit.query(a, b); }, [](int a, int b) { return a + b; });
            return sm == 0 and c <= mn;
        };

        rep(h, H) {
            rep(w, W) {
                int u = gridtoid(h, w, W);
                que_unused.push({get_wait(h, w), dist[h][w], u});
            }
        }

        vector<int> curA(K, -1);
        array<vector<int>, T + 1> adj;
        rep(k, K) {
            adj[D[k]].eb(k);
        }

        vector<array<int, 4>> curans;
        ...
        rep(t, 1, T + 1) {
            ...
            while (curk < K and que_unused.size()) {
                ...
                if (is_applicable(h, w, D[k])) {
                    add_agri(h, w, k);
                    curA[k] = i;
                    curans.pb({k + 1, (int)h, (int)w, (int)t});
                    curk++;
                } else {
                    // このマスはこの日には使わない
                    tmp.pb({wa, di, i});
    ...
    array<BIT<int>, T + 1> useda3d;
    array<BIT<int>, T + 1> usedb3d;
    array<BIT<int>, T + 1> plant3d;
    array<BIT<int>, T + 1> harvest3d;
    rep(t, T + 1) {
        useda3d[t] = BIT<int>{N};
        usedb3d[t] = BIT<int>{N};
        plant3d[t] = BIT<int>{N};
        harvest3d[t] = BIT<int>{N};
    }
    // A[i] := K[i]をどのセルで使ってるか
    vector<int> A(K, -1);
    ...
    auto is_assignable = [&](int k, int t) -> int {
        ...
        for (auto [_, u] : distorder) {
            ...
            // 祖先(植える時と収穫時だけでいいはず)
            if (ok1) {
                ok2 &= !hld.query(u, root, 0, [&](int a, int b) { return useda3d[t].query(a, b); }, [](int a, int b) { return a + b; });
                ok2 &= !hld.query(u, root, 0, [&](int a, int b) { return usedb3d[D[k]].query(a, b); }, [](int a, int b) { return a + b; });
            }
            if (ok1 and ok2) {
                return u;
            }
    ...
    for (auto& [k, h, w, t] : ans) {
        ...
        rep(tt, S[k], t, -1) {
            // この時点で植えに行けるか祖先チェック
            int ok = !hld.query(u, root, 0, [&](int a, int b) { return useda3d[tt].query(a, b); }, [](int a, int b) { return a + b; });
            if (ok) {
                rep(ttt, t + 1, tt + 1) {
    ...
    while (timer.get_elapse() < TL2) {
        ...
        if (v != -1) {
            ...
            if (u != -1) {
                ...
                ans[i1] = {k1 + 1, (int)h1, (int)w1, S2[k1]};
                ans.pb({k2 + 1, (int)h2, (int)w2, S2[k2]});
                B[k2] = M;
                M++;
            } else {
                remove_k(k2);
