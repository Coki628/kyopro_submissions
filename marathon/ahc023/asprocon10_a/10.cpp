...
　これで3641万！結構上がってきた。
・次の段階のために整えた。
・木の状態チェックをHLD*BITで高速化。これで差分チェックがかなり高速にできるはず。
　ついでにこの状態でTLめいっぱい伸ばした版を出してみた。点数ほぼ変わらず…。
*/
...
auto solve(int testcase = -1) {
    ...
    vector<BIT<int>> used3d(T + 1, BIT<int>{N});
    ...
    for (auto& [k, h, w, t] : ans) {
        ...
        rep(tt, S[k], t, -1) {
            // この時点で植えに行けるか祖先チェック
            int ok = !hld.query(u, root, 0, [&](int a, int b) { return used3d[tt].query(a, b); }, [](ll a, ll b) { return a + b; });
            if (ok) {
                rep(ttt, t, tt) {
                    used3d[ttt].add(hld.in[u], -1);
                }
    ...
    for (auto k : abandoned) {
        ...
        for (auto [_, u] : distorder) {
            ...
            if (ok) {
                ...
                // 祖先(植える時と収穫時だけでいいはず)
                if (ok1) {
                    ok2 &= !hld.query(u, root, 0, [&](int a, int b) { return used3d[t].query(a, b); }, [](ll a, ll b) { return a + b; });
                    ok2 &= !hld.query(u, root, 0, [&](int a, int b) { return used3d[D[k]].query(a, b); }, [](ll a, ll b) { return a + b; });
                }
                if (ok1 and ok2) {
                    // 植える
                    rep(tt, t, D[k] + 1) {
