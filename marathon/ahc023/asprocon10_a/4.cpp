/*
・初期解。思うように適当に貪欲。スコアは296万。
・無駄にキューに戻せてない分があったのでバグ修正。
　ぐっと良くなって1814万。でも順位表見る感じここがやっとスタートラインぽい。
・左端ソートをベースにしたら意外と伸びた。これで2541万。
・PQをsetにしてセルの重みの動的更新に対応させた。
　このままだと大した重み付けを作ってないのでほぼ変化はない。
*/
...
auto solve(int testcase = -1) {
    ...
    // 周囲の状況によって重み付け
    auto get_wait = [blocked, H, W](int h, int w) {
        int bcnt = 0;
        rep(d, 4) {
            int revd = d % 2 == 0 ? d + 1 : d - 1;
            auto [dh, dw] = dir4[d];
