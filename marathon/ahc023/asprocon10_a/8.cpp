...
auto solve(int testcase = -1) {
    ...
    while (timer.get_elapse() < TL) {
        ...
        HeavyLightDecomposition curhld(curnodes, r);
        ...
        auto is_applicable = [&](int h, int w, int c) {
            ...
            ll mx = curhld.query(u, r, -INF, [&](ll a, ll b) { return segmx.query(a, b); }, [](ll a, ll b) { return max(a, b); });
            ...
            if (ok) {
                ...
                rep(tt, t + 1, D[k] + 1) {
                    ...
                    auto dfs = [&](auto&& f, int u) -> void {
                        ...
                        if (used3d[h][w][tt - 1] != used3d[h][w][tt]) {
                            ok1 = false;
                        }
                        for (auto v : nodes[u]) {
                            f(f, v);
