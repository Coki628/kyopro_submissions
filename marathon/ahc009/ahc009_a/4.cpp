/*
・山登り、確率DP
・出来上がった文字列を、1点更新して時間いっぱい山登りした。
　この後更新箇所を複数にするのもやったんだけど、結局これのがスコア良かった。
　ヴィジュアライザーのとスコア計算が合わないので、
　確率DPが絶対どっかバグってるのは分かってたんだけど、本番中に解決はできず…。
・最初の200文字埋めは同じ文字100%に変更した。
・高速化のために3次元のDP配列はvectorから生配列にして、
　生成は1回だけでスコア計算の都度愚直ループで0埋め初期化してる。
・これで最高値で16億くらい。
*/
        ...
        rep(i, N) {
            ...
            rep(h, 1, H-1, 2) {
                rep(w, 1, W-1, 2) {
                    ...
                    ll nh = h + dh*2;
                    ll nw = w + dw*2;
                    if ((nh < 0 or nw < 0 or nh >= H or nw >= W) or
                        grid[nh][nw] == '#'
                    ) {
                        dp[i+1][h][w] += dp[i][h][w];
                    } else {
