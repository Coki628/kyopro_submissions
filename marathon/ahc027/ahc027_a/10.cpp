...
・経路順にちょっと乱択入れて、何回かやって一番いいのみたいにしたけど、
　何回か試すために1度の回数を少なくしてしまっているのもあり、スコアは伸びず。
*/
...
auto solve(int testcase = -1) {
    ...
    // 全頂点対で復元配列を前計算
    int K = 10;
    auto prv = listnd(K, N, N, -1);
    auto dist = listnd(K, N, N, 0LL);
    rep(k, K) {
        rep(i, N) {
            auto bfs = [&](vvi &nodes, const vector<int> &src) {
                int N = nodes.size();
                vector<ll> res(N, INF);
                queue<int> que;
                for (int s : src) {
            ...
            dist[k][i] = bfs(nodes1, {(int)i});
        }
    }

    auto find_next = [&](int k, int u) {
        auto [ch, cw] = idtogrid(u, W);
        vector<double> score(N, -1);
        rep(v, N) {
            if (v == u) continue;
            auto [nh, nw] = idtogrid(v, W);
            double di = dist[k][u][v];
    ...
    ll mxscore = INF;
    int opcnt = 0;
    while (opcnt == 0 or timer.get_elapse() < TL) {
        // reset
        ...
        auto dfs = [&](auto &&f, int u, int prv) -> void {
            ...
            for (auto v : nodes1[u]) {
                ...
                auto [nh, nw] = idtogrid(v, W);
                curans += mp[u][v];
                update2(curans.size(), nh, nw);
                f(f, v, u);
                curans += mp[v][u];
                update2(curans.size(), ch, cw);
            }
        };
        dfs(dfs, 0, -1);
        ...
        int LIMIT = 1e4;
        int u = 0;
        while (curans.size() < LIMIT and timer.get_elapse() < TL) {
            // auto [mxt, v] = seg.query(0, N);
            int k = randrange(0, K);
            int v = find_next(k, u);
            assert(v != u);
            auto route = get_route(prv[k][u], u, v);
            assert(route.size() >= 2);
            rep(i, route.size() - 1) {
                int a = route[i];
                int b = route[i + 1];
        ...
        // 帰り
        auto route = get_route(prv[randrange(0, K)][u], u, 0);
        rep(i, route.size() - 1) {
            int a = route[i];
            int b = route[i + 1];
            int d = etod[uvtoe[a][b]];
        ...
        assert(curans.size() <= 1e5);
        ll score = calc2(curans);
        if (mxscore > score) {
            mxscore = score;
            ans = curans;
        }
