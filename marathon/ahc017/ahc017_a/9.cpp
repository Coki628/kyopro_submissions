/*
・最短パスに使われた回数を数えてコストにしてみる。ちょい上がりしたけどほぼ誤差程度。
・結局これが最終提出に。今回はいい方針生えなかったなー。
　更新時の差分チェックで愚直に全部見ないで隣接とか代表だけで推測するのが勝ち筋だったみたい。
*/
...
auto solve(int testcase = -1) {
    ...
    vector<int> stcost(M);
    // STを生成
    auto make_st = [N, M, etop, edgecnt, pathcost](vector<int> &stcost) -> bitset<3000> {
        vector<array<int, 4>> edges;
        rep(e, M) {
            auto [u, v] = etop[e];
            // // 1. STに使われた回数, 2. 両端の次数, 3. ランダム
            // edges.pb({stcost[e], -(edgecnt[u] + edgecnt[v]), (int)randrange(0, MX), (int)e});
            // 1. STに使われた回数, 2. 最短路に使われた回数, 3. ランダム
            edges.pb({stcost[e], pathcost[e], (int)randrange(0, MX), (int)e});
        }
        sort(ALL(edges));
        UnionFind uf(N);
        bitset<3000> res;
        rep(i, M) {
            auto [_, __, ___, e] = edges[i];
            auto [u, v] = etop[e];
            if (not uf.same(u, v)) {
                // 使われた辺にコストをかける
                stcost[e]++;
