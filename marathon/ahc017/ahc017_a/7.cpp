/*
・使った辺にコストつけるやつ。とりあえず貪欲1回だけ。
　これでもスコアはめいっぱいやった方に近いくらいにはなった。
*/
...
auto solve(int testcase = -1) {
    ...
    vector<int> stcost(M);
    // STを生成
    auto make_st = [N, M, etop](vector<int> &stcost) -> bitset<3000> {
        vector<array<int, 3>> edges;
        rep(e, M) {
            edges.pb({stcost[e], (int)randrange(0, MX), (int)e});
        }
        sort(ALL(edges));
        UnionFind uf(N);
        bitset<3000> res;
