/*
・コストを考えてみる。
・水源 - 家 ペアを結ぶ線分からの距離を使った。
　1回で壊せない時にも相応に重みをつけた。
　これで手元402M、提出514M。。
・同じ箇所をキューに何回も詰めないようにした。
　あんま変わらずで手元398M。
・水源・家はコストを上げずに優先するようにした。
　あんま変わらずだけど手元386M。
*/
        ...
        if (res > 0) {
            for (auto [dh, dw] : dir4) {
                ...
                if (broken[nh][nw]) {
                    ...
                    reserved[v] = 1;
                    que.push({cost[nh][nw], v});
                }
            }
        } else {
            // 水源・家はコストを上げない
            cost[ch][cw] += is_water[u] or is_house[u] ? 0 : PENA;
            que.push({cost[ch][cw], u});
        }
    }
