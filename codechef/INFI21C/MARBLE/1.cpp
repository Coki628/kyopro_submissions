...
void solve() {
    ...
    for (char c : ascii_lowercase) {
        string curs = replace(S, "?", {c});
        string curt = replace(T, "?", {c});
        ll cnt = 0;
        rep(i, N) {
            if (curs[i] == curt[i]) {

            } elif (vowels.find(curs[i]) != -1 and vowels.find(curt[i]) != -1) {
                cnt += 2;
            } elif (vowels.find(curs[i]) != -1 or vowels.find(curt[i]) != -1) {
                cnt++;
            } elif (vowels.find(curs[i]) == -1 and vowels.find(curt[i]) == -1) {
                cnt += 2;
            }
