/*
・Test Set 2がWAになるけど、よく分からん。
・1つ前と比べて、桁が大きいならそのまま、
　小さいならなら同じ桁数に合わせてにぶたんでギリギリのところで超えさせる。
　超えられないなら1桁大きくして全0に。
　同じ桁数だったら、数値で大小比較して小さいなら1桁0を足す。
　ってやったんだけど、何かダメなケースがあるんだろうな。。　
・pythonで通した後、C++でもint128とか使って頑張ってみたけどなぜかTLE。。。
*/
...
__int128_t pow(int x, __int128_t n) { __int128_t res = 1; rep(_, 0, n) res *= x; return res; }
...
template<typename F> __int128_t bisearch_min(__int128_t mn, __int128_t mx, const F &func) { __int128_t ok = mx, ng = mn; while (ng+1 < ok) { __int128_t mid = (ok+ng) / 2; if (func(mid)) ok = mid; else ng = mid; } return ok; }
...
void solve() {
    ...
    auto A = LIST(N);
    vector<string> S(N);
    rep(i, N) S[i] = tostr(A[i]);

    ll ans = 0;
    rep(i, 1, N) {
        auto s = S[i-1];
        auto t = S[i];
        ll len_diff = (ll)s.size()-(ll)t.size();
        if (len_diff > 0) {
            __int128_t M = pow(10, len_diff);
            __int128_t res = bisearch_min(-1, M, [&](__int128_t m) {
                return t+zfill(tostr(m), len_diff) > s;
            });
            if (res == M) {
                S[i] += string(len_diff+1, '0');
                ans += len_diff+1;
            } else {
                S[i] = t+zfill(tostr(res), len_diff);
                ans += len_diff;
            }
        } elif (len_diff == 0) {
            if (t <= s) {
