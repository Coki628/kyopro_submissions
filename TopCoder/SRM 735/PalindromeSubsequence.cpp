 ...
 * ・自力AC？
 * ・回文、ギャグ
 * ・とりあえずよく考えると、文字2種類しかないんだから、aとbで完全に分けちゃえばサイズ2は少なくとも達成できる。
 * 　どっちか1つの文字しかなければサイズ1にできる。
 * ・とりあえずこれで適当にぶん投げたらシステス落ちた。最初から回文になってるパターンを見落としてた。
 * 　確かにそれでもサイズ1になるよね。サンプルにも入ってたしちゃんと確認しよう。。
 */
...
class PalindromeSubsequence {
    ...
    vector<int> optimalPartition(string s) {
        ...
        if (check(s) or s.find('a') == -1 or s.find('b') == -1) {
            vector<int> ans(N, 1);
            return ans;
        }
        vector<int> ans(N);
