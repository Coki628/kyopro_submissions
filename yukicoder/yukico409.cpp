 ...
 * ・なんとか自力AC！
 * ・式変形をすごく頑張る。Bの係数を等差数列の和にしたり、その後もごりごり式変形。
 * 　dp[i] = min{ dp[j] + (i-j)*(i-j+1)/2*B - (i-j)*A }
 * 　      = (i^2+i)/2*B - i*A + min{ -B*i*j + dp[j] + (j^2-j)/2*B + j*A }
 * 　と変形できて、min部分を f(x) = -Bjx + dp[j]+(j^2-j)/2*B+j*A
 * 　とすれば a = -B*j, b = dp[j]+(j^2-j)/2*B+j*A の ax + b の直線の式になる。
 * 　ここまで来れば、新しいdp[i]を更新する度に直線の式を加えていくことでDPできる。
 * ・注意点として、新しい直線の式を追加する時、更新後のdp[i]を使ってしまうと、
 * 　その日にドーナツを食べなかった情報まで式に含めてしまうので、
 * 　遷移前+D[i]となる、dp[i-1]+D[i]を使うこと。
 */
...
int main() {
    ...
    lct.add_line(0, dp[0]);
    rep(i, 1, N+1) {
        // この日にドーナツを食べる分の直線の式を追加
        lct.add_line(-i*B, dp[i-1]+D[i] + (i*i-i)/2*B + A*i);
        dp[i] = lct.query(i) + (i*i+i)/2*B - A*i;
    }
    ll ans = dp[N];
    print(ans);
    return 0;
