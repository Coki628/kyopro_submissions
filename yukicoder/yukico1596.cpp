...
・きっちり自力AC！
・二項係数数え上げ、主客転倒、余事象
・いわゆる数え上げ系だけど考察ほぼ詰まらず進められたのは嬉しい。
・全経路を見る問題なので、まともに数えると無理で、
　経路を1本ずつ見ていくのではなく、コストを先に見て、
　そのコストがかかる経路が何本あるかを考える、の方針を取る。
　この手の主客転倒テクは最近意識して使うようにしてたからか結構慣れてきたかもしれん。
・で、コストを見るんだけど、今回はほぼ全体がコスト1で、
　ある特定の場所だけコスト0なので、0のところだけ数えて、
　全部が1であった場合からそれを引く、の方針を取る。
　この手の余事象使うのもよく使われる典型考察よね。
・ここまで来れば、あとは経路の数え上げなので、nCr使っていくのと、
　ある辺が使われる通り数は、入りのマスを通る数*出るマスを通る数、
　ってのが見えれば解ける。
*/
...
void solve() {
    ...
    cin >> N >> M;
    ModTools mt(MOD, N*2);
    mint ans = mt.nCr(N*2, N)*N*2;
    rep(i, M) {
        ll t, x, y;
        cin >> t >> x >> y;
        if (t == 1) {
            ll x2 = x+1;
            ans -= mt.nCr(x+y, x)*mt.nCr((N-x2)+(N-y), N-x2);
        } else {
            ll y2 = y+1;
            ans -= mt.nCr(x+y, x)*mt.nCr((N-x)+(N-y2), N-x);
        }
    }
    print(ans);
}
