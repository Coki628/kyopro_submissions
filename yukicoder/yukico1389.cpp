 ...
 * ・自力AC！
 * ・考察して、累積和する。
 * ・全体の総和が分かっているので、そこから、自分以外の総和Biを引くと、
 * 　自分の元の値(仮)になる。で、この元の値から自分以外の区間和を高速に取れるように累積和を取る。
 * 　この区間和は、どこか1つの要素が誤っているので正しい値にならないが、
 * 　誤った要素のあるところだけは、自分以外の区間和なので正しい値になる。
 * 　誤った要素は正しい値の2倍とあるので、元の値(仮)を/2した値が、
 * 　上記区間和と一致したらそれが答えになる。
 * ・公式解説は何やら難しそうな式変形っぽいことをしてた。でも分かればそっちのが簡単なんだろうな…。
 */
...
int main() {
    ...
    rep(i, N) {
        ll sm = acc[i]-acc[0] + acc[N]-acc[i+1];
        if (B[i]%2 == 1) continue;
        if (B[i]/2 == sm) {
            ll ans = B[i]/2;
            print(ans);
