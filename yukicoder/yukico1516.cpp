...
・自力AC！BM強い。。
・BM、角松列、DP
・愚直DPで100項くらいまで求めておいて、BMに渡す。これで通る。
　想定解はなんか遷移ゴチャゴチャ行列累乗かDPに繰り返し2乗法絡めた難しいやつだった。
*/
...
void solve() {
    ...
    ll N2 = 100;
    ll M = N2*K;
    auto dp = list4d<mint>(N2+1, M+1, K, K+1, 0);
    rep(j, K) {
        dp[1][j][j][K] = 1;
    }
    rep(i, 1, N2) {
        rep(j, M+1) {
            rep(k1, K) {
                rep(k2, K+1) {
                    ...
                    rep(l, K) {
                        if (k2 == K and k1 != l) {
                            dp[i+1][j+l][l][k1] += dp[i][j][k1][k2];
                        } elif ((k2 < k1 and k1 > l or k2 > k1 and k1 < l) and k2 != l) {
                            dp[i+1][j+l][l][k1] += dp[i][j][k1][k2];
                        }
                    }
                }
