/*
・自力AC！解けなくて長らく放置してたやつなので嬉しい。
・nCr数え上げ、重複組み合わせ、主客転倒、寄与、前計算
・まず、実際の操作を考えようとすると指数時間になって無理がある。
　こういうのは主客転倒で見るといい場合が多い。今回なら、
　ある要素が何回のパターンで使われるか、で考える。
・要素を1つ固定すると、各操作K回でl,rがそれぞれどこに置かれれば
　その要素が最後まで残るか、みたいに考えることができて、
　これはその要素の左側と右側で重複組み合わせやって
　nHr(i+1, K)*nHr(N-i, K) とできる。ここにA[i]もかけて、
　これを全要素にやれば答えが出る。
・ただし今回はKが大きいのでこれだけだとダメで、いつも通りMOD逆元の前計算では通らない。
　そこで使われるnCrに注目すると、右辺は常にK固定で、使うパターンがN個くらいしかないので、
　左辺が小さい方から前計算しておけば、答えを求めるパートでは計算量がかからずに済む。　
*/
...
void solve() {
    ...
    vector<mint> nHr(N+1);
    nHr[1] = 1;
    rep(i, 1, N) nHr[i+1] = nHr[i]*(K+i)/i;
    mint ans = 0;
    rep(i, N) {
        // ans += mt.nHr(i+1, K)*mt.nHr(N-i, K)*A[i];
        ans += nHr[i+1]*nHr[N-i]*A[i];
    }
    print(ans);
}
