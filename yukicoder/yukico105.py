"""
・一応自力AC(問題文にヒント書いたあったけど。。)
・幾何、誤差
・座標間の角度をatan2で求めるのを知ってればとりあえず解ける。
・見る基準を(1, 0)に決め打っといて、6つのうち1つはそれっぽいのが出るのでそれが答え。
・ただし誤差がつらい。怒りのDecimalで投げても変わらず。atan2に渡した時結局floatが返るのね。。
・結局角度を出す関数内での0未満判定にEPS入れたら通った。
"""
...
for _ in range(INT()):
    ...
    for i in range(6):
        ...
        # (1, 0),(x, y)間の角度
        res = gm.get_degree((1, 0), (0, 0), (x, y))
        # 50度未満ならここがそうだとみなす
        if 0 < res+EPS and res < 50-EPS:
