/**
 * 参考：https://yukicoder.me/problems/no/599/editorial
 * 　　　https://kmjp.hatenablog.jp/entry/2017/11/25/0900
 * ・蟻本演習4-7-3
 * ・文字列検索、ローリングハッシュ、回文、分割するDP
 * ・区切る位置を決めて、それより手前から貰うDPみたいにする。
 * ・回文判定なので半分までしかやらないのは分かるんだけど、
 * 　最終的に全体の通り数が1箇所に集まってなくて、別途足しわせなきゃいけないのが
 * 　なんとなくモヤっとする。。
 * ・って思ったけど、中途半端なところでDP終わった感じになってるから、
 * 　それで最後に全体から集めてくるのはまあ自然なことか。。
 */
...
int main() {
    ...
    // dp[i] := iの手前で区切る時の分け方の通り数
    vector<mint> dp(N+1);
    dp[0] = 1;
    rep(r, 1, N/2+1) {
        rep(l, 0, r) {
            if (rh.query(s_table, l, r) == rh.query(s_table, N-r, N-l)) {
                dp[r] += dp[l];
            }
        }
    }
