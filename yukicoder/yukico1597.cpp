/*
参考：ツイートとか
・自力ならず。。FFTで殴れたらいいなとか言ってて終わった。。
・二分探索、にぶたんinにぶたん
・MODがなければ億マス計算的なやつで解けるんだけど、
　MODののところで大小関係変わっちゃうから無理じゃん。ってなってた。
・実際はよく見ると、A,Bの要素にP以上のものは出てこないので、
　a+bが取りうる値は2P未満くらいだと分かり、[0,P)と[P,2P)で場合分けして考えれば、
　ある数x以下の要素数を数え上げられる。
・境界がバグりそうですごい嫌になるやつだが、落ち着いて整理すると、
　a+b <= x, a+b < P, a+b <= P+x の3点が見たいので、それぞれ式変形して、
　以下の方をright、未満の方をleftでbisectすればいい。
*/
...
void solve() {
    ...
    sort(ALL(A));
    sort(ALL(B));
    ll res = bisearch_min(-1, P, [&](ll x) {
        ll cnt = 0;
        for (ll a : A) {
            ll l = bisect_right(B, x-a);
            ll m = bisect_left(B, P-a);
            ll r = bisect_right(B, P+x-a);
            cnt += l;
            cnt += r-m;
        }
        return cnt >= K;
