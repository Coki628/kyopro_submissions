/*
・自力AC！
・前に見て考えたけど解けなかったやつ、さっき得た知見をベースに考えたら解けた。
・クエリ先読み、集合2つで効率よく管理
・正確には今回は集合2つじゃなくて片方BITだけど、考え方は近い。
・まず前提として、題意より自分より手前の情報には興味がないので、
　後ろから見て確定していくことにする。また、ある位置より右をチェックする形になるので、
　クエリ先読みで左端毎にクエリを持って後から処理する。
・題意の「高い要素」は、自分より左側に1つでも自分より大きい要素が現れると該当しなくなる。
　また、一度該当しなくなったら、その先左端が伸びても復活することはない。
　なので今見ている要素より小さいものを候補から除いて、
　その状態で今欲しい区間でいくつ該当するかチェックして、
　最後に自分を候補に加える、といった流れで進めていく。
　候補から外れるかどうかのチェックはsetなりで持っておいて、そこに一緒にindexも持たせて、
　BITの方からも同時に削除できるようにする。これでBITでも正しい状態が保持されるので、
　クエリにはそっちから答えればいい。
*/
...
void solve() {
    ...
    vector<ll> ans(Q);
    rep(l, N-1, -1, -1) {
        while (se.size() and (*se.begin()).first < A[l]) {
            auto [a, i] = *se.begin(); se.erase(se.begin());
            bit.add(i, -1);
        }
        bit.add(l, 1);
        se.insert({A[l], l});
