/**
 * 参考：https://www.hamayanhamayan.com/entry/2017/11/25/014723
 * 　　　https://rustforbeginners.hatenablog.com/entry/yukicoder-599
 * ・蟻本演習4-7-3
 * ・文字列検索、ローリングハッシュ、回文、区間DP
 * ・こっちの方針のが直感的にはしっくり来るかも。
 * ・一番外側から始めて、中央まで区間を狭めて潜って、中央から確定していって、
 * 　外側を決める時には、その位置より内側の区間で条件を満たすもの全てから遷移させる。
 * ・N<=1万で2乗なので計算量は厳しめ。普通に区間DPと考えると、状態N^2の遷移Nで無理だが、
 * 　添字l,rは同時に動いて、全ての場所には遷移しないので状態は高々Nになる。
 * ・それでもメモをmapで持ったりでlogが乗っているのもあり、なんとかAC1.88秒。(でも制約4秒)
 */
...
int main() {
    ...
    // memo[{l, r}] := 区間[l,r)での通り数
    map<pll, mint> memo;
    auto rec = [&](auto&& f, ll l, ll r) -> mint {
        if (memo.count({l, r})) return memo[{l, r}];

        // 区間長1or0は条件を満たしているとみなして1
        mint res = 1;
        ll i = l+1, j = r-1;
        while (i <= j) {
            // [l,r)より内側の区間について、条件を満たすなら遷移させる
