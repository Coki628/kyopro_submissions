 ...
 * ・文字列検索、ローリングハッシュ、LCP
 * ・自力AC！
 * ・かなり厳しめの制約から、ロリハ解を落とそうという意思が伝わってくるが通ってしまった。。
 * ・300万クエリに文字列長総和80万のlogが乗ってくるが、AC0.75秒。(制約1秒)
 * 　まあ、これ以上きつくしちゃうと他の言語で想定解まできつくなったりすんだろうね。。C++は強い。
 */
...
int main() {
    ...
    cin >> N;
    vector<string> S(N);
    vector<vector<u_int64_t>> table(N);
    RollingHash rh;
    rep(i, 0, N) {
        cin >> S[i];
    ...
    rep(i, 0, M) {
        A[i]--;
        B[i]--;
        ans += rh.lcp(table[A[i]], 0, S[A[i]].size(), table[B[i]], 0, S[B[i]].size());
    }
    print(ans);
    return 0;
}
