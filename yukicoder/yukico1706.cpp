/*
・ライブラリ整備：BM
・なんとか自力AC！大変だったけど、解けてよかった。
・BM、確率DP、数え上げ
・まず愚直なDPを考えて、時間差で到着する部分は、移動中か到着中みたいな状態2つを持つ。
　で、しばらくはうまく複数台のバスに対する確率をDPに乗せられず悩んだんだけど、
　どのバスも同等のものと考えられるから、じゃあ1台でやって最後にM台全部の確率にできる？
　って気付いて考察が進んだ。
・1台なら普通に1=100%の状態から始める確率DPなんだけど、
　C個のバス停を全部状態に持つと、結局それだけで10万あるので間に合わない。
　ここで、駅1以外の駅は、これもまた同じものがたくさんあるだけと考えることができると気付く。
　これで駅の状態は駅1かそれ以外かの2通りにまとまった。
　それ以外駅から移動する時の遷移がちょっとややこしい。
・これである1台のバスが最終的にバス停1にいる確率は分かった。
　この先もまあ数え上げ考察が必要。欲しいのはバス停1に1台以上いる確率だが、
　○○以上はだいたい求めにくいので、まず1台もいない確率から考える。
　これならさっきの1台分のバス停1にいる確率の余事象で取れる。
　M台全て条件を満たさないといけないので、M回掛ける。
　これでM台全てがバス停1にいない確率が取れるので、さらに余事象で答えになる。
・ここまで詰めると、結局状態が大きいのはNだけになって、
　遷移は4*4の行列に乗るので、行列累乗に乗せて第N項を求めればいい。
　今回はBMのverifyが目的なので、適当に100回分くらいの項をDPで取って、それをBMに突っ込む。
・この使い方をする限り、一連の流れは同じっぽいのでBMで関数化しといた。
*/
...
void solve() {
    ...
    ll N2 = 100;
    FPS<mint> A;
    auto dp = list3d<mint>(N2+1, 2, 2, 0);
    dp[0][0][0] = 1;
    rep(i, N2) {
        rep(j, 2) {
            rep(k, 2) {
                ...
                if (j == 0 and k == 0) {
                    // 留まる
                    dp[i+1][0][0] += dp[i][j][k]/C;
                    // バス停1以外へ
                    dp[i+1][1][1] += dp[i][j][k]*(C-1)/C;
                }
                // バス停1以外から
                if (j == 1 and k == 0) {
                    // 留まる
                    dp[i+1][1][0] += dp[i][j][k]/C;
                    // バス停1へ
                    dp[i+1][0][1] += (dp[i][j][k]*(C-1)/C)/(C-1);
                    // それ以外へ
                    dp[i+1][1][1] += (dp[i][j][k]*(C-1)/C)*(C-2)/(C-1);
                }
            }
        }
        A.eb(dp[i+1][0][0]);
        // rep(j, C) {
        //     dp[i+1][j][0] += dp[i][j][0]/C;
        //     dp[i+1][j][0] += dp[i][j][1];
        //     rep(k, C) {
        //         if (k == j) continue;
        //         dp[i+1][k][1] += dp[i][j][0]/C;
        //     }
        // }
    }
    auto res = BM(A, N-1);
    ...
    // あるバスがバス停1以外にいる確率
    mint p = (mint)1-res;
    // 全てのバスがバス停1以外にいる(バス停1に何もいない)確率
    mint pall = p.pow(M);
    // バス停1にバスが1台以上いる確率
    mint ans = (mint)1-pall;
    print(ans);
}
