/*
参考：https://jupiro.hatenablog.com/entry/2020/09/22/061642
　　　https://qiita.com/DaikiSuyama/items/a55f9884e4cb6bd31ab7#d%E5%95%8F%E9%A1%8C
・昔解けなくて寝かしてたやつ。最近類題来たと思ったらそうでもなかった。解説AC。
・フェルマーの小定理
・使う論点としてはyukico1140が類題っぽいので、そちらのコメントも参照。
　合同式の右辺を1にすることで左辺の指数に変数作って、そこから式変形して解く。
　2^(p-1)n ≡ 1 と、求めたい形である 2^(p-1)n ≡ (p-1)n (x=(p-1)nと置く) を使って、
　(p-1)n ≡ 1 になるnを探すと、これはp-1の逆元とできる。
　このnが分かれば x = (p-1)*n はすぐ求まる。
・p = 2 の時は2冪と互いに素でないのでこれが使えないが、サンプルにあるし、それ1つくらい探してもいい。
*/
