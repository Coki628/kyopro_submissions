/*
・きっちり自力AC！
・まず偶奇場合分け、偶数は隣り合う数のGCDが必ず1になる(よね？)性質を使う。
　奇数はmod 4で1なら(N-1)/2から+-1、
　mod 4で3なら+-2ってやるといい感じに互いに素っぽい奇数同士に。
　奇数同士とはいえ一発で決まるとも限らなそうだから、
　和が変わらないように+2,-2してgcdが1になるまでやる、ってした。
　制約でNが10以上だったので、極端に小さくておかしくなるのはないと踏んだ。
・実際は互いに素なペアなんてたくさんあるので、
　もっと適当に雑に探しても大丈夫だったらしい。。
*/
...
void solve() {
    ...
    assert(ans[0]+ans[1]+ans[2] == N);
    assert(gcd(ans[0], ans[1]) == ans[2]);
    print(ans);
}
