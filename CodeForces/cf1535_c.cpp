/*
・きっちり自力AC！
・ある左端から見て右にどこまで伸ばせるかを前計算する方針。
　基本的には?のところは自由に進めて0,1のところは交互になる必要がある。
　ただし?でも一度交互を始めた後は0,1に縛りがあるので、
　次に0,1が来た時には続けられるかの判定が要る。
　また、伸ばせる右端が途切れてリセットされた時に、
　その位置から先がちょうど?だったらその分は伸ばせるので、
　右に?がいくつ続くかも別途前計算。
・これで考慮足りてるか自信はなかったけど通ったのでまあOK。。
*/
