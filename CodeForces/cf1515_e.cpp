/*
・何も方針が立たず、とりあえずこれは愚直解。
　N=8くらいまで見てOEISにぶん投げたけど何も出ず。。
*/
...
void solve() {
    ...
    ll ans = 0;
    auto rec = [&](auto&& f, vector<ll> &cur) -> void {
        if (sum(cur) == N) {
            ans++;
            return;
        }
        rep(i, N) {
            ...
            if (cur[i]) continue;
            cur[i] = 1;
            if (i >= 2 and cur[i-2] and cur[i-1] == 0) cur[i-1] = 1, f1 = 1;
            if (i < N-2 and cur[i+2] and cur[i+1] == 0) cur[i+1] = 1, f2 = 1;
            f(f, cur);
            cur[i] = 0;
            if (f1) cur[i-1] = 0;
            if (f2) cur[i+1] = 0;
