/*
・みんなの方針。以下、自分の方針で使った考えと照らし合わせていく。
・「なるべく通る頂点ペアが少ない辺」というのは、これは葉と繋がる辺そのもので、
　であれば隣接リストのサイズチェックだけですぐ個数が出る。
・「これらの辺が最も多く使われるパス」というのは、これは木なので、
　葉を通るのはどんなパスであれ最大2回で、どんな木でも絶対2回通るパスはある。
　(これも葉だと分かってしまえば結構自明に納得がいく)
・以上より、S*2/葉の数 が答え。道理でみんな早かったわけだ…。
*/
...
void solve() {
    ...
    ll leafcnt = 0;
    rep(i, N) {
        if (nodes[i].size() == 1) leafcnt++;
    }
    ld ans = S*2/(ld)leafcnt;
    print(ans);
}
