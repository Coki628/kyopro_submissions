 ...
 * ・自力AC！
 * ・素因数分解、osa_k法、クエリ系
 * ・全体GCDだと全ての要素がその素因数を持ってるのが条件になるので、各要素を素因数毎に管理。
 * 　それとは別に、全体のカウントとしての
 * 　mp[{a, b}] := 素因数aをb個持っている要素の数
 * 　みたいなのを作って、このカウントがNになったら、答えに*aする、といったことをした。
 * ・20万の内側で√Nかけて素因数分解してるので計算量は心配。システス通るかなー。。
 * ・通った！！やったね。制約2.5秒でAC1.62秒。やっぱ素因数分解の√くらいならまあまあ軽いのかな。
 * ・せっかくosa_k法の素因数分解作ったのでこっちでもやってみた。
 * 　やっぱり全然速くなって、AC1.62秒→0.58秒。
 */
...
void solve() {
    const ll MX = 200000;
    auto table = eratosthenes_sieve(MX);
    ll N, Q;
    cin >> N >> Q;
    auto A = LIST(N);
    ...
    rep(_, Q) {
        ...
        cin >> i >> x;
        i--;
        for (auto [k, v] : factorize(table, x)) {
            rep(j, 1, v+1) {
                mp[{k, C[i][k]+j}]++;
                if (mp[{k, C[i][k]+j}] == N) {
                    ans *= k;
