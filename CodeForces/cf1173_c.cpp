/*
・自力ならず。むずかった。
・まず、元の状態から続けて達成できる場合と、
　新たに1から順番に置き直さないといけない場合で場合分けする。
・後者の場合はsetとかうまく使って、どこまで待ってから
　1を置き始めないといけないか、で確認できる。こっちはほぼできてた。
　と思ったらできてなかった。chmaxする前の初期値を0にしてたんだけど、
　1から新たに置き直さないといけないので操作回数がNを下回ることはなく、
　逆に0だと1枚引いてすぐ置いて終了、みたいのでWAになったのでダメだった。。
・前者の場合のがシミュレーションがむずかった。
　これもsetとかうまく使って状態管理する。詳細はソース内コメントにて。
*/
...
void solve() {
    ...
    rep(i, N) {
        if (B[i] == tmp) {
            // 1から連続している場所をチェック
            if (tmp == 1 or (i != 0 and B[i-1] == B[i]-1)) {
                tmp++;
                good[i] = 1;
            } else {
                break;
    ...
    // ここが0だとダメで、こっちでNを下回ることはないのでNから始める
    ll ans = N;
    rep(i, N) {
        if (B[i] == cur) {
            chmax(ans, N+i+1-(B[i]-1));
        }
        have.insert(B[i]);
        while (have.count(cur)) cur++;
    }
