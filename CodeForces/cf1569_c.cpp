/*
・自力AC！苦手系だったけどちゃんと通せたのは良かった。
・nCr系数え上げ、余事象
・まずどういうのが該当するか考えると、最大値の要素が2つ以上あったら自明に全通りOK。
　最大値要素が1つとして、2番目に大きい要素が2以上離れた値だと、
　最後に最大値要素がどうやっても残ってしまって全てNG。
　2番目要素の差が1なら、それを1つでも最大値より右に置けばOKとなる。これをどうにかして数え上げる。
・そこで、これは余事象の方が数えやすかった。そっちで考えると該当するのは、
　2番目要素が全て最大値より左にあるパターン。最大値と2番目以外はどこでもいい。
　ここから、最大から2番目要素の数をcnt1、最大値でも2番目でもないやつの数をcnt2と置く。
　数え方に2つ方針があって、1つはN個の席がある所に、先に置き方が自由なcnt2を置ききって、
　残ったN-cnt2席を、左にcnt1、右に最大値が来るように置くとするもの。
　N個からcnt2個を並べて、cnt1と最大値の並びは一意、cnt1は複数あれば順序自由なのでここも並びを掛ける。
　式にすると nPr(N, cnt2) * cnt1!
　もう1つは、先に確定しているcnt1と最大値を置いて、それらの隙間にcnt2個の要素を自由に並べるもの。
　cnt1個の並びを決めて、置く隙間の数はcnt1+2箇所からN箇所までcnt2回、1ずつ増えていく。
　式にすると cnt1! * nPr(N, cnt2) 
　どちらで考えても、同じになる。これが余事象なので、全体となるN!からこれを引けばOK。
*/
...
void solve() {
    ...
    ll cnt2 = N-cnt1-1;
    mint ans = mt.fact[N] - mt.nPr(N, cnt2)*mt.fact[cnt1];
    print(ans);
}
