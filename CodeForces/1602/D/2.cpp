/*
・結構みんな区間辺使ってたみたいなので、そっちでも解いてみた。
・区間に辺を張るテク、復元
・前の頂点をどう持たせるかと思ったけど、ちょうどコストが抽象化されていたので、
　そこに頂点番号も持たせた。区間辺テクはコスト0の辺も色々張ってうまいこと区間を表現してるぽいので、
　本来の辺からの遷移以外では、遷移元の持つ前の頂点の情報を引き継ぐ形にしたらうまくいった。
・ちなみに30万頂点で、Beatsが0.44秒、こっちが0.72秒。Beatsのがちょい速いけど30万でこれなら全然使える。
　この問題で使えるなら、Beats使いたくなるような区間chminしたいDPで、結構幅広く使えるような気もする。
*/
...
struct cost {
    ...
    cost(ll w) : w(w) {}
    cost(ll w, int id) : w(w), id(id) {}

    cost operator+(const cost &c) const {
        // 辺が元の頂点番号を持っていれば使う、なければ前のを引き継ぐ
        return {this->w+c.w, c.id == -1 ? this->id : c.id};
    }
...
void solve() {
    ...
    reverse(ALL(A));
    reverse(ALL(B));
    range_edge_graph<cost> reg(N+1);
    rep(i, N) {
        ll l = i-B[i];
        ll r = l+A[l];
        reg.add_edge(i, i+1, l+1, r+1, {1, i});
    }
