...
・自力AC！！
・Beats、区間chmin、復元
・貰うDPにするうまい方法が出なかったので、Beatsで区間chminした。
　Beatsにペアを持たせる術はなかったけど、グリッド1次元にする時みたいにしてIDも持たせれば、復元もできた。
*/
...
void solve() {
    ...
    reverse(ALL(A));
    reverse(ALL(B));
    SegmentTreeBeats seg(N+1);
    seg.update_val(1, N+1, INF);
    seg.update(0, 0);
    rep(i, N) {
        ll l = i-B[i];
        ll r = l+A[l];
        if (seg[i] == INF) continue;
        auto [val, j] = idtogrid(seg[i], N+1);
        seg.update_min(l+1, r+1, gridtoid(val+1, i, N+1));
        // seg.print(N+1);
    }
    ll i = N;
    vector<ll> ans = {N-i};
