/*
・自力ならず。終了後テストケース見て色々粘ってAC。
・MOD累積和、二分探索
・まじでMOD累積和の添字バグらせがやばい。。
・元々方針が微妙に良くなかった。左端固定で右端伸ばす感じにしようとしてたけど詰められず。
　結局部分列に含まれるのって素数1個と周囲に連続した1だけなので、
　素数を軸に左端と右端を1が連続する限り伸ばす、ってやった。
　これを色々やってまあACはしたんだけど、本当にバグらせ過ぎた。
　MOD累積和だけでもややこしくなるのに、にぶたんしないといけなかったから、
　境界とかの管理がすごく大変になってしまった。
*/
...
void solve() {
    ...
    rep(i, N) {
        if (primes[A[i]]) {
            ll di, mo;
            tie(di, mo) = divmod(i, X);
            ll res = bisearch_min(-1, di, [&](ll m) {
                return acc1[i+X]-acc1[X*m+mo] == di-m;
            });
            ll lcnt = acc1[i+X]-acc1[X*res+mo];
            res = bisearch_max(di+1, M/X, [&](ll m) {
                return acc1[X*m+mo]-acc1[i+X] == m-(di+1);
            });
            ll rcnt = acc1[X*res+mo]-acc1[i+X];
            ans += (lcnt+1)*(rcnt+1)-1;
        }
    }
    print(ans);
