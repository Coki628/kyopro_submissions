...
・自力AC！
・累積XOR、区間XOR
・最初、全XORだけ確認してWA。。これだと同じやつ2つのパターンはカバーするけど、
　題意は同じやつ3つ以上でもいいんだよね。。
　通した時は制約から2乗できるなってエスパーして、
　2点固定3つ区間全探索で通したけど、今落ち着いて考えると、
　多分4つ同じようにできるなら2つにもできるし、5つなら3つにできるから、
　区間数2と3だけ確認すればいいんだね、って思ってる。
*/
...
void solve() {
    ...
    rep(i, N+1) {
        rep(j, i, N+1) {
            if (acc.query(0, i) == acc.query(i, j) and acc.query(i, j) == acc.query(j, N)) {
                YES();
                return;
            }
        }
