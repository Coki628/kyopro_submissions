...
・和が奇数なら即0、部分和DPしてsm/2があるなら、
　奇数の要素があればそれを除く、偶数しかない時は…分からず。
　最小値取ってWAになるけど反例が思いつかんー、ってしてた。
・部分和DP
・一見計算量渋そうに見えるけど、N=100で各要素2000だと、
　2000*100*100=2000万のDPなので全然間に合う。
・さて、その先の正解は、2で割れる回数が少ないものを使う、だった。
　まあ奇数が最善って考えると、それも確かになんだけど、
　多分こっちの方針だと本質が見えづらくて、
　もう1つみんなが多くやってた、先にGCDで割る、が良かったんだと思う。
　2つのグループに分けるのに、全要素が2で割れるなら状態は変わらないから割る。
　これをやれるだけやるとどこかが奇数になるので、そこが使えると分かる。
*/
...
void solve() {
    ...
    auto dp = list2d(N+1, sm+1, false);
    dp[0][0] = 1;
    rep(i, N) {
        rep(j, sm+1) {
            if (dp[i][j]) {
                dp[i+1][j] = dp[i][j];
                if (j+A[i] <= sm) dp[i+1][j+A[i]] = dp[i][j];
            }
        }
    }
    if (dp[N][sm/2]) {
        ...
        ll idx = min_element(ALL(B))-B.begin();
        print(1);
        print(idx+1);
    } else {
        print(0);
