...
・きっちり自力AC！
・ビット毎に見て桁DP、数え上げ
・上位ビットから順に値を配置していくとすると、
　ある時点でandとxorの大小関係が確定して、それに基づいて状態管理できる。
　全ビット立ってるかどうか、立ってるビットの数の偶奇
　の2点に注目して場合分けすれば、適切に遷移を作れる。
*/
...
void solve() {
    ...
    vector<mint> dp0(K+1), dp1(K+1);
    dp0[0] = 1;
    mint half = pow(2, N-1, MOD);
    rep(i, K) {
        if (N%2 == 0) {
            // 全ビット立ちかつ偶数
            // and > xor
            ...
            // その他で偶数
            // and == xor
            dp0[i+1] += dp0[i]*(half-1);
            dp1[i+1] += dp1[i]*(half-1);
            // その他で奇数
            // and < xor
            dp1[i+1] += dp1[i]*half;
        } else {
