/*
・きっちり自力AC！
・dp[i][j] := 文字列Tをi番目まで見て、文字列Sで今見ているのがj番目の時の到達可否
　みたいなやつを行きと帰りで2回やった感じ。
*/
...
void solve() {
    ...
    auto dp0 = list2d(M+1, N+1, false);
    auto dp1 = list2d(M+1, N+1, false);
    rep(i, N) {
        dp0[0][i] = 1;
    }
    rep(i, M) {
        rep(j, N) {
            if (dp0[i][j] and T[i] == S[j]) {
                dp0[i+1][j+1] = 1;
                dp1[i+1][j] = 1;
            }
        }
    ...
    rep(i, M) {
        rep(j, N-1, -1, -1) {
            if (dp1[i][j+1] and T[i] == S[j]) {
                dp1[i+1][j] = 1;
            }
        }
    }
    
    ll ans = sum(dp0[M])+sum(dp1[M]);
    if (ans) YES();
    else NO();
}
