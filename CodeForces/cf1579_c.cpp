...
・きっちり自力AC！
・グリッド、実装系
・あんまり解かれてなくて最初飛ばしちゃったんだけど、ちゃんと見たら結構すぐ解けた。
・V字の谷に当たるマスを軸にして、そこから伸ばせるだけ伸ばす、を全マスで試す、ってやった。
　それがK以上に行くなら塗る。上書きOKなので、やれるとこ全部やっていい。
　これで元のグリッドと一致させられればOK。
*/
...
void solve() {
    ...
    auto check = [&](ll h, ll w) -> ll {
        ll res = 0;
        ll h1 = h;
        ll w1 = w, w2 = w;
        while (1) {
            w1--, w2++, h1--;
            if (w1 < 0 or w2 >= W) break;
            if (h1 < 0) break;
            if (grid[h1][w1] == '.' or grid[h1][w2] == '.') break;
            res++;
        }
        return res;
    };
    ...
    auto paint = [&](ll h, ll w, ll k) -> void {
        ll h1 = h;
        ll w1 = w, w2 = w;
        ans[h][w] = '*';
        rep(_, k) {
