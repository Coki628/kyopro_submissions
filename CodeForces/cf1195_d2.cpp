/*
・なんとか自力AC！間に合ってよかった。
・ペアになる相手方が何桁あるかで集計してまとめておく。
　相手の桁数が分かれば、自分はどこの桁がどこに使われるか分かる。
　自分の桁が少ない時はD1と同じ感じ、多い時はそれが全部大きい桁に付く。
　大きい部分は左辺の時と右辺の時両方で使われるから*2する。(デバッグで気付いた)
　10^9だから10桁あるのにmax9桁と思い込んでWA。。まあこれも気付けてよかったけど…。
*/
...
void solve() {
    ...
    rep(i, N) {
        ...
        reverse(ALL(s));
        rep(len2, 1, MXD+1) {
            ll mnlen = min(len, len2);
            ll mxlen = max(len, len2);
            rep(j, mnlen*2) {
                ans += toint(s[j/2])*pow(10, j, MOD)*C[len2];
            }
            if (len > len2) {
                rep(j, mnlen*2, len+len2) {
                    ans += toint(s[j-len2])*pow(10, j, MOD)*C[len2]*2;
                }
            }
        }
    }
