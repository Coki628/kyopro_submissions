/*
参考：https://twitter.com/laycrs/status/1408483554107953153
・自力ならず。。考察出ないー。。
・主客転倒、寄与
・まず重要な考察として、与えられる数列は頂点1からの最短距離で、
　0,3,9,6とか適当に並ぶんだけど、これをソートしたものを
　一直線に次の頂点に向かう最短ルートとみなしていい。
　言われてみると確かにそうなんだけど、本番中は、負辺を通って
　距離の小さい方に移動したらゴチャゴチャするんじゃないかとか考えてダメになった。
・この一直線の最短路が作れるとあとは結構考えてた通りで、
　この最短ルートに当たる部分の逆辺に、ちょうど+-0になる辺を張る感じ。
　他もそれと同等に当たるコストをそれぞれ張っていくんだけど、
　これは辺を1本ずつ見るのは無理だけど、
　一直線になってるから位置毎にコストをまとめられて、
　例えばi,i+1間を通る辺が何本あるかは、
　そこより左の頂点数*右の頂点数みたいに求められるので、
　実際の答えは最短路の分で1本は相殺されちゃうので-1して、
　これをその辺のコストと掛けていけば答えが出る。
*/
...
void solve() {
    ...
    ll ans = 0;
    rep(i, N-1) {
        // ここに張る辺のコスト*(ここより左の頂点数*右の頂点数-1)
        ans += (A[i]-A[i+1])*((i+1)*(N-i-1)-1);
    }
    print(ans);
}
