/*
・自力ならず。。
・基本的には、0なしを先に分けて、後は0が出る位置が左か右かで場合分け。
・惜しかった。だいたいの方針は合ってたのに、
　詰めのところで倍数とビットの関係を勘違いしてしまったため、最後まで合わなかった。
　右に0が出るパターンの時に、末尾桁0で合わせれば以降は2の倍数でいけるとか思ったんだけど、
　確かに両方2の倍数だけど、一方が他方の倍数とは限らないよね。。
　逆に0を含むかの方でずらせば、これはビットシフトしたみたいな形になるから
　ちょうど常に2倍になるんだね。。
*/
...
void solve() {
    ...
    if (not count(ALL(S), '0') or not count(ALL(S), '1')) {
        vector<ll> ans = {1, N-1, 2, N};
        print(ans);
        return;
    }
    ...
    if (idx < N/2) {
        idx++;
        vector<ll> ans = {idx, N, idx+1, N};
        print(ans);
    // 右の方に0
    } else {
        idx++;
        vector<ll> ans = {1, idx, 1, idx-1};
        print(ans);
    }
}
