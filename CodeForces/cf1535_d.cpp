/*
・終了後20分くらいで自力AC。間に合わなかった。。
・セグ木応用、二分木、2冪、クエリ系
・セグ木でやるようなことを、セグ木の外でやる。
　試合結果の文字列は、二分木の頂点と対応させることができて、
　結果が更新されたら、自分の親側にだけ更新を反映させる。
　ただし番号の対応付けは文字列をリバースさせた方が高さが合う。
　これだと同じ高さ同士の左右も逆転してしまうが、
　それは更新時の左辺と右辺の勝利関係の逆転させればOK。
・終わってみればもっと早く詰めれそうに思えるのに、
　実際はめちゃくちゃ時間かかった。ダメだなー、まだまだ。
*/
...
struct Node {
    ...
    Node(ll cnt, char c) : cnt(cnt), c(c) {};
...
void solve() {
    ...
    rep(i, 1, 2*sz) {
        if (S[i] == '?') nodes[i] = Node({2, S[i]});
        else  nodes[i] = Node({1, S[i]});
    }
    for (int k=sz-1; k>0; k--) {
        // nodes[k] = f(nodes[2*k], nodes[2*k+1]);
        if (nodes[k].c == '?') {
            nodes[k].cnt = nodes[2*k].cnt+nodes[2*k+1].cnt;
        } elif (nodes[k].c == '0') {
            nodes[k].cnt = nodes[2*k+1].cnt;
        } else {
            nodes[k].cnt = nodes[2*k].cnt;
        }
    }
