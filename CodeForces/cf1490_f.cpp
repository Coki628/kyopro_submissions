/**
 * ・自力AC！
 * ・状態を持ち替えて累積和を取ると、各個数iについてO(1)チェックできるようになるのでこれで見る。
 * ・個数毎にまとめておくと、個数iに揃えるために必要な処理は、
 * 　自分より少ない数を全部消す、と、自分より多い数の多い部分を消す、なので、
 * 　前者は普通に区間和、後者は区間和から自分の数*区間長を引けば出る。
 */
...
void solve() {
    ...
    vector<ll> acc(M+1);
    rep(i, M) acc[i+1] = acc[i]+B[i];
    B.insert(B.begin(), 0);
    ll ans = INF;
    rep(i, 1, M+1) {
        ll lsm = acc[i-1]-acc[0];
        ll rsm = acc[M]-acc[i-1]-B[i]*(M-i+1);
        chmin(ans, lsm+rsm);
    }
    print(ans);
}
