...
・きっちり自力AC！
・幾何、式変形、場合分け、ビット全探索
・大昔になぜか頑張って解いたやつだったけど、まあほとんど方針は覚えてなかったしOK。
・結局、直接マンハッタン距離で行くか、斜めの線を使うかなんだけど、
　入る時x or y方向に斜めの線に入る、出る時x or y方向に斜めの線から出る
　みたいな4通りが考えられて、これ全部if文で場合分けするとやばいやつで、
　ビット全探索でうまいことまとめるよう努めた。
　この手の4通りとか8通りくらいの場合分けで今まで地獄実装に走りがちだったので、
　落ち着いてビット全探索にまとめられたのは良かったと思う。
　過去解いたやつ見たけど、相当やばになってたもん。。
・終了後、色々整理。幾何クラス使ったりfor文にまとめたり。
*/
...
void solve() {
    ...
    rep(bit, 1<<2) {
        ...
        ld dist = P[0].manhattan(ST[0]) + ST[0].abs(ST[1]) + ST[1].manhattan(P[1]);
        chmin(ans, dist);
    }
    print(ans);
}
