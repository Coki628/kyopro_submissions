/**
 * 参考：https://codeforces.com/contest/1479/submission/106794498
 * ・遅延セグ木でDP
 * ・区間加算、区間最大値取得、1点更新
 * ・実はD2の方とほぼ同じDPをすることでD1も解ける。
 */
...
int main() {
    ...
    rep(i, 1, N+1) {
        ...
        ll mx = max(max(seg.query(0, A[i])+1, seg.query(A[i]+1, N+1)+1), seg[A[i]]);
        if (A[i-1] != A[i]) {
            // 全体を+1
            seg.update(0, N+1, 1);
        }
        // 位置i-1にある要素を更新
        seg.update(A[i-1], mx-seg[A[i-1]]);
    }
