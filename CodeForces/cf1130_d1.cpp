...
・終了後自力AC！
・ソートして貪欲シミュレーションする。
　先に到着できるようなものが後になるように、順番に駅で荷物を取る。
・設定が、いくらでも乗せられるけど積み下ろしは1つずつみたいな話だと思ってたら
　積み込みは1つずつだけど、下ろすのは1回でまとめていけたみたい。
　そういう風に直したらサンプル合って、投げたら無事AC。。
*/
...
void solve() {
    ...
    rep(x, N) {
        ...
        while (1) {
            if (not cur[i].empty()) {
                auto [d, b] = cur[i].back(); cur[i].pop_back();
                se.insert(b);
            }
            if (se.count(i)) {
                cnt += se.count(i);
