/*
参考：https://twitter.com/ricky_quikem/status/1439992844308004866
　　　https://img.atcoder.jp/abc123/editorial.pdf
　　　https://codeforces.com/contest/1574/submission/129365181
・自力ならず。むずいけどためになったな。
・鳩の巣原理、上位K個の列挙、優先度付きキュー
・鳩の巣っぽいの自分で気付いたのは初めてかも。禁止の条件はM個なので、
　大きい方からM+1個以上列挙すれば必ず当たる。とまでは思ったんだけど、列挙の方法が分からず。。
・やり方はABC123dの解説に書いてあった。りきぽんさんのツイートより。
　最大値の状態から、1つずつ動かして状態遷移させたものをpriqueに詰める。
　こうすると最適な方から優先して次の遷移元とできるので、これを必要な数集まったら打ち切ればいい。
・ちなみに、setの中身がvector<ll>よりpair<ll, vector<ll>のが速いのが以外だった。
　前者は1.28秒、後者は1.75秒くらい。最初からvector比較になるより、値比較が先に入った方がいいってことか。
　あとll -> intもまあ予想通りちょっと速くなる。それで1.19秒くらい。
・さらにもう一押し。vectorの代わりにarrayを使った。固定長なのでNと合わせられなくてもやっとするけど、
　やっぱり速くなってAC0,8秒。
*/
