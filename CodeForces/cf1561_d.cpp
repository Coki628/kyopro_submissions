/*
・これはMLE。今回メモリの制約がきつくなってて、(128MB)
　割り算の遷移を予め陽に作っておくとダメだった。
　pllをpiiにしたり、piiをll1つで持ったりしたんだけど変わらず。。
*/
...
void solve() {
    ...
    rep(i, 1, N+1) {
        ...
        while (i/j > 1) {
            ll d = i/j;
            ll k = bisearch_max(j, N+1, [&](ll m) {
                return i/m == d;
            });
            nodes[d].pb(i*MOD + k-j+1);
            j = k+1;
        }
        nodes[1].pb({i*MOD + i-j+1});
    }

    vector<ll> dp(N+1), acc(N+1);
    dp[N] = acc[N] = 1;
    rep(i, N-1, 0, -1) {
        dp[i] += acc[i+1]%M;
        dp[i] %= M;
        for (auto p : nodes[i]) {
            auto [j, c] = divmod(p, MOD);
            dp[i] += (dp[j]*c%M)%M;
            dp[i] %= M;
        }
        acc[i] += (dp[i]+acc[i+1])%M;
        acc[i] %= M;
    }
    ll ans = dp[1];
    print(ans);
