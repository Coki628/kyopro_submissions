/*
参考：https://twitter.com/shojin_pro/status/1465010106525884421
・自力ではないかな。。TL見てなきゃ問題理解ができたか分からん。
・UF応用
・問題文が難読。でもCに粘着しないでこっち取り組んだら理解できたかなぁ？
・ようは必須で繋ぐ人の条件が決まっていて、それらを全て繋いだ上で、
　最大連結成分の要素数を最大化する感じ。これを1～M辺までの各パターン求める。
・やりたいことが分かってしまえば、2乗が効く制約なので、
　UF使って愚直シミュみたいにできる。毎回新しくUF作って、
　必須の連結をやって、余った回数で大きい連結成分から貪欲に繋ぐみたいな。
　最大連結成分がどれなのかは、一応効率良く取得できるように
　setなりに詰めてUFの連結状況と合わせて管理しておく。
　set側の操作をする前にUFからroot取得するのを忘れずに。
*/
...
void solve() {
    ...
    rep(i, M) {
        ...
        rep(_, cnt) {
            ll a = se.rbegin()->second;
            ll b = next(se.rbegin())->second;
            se.erase({C[a], a});
            se.erase({C[b], b});
            ll r = uf.merge(a, b);
            C[r] = C[a]+C[b];
