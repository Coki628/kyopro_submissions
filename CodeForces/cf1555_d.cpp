...
・きっちり自力AC！
・回文、累積和、前計算、クエリ
・使える文字は3種類しかないので、3文字で繰り返す6パターン以外に正解がない。
　これらに持っていくためのコストを前計算しておいて、各クエリは6つ試して最小を取る。
*/
...
void solve() {
    ...
    vector<Accumulate<ll>> acc(L);
    rep(i, L) {
        acc[i] = Accumulate<ll>(cost[i]);
    }
