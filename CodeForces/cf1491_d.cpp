 ...
 * ・なんとか自力AC！
 * ・グラフとビット
 * ・近い方針まで出ていたけど詰めきれず、と思ったら最後の最後で解法ガチャ当ててAC。
 * 　最後まで諦めないの大事ね。
 * ・とりあえず条件から構成されるグラフを考えると、上位ビットに1個ずらすみたいなのができると分かる。
 * 　元の値より小さくはできないこと、立ってるビットを増やすことはできないこと、
 * 　下位ビットから貪欲に増やしていったとして、それでも1にできないビットがあったらNG、
 * 　を必要条件として並べていったら通った。。
 */
...
void solve() {
    ...
    rep(i, Q) {
        ...
        if (v < u or popcount(u) < popcount(v)) {
            NO();
            continue;
        }
