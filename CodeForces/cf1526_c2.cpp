/*
・なんとか自力AC！危なかった。。
・遅延セグ木、区間最小値取得・区間加算、ソートして貪欲
・正の数は自明に使える。全部使ったものとして累積和しておいて、
　負の数は絶対値の小さい方から順で、ソートして順番に使ってみる。
　(同値だったらindex大きい順、これは必須から分からんが念のため)
　そこから先で0未満になってしまう場所があるなら見送るし、ないなら使う。
　0未満チェックは区間最小値取得、累積和の状態更新は区間加算でできるので、
　遅延セグ木でぶん殴る。多分、もっと簡単にできるやつ。。
*/
