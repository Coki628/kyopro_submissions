...
template<typename Monoid, typename F>
SegmentTreeIndex<Monoid, F> get_segment_tree(int N, const F& f, const Monoid& M1) {
    return {N, f, M1};
}

template<typename Monoid, typename F>
SegmentTreeIndex<Monoid, F> get_segment_tree(const F& f, const Monoid& M1) {
    return {f, M1};
}
