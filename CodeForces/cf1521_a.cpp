/*
参考：https://twitter.com/MohutonLab_comp/status/1390710978765942789
　　　https://twitter.com/titia_til/status/1390711989840666630
・自力ならず。。
・式変形、和と積
・以下、なるべく自分でも思い付けそうな思考プロセスに噛み砕く。
・x,y,zともAの倍数であることを考えると、
　A*?(Bの倍数でない)+A*?(Bの倍数でない) = A*B*?(何でも)
　の形を取りたくなる。ここで、B+-1したものはBの倍数にならないので、
　A+A*(B-1) = A*B としてみる。概ねこれでいいが、B=2の時にx=yとなってしまうので、
　A*1の方をうまく変えられないか考える。
　そこで、A*(B+1)+A*(B-1) = A*B*2 とやると、うまく合う。B=1の時は無理。
*/
