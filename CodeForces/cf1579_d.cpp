...
void solve() {
    ...
    my_priority_queue<pll, vector<pll>> que;
    rep(i, N) {
    ...
    while (que.size() >= 2) {
        ...
        if (que.size()) b = que.pop();
        else break;
        ans.eb(a.second+1, b.second+1);
        a.first--;
        b.first--;
        if (a.first) que.emplace(a);
        if (b.first) que.emplace(b);
    }
    print(ans.size());
    rep(i, ans.size()) {
        print(ans[i]);
