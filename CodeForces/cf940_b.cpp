 ...
 * ・なんとか自力AC！
 * ・整数ダイクストラ
 * ・AGC044aとかABC188_fと同じような感じにやれる。
 * 　もっと速く通せるべきだったのに、持ってきて書き換える時に*a書き忘れたりバグらせて時間かかった。
 * ・でもこれは-1だけで+1がないやつだから、多分こういうのしなくてももっと適当にやれるやつ。
 */
...
int main() {
    ...
    while (!que.empty()) {
        ...
        // 全部-1だけで行った場合の到達コストをチェック
        if (cur > 1 and (!dist.count(1) or dist[1] > cost+abs(1-cur)*a)) {
            dist[1] = cost+abs(1-cur)*a;
        }
        if (cur >= K) {
            // -1して割るKする場合の遷移
            ll x = cur%K;
            ll nxt = (cur-x)/K;
            if (!dist.count(nxt) or dist[nxt] > cost+a*x+b) {
                dist[nxt] = cost+a*x+b;
                que.push({cost+a*x+b, nxt});
            }
