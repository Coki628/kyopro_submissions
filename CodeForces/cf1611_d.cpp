...
・終了後自力AC。本番は時間足りず。。
・HLD、区間和取得、最短距離
・ちょっと設定がややこしくて、順列Pの順番に頂点を見ていくんだけど、やりたいのは
　「直前に見た頂点よりも根への距離が長くなるように、この頂点に係る重さを設定する」
　みたいな感じ。実際は辺の重さだけど、全て根への辺なので、頂点属性とみなして問題ない。
　で、上記は式にするなら W[p] >= dist[prevp]-dist[pの手前まで]+1
　みたいな感じになるので、これを満たすようにWを設定していけばいい。
　根への途中にまだ未使用の頂点があると、そこのdistはどうやっても自分より大きくできないので、
　そういう場所があったら即NG判定とする。
*/
...
void solve() {
    ...
    rep(i, N) {
        if (i == root) continue;
        bit.add(hld.in[i], MOD);
    }
    vector<ll> ans(N), D(N);
    rep(i, 1, N) {
        bit.add(hld.in[P[i]], -MOD);
        D[P[i]] = hld.query(root, P[i], 0LL, [&](ll a, ll b) { return bit.query(a, b); },
            [](ll a, ll b) { return a+b; });
        // rootまでの間にまだ未使用の頂点があればNG
        if (D[P[i]] >= MOD) {
            print(-1);
            return;
        }
        // 今回必要な重み： W[p] >= dist[prevp]-dist[pの手前まで]+1
        ans[P[i]] = max(D[P[i-1]]-D[P[i]]+1, 0LL);
        // 今回分の重みをdist[p]に足す
        D[P[i]] += ans[P[i]];
        bit.add(hld.in[P[i]], ans[P[i]]);
    }
    print(ans);
}
