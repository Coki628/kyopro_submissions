/**
 * ・自力AC！頑張った。
 * ・実装頑張る。UnionFindとmapで状態管理して、A[i]が小さい方から見ていく。
 * ・問題設定が難解な感じだけど、ようはあるKを定めると、その時有効になる区間は一意に定まるので、
 * 　各A[i]について、ちょうどその値を含むような値を下から順番に見ていく。
 * ・区間長の管理は基本的にはUFにやらせて、区間長の種類数がどうかはmapで管理した。
 * ・種類数が1の時、区間の個数の最大チェックをして、更新ならKもa+1に更新、同値ならKはa+1と最小チェックする。
 */
...
void solve() {
    ...
    for (auto [a, i] : AI) {
        ...
        if (i) {
            if (can_use[i-1]) {
                ll sz1 = uf.get_size(i-1);
                ll sz2 = uf.get_size(i);
                C[sz1]--;
                if (C[sz1] == 0) C.erase(sz1);
                C[sz2]--;
                if (C[sz2] == 0) C.erase(sz2);
                uf.merge(i-1, i);
                ll sz3 = uf.get_size(i);
                C[sz3]++;
            }
        }
        if (i != N-1) {
        ...
        if (C.size() == 1) {
            if (C.begin()->second > mx) {
                mx = C.begin()->second;
                ans = a+1;
            } elif (C.begin()->second == mx) {
                chmin(ans, a+1);
            }
        }
    }
