/*
参考：https://twitter.com/keijak/status/1374158879555194883
　　　https://codeforces.com/contest/1121/submission/110774823
・Suffix Array、LCP、Sparse Table
・ロリハを捨ててupsolveした。方針はkeijakさんのを参考に。
・今回は出現済の文字列を網羅する必要はなく、一番長いもの(遠くへ遷移できるもの)
　が分かればいいので、これにはSAからのLCPが使える。
　このLCPは区間最小値を取れば任意の2点間のLCPが分かるので、
　Sparse Tableを準備しておけば、取得はO(1)にできる。
・LCPを取った時の後ろの点では、前の点で既にその部分列が出現済とみなせるので、
　LCPから重なった部分を除いた長さが、後ろの点からの遷移に使える。
・ロリハで3秒あっても全然足りなかったのに、こっちではAC0.22秒。
　やっぱ線形が1000万単位だとlogが渋くなるよねー。。。
*/
...
void solve() {
    ...
    // rsa : 元indexからsa上のindexを取得する
    vector<ll> rsa(N);
    rep(i, N) rsa[sa[i]] = i;
    SparseTable<int> st(lcp, [](int a, int b) { return min(a, b); });

    vvi nodes(N+1);
    rep(i, N) {
        rep(j, i+1, N) {
            // 開始位置i,jでのLCPを取る
            ll len = st.query(min(rsa[i], rsa[j]), max(rsa[i], rsa[j]));
            // 重なっている部分はノーカウントにする
            chmin(len, j-i);
            // 後ろ側の時には既に出現済とみなせるので、遷移を作る
            nodes[j].pb(j+len);
        }
    }
