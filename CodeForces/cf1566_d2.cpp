...
・きっちり自力AC！
・座圧、BIT、転倒数
・さっきのは1行だけだったけど、今回は複数行に分かれてる。
・とはいえ、場所がほぼ決まってること自体は同じなので、最終的にやるのはシミュするだけ。
　問題は行にどう割り振るかだけど、これも値が大きい人を前(や上)にはできないので、
　ソートしてM個ずつ区切ればいい。ただしこの時の同じ行内での操作前の並びはindex順にしたいので、
　元indexを覚えておく。同じ行内で今度は元indexでソートし直す感じ。
　ここまで来れば後は行毎にD1と同じことすればOK。
*/
...
void solve() {
    ...
    sort(ALL(AI));
    auto B = list2d(N, M, 0LL);
    rep(i, N) {
        vector<pll> cur;
        rep(j, M) {
            cur.eb(AI[i*M+j].second, AI[i*M+j].first);
        }
        sort(ALL(cur));
        rep(j, M) {
            B[i][j] = cur[j].second;
