/*
・なんとか自力AC。。
・制約5000だし明らかに2重ループでの何かを示唆してるんだけど方針立たず。
　強烈に嘘っぽい貪欲を詰めていったら通った。。。詳細はソース内コメントにて。
・ちなみに公式解説見たら、式にして変数2つで2重ループ全探索できたらしい。。
*/
...
void solve() {
    ...
    // 1,2に偏りがあったら3を使って調節する
    if (adjli[1].size() <= adjli[2].size()) {
        while (not adjli[3].empty() and adjli[1].size() < adjli[2].size()) {
            adjli[1].pb(adjli[3].back()); adjli[3].pop_back();
        }
    } else {
        while (not adjli[3].empty() and adjli[2].size() < adjli[1].size()) {
            adjli[2].pb(adjli[3].back()); adjli[3].pop_back();
        }
    }

    // 残った3を交互に割り振る
    vvl ans(2);
    while (adjli[3].size() >= 2) {
        rep(i, 2) {
            ll j = adjli[3].back(); adjli[3].pop_back();
            ans[i].pb(j);
        }
    }
    // 1つ残ったら、1,2の少ない方に付ける
    if (adjli[3].size() == 1) {
        // 3が単独で余ったら均等にできないのでNG
        if (adjli[1].empty() and adjli[2].empty()) {
            print(-1);
            return;
        }
        if (adjli[1].size() <= adjli[2].size()) {
    ...
    // 1,2を少ない方がなくなるまで交互に割り振る
    while (adjli[1].size() >= 1 and adjli[2].size() >= 1) {
        ll i = adjli[1].back(); adjli[1].pop_back();
        ans[0].pb(i);
        i = adjli[2].back(); adjli[2].pop_back();
        ans[1].pb(i);
    }
    // 1,2残った方に対する処理
    if (adjli[1].size() >= adjli[2].size()) {
        // 残った1が0より多かったら相殺しきれないのでNG
        if (adjli[1].size() > adjli[0].size()) {
            print(-1);
            return;
        }
        // 残った1をカウントされない側に全部詰める
        while (adjli[1].size() >= 1) {
            ll i = adjli[1].back(); adjli[1].pop_back();
            ans[1].pb(i);
            i = adjli[0].back(); adjli[0].pop_back();
            ans[0].pb(i);
        }
        // 最後に残った0を交互に詰める
        ll i = 0;
        while (adjli[0].size() >= 1) {
            ll j = adjli[0].back(); adjli[0].pop_back();
            ans[i].pb(j);
            i = 1-i;
        }
    } else {
        if (adjli[2].size() > adjli[0].size()) {
            print(-1);
            return;
        }
        while (adjli[2].size() >= 1) {
            ll i = adjli[2].back(); adjli[2].pop_back();
            ans[0].pb(i);
            i = adjli[0].back(); adjli[0].pop_back();
            ans[1].pb(i);
        }
