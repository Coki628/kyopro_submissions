/*
・自力WA。。
・約分して同じになるやつのこと考慮できてないのに気付いたのが、
　だいぶ終了近くなってからだった。。
・でもそれが分かっても、じゃあいい方針が出たかと言うと出なかった。。
*/
...
void solve() {
    ...
    vector<ll> ans(N);
    rep(len, 1, N+1) {
        ll cnt = acc.query(0, len);
        ll seg = 1;
        chmax(ans[len-1], seg);
        if (len >= N-len+1) continue;
        rep(i, len, N-len+1, len) {
            seg++;
            if (acc.query(i, i+len) == cnt) {
                chmax(ans[i+len-1], seg);
            } else {
                break;
            }
        }
