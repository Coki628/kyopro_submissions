/*
参考：https://codeforces.com/blog/entry/93105
・参加中ちょっと考察してて気になってたので、頑張って解説AC！疲れたー。。
・BIT2本、式変形、平方分割
・まず、modについての以下の式を知らないとスタートラインに立てない。
　x mod y = x - y * floor(x, y)
・この式をベースに、さらに式変形をしてまとめて計算できる形に落としていく。
・全体の流れは、今見ている要素と、これまでに出た要素のペアをまとめて計算することで進める。
・「今見てるやつ mod これまでのやつ」と「これまでのやつ mod 今見てるやつ」に分けて考える。
　これで式がだいぶ固定できて、あとはfloorをどう処理するかって感じになる。
　そこで、floorした答えが同じものをまとめて計算する。答えが同じになる範囲は、
　割り算と掛け算でそれぞれ求めることができる。(大変だったけど)
・あと途中まで、意地悪な値がたくさん来たらこれでも遅くなるじゃん、って思ってたんだけど、
　よく見ると要素は制約よりdistinctなので問題ない。
・さて、掛け算の方はいかにも調和級数っぽい見た目をしていてlogで済みそうな感じだが、
　割り算の方はそうでもなかった。そのまま行くと普通にN回かかるループになる。
　デバッグして状況を精査すると、大きい値では最後の方でほぼ区間長0みたいなのを
　たくさん見ているのが分かった。ここを何とかする。
・√で分けて、大きい部分は割り算で進めて、小さい部分は1ずつ進めるみたいにした。
　これで何とかAC。3.96秒でギリギリ。(制約4秒)
・これだと確かに平方分割っぽい√にBITのlogがかかってるので、まあ20万に乗るときついよなぁって感じ。
・公式解のコードは何かもっとどシンプルに仕上げていたが、
　どうしてああなるのかよく分からない。。
・ちなみに試しにllをできるだけintにしてみたりしたけど、速度変わらなかった。
*/
...
void solve() {
    ...
    rep(i, 1, N) {
        ...
        // A[i] mod (A[x] as A[0]~A[i-1])
        ans[i] += i*A[i];
        ll j = 1;
        ll sqa = ceil(sqrt(A[i]));
        while (1) {
            // floor(A[i], A[x])がjになる区間
            ll l = A[i]/(j+1)+1;
            ll r = A[i]/j+1;
            // √A[i]以下になったら後は1ずつ進める
            if (r <= sqa) {
            ...
            ans[i] -= bitsm.query(l, r)*j;
            j++;
        }

        // (A[x] as A[0]~A[i-1]) mod A[i]
        ans[i] += bitsm.query(0, MX+1);
        j = 1;
        while (1) {
            // floor(A[x], A[i])がjになる区間
            ll l = A[i]*j;
            ll r = A[i]*(j+1);
            if (l > MX) break;
            chmin(r, MX+1);
            ans[i] -= bitcnt.query(l, r)*j*A[i];
            j++;
        }
        bitcnt.add(A[i], 1);
        bitsm.add(A[i], A[i]);
