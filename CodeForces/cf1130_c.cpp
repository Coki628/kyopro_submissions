...
・きっちり自力AC！
・グリッドBFS
・なんとなくこどふぉ前半だと少なめなこういう最短経路系。
　そのせいか考察易しめでも、あっとこで出るよりも解かれない印象。
・さて、今回は始点から行けるマスと終点から行けるマスをBFSで事前に調べておいて、
　あるマスとあるマスを見た時、一方が始点から行けて他方が終点から行けるなら、
　そこからのトンネルを掘るコストを確認して、これを全部でchminする。で解ける。
　グリッドが50*50=2500と小さいので、全マスのペアでN^4になって間に合う。
*/
...
// グリッドBFS
vvi bfs(vector<string> &grid, vector<pii> src) {

    int H = grid.size();
    int W = grid[0].size();
    auto res = list2d(H, W, -1);
...
void solve() {
    ...
    auto grid = build_grid(N, N, '1');
    auto res1 = bfs(grid, {{sh, sw}});
    auto res2 = bfs(grid, {{gh, gw}});

    ll ans = INF;
    rep(h1, 1, N+1) {
        rep(w1, 1, N+1) {
            if (res1[h1][w1] == -1) continue;
            rep(h2, 1, N+1) {
                rep(w2, 1, N+1) {
                    if (res2[h2][w2] != -1) {
                        chmin(ans, (h1-h2)*(h1-h2)+(w1-w2)*(w1-w2));
                    }
                }
            }
        }
