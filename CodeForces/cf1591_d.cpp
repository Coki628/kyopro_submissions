/*
・嘘貪欲でWAになった後、良い解法も出ないのでマラソン練習。
・山登り
・複数テストケースはNの大きさ毎に2秒を按分する感じにした。
・ローカルとかACのコードテストだと一応ちゃんと動いてるっぽいのに、
　CFのコードテストだと異常に遅くなってしまうので、
...
struct Timer {
    ...
    int64_t get_diff() {
        return chrono::duration_cast<chrono::microseconds>(this->now() - start_time).count();
    }
};
...
void solve(ll t) {
    ...
    while (cnt) {
        ...
        ll i = bit.bisearch_fore(0, N-1, x);
        assert(i != -1);
        ll j = bit.bisearch_fore(0, N-1, y);
        assert(j != -1);
        ll k = bit.bisearch_fore(0, N-1, z);
        assert(k != -1);
