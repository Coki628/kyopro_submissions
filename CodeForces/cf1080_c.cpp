/*
・なんとか自力AC！
・重なっている部分は、max(開始位置)〜min(終了位置) みたいなのを
　XY両方についてやれば出る。
・基本的には、白を塗って、黒を塗って、重複部分の白を引く、みたいにやればいいけど、
　位置の偶奇によって、1つ多くなるのがどっち側かが変わるので、
　それを考慮して足し引きする。この偶奇が考えてた方と逆だったりして
　サンプルなかなか合わなくて時間かかった。。
*/
...
void solve() {
    ...
    rep(i, 3) {
        ...
        st[i] = (h[i*2]+w[i*2])%2;
        // その領域の和
        sm[i] = max(h[i*2+1]-h[i*2]+1, 0LL)*max(w[i*2+1]-w[i*2]+1, 0LL);
    }

    ll wcnt = ceil(H*W, 2), bcnt = H*W/2;
    wcnt += sm[0]/2;
    bcnt -= sm[0]/2;
    if (st[0] == 1 and sm[0]%2 == 1) {
        wcnt++;
        bcnt--;
    }
    bcnt += sm[1]/2;
    wcnt -= sm[1]/2;
    if (st[1] == 0 and sm[1]%2 == 1) {
        bcnt++;
        wcnt--;
    }
    wcnt -= sm[2]/2;
    bcnt += sm[2]/2;
    if (st[2] == 1 and sm[2]%2 == 1) {
        wcnt--;
        bcnt++;
    }
    print(mkp(wcnt, bcnt));
