 ...
 * ・自力AC！と思ったのにシステス落ち…。
 * ・前後から貪欲、二分探索
 * ・前後からの貪欲を両方調べて、どこまで前から使うか、みたいのを全探索するこれはたまに見る。
 * ・ほぼ方針は合ってたのにシステス落ち。なんで、と思ったら、添字ミスとかかと思ったのに違って、
 * 　空く隙間の最大値を求めたいのに、片側に全部寄っちゃってるやつまで答えの対象に
 * 　含んでしまっていた。。
 * ・前後からやるので閉区間でやったのを、半開区間にしてみた。こっちのが見やすい？
 * 　前からやるだけなら半開区間一択だけど、後ろからも同じことやる時はどっちにするか悩む。
 */
...
void solve() {
    ...
    ll ans = 0;
    rep(i, N) {
        // 片側から全て取っている場合は答えに合わないので除く
        if (L[i] == 0 or L[i] == M) continue;
        ll j = bisect_right(R, L[i]);
        chmax(ans, j-i);
    }
    print(ans);
}
