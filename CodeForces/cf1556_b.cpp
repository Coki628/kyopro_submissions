/*
・なんとか自力AC。。
・区間反転かと思って遅延セグ木出してきたのに、やってみたら1点反転だった。。
　これならBITでもっとさくっと作れたっぽいよね。。
*/
...
void solve() {
    ...
    rep(i, N) {
        if (seg[i] != ok1[i]) {
            if (ok1[i] == 1) {
                ll j = seg.find_first(i+1, [&](Node a) { return a.cnt1 >= 1; });
                if (j == -1) {
                    cnt1 = INF;
                    break;
                }
                ...
                ll j = seg.find_first(i+1, [&](Node a) { return a.cnt0 >= 1; });
                if (j == -1) {
                    cnt1 = INF;
                    break;
                }
    ...
    rep(i, N) {
        if (seg2[i] != ok2[i]) {
            if (ok2[i] == 1) {
                ll j = seg2.find_first(i+1, [&](Node a) { return a.cnt1 >= 1; });
                if (j == -1) {
                    cnt2 = INF;
                    break;
                }
                ...
                ll j = seg2.find_first(i+1, [&](Node a) { return a.cnt0 >= 1; });
                if (j == -1) {
                    cnt2 = INF;
                    break;
                }
