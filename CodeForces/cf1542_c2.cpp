/*
参考：https://twitter.com/laycrs/status/1411340241302999044
・終了後1時間くらいかけてなんとか自力AC…！
・素因数分解、約数系包除
・実験すると、f(i)が取る値の種類はかなり少なそうなのが分かる。
　そしてiになるのは2,3,4..を全て約数に含む数、みたいなものになる。
　これはiが10^16を超えるところまで見てもf(i)の値がせいぜい15〜20個くらいだった。
・あとはそれぞれの値が何回使われるのかが数えたくて、
　これは最初にその値になるiの倍数が該当する。
・なんだけど、普通に数えると重複してしまうので、
　大きい方から見ていって、自分の倍数が持つ通り数を引くみたいなやつをやる。
　よく見ると今回は自分より大きいところの最初のiは全て自分の倍数なので、
　ようは自分より上を全部引いていくといい。
・さんざん合わなかった原因は、前処理パートの素因数分解で、
　pow(k, v)ってやるべきところをk*vってやってたことだった。。
　重複カウントの方疑ってめっちゃ調べてたからきつかったな。
　これ同じの何回もやってるからほんと困る。しかも小さい数だと
　絶妙にサンプル合っちゃったりするから発見が遅れる。。
・Layさんのツイート見たけど、準備パートの素因数分解とか頑張ってたところ、
　LCMすればいいだけじゃん。。これならバグらせなかったのに。。
*/
...
void prep() {
    ...
    while (1) {
        cur = lcm(cur, div);
        if (V.size() and V.back().second == cur) V.pop_back();
        V.pb({div+1, cur});
        if (cur > MX) break;
        div++;
    }
