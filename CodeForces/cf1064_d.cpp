 ...
 * ・きっちり自力AC！
 * ・グリッドダイクストラ
 * ・ダイクストラをちょっといじる。具体的には、
 * 　コストとして距離じゃなくて左右に動いた回数を持つ。
 * ・左右で条件が2つになるので、同じロジックでダイクストラがちゃんと機能してくれるか
 * 　心配だったけど、無事1発AC。2000^2=400万グリッドでAC0.72秒。
 */
...
using P = tuple<pll, int, int>;
pair<vvl, vvl> dijkstra(vector<string> &grid, pii src, ll llimit, ll rlimit) {

    int H = grid.size();
    int W = grid[0].size();
    auto L = list2d(H, W, INF);
    auto R = list2d(H, W, INF);
    ...
    R[sh][sw] = 0;
    while (!que.empty()) {
        auto [lrcnt, h, w] = que.top(); que.pop();
        auto [lcnt, rcnt] = lrcnt;
        if (L[h][w] < lcnt and R[h][w] < rcnt) {
            continue;
        }
        for (auto [dh, dw] : directions) {
            ...
            if (dw == -1) {
                if (lcnt >= llimit) continue;
                lcnt++;
            }
            if (dw == 1) {
                if (rcnt >= rlimit) continue;
                rcnt++;
            }
            if (lcnt < L[nh][nw] or rcnt < R[nh][nw]) {
                chmin(L[nh][nw], lcnt);
                chmin(R[nh][nw], rcnt);
                que.push({{lcnt, rcnt}, nh, nw});
            }
            if (dw == -1) lcnt--;
            if (dw == 1) rcnt--;
        }
...
void solve() {
    ...
    auto grid = build_grid(H, W, '*');

    auto [L, R] = dijkstra(grid, {sh, sw}, llimit, rlimit);

    ll ans = 0;
    rep(i, 1, H+1) {
        rep(j, 1, W+1) {
            if (L[i][j] != INF and R[i][j] != INF) {
                ans++;
            }
        }
    }
