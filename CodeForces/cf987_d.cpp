/**
 * ・自力AC！こういうグラフ問題はやっぱり結構好き。
 * ・グラフ、逆から見る、多点スタートBFS
 * ・商品数のが少ないので、商品側からどこの町にいくらで到達できるかBFSする。
 * 　各町で全商品への距離が分かるので、ソートして短い方からs個使う、とすればOK。
 * ・10万のグラフに100回BFSはなかなか攻めてるなぁと思ったけど、AC0.78秒とC++でもそれなりにかかった。
 * 　ダイクストラを落としたい制約なのかな多分。
 */
...
void solve() {
    ...
    auto dist = list2d(N, K, -1);
    rep(i, K) {
        auto res = bfs(nodes, adj[i]);
        rep(j, N) {
            dist[j][i] = res[j];
        }
    }
