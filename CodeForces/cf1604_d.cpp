/*
参考：https://codeforces.com/blog/entry/96460
・自力ならず。。
・場合分け、整数
・x == y は自明にxでいい。x > y はnをある程度大きくするとすれば、
　y mod n のnが無視できるようになり、n mod x = y となるnを探すだけになるのでこれはx+yが常に有効。
・x < y の場合が難しい。ここをどうすれば導けたかを考えてみる。
　まず、手元で適当なケースを試して (x+y)/2 が全て合うので、ここで諦めてしまったのが良くない。
　愚直全探索コードを書いて、x,y両方100まで試したらたくさん反例が出た。例えばx=4,y=14とか。
　このくらいの労力で反例見つかるなら、もう少し泥臭くあがくべきだった。
・で、この反例がなぜダメかを手元で精査すると、n mod x のnが (4+14)/2=9 だと、
　9では4で割った時に5全部が余りになってはくれず、4は商に入ってしまうので余りは1になってしまう。
　4を大きくしたいがそこはxで固定なので、9をどうにかすると考えると、
　ここで発想の飛躍がいずれにしろ必要な気はするんだけど、めいっぱい大きくすると、
　13になったら、n mod x の値はmod 4で維持されたまま、y mod n の方も余りがちょうど1になる。
　そのまま半分の位置を取るとx側が少なすぎたので、xに下駄を履かせて、そこからyとの半分の位置を取った感じ。
　これを式にして表すと y-(y%x)/2 となる。
*/
