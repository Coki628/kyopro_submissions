...
・自力ならず。。
・前計算、数え上げDP
・重要な考察として、例えば1の状態は、0が1回遷移した状態と変わらない。
　ここから、1〜9までの数字は、0からその回数だけ多く遷移したものとみなせる。
・これなら初期値を0として最大回数の20万ちょっとまでの遷移を前計算しておけば、
　各回には桁数分のチェックだけすればよくなるので間に合う。
・実は考察をもう一段浅くして、普通に0~9それぞれから始まるものを全て持っても間に合う。
　これなら思いつくべきだったよなぁ。
　結局操作回数が最大20万だから、愚直DP前計算で全然よかったんだよね。。
*/
