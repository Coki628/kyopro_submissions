 ...
 * ・自力AC！
 * ・ランレングス圧縮して貪欲
 * ・同じ値の要素毎に区間にしてまとめる。前に自分が使った数が1つだけだったり、
 * 　前の区間との間が2以上空いている時は、2箇所ばらけて置けるので+2、そうでない時は+1する。
 */
...
int main() {
    ...
    rep(i, 1, N+1) {
        ll m = adj[i].size();
        if (m) {
            ans += min(adj[i][0].second-adj[i][0].first, 2LL);
            ll prvcnt = min(adj[i][0].second-adj[i][0].first, 2LL);
            rep(j, 1, m) {
                auto& [l1, r1] = adj[i][j-1];
                auto& [l2, r2] = adj[i][j];
                if (prvcnt == 1 or l2-r1 >= 2) {
                    ans += min(r2-l2, 2LL);
                    prvcnt = min(r2-l2, 2LL);
                } else {
                    ans++;
                    prvcnt = 2;
                }
