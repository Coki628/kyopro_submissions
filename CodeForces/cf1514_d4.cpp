/*
参考：https://scrapbox.io/data-structures/Range_Mode_Query
・のしさんの区間最頻値取得ライブラリ
・デフォルトだと最頻値の要素の値を返すので、要素数を返すようにする。
・数列の長さより大きい要素があるとNGのようなので、適宜座圧とかやる。
・llをintにしたらギリギリ通った。AC2.85秒。(制約3秒)
・なんかコンテスト終了後に凶悪なケースが追加されてるっぽい。
　コンテスト中なら1.5秒とかで通ってたみたい。
　なんならゆうさんのセグ木解の方も通ってたぽい。
*/
...
class range_mode_query {
    ...
    public:
        ...
        b_node(const T value, const T rank) : value(value), rank(rank) {}
    };
    ...
    public:
        ...
        s_node(const T mode, const T freq) : mode(mode), freq(freq) {}
    };
...
public:
    ...
    range_mode_query(const std::vector<T> a) : t(1) {
        const T n = a.size();
        ...
        for (T f = 0; f * t <= n; f += 1) {
            std::vector<T> freq(n, 0);
            s_node cur(0, 0);
            for (T l = f + 1; l * t <= n; l += 1) {
                for (T i = (l - 1) * t; i != l * t; i += 1) {
                    const T e = a[i];
                    freq[e] += 1;
                    if (freq[e] > cur.freq)
                        cur = s_node(e, freq[e]);
    ...
    s_node query(const T first, const T last) const {
        assert(first < last);
        assert(last <= b.size());
        T bf = first / t + 1;
        T bl = last / t;
        if (bf >= bl) {
            s_node cur(0, 0);
            for (T i = first; i != last; i += 1) {
                const b_node &e = b[i];
                const std::vector<T> &v = q[e.value];
                T idx;
...
void solve() {
    ...
    rep(i_, 0, Q) {
        ...
        ll mx = rmq.query(l, r).freq;
        ll cnt = 1;
        cnt += max(max(mx-ceil(r-l, 2), 0LL)*2-((r-l)%2 == 0), 0LL);
        print(cnt);
    }
}
