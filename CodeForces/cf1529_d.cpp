/*
参考：https://codeforces.com/contest/1529/submission/117249488
・自力ならず。。
・約数の個数、累積和同時進行DP
・Siroさんのコードが分かりやすそうだったので、それを参考に解釈。
・iをペアの数だとすると、iが1増える度に、外側に1つ大きく覆ったような形で
　前の状態が全て使える。ここでの前の状態はdp[i-1]ではない。
　もちろんそこも使えるのだが、それ以外にも外側を2,3,4ペアと多く覆うことで、
　dp[i-2],dp[i-3]..のパターンも全て使うことができる。
　これはdpと同時進行で累積和も進めてくやつをやればすぐ出せる。
　1つ前の状態がそのまま使えそうなのは思ったけどそれだけだと合わなくて、
　どうなんだろうって悩んだけど、前のやつ全部使えるんだね。。
・あとそれとは別に毎回増えるのは、同じ大きさで全て並べるやつ。
　これはそのiの約数の個数と一致するので、篩で前計算。
・本番中はnCrとか階乗こねくり回すものだと思って迷走して2時間終わり。
　累積和DPって言われても、なかなか理解できなかった。。はー。。。
*/
...
void solve() {
    ...
    auto divs = eratosthenes_sieve(N);
