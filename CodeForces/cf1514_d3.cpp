/*
参考：https://twitter.com/yuusanlondon/status/1384176199061016577
・ゆうさんのセグ木解
・セグ木にmapを乗せるんだけど、クエリと構築時で違う処理をするので、
　クエリの処理はセグ木の外でやってる。
・ほぼ同じことしたのに、なぜかpypyより先にTLEするので、今度調べる。
　原因は分かった。クエリの時Nodeに対して参照アクセスじゃなくて、
　毎回生成みたいになってたから遅かったんだ。
　そこを参照だけするようにしたら速くなった。
　でもそれでも先のケースでTLE。。まあやっぱダメなんかなー…。
・全部intにしてみたけどやっぱダメだった。
*/
...
auto f = [](Node a, Node b) -> Node {
    if (a.C.size() < b.C.size()) swap(a, b);
    a.sz += b.sz;
    for (auto [k, v] : b.C) {
        a.C[k] += v;
    }
    if (a.C[a.most] >= ceil(a.sz, 2)) {
        a.most = a.most;
    } elif (a.C[b.most] >= ceil(a.sz, 2)) {
        a.most = b.most;
    } else {
        a.most = -1;
    }
    ...
    rep(_, Q) {
        ...
        for (a += seg.sz, b += seg.sz; a < b; a >>= 1, b >>= 1) {
            if (a & 1) {
                ids.pb(a);
                auto& node = seg.seg[a++];
                if (node.most != -1) se.insert(node.most);
            }
            if (b & 1) {
                auto& node = seg.seg[--b];
                ids.pb(b);
