/*
・きっちり自力AC！
・基本的に中央値にするといい感じになるやつ。
　例えばXについて真ん中のやつのところまで、Xが小さいのと大きいのを
　それぞれ同じY座標のまま引っ張ってきて、次に今度はX座標を固定して
　真ん中のやつのX座標位置でYの上から下まで繋げる、ってやるとできる。
*/
...
void solve() {
    ...
    vector<pll> XI = {{X[0], 0}, {X[1], 1}, {X[2], 2}}, YI = {{Y[0], 0}, {Y[1], 1}, {Y[2], 2}};
    sort(ALL(XI));
    sort(ALL(YI));
    ...
    rep(y, YI[0].first, YI[2].first+1) {
        ans.insert({xm, y});
    }

    print(ans.size());
    for (auto xy : ans) print(xy);
}
