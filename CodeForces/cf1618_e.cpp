/*
参考：https://twitter.com/mkawa2/status/1470799219678380032
・自力ならず。。これは解けるべきだったろうな。
・式変形、総和、差分
・まず、求める列ansの総和は出る。Aの総和を1~Nの等差数列の和で割ればいい。
　ここまではできた。ここから個々の値が出なかった。
・差分に注目する。差分取ると、-a1*2+a2+a3みたいなやつになって、
　総和が分かってるとそれはa1+a2+a3だから、これを合わせると-a1*3だけが残る。
　この3はNなので、つまり/-Nすればa1になるので、
　まとめると、((a[i]-a[i-1])-sm)/-Nを各iでやってけば答え。
　割り切れない時とかは適宜NGとする。
*/
...
void solve() {
    ...
    auto [sm, m] = divmod(sum(A), N*(N+1)/2);
    if (m) {
        NO();
        return;
    }
