/*
・さくっと自力AC！
・区間chmax、1点更新、1点取得、SegTreeBeats
・今回時間なくてD全く見れなかったので、一応見てみたらびっくり。
　区間chmaxがBeatsで殴れって言ってるじゃん。
　見てたら絶対通せてたやつじゃん、もったいない…。
・ちなみに20万のNと20万クエリで、AC0.65秒。やっぱさすがにちと重めかな。
*/
...
void solve() {
    ...
    rep(i, Q) {
        ...
        if (op == 1) {
            ...
            cin >> p >> x;
            p--;
            seg.range_update(p, p+1, x);
        } else {
            ll x;
            cin >> x;
            seg.range_chmax(0, N, x);
        }
    }
    vector<ll> ans(N);
    rep(i, N) ans[i] = seg.get_sum(i, i+1);
    print(ans);
}
