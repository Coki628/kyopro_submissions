/*
・きっちり自力AC！
・2人が1から順に素材を集めて行くが、詰まるところある値に対して、
　その前にいた2点から、それぞれが新しい2点のうちどちらかに行くしかないので、
　a1->a2,b1->b2と行くか、a1->b2,b1->a2と行くか、みたいな2通りから、
　小さい方を取っていくのを順次見ていけばOK。2人に性質上の違いはないので、
　具体的にどっちがどっちに行ったとかは考えなくていい。
*/
...
void solve() {
    ...
    rep(i, 1, N+1) {
        ...
        ll nxb = adjli[i][1];
        ll mn = INF;
        ans += min(abs(cura-nxa)+abs(curb-nxb), abs(cura-nxb)+abs(curb-nxa));
        cura = nxa;
        curb = nxb;
    }
    print(ans);
