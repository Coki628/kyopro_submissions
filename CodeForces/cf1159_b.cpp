...
・なんとか自力AC！粘ってよかった。
・累積min、二分探索
・話を整理すると、ペアの距離によって(j-i)*kは一定になり、これが使われる場所は、
　ざっくり言うと最初の方が最後の方になる。
　ペアの距離が長いパターンでは、数列の真ん中らへんは使われない。
・ここから、数列を予め前後から累積minしておけば、ペアの距離ごとには、
　OKかどうかをO(1)で判定できる。ペアの距離の種類はまあNくらいなので、
　kをにぶたんすれば内側でN回しても間に合う。
・概ね方針は合ってたけど、WAが取れずに詰めで時間かかった。
　累積minのindexは回文判定みたいに動くんだけど、
　ペアの距離はここの(j-i)ではなく別の動きで、N-1から1ずつ減っていく、
　と気付くのにかなり時間がかかった。。気付いたらそれはそうなんだけどね。。
*/
...
void solve() {
    ...
    auto accmn = A;
    rep(i, 1, N) {
        chmin(accmn[i], accmn[i-1]);
    }
    auto accmnrev = A;
    rep(i, N-2, -1, -1) {
        chmin(accmnrev[i], accmnrev[i+1]);
    }

    auto res = bisearch_max(0, MOD, [&](ll m) {
        ll k = N-1;
        rep(i, N) {
            ll j = N-i-1;
            if (k*m > min(accmn[i], accmnrev[j])) {
                return false;
            }
            k--;
        }
