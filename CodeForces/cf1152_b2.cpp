/**
 * ・なんとか自力AC。。時間かかった…。
 * ・上位ビットから0か1に揃えていって、最後の1だけ別途立てる。
 * 　ただしこれだけだと、途中で全ビット立った時に+1でおかしくなるので、
 * 　途中で全ビット立ったらbreakする。
 * ・bit_lengthとbinのverifyをしておいた。
 */
...
void solve() {
    ...
    rep(i, MXLOG, 0, -1) {
        if (cur>>i & 1) {
            ...
            ans.pb(i+1);
            // print(bin(cur));
            if (popcount(cur) == bit_length(cur)) break;
            cur++;
            cnt++;
            // print(bin(cur));
        }
