...
bool check(vector<string> &grid, ll i, ll j) {
    rep(h, i-1, i+2) {
        rep(w, j-1, j+2) {
            if (h == i and w == j) continue;
            if (grid[h][w] == '.') return false;
        }
    }
    return true;
}

void paint(vector<string> &grid, ll i, ll j) {
    rep(h, i-1, i+2) {
        rep(w, j-1, j+2) {
            if (h == i and w == j) continue;
            grid[h][w] = '#';
