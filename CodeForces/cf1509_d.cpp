/*
参考：https://twitter.com/laycrs/status/1383102241255530499
・自力ならず。。
・使われてる01が多い2つを使おうとしたのは良かった。その後が詰めきれず。
　片方優先で全部並べて、他方の残ってるやつを続ける、でWA。
　それを優先する方逆でも試す、でもWA。とかやってて終了。
・正解は、両方並行で進めていって、両者異なる箇所に来たら
　01で少ない方を優先で置く、ってやったらAC。
　この時にどっち置くのがいいのかなんか非自明だったけど、
　まあWAなら両方やるつもりだった。
　多分だけど、N以上ある多い方は、そこで優先して使わなくても、
　どのみち重なる箇所が多くて進むので、少ない方優先で進める、みたいな話だと思う。
*/
...
void solve() {
    ...
    vector<unordered_map<char, ll>> C(3);
    rep(i, 3) {
        C[i] = Counter(S[i]);
    }
    vector<ll> ok0, ok1;
    ...
    if (ok0.size() >= 2) {
        ...
        while (j < N*2 and k < N*2) {
            if (s[j] == '0' and t[k] == '0') {
                ...
                k++;
                ans += '0';
            } elif (s[j] == '1' and t[k] == '1') {
                j++;
                k++;
                ans += '1';
            // 01が食い違った時は少ない方(ここでは1)を優先して進める
            } elif (s[j] == '0' and t[k] == '1') {
                k++;
                ans += '1';
            } else {
                j++;
