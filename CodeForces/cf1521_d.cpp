/*
・Cのインタラクティブがやりたくならない見た目をしていたのでこっちに注力。
・直径をベースにして残りを適当にそこに続けて繋ごうとか考えたけどWA。。
*/
...
void solve() {
    ...
    dfs(dfs, 0, -1);
    ll mxi = max_element(ALL(dist))-dist.begin();
    dist.assign(N, 0);
    dfs(dfs, mxi, -1);
    ll mxj = max_element(ALL(dist))-dist.begin();

    set<ll> path;
    auto dfs2 = [&](auto&& f, ll u, ll prv) -> bool {
        if (u == mxj) {
            path.insert(u);
            return 1;
        }
    ...
    ll cur = mxi;
    vvl ans;
    auto dfs3 = [&](auto&& f, ll u, ll prv) -> ll {
        ll leaf = u;
        for (ll v : nodes[u]) {
            if (v == prv) continue;
            ll res = f(f, v, u);
        ...
        if (prv != -1 and nodes[prv].size() >= 3 and not path.count(u)) {
            vector<ll> tmp(4);
            tmp[0] = prv+1;
            tmp[1] = u+1;
            tmp[2] = u+1;
