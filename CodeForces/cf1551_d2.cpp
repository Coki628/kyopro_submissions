/*
・終了後時間かけてAC。まあテストケース見たし自力ではないかな。。
・構築、実装
・大まかな方針は立ってたけどまあめんどい。左上から2個ずつ、必要な数まで横置きして、
　残りを縦置き。奇数なら最終行も置く。
・場合分けを減らしたいので、列が奇数なら縦横スワップで奇数行にしてる。
・地味に値が26種類しか使えないので、重複チェックが面倒。自分より手前を見る。
　回数多いし長くなるので別関数に出した。ついでに次の文字に進めるやつも。
・これでもWAが出て何でー、ってなったけど、結局横置きの必要な数を減らしてくに当たって、
　必須で横置きになる最終行の分を予め引いてなかった。。
・実装ゴチャゴチャするとしょうもないミスが出るのは何とかならんかなぁ。。
*/
...
bool check(vector<string> &grid, ll i, ll j, char c1, char c2) {
    if (i and (grid[i-1][j] == c1 or grid[i-1][j] == c2)) return false;
    if (i and (grid[i-1][j+1] == c1 or grid[i-1][j+1] == c2)) return false;
    if (j and (grid[i][j-1] == c1 or grid[i][j-1] == c2)) return false;
    if (j and (grid[i+1][j-1] == c1 or grid[i+1][j-1] == c2)) return false;
    return true;
}

bool check2(vector<string> &grid, ll i, ll j, char c) {
    if (i and grid[i-1][j] == c) return false;
    if (i and grid[i-1][j+1] == c) return false;
    if (j and grid[i][j-1] == c) return false;
    return true;
}
...
void solve() {
    ...
    if (H%2 == 0 and W%2 == 0) {
        if (K1%2 == 1) {
            ...
            rep(i, 0, H, 2) {
                rep(j, 0, W, 2) {
                    while (not check(ans, i, j, c, next(c))) c = next(c);
                    if (K1 > 0) {
                        ans[i][j] = c;
                        ans[i][j+1] = c;
                        c = next(c);
        ...
        ll fixed = W/2;
        if (N-fixed < K2 or (N-fixed)%2 != K2%2) {
            NO();
        } else {
            YES();
            K1 -= fixed;
            ...
            rep(j, 0, W, 2) {
                while (not check2(ans, H-1, j, c)) c = next(c);
                ans[H-1][j] = c;
                ans[H-1][j+1] = c;
            }
