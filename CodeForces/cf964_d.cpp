/**
 * 参考：https://twitter.com/ItoHdak_kyopro/status/1361835803526135810
 * ・自力ならず。見抜けると勝てるやつ、見抜けなくて負け。
 * ・木の走査、DFS2回
 * ・まず頂点が偶数個の木は辺が奇数個あるのでNG。言われたら自明だけどこれも気付かなかった。。
 * ・で、実は始点とかどこでもよくて、まずは普通にDFSする。
 * 　ここで戻り時にチェックしていくけど、辺が奇数個ならその時は何もしない。
 * 　偶数個だったら、その頂点を消すとする。すると、その子頂点は辺が1本減って偶数個、
 * 　その子も残ってるやつは辺が奇数個だったはずなので偶数個、となっていって、
 * 　結果的に部分木を全部処理できる。これ意味分かった時めっちゃアハ体験だった。
 * ・このロジックが分かればあとは実装。普通のDFSの中で、子孫頂点を後から消しに行く用の、
 * 　もう1つ別のDFSを用意すればいい。
 */
