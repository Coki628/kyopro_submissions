/**
 * ・自力AC！
 * ・制約が2重ループできるよって言ってるけど方針が立たずO(1)を頑張って詰めた。
 * ・最初の2マスには6手かかって、その先は基本的に3手ずつ。
 * 　左右どっちを先に行くかで往復コストが変わるので、その小さい方にする。
 * 　初期位置が端の場合は片側に突き進む確定なので場合分け。
 */
...
void solve() {
    ...
    if (K == 1) {
        ...
        // 左先か右先かで小さい方
        ans = min(7 + (l-1)*4+(N-K)*3, 7 + (K-1)*3+(N-r)*4);
    }
    print(ans);
}
