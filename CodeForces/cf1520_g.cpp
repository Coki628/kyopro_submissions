/*
・超頂点ダイクストラ
・ワープの頂点を作るやつ。こっちはグリッドのまま持ってるのに何か通ってて、
　1次元グラフにした方はhackされてた…。
　MLEみたいだし、ワープの辺を陽に持たせたのが良くないのかな。。
*/
...
vvl dijkstra(vvi &grid, pii src, ll C) {
    ...
    while (!que.empty()) {
        ...
        if (h == -1 and w == -1) {
            ...
            rep(nh, 1, H) {
                rep(nw, 1, W) {
                    if (grid[nh][nw] != -1 and grid[nh][nw] != 0) {
                        if (dist+grid[nh][nw] < res[nh][nw]) {
                            res[nh][nw] = dist+grid[nh][nw];
                            que.push({dist+grid[nh][nw], nh, nw});
                        }
                    }
                }
            }
            ...
            for (auto [dh, dw] : directions) {
                ...
                if (dist + C < res[nh][nw]) {
                    res[nh][nw] = dist + C;
                    que.push({dist+C, nh, nw});
                }
            }
            if (grid[h][w] != 0 and dist+grid[h][w] < warp) {
                warp = dist+grid[h][w];
                que.push({dist+grid[h][w], -1, -1});
            }
        }
    }
    return res;
