/*
・きっちり自力AC！飲酒参戦だった割にはまあ頑張れた方かもしれない。
・繰り上がりのある桁DP、2変数を扱う桁DP
・状態として、直前桁と、2つ前の桁を持って下位桁からDPしていく。
　遷移はa,bの2変数なので、0~9にするのを両方で10*10通り試す。
　最近この手のDPをやってた(ABC172f,ABC138f)から、いくらかやりやすかった。
*/
            ...
            rep(prv, 2) {
                rep(d1, 10) {
                    rep(d2, 10) {
                        ll sm = d1+d2+prvprv;
                        ll prvprv2 = prv ? 1 : 0;
                        auto [di, mo] = divmod(sm, 10);
                        if (mo == A[i]) {
                            ll prv2 = di;
                            dp[i+1][prvprv2][prv2] += dp[i][prvprv][prv];
                        }
                    }
                }
            }
