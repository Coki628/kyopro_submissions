/*
参考：https://twitter.com/keijak/status/1471506884406366215
　　　https://codeforces.com/blog/entry/97920
・自力ならず。。
・インタラクティブ
・隣接3人でクエリを出す方針は合ってた。
　で、白黒1人ずつ分かれば、その2人で照会かけて全部分かるっていうのも分かった。
　でも最初の2人の見つけ方が違ってた。
　クエリ3つ合わせて5人の真ん中が決まらないかみたいな感じに見てしまったんだけど、
　これだとうまくいかないっぽい。
・隣接2クエリで値が異なった場合、その時の4人の両端になるものが分かる。
　そこが該当の方でなければ、そっちに値が変化する理由がないからね、確かに。。
・クエリ結果が全部片方の色になってしまうケースがあると思って、
　余計な確認処理を入れてたのが余計にupsolveを遅らせてた。
　隣接クエリ3つが全部同じ色なら真ん中はその色だ、みたいなチェックを入れてたんだけど、
　これ全然反例あったわ。5人の真ん中だけ違う色だとおかしくなる。
　全体の1/3超～2/3未満より偏ることがないって制約があるから、
　これがクエリ結果が全部片方の色になることを無くしてるようだ。
・余計な確認とか全部消したら無事AC。悲しい。。
*/
...
void solve() {
    ...
    // 制約より、クエリで隣同士が違う場所は必ずあるようだ
    assert(ans[0].size() and ans[1].size());

    // 両方の色が揃えばそれらを使って一意にできる
    rep(i, N) {
        if (B[i] == -1) {
            ll res = ask(ans[0][0], ans[1][0], i);
            ans[res].eb(i);
        }
    }
    answer(ans[0]);
