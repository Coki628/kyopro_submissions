...
・自力ならず。。
・区切るDP、ロリハ
・出現済の部分列をロリハとsetで管理するも、制約がきつく(5000^2log)、TLE。
　unordered_setにしたり、色々あがいたけど結局ダメだった。
　まあ多分これはロリハを落としたい制約なんだよね、きっと。
*/
...
void solve() {
    ...
    rep(i, N) {
        ...
        rep(j, i+1, N+1) {
            if (j-i > i) break;
            if (se.count(rh.query(table, i, j))) {
                chmin(dp[j], dp[i]+b);
            }
        }
        rep(j, 0, i+1) {
            se.insert(rh.query(table, j, i+1));
        }
    }
    ll ans = dp[N];
    print(ans);
