/**
 * 参考：https://twitter.com/KaikeyPro/status/1370013584869130243
 * 　　　https://codeforces.com/blog/entry/57840
 * ・自力ならず。
 * ・式変形、二分探索
 * ・自分が立ててた式がそもそも間違ってた。
 * 　x = N^2 - M^2 だと思ってたけど、x = N^2 - floor(N/M)^2 だった。
 * 　考えたら確かにそう。小さいケースでうまく数字が合っちゃって気付けなかった。。
 * ・で、これが分かると式変形で、floor(N/M)^2 == N^2 - x とできるので、
 * 　Nを固定すればMはにぶたんで探せる。
 * ・100*Nの候補*logくらいの計算量でTL1秒なので厳しめ。
 * 　雑にNの候補を10万個でやったらTLEして、N^2 > 20億でbreak(これで5万弱だった)したらAC0.59秒。
 * ・にぶたんの上限も雑にINFだったのを20億にしたら早くなってAC0.31秒。
 * 　やっぱ時間厳しめな時はこういうとこ丁寧にやるべきだなー。
 */
...
void solve() {
    ...
    rep(i, N) {
        ...
        rep(n, 1, M) {
            ll nn = A[n];
            if (nn-x <= 0) continue;
            ll m = bisearch_max(0, MX, [&](ll m) {
                return pow(n/m, 2) >= nn-x;
            });
            if (m and pow(n/m, 2) == nn-x) {
                ans = {n, m};
                ok = 1;
                break;
            }
