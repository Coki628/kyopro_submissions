/*
・終了後自力AC！
・Eに時間取られ過ぎた。。こっち取り組めばよかったかな。でも時間かかったしどっちもどっちか。
・差分GCD、添字いっぱいDP
・dp[i][j][k][l] := i番目まで見て、j個取った状態で、最後に取ったのがk番目で、gcdの状態がl、への到達可否
　みたいなやつを頑張ったら通った。。
・遷移の注意点として、最初の1つとそれ以降で動き方が異なるので場合分けする。
　最初の1個を入れる時は常にGCDは0扱いで、2個目以降が前回との差分でって感じ。
・制約は小さめなんだけど、計算量が微妙だった。Nが合計100以下だから N^3*GCDの種類数 = 100万*GCDの状態数
　くらいだと思うんだけど、そこまでGCDの状態数少ないかなぁって感じはする。まあ余裕で通ったんだけど。
*/
...
void solve() {
    ...
    auto dp = list3d(N+1, N/2+1, N, set<ll>());
    rep(i, N) {
        rep(j, N/2+1) {
            if (j == 0) {
                dp[i+1][1][i].insert(0);
                ...
                rep(k, i) {
                    for (auto v : dp[i][j][k]) {
                        dp[i+1][j][k].insert(v);
                        if (j+1 < N/2+1) {
                            dp[i+1][j+1][i].insert(gcd(v, abs(A[i]-A[k])));
                        }
                    }
                }
            }
