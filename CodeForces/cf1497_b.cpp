/**
 * ・なんとか自力AC！
 * ・mod Mで考える。すると、mod Mでペアにする時、和がMになるのが基本的に隣同士にできる、となる。
 * 　つまりmod MのiとM-iが隣同士になる。0のところは自身と組む。
 * 　Mが偶数ならM/2も自身と組む。
 * ・これを適切に数えればいいのだが、ループの終わり位置をミスっててペナ。時間食った。
 */
