/**
 * ・自力AC！
 * ・実装頑張る。文字列でゴニョゴニョやるので、pythonのが書きやすそう。zfillとか作ろうかな。。
 * ・zfill作った。
 */
...
bool check(ll a, ll mod) {
    auto s = zfill(to_string(a), 2);
    string t = {mp[s[1]], mp[s[0]]};
...
void solve() {
    ...
    string s;
    cin >> s;
    ll hh = toint(s.substr(0, 2));
    ll mm = toint(s.substr(3, 2));

    string ans;
    bool ok = 0;
    while (1) {
        while (1) {
            if (check(hh, M) and check(mm, H)) {
                string t1 = zfill(to_string(hh), 2);
                string t2 = zfill(to_string(mm), 2);
                ans = t1+":"+t2;
                ok = 1;
                break;
            }
