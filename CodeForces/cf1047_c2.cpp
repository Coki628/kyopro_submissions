 ...
 * ・自力AC！
 * ・素因数分解、osa_k法
 * ・全要素に素因数分解やるんだけど、計算量がきつい。
 * 　osa_k法使って、√じゃなくてlogでやったら何とかAC0.998秒。。
 * ・ちょっと想定解に寄せてみる。
 * 　先に全部GCD取って約分しておくと、全要素共通で持ってる部分を消せるので、
 * 　素因数を個数管理しないで存在チェックだけすればいいようになる。
 * ・でもこれでもAC0.982秒。多分素因数分解がmapでやってるからそもそも重いんだろうな。
 * 　vector版作れば速いんだろうけど個数管理しづらくなるし、悩みどころ。。
 */
...
void solve() {
    ...
    map<ll, ll> mp;
    rep(i, N) {
        for (auto [k, v] : factorize(table, A[i])) {
            mp[k]++;
        }
    }
