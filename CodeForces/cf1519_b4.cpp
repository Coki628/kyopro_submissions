/*
・グリッドDP、前計算
・bitset高速化してもLayCurseさんのより全然遅いので何でか調べたら分かった。
　これH,W,Kしか与えられないんだから、DPは前計算で最大のやつ1回やればいいだけじゃん。。
　そうしてみたら、普通のDPで1億でも余裕でAC0.09秒。。
・これは気付くべきだったなぁ…。
*/
...
bool dp[100][100][10001];

void prep() {
    ll H = 100, W = 100, K = 10000;
    dp[0][0][0] = 1;
    rep(h, 0, H) {
        rep(w, 0, W) {
            rep(k, 0, K+1) {
                if (dp[h][w][k]) {
                    if (h+1 < H and k+w+1 <= K) {
                        dp[h+1][w][k+w+1] = dp[h][w][k];
                    }
                    if (w+1 < W and k+h+1 <= K) {
                        dp[h][w+1][k+h+1] = dp[h][w][k];
                    }
                }
            }
        }
