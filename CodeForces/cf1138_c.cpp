/*
参考：https://codeforces.com/blog/entry/65825
・自力ならず。。
・グリッド、行列独立、ソートしてにぶたん
・変な座圧とか考えちゃって全然正解の方針に行けなかった。。
・解いてみれば、行列独立とかソートにぶたんとか基本じゃんね。。
*/
...
void solve() {
    ...
    auto ans = list2d(H, W, 0LL);
    rep(i, H) {
        ll N = rows[i].size();
        rep(j, W) {
            ll M = cols[j].size();
            ll hlcnt = bisect_left(rows[i], grid[i][j]);
            ll hrcnt = N-bisect_right(rows[i], grid[i][j]);
            ll wlcnt = bisect_left(cols[j], grid[i][j]);
            ll wrcnt = M-bisect_right(cols[j], grid[i][j]);
            ans[i][j] = max(hlcnt, wlcnt) + 1 + max(hrcnt, wrcnt);
        }
    }
    rep(i, H) {
        print(ans[i]);
