/*
参考：https://nyaannyaan.github.io/library/rbst/lazy-reversible-rbst.hpp
　　　https://nyaannyaan.github.io/library/verify/verify-yosupo-ds/yosupo-dynamic-sequence-range-affine-range-sum.test.cpp
・なんとか自力AC。
・区間反転、平衡二分木
・激ヤバデータ構造を窃盗してくると通せる。Nyaanさんに感謝。
*/
...
void solve() {
    ...
    rep(_, Q) {
        ...
        cin >> x;
        x--;
        ll i = bisect_right(L, x) - 1;
        ll a = min(x, R[i] + L[i] - x);
        ll b = max(x, R[i] + L[i] - x);
        rbst.reverse(root, a, b + 1);
    }
    string ans;
    rep(i, N) {
        ans += 'a' + rbst.fold(root, i, i + 1);
    }
    print(ans);
}
