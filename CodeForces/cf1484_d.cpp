/**
 * ・自力ならず。
 * ・mapで愚直シミュっぽいことをしたんだけど、TLE。
 * 　多分これだと、最後の方で毎回1個だけ減るとかを繰り返されるとつらい。
 * 　コンテスト後のTLは見れなかったし、公式解説もまだ出てないのでまあupsolveは保留で…。
 */
...
void solve() {
    ...
    while (updated) {
        ...
        for (auto itr = begin(mp); itr != end(mp); itr++) {
            auto [k1, v1] = *itr;
            if (next(itr) == mp.end()) break;
            auto [k2, v2] = *next(itr);
            if (gcd(v1, v2) == 1) {
                if (next(mp.find(k2)) == mp.end()) {
                    last_deleted = 1;
                }
                mp.erase(k2);
                ans.pb(k2+1);
