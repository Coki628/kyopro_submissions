 ...
 * ・なんとか時間内ギリギリ自力AC！
 * ・2-SAT
 * ・場合分けでWAが取れずに死んで、怒りの2-SATで通した。
 * ・2-SATは基本的にor条件を並べていく仕組みなので、A&Bを条件にしたい時
 * 　どうするのかなかなか分からなかったんだけど、A|A and B|B ってやったらできた。
 */
...
void solve() {
    ...
    rep(i, M) {
        if (A[i] == N) {
            // A&B -> A|A and B|B
            ts.add_clause(i, true, i, true);
            ts.add_clause((i+1)%M, true, (i+1)%M, true);
        }
        if (A[i] == 0) {
            ts.add_clause(i, false, i, false);
            ts.add_clause((i+1)%M, false, (i+1)%M, false);
        }
        if (A[i] == N-1) {
            ts.add_clause(i, true, (i+1)%M, true);
        }
        if (A[i] == 1) {
            ts.add_clause(i, false, (i+1)%M, false);
        }
    }
    if (ts.satisfiable()) YES();
    else NO();
}
