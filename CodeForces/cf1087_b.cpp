/*
・自力AC！こういうのみんな早くて困る。
・実験すると、Nの候補は例えばK=3なら、0,0,0,1,2,0,2,4,0,3,6,0... みたいに、
　mod K周期で1の倍数、2の倍数、3の倍数、といった形を取っていくと分かる。
　これでmod K = 0の時の0,1,2,3を固定すると、その周期でNまで届くかどうか、
　と、ちょうどNがあるかどうか、が分かるので、該当するものを見つけたら、
　その時の回数(これも計算一発で出る)を出してそれが答え。
*/
...
void solve() {
    ...
    rep(a, 1, N+1) {
        // この周期でNまで届くかどうか and ちょうどNがあるかどうか
        if (a*(K-1) >= N and N%a == 0) {
            // この周期までの数 + この周期での数
            ll ans = K*a + N/a;
            print(ans);
            return;
