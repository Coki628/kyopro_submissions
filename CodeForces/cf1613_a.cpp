/*
・きっちり自力AC！
・結構遠回りしてる気もするけど、きっちりフォーマット合わせて比較していく。
　まずは桁数で見て、桁が同数なら、0でない桁が大きい方と同じ所まで小さい方の0を持ってきて、
　数値に戻して比較、ってやった。(桁数一緒なら文字列のままで良かったか…)
*/
...
void solve() {
    ...
    string s1 = tostr(x1), s2 = tostr(x2);
    ll d1 = s1.size()+p1;
    ll d2 = s2.size()+p2;
    if (d1 < d2) {
        print("<");
    ...
    ll num1 = toint(s1), num2 = toint(s2);
    if (num1 < num2) {
        print("<");
    } elif (num1 > num2) {
        print(">");
