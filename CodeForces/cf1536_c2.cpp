/*
参考：https://twitter.com/SSRS_cp/status/1401579049667891201
・自力ならず。。
・DとKの出現数と、その時点での比率(約分済)を持ちながら進む。
　そうすればその場所での比率のカウントが答え。
・比率をキーにしてカウントのアイディアは出なかったな。。
　それに、そう聞いてもなんか最初しっくり来なくて、その時点での比率が分かっても、
　前に同じ比率だったところからの増分がちょうど同じ比率になるとは
　限らなくない？って思ったんだけど、なるね。多分。。
・ほんとこういうの自明に見えない時は見えないんだよなぁ。。
*/
...
void solve() {
    ...
    map<pll, ll> C1;
    vector<ll> C2(2), ans(N);
    rep(i, N) {
        C2[A[i]]++;
        ll g = gcd(C2[0], C2[1]);
        C1[{C2[0]/g, C2[1]/g}]++;
        ans[i] = C1[{C2[0]/g, C2[1]/g}];
    }
    print(ans);
}
