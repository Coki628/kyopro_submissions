/*
参考：https://codeforces.com/blog/entry/97179
　　　https://twitter.com/laycrs/status/1463201470392639489
・自力ならず。。
・決め打ち二分探索、前から貪欲
・両側に条件ついてるしこんなんDPも無理そうだし何なん？？ってなってて終了。。
　にぶたんもちょっと考えたけど、招待人数決め打った所で条件がいい感じになる筋が全く見えなかった。
・実際は人数決め打つと、その時点でのある人以下の人数と以上の人数が一意に定まるので、
　その人を入れることができるかどうかの判定ができるようになる。これは説明聞いたら分かった。
・なんだけど、それでも前から貪欲で大丈夫なことが自分の中であんましっくりきてない。。
　条件の範囲によっては、後ろにいるやつ選んだ方が得になったりしないのかなー。
・あーでも今色々考えてたらある程度納得してきたかも。
　人数決め打ってると、条件ABの範囲に当てはまるかどうかだけが大事で、
　それを超えてキャパに余裕があるかどうかはどうでもよくなるから、
　だったらその時点で条件に合うやつは入れた方が得、って感じか。
　(仮に後ろのやつも条件に合ったとして、後ろのやつのキャパにどんなに余裕があっても、
　人数決めて判定してるとそれが関係ないから、結局前のやつ入れるのと変わらないので)
*/
...
void solve() {
    ...
    ll res = bisearch_max(0, N+1, [&](ll m) {
        ll cnt = 0;
        for (auto [a, b] : AB) {
            if (cnt <= b and m-cnt-1 <= a) {
                cnt++;
            }
        }
        return cnt >= m;
