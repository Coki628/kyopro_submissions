/*
・自力AC！ここはよく通した。
・部分列にLISみたいなチェックをして、3以上ならNG。
　区間長5以上だと絶対無理になるので、区間があまり伸びず2重ループ+breakで通る。
・部分列の切り出しがO(部分列長)かO(全体)か自信なかったんだけど、
　一応毎回切り出してLISライブラリに突っ込む版も試してみた。通った。
　けどやっぱちょっと遅いね。添字でやった方はAC0.12秒くらいなのに、
　こっちは0.23秒くらいかかってる。
*/
...
void solve() {
    ...
    rep(l, N) {
        rep(r, l+1, N+1) {
            vector<ll> cur = {A.begin()+l, A.begin()+r};
            auto rev = cur;
            reverse(ALL(rev));
            if (LIS(cur, 1).size() < 3 and LIS(rev, 1).size() < 3) {
                ans++;
            } else {
                break;
            }
