/**
 * ・きっちり自力AC！
 * ・縦横それぞれの総和はNで決まっているので、
 * 　そのNをそれぞれどこに割り振るか。
 * 　偶奇で分けて管理して、最小値以外は1しか使わないで、
 * 　余った分は全部最小値のところに割り当てるものとしてコストを出す。
 * 　これを何個目の区間まで使うか全探索する。
 */
...
void solve() {
    ...
    ll sma = A[0], smb = A[1], rest = N-1, cura = A[0]*N, curb = A[1]*N;
    multiset<ll> sea, seb;
    sea.insert(A[0]);
    seb.insert(A[1]);
    ll ans = cura+curb;
    rep(i, 2, N) {
        if (i%2 == 0) {
            ...
            sea.insert(A[i]);
            rest = N-sea.size();
            cura = (*sea.begin())*rest + sma;
        } else {
            smb += A[i];
            seb.insert(A[i]);
            rest = N-seb.size();
            curb = (*seb.begin())*rest + smb;
        }
        chmin(ans, cura+curb);
    }
    print(ans);
