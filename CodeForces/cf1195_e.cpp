/*
・せっかく2Dセグ木を整備したので思い出したやつをやってみた。
・当時みんなも言ってたけど、メモリがまじできつい。
　でも実際生配列取ってみると、8200*8200欲しいけどギリギリ無理で、8000*8000ならいける。
　2冪の区切りが4000ちょっとで制約が最大3000なので、
　おそらく最大ケースでは後ろの方使ってない領域がかなりあるように思える。
　なので8000以上の場所にはアクセスしないような条件付けてみたらいけた。
　やっぱり使ってなければ後ろが欠けてても大丈夫なんだね。
　まあでもTLもギリギリで1.87秒。900万にlog2つ乗るしね。メモリも256制限で250ちょい。危ねー。
*/
...
struct SegmentTree2D {
    ...
    SegmentTree2D(vector<vector<T>> &grid, const F f, const T &M1) : f(f), M1(M1) {
        ...
        for (int i = 0; i < (int)grid.size(); i++)
            for (int j = 0; j < (int)grid[0].size(); j++)
                if (H+i < M and W+j < M) dat[H+i][W+j] = grid[i][j];
        build();
    }
    ...
    void build() {
        for (int i = 2*H-1; i > H-1; i--)
            for (int j = W-1; j > 0; j--)
                if (i < M and 2*j+1 < M) dat[i][j] = f(dat[i][2*j], dat[i][2*j+1]);
        for (int i = H-1; i > 0; i--)
            for (int j = 1; j < 2*W; j++)
                if (2*i+1 < M and j < M) dat[i][j] = f(dat[2*i][j], dat[2*i+1][j]);
    }
        ...
        for (Lx += H, Rx += H; Lx < Rx; Lx >>= 1, Rx >>= 1) {
            if (Lx & 1) {
                ...
                for (T Ly1 = Ly + W, Ry1 = Ry + W; Ly1 < Ry1; Ly1 >>= 1, Ry1 >>= 1) {
                    if (Ly1 & 1 and Lx < M and Ly1 < M) vLy = f(vLy, dat[Lx][Ly1++]);
                    if (Ry1 & 1 and Lx < M and Ry1 < M) vRy = f(dat[Lx][--Ry1], vRy);
                }
                vLx = f(vLx, f(vLy, vRy));
                Lx++;
            }
            if (Rx & 1) {
                ...
                T vLy = M1, vRy = M1;
                for (T Ly1 = Ly + W, Ry1 = Ry + W; Ly1 < Ry1; Ly1 >>= 1, Ry1 >>= 1) {
                    if (Ly1 & 1 and Rx < M and Ly1 < M) vLy = f(vLy, dat[Rx][Ly1++]);
                    if (Ry1 & 1 and Rx < M and Ry1 < M) vRy = f(dat[Rx][--Ry1], vRy);
                }
                vRx = f(f(vLy, vRy), vRx);
            }
        }
...
void solve() {
    ...
    rep(i, M) rep(j, M) dat[i][j] = MOD;

    auto seg = get_segment_tree_2d(H, W, [](int a, int b) { return min(a, b); }, MOD);
    ll cur = g0;
    rep(i, N) {
