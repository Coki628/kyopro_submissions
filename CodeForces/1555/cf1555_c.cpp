...
・きっちり自力AC！
・ゲーム、全探索
・結局のところ、ボブは上に残ったやつか下に残ったやつのどちらかを取るしかない。
　各コースで上下に残る数を持っておいて、ボブはそのmax、アリスが全体からminを取れば答え。
*/
...
void solve() {
    ...
    ll bob1 = 0;
    ll bob2 = sum(grid[1])-grid[1][N-1];
    ll bob = max(bob1, bob2);
    rep(i, N-2, -1, -1) {
        bob1 += grid[0][i+1];
        bob2 -= grid[1][i];
        chmin(bob, max(bob1, bob2));
    }
    print(bob);
}
