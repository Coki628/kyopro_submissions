...
・自力AC！
・最大流、二部マッチング、復元
・基本的には最大マッチングして、残ったところを適当に繋ぐのだが、
　自分自身を選べない制約があるのでどうするか考える。
・ここで、自分を選びそうになった時、その頂点にもマッチングしたい要素があるはずなので、
　そこと繋ぎに行く。代わりに余ってしまうものが出てくるが、それは今つなごうとした方に繋げばいい。
　自分自身になってしまうこともないし、マッチング数も減らさずに付け替えができる。
・各頂点行き先が1本ずつしかないので、これフロー流さなくても適当貪欲で最大マッチングにならない？
　って思ってやってみたら、やっぱりなった。これでも大丈夫で問題なくAC。。
*/
...
void solve() {
    ...
    // rep(i, N) se.insert(i);
    // vector<ll> ans(N, -1), toi(N, -1);
    // for (auto [from, to, cap, flow] : mf.edges()) {
    //     if (flow and from < N and N <= to and to < N*2) {
    //         to -= N;
    //         ans[from] = to;
    //         se.erase(to);
    //         toi[to] = from;
