...
・なんとか自力AC。。
・調和級数、約数列挙、場合分け
・0回で済むのは当然別処理するとして、実はどんな時も2回やれば達成できる。
　Nで割るとすれば、Nより小さい全ての数では割り切れないので、N以外全部消せる。
　あとはNの約数でないような適当な値でもう1回やればいい。
　これで残るは1回でいけるものを網羅すればいいだけになった。
　Nが30万なので、まあ何とかならないかと思って愚直に最大N回約数列挙した。
　これでNGな約数を網羅できるので、2~NでOKな値があるかチェックする。
　調和級数logでいけそうな問題の雰囲気を察しながらもO(N√N)で通してしまった。AC1.34秒。
・後からちゃんと考えた。やっぱり調和級数でいけた。NGな地点を先に配列に埋め込んでおいて、
　そこに2~Nの調和級数ループ走らせればいい。これはやっぱ速い。AC0.03秒。
*/
