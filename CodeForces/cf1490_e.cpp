 ...
 * ・自力AC！
 * ・ソートして累積和
 * ・ある値の人が優勝まで行けるかは、「自分以下を全部集めて、1つ上に勝てるか」を
 * 　繰り返しててっぺんまで行けるかによる。
 * 　これは累積和を取っておけば、各位置O(1)で確認が取れる。
 * ・無理な箇所が来たらそこまでのを一度リセットして進めていって、最後まで残ったのが答え。
 * 　indexを使うので、ソートする前にペアにして取っておくこと。
 */
...
void solve() {
    ...
    acc[0] = AI[0].first;
    rep(i, 1, N) {
        acc[i] = acc[i-1]+AI[i].first;
    }

    ll cnt = 0;
    vector<ll> ans;
    rep(i, N-1) {
        if (acc[i] >= AI[i+1].first) {
            cnt++;
            ans.pb(AI[i].second+1);
        } else {
            cnt = 0;
            ans.clear();
        }
