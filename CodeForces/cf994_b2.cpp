/**
 * ・自力AC！
 * ・強さでソートして弱い順にやる。終わったやつmultisetなりに詰めておけば次以降大きいK個取れる。
 * 　最初重複考えてなくてsetに詰めててペナ…。
 * ・multiset使ったけど、優先度付きキューでやる方が普通っぽいよね。
 * ・はい、やってみた。C++のプリキューは第3引数なしだと降順ソートになって今回はちょうどいいね。
 */
...
void solve() {
    ...
    for (auto [a, i] : AI) {
        ...
        ll cnt = 0;
        vector<ll> tmp;
        while (!que.empty() and tmp.size() < K) {
            ll b = que.top(); que.pop();
            ans[i] += b;
            tmp.pb(b);
        }
        for (ll b : tmp) que.push(b);
        que.push(B[i]);
    }
    print(ans);
}
