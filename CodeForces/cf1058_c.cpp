/**
 * ・自力AC！けどここでモタついたのが今日の敗因。。
 * ・区間の数を2以上で自由な数に区切れる。こっち側に注目してると計算量が死ぬ。
 * ・Nは100だし各値も0〜9しかないので、総和が小さい。
 * 　こっちに注目すると、各区間に値をxずつ詰めるの全探索ができることに気付ける。
 * ・で、チェック処理はランレングス圧縮みたいな感じをやってくんだけど、
 * 　ちょっと罠があって、最後に0が連続するのは手前の区間に押し込めるのでOKになる。
 * 　0の扱いが怪しいのは結構疑ってたんだけど、これ気付くのに結構時間かかってペナ+タイムロス。
 * 　まだまだだなぁ、ってなるね。
 */
...
void solve() {
    ...
    rep(x, 1000) {
        ...
        if (cur == x) cnt++;
        if (ok and (cur == x or cur == 0) and cnt >= 2) {
            YES();
            return;
        }
    }
