/*
・自力AC！時間かかったけどまあ通せてよかった。
・区間or更新、ビット毎にDP
・前半パートはなんか貪欲に置くとか色々迷走したんだけど、
　結局どういう列が全区間の条件を満たすか考えると、全区間orすればいいとなる。
　これはビット毎に個数を持って、1つでもそのビットの立つ区間があれば立てる、とした。
・後半パートは部分列の数え上げなので、その要素を使う/使わないでDPする。
　ビット毎に見れば、xorなので今見ているビットが立ってるかどうかだけ状態に持てばOK。
*/
