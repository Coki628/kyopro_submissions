/*
参考：https://codeforces.com/contest/1609/submission/137225904
・あまりに今回は自分の実装方針がダメ過ぎたので、色々人のを観察した。
・これはPCTさんのを参考にしたもの。そもそもMOD累積和使ってない。
　結局1を伸ばせる区間は次の素数が来れば終わるので、毎回始点から左右に伸ばすのを普通にやっても、
　内側はトータルで+Nくらいにしかならず、2乗ループにはならない。
　これなら添字がどうだって話も全然問題にならない。
*/
