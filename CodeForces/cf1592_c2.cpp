/*
参考：https://codeforces.com/blog/entry/95583
　　　https://twitter.com/laycrs/status/1444702692115320833
・自力ならず。。
・木、DFS、XOR系
・0にならない時、連結成分は3つでいい。TLのみんなのこれが何でか分からなかったけど、
　その最も大事な考察は公式解説の一番上に書いてあった。
　連結成分同士でXORを等しくするので、その等しい成分同士がマージするとすると、
　0でない時、1つ付ければ0になるし、2つ付ければ元の値に戻る。で、これは一般に言えて、
　例えば成分7つが達成できるなら、2n個くっつけて3個にもできる。
　これに気付けないと、この問題を解くのは無理だった。
・自分はビット毎に見たのでその方針で話を進めるけど、全ビットが偶数個立っているなら、
　どこで何を切って2つにしても成分が等しくなるので即YES。
　そうでない場合、立てる必要のあるビットは明らかになるので、
　これに基づいて、条件を満たす部分木から積極的に切っていけば、
　これで3つ以上にできるか(それとKが3以上か)をチェックすればOK。
・で、実はビット毎に見る必要なんてなくて、全体で全XORを取れば同じことだった。。
　これはそっちでやった版。
*/
