/*
・自力AC！
・ちょっとややこしめなDP。てか問題設定が読みにくめ。
・ようは2次元DPで区間加算の遷移ができそうならよかったので、
　区間加算BITで殴った。でも貰うDPにすれば区間和取得で普通のBITでよかったよね。
・なんかサンプル合ったのに通らなくて何かと思ったら、
　問題設定の通りに後ろからDPしたおかげで、偶奇の前後が逆の時があった。。
*/
...
void solve() {
    ...
    vector<BIT2<mint>> dp(K, {N-1});
    dp[0].add(0, 1);
    mint ans = 1;
    rep(i, K-1) {
        if (i%2 == 0) {
            rep(j, N-1) {
                dp[i+1].add(j, N-1, dp[i][j]);
            }
        } else {
            rep(j, N-2, -1, -1) {
                dp[i+1].add(0, j+1, dp[i][j]);
            }
        }
        ans += dp[i+1].query(0, N-1);
    }
    if (K > 1) ans += 1;
    print(ans);
}
