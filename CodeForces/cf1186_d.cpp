...
・なんとか自力AC。時間かかった…。
・いずれにせよ整数部は確実に総和に加わる。
　後は小数部が0じゃなければ、切り上げて+-1するかしないか。
　元の総和が0になることが保証されているので、これで届かないことはない(はず)。
　元の数列を正負で分けて持って、どれを切り上げるかをよしなに見ていく。
・不等号に等号入れてないやつで時間溶かした。。
　小数部が0だと考えなくていいから等号除いてたやつを、
　そのまま整数の方に使ってたから。。
　整数の方は0.001とかでも正数側で使うから、a >= 0 にしないとだった。。
*/
...
pll convert(string &s) {
    ...
    ll a, b;
    if (s[0] == '-') {
        a = -toint(s.substr(1, i-1));
    } else {
        a = toint(s.substr(0, i));
    }
    b = toint(s.substr(s.size()-5));
    return {a, b};
}
...
void solve() {
    ...
    rep(i, N) {
        ...
        sm += a;
        if (b and a >= 0) A.pb(i);
        elif (b and a < 0) B.pb(i);
    }

    while (sm < 0) {
        auto i = A.back(); A.pop_back();
        ans[i]++;
        sm++;
    }
    while (sm > 0) {
        auto i = B.back(); B.pop_back();
        ans[i]--;
        sm--;
    }
