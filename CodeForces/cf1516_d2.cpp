/*
参考：https://twitter.com/SSRS_cp/status/1384893896296108033
　　　https://twitter.com/KaikeyPro/status/1384894653456134149
・自力ならず。。でも自分が向かってた方針で間違ってはなかった。
・素因数分解、尺取り、ダブリング、二分探索
・題意を満たそうとすると、互いに素な区間みたいのが知りたくて、
　素因数分解しながら尺取りみたいなことをしようとして無限バグらせで死亡。。
　でも終了後ちょっと直したらちゃんと動いた…。
・で、この区間が取れれば、ある左端から見て最大でどこまで行けるかの右端が分かるので、
　これを1回目の移動としてダブリングを構築すればクエリに答えられる。
　まあこのダブリングには気付けなかったから、
　尺取りうまくってても結局ダメだったんだけどね…。
・ちなみに尺取りパートは、素数列挙して各素因数毎に出現位置を通って、
　それぞれ累積minしたやつを全部合わせる、みたいにやっても作れるぽい。
　見た限りこっちでやってる人が多そうだった。
・ちなみに10万のN√Nで素因数分解尺取りやってるんだけどAC0.66。速い。
　map使わない速い素因数分解、最近入れたのが役に立ったね。
　これ制約1秒なのは、2秒でもよかったんじゃあ、って思うけどね。。
・SSRSさんの見てダブリング二分探索のにぶたんしなくていいやつやってみた。
　これだとlog2つは乗らないから、計算量きつい時はこっちのがいいかも。
　(今回は素因数分解パートがボトルネックぽいからあんま変わらなくて0.61秒。)
*/
...
void solve() {
    ...
    ll MXLOG = bit_length(N+1);
    auto nxt = doubling(MXLOG, B);
