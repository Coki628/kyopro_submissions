...
・これでやっとAC。
・DP遷移高速化、累積和同時進行DP
・色々やったんだけど、最後に効いたのが割り算の遷移の工夫で、
　小さい数の時は明らかに1ずつ値が変わるので1ずつ進めて、
　ある程度大きい所からは同じ値が続くのでにぶたんで次の変化する位置を探す。
　この境界を√Nにして平方分割っぽくしたら通らなくて、
　固定値で1000にしたら通った。AC5.44秒。(制約6秒)
*/
...
void solve() {
    ...
    vector<ArbitraryModInt> dp(N+2), acc(N+2);
    dp[N] = 1;
    const ll D = 1000;
    rep(i, N, 0, -1) {
        dp[i] += acc[i+1];
