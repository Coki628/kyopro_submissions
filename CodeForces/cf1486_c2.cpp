/**
 * 参考：https://twitter.com/yosswi414_0/status/1362445301198376961
 * ・自力ならず。この発想は全然出なかった。そういうことかー。
 * ・インタラクティブ、二分探索
 * ・まず全体での2番目の位置を確認。で、ここを軸足にして右か左に聞けば、
 * 　最大がどっち側にあるかは分かる。
 * ・そうしたらそっち側に向かって、2番目のものが変わるかどうかの境界を二分探索すれば、
 * 　その境界に最大値がある。
 * ・普通に半開区間でやったけど、こういう左右対称にやりたい時は閉区間のが分かりやすいかもー、
 * 　と思ったりした。
 * 　…で、ちょっとやってみたけど、結局+1-1するところは出てくるので、あんま変わらないかも。。
 */
...
void solve() {
    ...
    ll base = ask(0, N);
    if (base == 0 or ask(0, base+1) != base) {
        ll res = bisearch_min(base+1, N, [&](ll m) {
            return ask(base, m) == base;
        });
        answer(res-1);
    } else {
        ll res = bisearch_max(0, base, [&](ll m) {
            return ask(m, base+1) == base;
        });
        answer(res);
    }
}
