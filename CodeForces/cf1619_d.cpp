...
・自力AC！
・二分探索、最小値の最大化
・最小値の最大化はにぶたん！判定パート悩んだ。
　結局、N-1個も選べるので、2人以上満足しそうな店を優先して取って、
　残りはまだ満たしてないやつが含まれる店をよしなに、ってやったら通った。
*/
...
void solve() {
    ...
    ll res = bisearch_max(0, MOD, [&](ll m) {
        ...
        rep(i, M) {
            ...
            que.push({cur[i].size(), i});
        }
        vector<int> ok(N);
        ll cnt = 0;
        while (cnt < N-1 and que.size()) {
            auto [sz, i] = que.top(); que.pop();
            bool used = false;
            for (ll a : cur[i]) {
                if (not ok[a]) {
                    ok[a] = true;
