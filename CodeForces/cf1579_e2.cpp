...
・終了5分前くらいにAC！
・転倒数、BIT
・前に置くか後ろに置くか、その時点で転倒数の増加が少ない方にやる。未証明ぶん投げAC。。
・どうしてそれでOKか、後からゆっくり考えてみたけど、結局その時前にしても後ろにしても、
　後からやる要素にとっての相対位置は変わらないから、その時の操作は後の操作の優劣に影響しない。
　だったらその時一番いいのをやり続けるべきだよね、ってお気持ちでまあまあ納得した。
*/
...
void solve() {
    ...
    ll ans = 0;
    rep(i, N) {
        ll lsm = bit.query(0, A[i]);
        ll rsm = bit.query(A[i]+1, M);
        if (lsm <= rsm) {
            ans += lsm;
        } else {
            ans += rsm;
