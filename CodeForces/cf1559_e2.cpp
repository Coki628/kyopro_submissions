/*
参考：https://codeforces.com/contest/1559/submission/125965646
・upsolveした。
・メビウス関数、約数系包除、部分和DP、貰うDP、累積和でDP遷移高速化
・まずgcd毎に1~M=gを見るとして、各gで
　dp[i][j] := i番目まで見て、和がjとなり、gcdがgの倍数となる通り数
　みたいなDPをする。gcdがgの倍数となるためには、
　各遷移で足される値はgの倍数でなくてはならないので、このことに基づいた遷移をする。
　これを最終的にgcd=1のものに限定するには、
　DPが全部終わった後で、約数系包除を適用していけばいい。
・状態として使う値もgの倍数のみに限定されるので、そこをmapで持てば通らないかなー、
　って思ったけど、これだと状態は減ってもその内側の遷移はまだ愚直だから無理だった。。
・さて、遷移を高速化するために、遷移する範囲を1つの区間にする。
　各gでDPしていく時、gの倍数以外使わない訳だから、実質gを1とみなして、
　/gの世界で値を見るみたいなことをやれば、遷移はg飛ばしではなく連続になるので、
　累積和で貰うDPの遷移高速化みたいなのが作れる。
・これで無事AC。計算量は多分M*N*logM=10万*50*logだから500万*logで、やっぱり結構重くて、AC1.03秒。
*/
...
// メビウス関数
vector<int> eratosthenes_sieve(ll n) {
    ...
    rep(g, 1, M+1) {
        ...
        rep(i, N) {
            ...
            rep(j, D+1) {
                ll l = max(j-rd, 0LL);
                ll r = max(j-ld+1, 0LL);
                dp[i+1][j] = acc.query(l, r);
            }
        }
        C[g] = sum(dp[N]);
    }

    auto mobius = eratosthenes_sieve(M);
    mint ans = 0;
    rep(g, 1, M+1) {
        ans += C[g]*mobius[g];
    }
