/*
参考：https://twitter.com/kiri8128/status/1416454545480114176
　　　https://twitter.com/noshi91/status/1416452852218695680
・自力ならず。条件いっこ足りなかった。。
・文字列、辞書順最小、場合分け
・題意の操作である「prefixでsuffix」みたいなやつの長さを小さくすることを考えると、
　文字1種類の時以外はほぼ1〜2くらいで収まることに気付く。
　そこでその最初の1〜2文字を決めて、残りは辞書順に並べるとする。
・条件の内訳はほぼきりさんのツイートの通り。
　でも自分のは1つ考察が抜けていて、1個しかない文字が1つでもあれば、
　それを使ってf(t)=0にできることに気付かなかった。(全部1〜2で考えてた)
　これを踏まえると、無事ACした。(ソース内コメント「x,aaaabbbbbccccc...」の部分)
*/
...
void solve() {
    ...
    for (auto [k, v] : C) {
        if (v == 1) {
            string ans = {k};
            C[k]--;
            auto c = (*C.begin()).first;
            ans += string(C[c], c);
            while (next(C.find(c)) != C.end()) {
                c = (*next(C.find(c))).first;
                ans += string(C[c], c);
            }
            print(ans);
            return;
    ...
    char c1 = (*C.begin()).first;
    char c2 = (*next(C.begin())).first;
    string ans = string(2, c1);
    C[c1] -= 2;
    // aa,bababacacacaccccdddd...
    if (C[c1] <= (N-2)/2) {
        while (C[c1] > 0) {
            ans += c2;
            C[c2]--;
            ...
            C[c1]--;
            if (C[c2] == 0) {
                if (next(C.find(c2)) == C.end()) break;
                c2 = (*next(C.find(c2))).first;
            }
        }
        assert(C[c1] == 0);
        ans += string(C[c2], c2);
        while (next(C.find(c2)) != C.end()) {
            c2 = (*next(C.find(c2))).first;
            ans += string(C[c2], c2);
        }
        print(ans);
    // ab,aaaaaaacbbbbccccdddd...
    // or
    // ab,bbbbbbbaaaaaaaa
    } else {
        ans.pop_back();
        ...
        C[c2]--;
        // 3種類目があるか
        if (next(C.find(c2)) != C.end()) {
            char c3 = (*next(C.find(c2))).first;
            ans += string(C[c1], c1);
            ans += c3;
            C[c3]--;
            ans += string(C[c2], c2);
