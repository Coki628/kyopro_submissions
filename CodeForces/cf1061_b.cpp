/*
・自力ならず。。
・WAのテストケース見て、よくよくチェックすると自分の見落としていたポイントが分かった。
・前提として、ソートして横の同値の数と縦の差分が分かるようにしておく。
　で、同値の横の長さと縦の差分で長方形を作って、これの多い方の分使う。
　っていうのは、だいたい合ってたけど1つ足りなかった。
・これで縦のほうが多かった時、横より多かった分は、この長方形内で使わないで、
　横に自由に動かして使えるコマになる。横のが長い場所が来た時、
　この余りから補填すれば、縦の長さより余計に使う数を減らせる。
・いやー、むずい。こういう貪欲詰めは無理な時ほんと無理だからきつい。
*/
...
void solve() {
    ...
    ll cnt = 0, keep = 0;
    rep(i, 1, L) {
        ll h = rle[i].first-rle[i-1].first;
        ll w = rle[i].second;
        if (h >= w) {
            // 縦に使って余った分を保持しておく
            keep += h-w;
            ...
            ll need = w-h;
            // そこまでで余ってた分を使える
            ll use = min(need, keep);
            need -= use;
            keep -= use;
            // それでも足りない分だけここでのコストに足す
            cnt += need;
        }
    }
    ll sm = sum(A);
