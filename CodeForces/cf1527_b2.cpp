...
・なんとか自力AC！
・ゲーム、回文
・こっちの難しい版は最初の状態が色々あるので考えることは多い。
・とりあえずあと1手で回文になるような状態は結構逆転されたりしそうなので、
　そこを中心に色々頑張って場合分けガチャガチャやったら通った。。
・実はうまく状態を持つとDPもできたらしい。すげー。。
*/
...
void solve() {
    ...
    if (cnt == 0) {
        if (count(ALL(S), '1') == N) print("DRAW");
        elif (N%2 == 1 and S[N/2] == '0' and count(ALL(S), '0') >= 3) print("ALICE");
        else print("BOB");
    } elif (cnt == 1) {
        if (count(ALL(S), '0') == 1) print("ALICE");
        elif (count(ALL(S), '0') == 2) print("DRAW");
        elif ((count(ALL(S), '0')+(N%2 == 1 and S[N/2] == '0'))%2 == 1) print("ALICE");
        else print("BOB");
    } else {
        print("ALICE");
    }
