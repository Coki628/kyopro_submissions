 ...
 * ・自力ならず。。悔しい。でもこういう整数できるかなみたいのは、やっぱ苦手…。
 * ・約数列挙
 * ・素数列挙しようとしたり、pからyまでの間で素数を探したりしたけどWAで終了。。
 * ・多分だけど、[2,p]の倍数だとダメな訳だから、pより大きい素数の倍数なら
 * 　素数じゃなくてもOKなんだよね。悩んだ末そこまでは考えたんだけど、
 * 　じゃあ素数でもないそれをどう判定すればいいか、が見えずに終わった。
 * ・結果、約数列挙すればよかった。これならその約数の中に[2,p]のどれかが
 * 　含まれていないか判定すればいい。1回の約数列挙はO(√N)かかってしまうけど、
 * 　yから線形にpに遡って進めて行くと、条件に合う数はそう多くない回数で見つかる。
 * 　p,y間が短かったら、見つけられずに-1終了するだけ。
 * ・倍数探すなら約数でとか定石じゃんねーこれも。あー、ダメだった。
 */
