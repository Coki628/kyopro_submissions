/*
・Mo's algorithm、個数の個数
・結局、セグ木パートを線形にできれば通る、だった。
　これやろうとしたんだけど、個数の個数みたいのを管理しないといけなくて、
　頭壊れて終了だった。。ソース内コメントも参照。
*/
...
void solve() {
    ...
    auto add = [&](int idx) {
        ...
        C1[A[idx]]++;
        C2[C1[A[idx]]]++;
        // 個数の個数が1で値の個数が最大値+1なら上げる
        if (C2[C1[A[idx]]] == 1 and C1[A[idx]] == mx+1) {
            mx++;
        }
    };
    auto erase = [&](int idx) {
        // 個数の個数が1で値の個数が最大値なら下げる
        if (C2[C1[A[idx]]] == 1 and C1[A[idx]] == mx) {
            mx--;
        }
        C2[C1[A[idx]]]--;
        C1[A[idx]]--;
