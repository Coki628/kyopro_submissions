/**
 * ・なんとか自力AC。。今日の戦犯。
 * ・絶対貪欲あるんだろうと思ったけど手堅く先に思いついたDPを、ってのが沼だった。
 * 　dp[i][j] := i番目まで見て、最後に*を変換したのがjの時の最小値、
 * 　ってやったら思った以上に遷移がゴチャゴチャした。
 * ・普通に前から貪欲で進めるだけ進むでよかったみたい…。
 * 　今そう言われて考えてたら、確かに毎回進める最大距離はKで一定だから、
 * 　あえて手前に着地するメリットはないんだよな。。
 */
...
void solve() {
    ...
    auto dp = list2d(N+1, N+1, INF);
    dp[0][0] = 0;
    rep(i, N) {
        rep(j, i+1) {
            if (S[i] == 'x') {
                chmin(dp[i+1][i+1], dp[i][j]+1);
            } elif (S[i] == 'y') {
                if (i-j < K) chmin(dp[i+1][i+1], dp[i][j]+1);  
            } elif (S[i] == '.') {
