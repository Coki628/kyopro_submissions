/*
・自力ならず。
・最小+ちょこっとを全部見ればいけるんじゃね？が嘘でtest 61でWA。。
*/
...
void solve() {
    ...
    auto count = [&](ll sh, ll gh, ll sw, ll gw) {
        ll res = 0;
        ll n = gh-sh;
        ll m = gw-sw;
        rep(h, sh, gh) {
            rep(w, sw, gw) {
                ...
                ll j = w-sw;
                if (
                    i == 0 and j == 0 or i == n-1 and j == m-1 or
                    i == 0 and j == m-1 or i == n-1 and j == 0
                ) continue;
                char need = i == 0 or j == 0 or i == n-1 or j == m-1 ? '1' : '0';
                if (grid[h][w] != need) res++;
            }
        }
        return res;
    };
