/**
 * ・きっちり自力AC！これは早かった。JOIにありそうな最短経路問題。
 * ・ダイクストラ、頂点追加
 * ・とりあえず全部の町でのコストが必要なので、どこか共通の始点を作ることを考えてみる。
 * ・すると始点の頂点を作って、そこから各町への移動コストを各町での開催費用にすると
 * 　ちょうどよさそう、となる。
 * ・さらに観察して、往復の移動費が発生するのは各町間のコストを単純に2倍したらいけそう、となる。
 * ・これらを踏まえたグラフを構築して、素直にダイクストラを流すと無事AC。
 */
