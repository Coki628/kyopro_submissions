/*
・終了後自力AC？ちょっとTL見ちゃったけど…
・MEX、セグ木にぶたん
・セグ木に値突っ込んで、0から見ていく。
　1つ手前が0だった場合、2つ手前以前で2以上ある一番近い所から持ってくる。
　これで1つ手前を埋めながら進めていけば、次の値を考える時、2つ手前以前の心配はしなくていい。
　今の位置に値がある場合、それを1つ先にどかしたとみたしてコスト追加したものを答えとする。
　今の位置はまだ値を足したりしていないので、余計なコストは乗らない。
*/
...
void solve() {
    ...
    rep(i, 1, N+1) {
        if (seg[i-1] == 0) {
            ll j = seg.find_last(i-1, [&](ll m) { return m >= 2; });
            if (j == -1) {
                break;
            }
            seg.update(j, seg[j]-1);
            seg.update(i-1, seg[i-1]+1);
            cost += i-j-1;
        }
        ans[i] = cost+seg[i];
    }
