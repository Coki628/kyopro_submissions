/*
・自力AC！ここまできっちり通せたのは嬉しいね。
・グリッド、頂点拡張ダイクストラ
・グリッドだけど、コストがマスじゃなくて辺にかかるので、
　1次元グラフにエンコードした方が管理しやすそうだな、となる。
　20手先で自分に戻ってくるということは、10手先に最短で行ける場所が見つかれば、
　帰りも同じ道通ればよさそうだね、となる。
　手数も状態に持って、全マスからの多点スタートにしてダイクストラする。
・プレテスACだけど1.79秒ですげー不安。。。
・1.98秒で通ってた！！でも危ないー。
　TL眺めてたら、想定解はDPみたい。それだとlog乗らないから余裕なのね。。
　確かに毎回k+1への遷移でkのまま横移動みたいになることがないから、
　DAGとみなせてDPでいける、ってことか。
*/
...
vvl dijkstra(vector<vector<pair<ll, ll>>> &nodes, vector<ll> src, ll K) {

    int N = nodes.size();
    auto res = list2d(K+1, N, INF);
    ...
    while(!que.empty()) {
        auto [dist, k, u] = que.top(); que.pop();
        if (res[k][u] < dist) {
            continue;
        }
        for (auto [v, cost] : nodes[u]) {
            if (dist+cost < res[k+1][v]) {
                res[k+1][v] = dist+cost;
                if (k+1 < K) que.push({dist+cost, k+1, v});
            }
        }
    }
    return res;
...
void solve() {
    ...
    vector<ll> src;
    rep(i, N) src.pb(i);
    auto res = dijkstra(nodes, src, K/2);
    auto ans = list2d(H, W, -1);
    rep(h, H) {
        rep(w, W) {
            ll i = gridtoid(h, w, W);
