/**
 * 参考：https://twitter.com/ItoHdak_kyopro/status/1362200719550345216
 * ・自力AC！
 * ・貰うDPの遷移を高速化
 * ・dp[i] := 位置iの石に辿り着ける最大人数 みたいにする。
 * 　遷移は[M,i)個目までの合計とその位置を通れる人数A[i]でminを取る。
 * 　でもこれだけだと多くなり過ぎることがあって、各区間長Mの範囲で乗り切れる最大人数より
 * 　最終的に多くゴールすることはないので、それもminを取る。
 * 　必要なことは並べたつもりで、十分かどうかの自信はないけど無事AC。
 * ・TLでひでろーさんのコメントより。後半の区間長Mのチェックだけで通る。
 * 　前半パートのDP全く意味なかった…。
 */
