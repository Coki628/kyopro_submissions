/*
参考：https://twitter.com/laycrs/status/1380922420194803712
・自力ならず。間に合わなかったけど、時間あっても微妙にダメだった。
・基本的には、その時のランクでそのまま購入を目指すか、
　次のランクに最速で上がるよう頑張るかを両方試していく。
・ざっくり方針は合ってたけど、必要数が負数になる時、
　次に残す分があるのに0みたいにしちゃってた。。
*/
...
void solve() {
    ...
    ll ans = INF, curk = 0, curday = 0;
    rep(i, N) {
        ll need = K-curk;
        ll days = ceil(max(need, 0LL), A[i]);
        chmin(ans, curday+days);
        if (i != N-1) {
            need = B[i]-curk;
            days = ceil(max(need, 0LL), A[i]);
            // 負数のneedはそのまま次に残す分にできる
            curk = days*A[i]-need;
            curday += days+1;
        }
    }
