/*
・きっちり自力AC！これを順当に通せたのは良かった。
・正負で場合分け、DP高速化、セグ木で遷移高速化
・正負は分けて考える。正負をまたぐ時は補充できるので、別の回と考えていい。
　それで、ソートして手前から1～K個ずつ片付けてくとすると、DPにできる。
　K個手前からの遷移をまとめたいので、セグ木で高速化した。
*/
...
void solve() {
    ...
    ll sm = 0;
    rep(p, 2) {
        auto dp = get_segment_tree(M[p]+1, [](ll a, ll b) { return min(a, b); }, INF);
        dp.update(0, 0);
        rep(i, 1, M[p]+1) {
            ll mn = dp.query(max(i-K, 0LL), i);
            dp.update(i, mn+B[p][i-1]*2);
        }
        sm += dp[M[p]];
    }
    ll ans = sm-mx;
