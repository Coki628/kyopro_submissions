...
・自力AC！ここまで通せたのはいい感じ。
・インタラクティブ
・1と2で市松を目指して、どっちか埋まったらあとは適当に置けばOKになる。
　実装ちょっと悩んだけど、市松のどっちなのかを先にそれぞれに配列に分けて持たせて、
　そこから順番に使っていく、ってやったらいい感じにできた。
*/
...
void solve() {
    ...
    rep(_, N*N) {
        ...
        if (lim1 == 0) {
            ...
            if (a == 1) {
                auto [i, j] = V2.back(); V2.pop_back();
                answer(2, i, j);
                se.erase({i, j});
                lim2--;
            } else {
                auto [i, j] = V1.back(); V1.pop_back();
                answer(1, i, j);
                se.erase({i, j});
                lim1--;
            }
