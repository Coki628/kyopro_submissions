/*
参考：https://twitter.com/laycrs/status/1383102294921601026
　　　https://codeforces.com/blog/entry/89644
・自力ならず。Div.1みんなDよりこっち解いてたりするのやっぱ凄い。
・解くために見えないといけない考察が2つある。1つは、
　この操作をした「降順は隣と1しか離れちゃダメな順列」が
　詰まるところどういったものなのかで、これは
　「自由に(複数でもOK)区間を決めて、そこでリバースした順列」になる。
　これも自分で気付くの無理そうだったけど、
　3歩譲って実験とか頑張って気づけたとする。ここからもう1つある。
・2つ目は、リバース区間に入っているかどうかを01にエンコードすると、
　N-1ビットの2進数がちょうど辞書順と対応しているということ。
　言われた後だから今は納得できるけど、これ気付かないだろー。
　順列とビットとかあんまり組み合わさらないから紐付けにくいし。。
・さて、理屈が分かったところで実装だけど、これもちょい面倒で、
　0-indexedにするためにK--するとか、数列の後ろの方を弱くしたいので、
　小さい値とするため01にエンコードした時リバースするとか、
　01の連続する区間を取るのがランレングス圧縮のちょっと変形したやつ
　みたいになってややゴチャゴチャしたりとか、数列はN-1あるけど
　そこまでビット見ようとしたらオーバーフローで死ぬからそこはbit_length(K)までとか、
　N-1が基準なせいで添字がバグりやすかったりとか、
　まあ大変って程じゃないけど、そんなにサクッとって感じでもなかった。
*/
...
void solve() {
    ...
    vector<pll> P;
    ll l = -1;
    if (not B.empty() and B[0] == 1) l = 0;
    rep(r, 1, N-1) {
        if (B[r-1] == 1 and B[r] == 0) {
            P.pb({l, r});
        } elif (B[r-1] == 0 and B[r] == 1) {
            l = r;
        }
    }
    if (not B.empty() and l != -1 and B[N-2] == 1) {
        P.pb({l, N-1});
    }
