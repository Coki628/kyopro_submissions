/**
 * 参考：https://twitter.com/keijak/status/1374885147229745152
 * ・自力ならず。悔しいー。
 * ・グラフ、後ろからシミュレーション
 * ・最初こんなんきっとUFだろ、って雑にぶちこんで撃沈。最終的に近い方針には来ていたものの詰めきれず。
 * 　後ろからシミュるんだけど、もう後ろに抜けた要素については考えない、っていう点が抜けてた…。
 * 　あとしばらく無向グラフかと思って時間溶かしてた。有向グラフだった。
 * ・予め辺はsetで管理しておいて、後ろに行けるだけ行く。
 * 　この時後ろに抜けた要素については、最初フラグ管理して抜けてるなら無視みたいにしたんだけど、
 * 　それだとTLE。辺に限りがあるからどうせさっさとbreakに当たると思ってたんだけど、
 * 　そうでもない時もあるみたい。
 * 　全要素mapで管理して、抜けたやつはerase、次の開始位置はmapのlower_boundで探せる。
 * 　ってやったら無事AC。
 */
...
void solve() {
    ...
    rep(i, N-2, -1, -1) {
        ll u = A[i];
        bool ok = 1;
        // 次の開始位置(まだ残ってる添字)はlower_boundで見つける
        for (auto itr=mp.lower_bound(i+1); itr!=mp.end(); itr++) {
            auto [j, v] = *itr;
            if (not nodes[u].count(v)) {
                ok = 0;
                break;
            }
        }
