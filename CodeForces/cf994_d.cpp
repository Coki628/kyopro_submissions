/**
 * ・なんとか自力AC！
 * ・題意がよく分からない。サンプルの挙動から頑張って推測して、
 * 　有り得そうな可能性をガチャガチャ総当りしたら通った…。(6ペナ)
 * ・具体的には、vectorで持ってたのをsetにする、i->jだけでなくj->i方向のループも回す、とかやった。
 */
...
void solve() {
    ...
    rep(i, N) {
        ...
        rep(j, M) {
            ...
            se.insert(B[j].first);
            se.insert(B[j].second);
            if (se.size() == 3) {
                for (ll a : se) {
                    if ((a == A[i].first or a == A[i].second) and (a == B[j].first or a == B[j].second)) {
                        V.insert(a);
                    }
                }
            }
