 ...
 * ・自力AC！いい感じ。
 * ・フロー、最大流、2部マッチング、復元
 * ・頭いい貪欲がありそうな問題だが、フローが流せそうでもある。
 * ・実際、始点→日にち→人→終点でフローを流せる。
 * 　基本は流量1で、人→終点だけは各人を使える上限であるceil(N, 2)にする。
 * ・20万頂点、20万辺のグラフになるが、しっかりAC0.20秒。
 * 　確か最大流は二部グラフになるならかなり速くて、10万単位のグラフでも
 * 　前に通せたやつがあったから、これもいけると思ったらやっぱりちゃんと通った。
 * ・TLで話してるの見たんだけど、これは各辺の流量が1ではないから、
 * 　本来は2部マッチングと同じ速度ではいけないかもしれないらしい。
 * 　フローの計算量は難しい。
 */
...
void solve() {
    ...
    rep(day, M) {
        ...
        rep(j, k) {
            ...
            cin >> f;
            f--;
            mf.add_edge(day, M+f, 1);
        }
    }
    rep(i, N) {
        mf.add_edge(M+i, t, ceil(M, 2));
    }
    ...
    if (res == M) {
        ...
        for (auto edge : mf.edges()) {
            // 流量のある辺が答えに使われているので、edge.flowを確認する
            if (edge.from < M and edge.flow == 1) {
                ans[edge.from] = edge.to-M+1;
            }
        }
        print(ans);
    } else {
