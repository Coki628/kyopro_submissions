...
・自力AC！！やったね。
・DFS、木DP
・考えることはアドホックぽいけど、最終的にやることは木DPっぽいものに落ち着く。
・基本的には 葉の合計 - (budの数 - budに含まれない葉があるか) になる。
　理屈としては、budを取ったら適当な葉に繋げば葉を1つ減らせて、それは全部のbudに対して言えて、
　ただしbudに含まれない葉がない場合、最初の1回はbudの葉を使わないといけなくて、
　この場合budが1個無駄になるため、この分を-1するようにしてる。
・これで後は必要な情報をDFSで集めてくればいい。ただしここで1WA。見落としてた点があった。
　それは、budを取った時その取られた部分がまたbudになる場合があること。
　この場合はそれも使えるので、budの親ではそれが元々なかったものとして状態の管理を進めるといい感じになった。
*/
...
void solve() {
    ...
    vector<ll> dep(N, 0), is_bud(N, 0);
    ll leafcnt = 0;
    bool has_other_leaf = 0;
    ...
    dfs(dfs, 0, -1);
    ll ans = leafcnt - max(sum(is_bud) - (not has_other_leaf), 0LL);
    print(ans);
}
