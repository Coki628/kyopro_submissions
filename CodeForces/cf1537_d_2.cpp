...
・自力ならず。。
・ゲームDP、実験
・愚直書いてそこに間違いはなさそうだったんだから、ちゃんと実験すればよかった。
　与えられるのNだけだからすぐできるし。。
・やってみるとすぐ見える。ざっと見るとまず偶数だと先手勝ち、奇数だと後手勝ち。
　よく見ると偶数なのに先手負けがたまにあって、
　そこだけ注目すれば、2の奇数乗の時だけ負けなのも分かる。
*/
...
void solve() {
    ...
    set<ll> ng;
    ll k = 1;
    while (pow(2, k) <= 1000000000) {
        ng.insert(pow(2, k));
        k += 2;
    }
    if (N%2 == 1 or ng.count(N)) {
        print("Bob");
    } else {
        print("Alice");
    }
