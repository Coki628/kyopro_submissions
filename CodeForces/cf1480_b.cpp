/**
 * ・システス落ち。。
 * ・攻撃力の低い敵から貪欲に倒しにいくシミュをした。方針は問題なかった。
 * 　落ちた原因は条件判定の >= hp が > hp になってたところだった。。
 * 　注意力大事も分かるけども、このくらいプレテス入れといて欲しいよね。。
 * ・って言ってたら何かオーバーフローするケースがあったとかでハック無効になって、
 * 　リジャッジでACになってた！運がいいー。
 */
...
void solve() {
    ...
    rep(i, 0, N) {
        ...
        ll t = ceil(AB[i].second, at);
        // 必要回数-1で力尽きてしまうようだと即NG
        if ((t-1)*AB[i].first >= hp) {
            NO();
            return;
        }
        hp -= t*AB[i].first;
