/*
・なんとか自力AC！
・隣り合う所が単調増加かどうかだけ見たらWAで、結構悩んだ。
　色々検討した末、これだとその2値の間に挟まるような要素がある時ダメなのが分かり、
　ソート済と比較して、ソート済でも隣り合うペアは区切らなくていい、とやって無事AC。
*/
...
void solve() {
    ...
    set<pll> good;
    rep(i, N-1) {
        good.insert({B[i], B[i+1]});
    }

    ll cnt = 0;
    rep(i, N-1) {
        if (good.count({A[i], A[i+1]})) cnt++;
    }
    ll need = N-cnt;
    if (need <= K) Yes();
    else No();
