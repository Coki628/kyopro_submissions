/*
参考：https://codeforces.com/blog/entry/89810
・公式解の方針。結構TLでもやってる人多そうだった。
・乱数、確率
・今回の題意では区間内で過半数を占めるような要素があればそれが欲しい。
　逆にそういうものがなければ、別に最多要素を取得できる必要もない。
　ここから、ランダムに適当な数を区間から抽出し、その時に選ばれた要素だけを
　最多かもしれないとみなしてチェックする、としていい。
　回数は公式解説に従って40回としてみた。過半数ならまあ2回に1回は当たるはずなので、
　40回やって外す可能性は2^(-40)となり、まずない。
・ちなみにある区間内でその要素が出現した回数を高速にチェックするには、
　隣接リストに要素毎でindexを持たせてにぶたんするといい。
　こういうのもパッと出るといいよなー。言われたらああってなるけど、
　自分ですぐどうすればいいか方針が出るようにならないとねー。。
・実際やってみて1つ得た知見があった。
　乱数のシード生成？のパートはちょっと計算量がかかるっぽくてTLEした。
　そこは外側で1回だけやって乱数取得の時にはやらないようにしたら無事AC2.25秒。
*/
...
void solve() {
    ...
    rep(_, Q) {
        ...
        rep(_, 40) {
            ll x = randrange(l, r);
            chmax(mx, (ll)bisect_left(adj[A[x]], r)-bisect_left(adj[A[x]], l));
        }
        ll cnt = 1;
