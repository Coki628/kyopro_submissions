/**
 * ・自力AC！
 * ・何がしたいのかを読み解くのにちょっと時間かかった。
 * ・ようは01が切り替わる瞬間だけ気をつければよくて、条件を満たすようにl,rを更新していく。
 * 　区間の文字をチェックしたり値のmin,maxを取ったりするけど、区間長が固定5なので、
 * 　特にセグ木とか考えないで普通にやって問題ない。
 */
...
int main() {
    ...
    rep(i, 4, N) {
        if (S.substr(i-4, 4) == "0000" and S[i] == '1') {
            ll mx = -INF;
            rep(j, i-4, i+1) chmax(mx, A[j]);
            chmax(l, mx+1);
        }
        if (S.substr(i-4, 4) == "1111" and S[i] == '0') {
            ll mn = INF;
            rep(j, i-4, i+1) chmin(mn, A[j]);
            chmin(r, mn-1);
        }
    }
    print(mkp(l, r));
