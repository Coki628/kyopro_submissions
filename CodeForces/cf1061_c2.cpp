/*
参考：https://twitter.com/Soleil_7103/status/1371627808166834179
　　　https://twitter.com/_yamate11/status/1371623602504704003
　　　https://codeforces.com/blog/entry/63384
・自力ならず。。
・エラトステネスの篩応用、約数列挙、貰うDP、inplace DP
・篩の気持ちになれば、全体の遷移はそう多くないはず、みたいな考察はまあよかった。
　B解けなくて焦ってたとはいえ、うまく落とし込めないのは実力不足。。
・まずDPの形として、今回は数列の何番目まで行ったかが重要で、
　その時の値がどうとかは遷移に影響しない。よって遷移元となる状態としては
　「今数列の長さがjで」が分かればいい。
　これでi側としては「数列i番目まで見て」とするが、こっちは使い捨てる。
・題意は、数列のi番目にはiで割り切れるものだけが遷移してこれる、なので、
　各位置iについて約数列挙しておけば、貰うDP遷移を網羅できる。
　今回は自分が篩を使って倍数からやろうとしていたので、
　近い方針だったYoikaさんのを参考に遷移を準備した。
　篩のループで自分の倍数全てに対して、自分を約数として渡していくような感じ。
　100万までの数で約数全部を隣接リストに陽に持っても大丈夫なんだな。。
　なんかすごい数大きくなりそうだけど、きっとlogくらいなんだろう。
・遷移が作れたら、後は準備した遷移に従って進めればいい。
　ナップザックを数列1個でやる時と一緒で大きい方からやらないとバグるので、
　約数はリバースさせるなりしておくこと。
・公式解に寄せた、毎回約数列挙する方針。多分こっちが主流。
　篩の100万まで全列挙よりは、√かかっても数列10万のが軽いみたいで、
　篩の方はAC1.43秒、こっちはAC0.94秒。
*/
