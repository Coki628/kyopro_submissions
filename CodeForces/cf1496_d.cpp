/**
 * ・なんとか自力AC！終了1分前でマジのブザービートだったな。。
 * ・ゲーム系
 * ・とりあえず、先手が勝てる見込みはかなり少なそうに思える。
 * 　実際動かしてみると、山の頂上しか可能性はなさそう。
 * ・で、その山の頂上であっても、他の所にもっと長い山があると、
 * 　そこで時間を使われてしまうので負ける。
 * ・さらに、今の山での勝負になるとして、ここはほぼサンプルエスパーなんだけど、
 * 　左右の高さが同じかつ、距離が偶数の時しか勝てない、ってした。
 * ・ほんとにお気持ちでしかやってなくて、システス全く自信なかったんだけど、
 * 　通ってしまった。最後まで諦めずに粘るもんだね。
 */
...
void solve() {
    ...
    SegTree<ll> segl([](ll a, ll b) { return max(a, b); }, -INF);
    segl.build(L2);
    SegTree<ll> segr([](ll a, ll b) { return max(a, b); }, -INF);
    segr.build(R2);

    ll ans = 0;
    rep(i, N) {
        if (L1[i] > 0 and R1[i] > 0) {
            ll l = i-L1[i];
            ll r = i+R1[i]+1;
            ll lmx = max(segl.query(0, l+1), segl.query(r, N));
            ll rmx = max(segr.query(0, l), segr.query(r-1, N));
            ll mx = max(lmx, rmx);
            if (mx >= max(L1[i], R1[i])) continue;
            if (L1[i] == R1[i] and L1[i]%2 == 0) ans++;
        }
    }
    print(ans);
}
