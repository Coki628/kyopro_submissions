/*
・ちゃんとした方もやった。
・同じ数字のところに注目して、jを固定すると、
　i0*(N-j)+i1*(N-j)+i2(N-j)+... みたいになって、これを変形すると、
　(i0+i1+i2+...)*(N-j) となってiのループは累積でまとめられる。
　jのループを回せば、ペアにできるiは自分より手前のものだけなので、
　これはそのjが終わる時に自分を値毎にmap管理した和に足す、とすればいい。
*/
