/*
参考：https://codeforces.com/contest/1529/submission/117248981
　　　https://codeforces.com/blog/entry/91058
　　　https://twitter.com/laycrs/status/1396872750703136769
・難しかったけど、グラフ強くなりたいしupsolveした。
・HL分解、区間和取得、DFS
・まず題意を整理すると、木1で全ての頂点が直系の関係でかつ、
　木2で全ての頂点が直系の関係にないような集合の最大値、
　って感じに多分なる。
・木1の条件を満たすために木1をDFSして、
　入りで頂点追加、戻りで頂点削除をする。
・木2の条件を満たすために、木2のグラフはHLDに展開しておく。
　DFSで木1の条件は満たすので、頂点追加の時に木2の条件を満たせるか
　チェックしていくようにする。具体的には、
　今見ている頂点の部分木に含まれる頂点があったらここは入れない。
　これは、なるべく葉に近い頂点を採用していった方が、
　直系に被る機会が少ないかな、という考えからでいいと思う。
　同じ理由で、根に向かうパスのチェックでは、逆にそっちを外して
　今見ている頂点を集合に追加する。
　この時、消す方の頂点を残した方が得するケースがないか
　だいぶ心配になったけど、よくよく考えていくと、
　DFSの都合上多分それは大丈夫かな、となる。多分。。
　あと根に向かうパス上に含まれる頂点が2つ以上あるかもと思ったけど、
　もしあったらそうなる前に消されるはずなので、これはない。
・ここまで来れば、後は戻り時に状態をきっちり戻しながら進めるだけ。
・よく考えたら、set併用しないでBIT1本で十分だったので、整理。
*/
    ...
    auto dfs = [&](auto&& f, ll u) -> void {
        ...
        if (bit.query(hld.in[u], hld.out[u]) == 0) {
            ...
            hld.add(0, u, [&](ll a, ll b) {
                auto res = bit.bisearch_fore(a, b-1, 1);
                if (res != -1) erased = res;
            });
            if (erased != -1) {
                bit.add(erased, -1);
