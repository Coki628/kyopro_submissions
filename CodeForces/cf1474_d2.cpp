/**
 * 参考：https://twitter.com/titia_til/status/1351570101980418049
 * ・前後から累積で前計算
...
bool check(vector<ll> A) {
    ll N = A.size();
    vector<ll> acc1(N, -1), acc2(N, -1);
    acc1[0] = A[0];
    rep(i, 1, N) {
        if (acc1[i-1] > A[i]) break;
        acc1[i] = A[i] - acc1[i-1];
    }
    acc2[N-1] = A[N-1];
    rrep(i, N-2, -1) {
        if (acc2[i+1] > A[i]) break;
        acc2[i] = A[i] - acc2[i+1];
    }
    rep(i, 0, N-1) {
        if (acc1[i] != -1 and acc1[i] == acc2[i+1]) {
            return true;
        }
    }
    return false;
