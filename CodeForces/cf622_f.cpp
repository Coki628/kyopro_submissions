...
・でもこの問題を見て、K+1次式になりそうって判断は数学できないと分からなそう。。
　それが分かると、K+2項までの和を計算しておけば、ライブラリに突っ込むだけになる。
・多分だけど、使う値の種類がK冪だけだからK次式っぽくて、
　和にするから次数1つ増えてK+1次式、って感じなんだと思う。
*/
...
void solve() {
    ...
    F[0] = 1;
    rep(i, 1, K+2) {
        F[i] = F[i-1]+pow((i+1), K, MOD);
    }
    auto res = lagrange_polynomial(F, N-1);
    print(res);
}
