...
参考：各種ツイート
・自力ならず。。
・区間リバース、真ん中固定、偶奇場合分け、累積和
・なんか先頭から見てて、前の値使えないじゃん、、って詰んでた。
　強い人にはお馴染みの典型要素っぽい。
・区間リバースは真ん中を固定すると、左右に伸ばした部分は共通になって再利用できる。
　これを利用すればN^2で全区間チェックできる。
・区間を前後に伸ばすので、添字がバグらないように気をつける。
　また、区間長の偶奇でも変わってくるので、同じ位置で偶奇両方見る。
・ソース内コメントも参照。
*/
...
void solve() {
    ...
    rep(i, N) {
        ...
        auto rev = A;
        // ここを中心とした区間で一番長いところまでをリバース
        reverse(rev.begin()+l, rev.begin()+r+1);
        auto ABrev = AB;
        rep(j, l, r+1) {
            ABrev[j] = rev[j]*B[j];
        }
        ...
        r = i;
        ll cur = ABrev[i];
        chmax(ans, cur+acc.query(0, i)+acc.query(i+1, N));
        l--;
        r++;
        // リバース区間を前後に1ずつ伸ばしていく
        while (l >= 0 and r < N) {
            cur += ABrev[l]+ABrev[r];
            chmax(ans, cur+acc.query(0, l)+acc.query(r+1, N));
            l--;
            r++;
        }
