 ...
 * ・グリッド、構築
 * ・あっとこで類題あったよね。
 * ・グリッド全体を十字で切ってABCDの領域みたいにして、欲しい連結成分の数だけ、
 * 　自分の所じゃない場所に点を打ち込んでく。
 */
...
void solve() {
    ...
    rep(i, H/2) rep(j, W/2) grid[i][j] = 'A';
    a--;
    rep(i, H/2) rep(j, W/2, W) grid[i][j] = 'B';
    b--;
    rep(i, H/2, H) rep(j, W/2) grid[i][j] = 'C';
    c--;
    rep(i, H/2, H) rep(j, W/2, W) grid[i][j] = 'D';
    d--;
