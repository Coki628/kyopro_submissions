...
・なんとか自力AC！危なかった。
・枝刈り全探索
・求める答えの右辺がi+jなので、ここはせいぜい2*Nくらいしか取らない。
　対して察辺はai*ajでこれはかなり大きくなる。でもそこは見る必要がない。
　問題文に強調してdistinctって書いてある。つまり全部1とかってことがないので、
　ペアの大半は大きくなりすぎてi+jと比較する必要がない。
　よってソートして値の小さいペアからチェックしていって、
　N*2より大きくなったところで枝刈りすればOK。
・最初、A[i]*A[j] == i+j+2 をなんとか式変形こねくり回して
　i,jを同じ側にできないかとかやってたんだけど無理だった。。
*/
...
#define rep3(i, a, b, c) for(int i = a, _bb = b; (a <= i && i < _bb) or (a >= i && i > _bb); i += c)
