/*
・これは考察ミスっててWA。。
・でも全方位のパーツは他の所で使えるかもしれないから取っておく。
・「その頂点を根とした木で、根から一番近い葉への距離」を求めてる。
　遠い方はやったことあったけど、近いのは初めてでちょっと工夫が必要だった。
　まず事前にその頂点が葉かどうかの情報を調べておいて持たせて、それによって遷移を場合分け。
　それと根は自分の頂点情報を(おそらく)全方位の最中に取れないから、根の頂点分の判定は最後に別途やる。
*/
...
void solve() {
    ...
    rep(i, N) {
        is_leaf[i] = nodes[i].size() == 1;
    }
    auto rr = get_rerooting(N, f1, f2, INF, false);
    for (auto [u, v] : edges) {
        rr.add_edge_bi(u, v, is_leaf[v], is_leaf[u]);
    }
    auto res = rr.build();
    rep(i, N) {
        if (is_leaf[i]) res[i] = 0;
    }
    ll ans = 0;
    rep(i, N) {
        if (res[i] >= K) ans++;
