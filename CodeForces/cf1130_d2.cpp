/*
・終了後自力AC！
・積める量と一度に下ろす量は自由なのに対し、乗せる時だけ1回に1つなので、
　ここを軸に考える。
・まずある場所で積む予定の個数があったら、その個数-1周は絶対必要。
　最初の回収だけは始点となる場所からの距離になる。
　で、最後のやつを下ろすために、最後に積んだ時点からいくらか必要になり、
　これは下ろす場所までの距離が一番小さいものを使うのが最善。
・これらをまとめると、回収する回数と、下ろす場所への最小距離を事前に取っておけば、
　始点となる場所iと回収する場所jを決めて、
　N*(回数-1)+(i~j間の距離)+下ろす最小距離、でここに必要な時間が分かる。
　後は各jに対してやったものの最大値が始点iでの必要時間なので答えで、
　制約N<=5000から、N^2は間に合うのでこれで大丈夫。
*/
...
void solve() {
    ...
    rep(i, N) {
        rep(j, i, i+N) {
            if (C[j%N] == 0) continue;
            chmax(ans[i], N*(C[j%N]-1)+(j-i)+mn[j%N]);
        }
    }
    print(ans);
}
