...
・自力AC！
・セグ木、区間OR、ビット、にぶたん
・種類が50しかないのでビットで管理。or演算をセグ木に乗せてにぶたんした。
　後ろ半開区間なのにjをj-1ってしないでバグって時間食ったのはもったいない。
・その要素が前から何番目なのかは、存在位置を別途BITで管理した。
・セグ木にぶたんで30万にlog2つ乗ってるからまあ重い。AC0.56秒。
・Funcionをラムダにしてみた。やっぱちょっと速くてAC0.42秒。
　この辺ちゃんと整備しようかなー。。
・整備した。にぶたんもちゃんといいやつにした。AC0.17秒。
*/
...
template<typename Monoid, typename F>
SegTree<Monoid, F> get_segment_tree(int N, const F& f, const Monoid& M1) {
  return {N, f, M1};
}
...
void solve() {
    ...
    auto seg = get_segment_tree(Q+N, [](ll a, ll b) { return a|b; }, 0LL);
    rep(i, N) {
        seg.set(Q+i, 1LL<<A[i]);
    }
    seg.build();
    ...
    rep(i, Q) {
        ...
        ll j = seg.find_first(0, [&](ll x) { return x>>t & 1; });
        ans[i] = bit.query(0, j+1);
        seg.update(Q-i-1, 1LL<<t);
        bit.add(Q-i-1, 1);
        seg.update(j, 0);
        bit.add(j, -1);
    }
