/*
・自力AC！ここまで通せて嬉しい。
・差分GCD、セグ木で区間GCD取得、セグ木にぶたん
・動きを観察していくと、差分の列でGCD取っていって、1になる所は無理そうに見える。
　実際これが当たりで、ある左端から1にならない所まで右端を伸ばす感じにやる。
*/
...
void solve() {
    ...
    auto seg = get_segment_tree(N-1, [](ll a, ll b) { return gcd(a, b); }, 0LL);
    rep(i, N-1) seg.set(i, B[i]);
    seg.build();

    ll mx = 0;
    rep(i, N-1) {
        ll j = seg.find_first(i, [](ll x) { return x == 1; });
        if (j == -1) j = N-1;
        chmax(mx, j-i);
    }
    ll ans = mx+1;
