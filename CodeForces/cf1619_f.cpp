/*
・本番中時間なくて、問題ちゃんと読めてなかったんだけど、簡単って聞いたから通しておいた。
・構築、プリキューでシミュ
・題意の理解が一番の難所まである。何やってるか把握するまでが。
　あとまあ構築だから実装がちとめんどい。とはいえそれでもD,Eより易しい気はする。
・プリキュー使ったとかのネタバレは聞いてたんだけど、これもまあ思いついたはず。
*/
...
void solve() {
    ...
    rep(_, K) {
        vector<bool> is_bigger(N+1);
        vector<pll> tmp;
        rep(_, m) {
            vector<ll> biggers;
            rep(_, d+1) {
                auto [cnt, i] = que.top(); que.pop();
                cnt++;
                tmp.eb(cnt, i);
                is_bigger[i] = true;
                biggers.eb(i);
        ...
        rep(_, M-m) {
            vector<ll> smallers;
            rep(_, d) {
                while (i <= N and is_bigger[i]) i++;
                if (i <= N) smallers.eb(i);
                i++;
            }
            assert(smallers.size() == d);
            cout << d << ' ';
            print(smallers);
        }
    }
