/*
・きっちり自力AC！
・A[i][j] > B[i][j]なところをスワップさせて、うまくいってるか確認する。
・1次元の場合で考えると、全部を大きい方と小さい方にそれぞれ寄せた方が
　うまくいきそうなのが見えてくる。実際、2次元でもこの考えが適用できる。(証明AC…。)
*/
...
void solve() {
    ll H, W;
    cin >> H >> W;
    auto A = list2d(H+1, W+1, 0LL);
    auto B = list2d(H+1, W+1, 0LL);
    rep(i, 1, H+1) {
        rep(j, 1, W+1) {
            cin >> A[i][j];
        }
