/*
・自力AC
・脳筋なのでcomb(7, 3)通りと部分列2^3通りを全探索した…。
　しかも内側の2^3は何か3通り遷移する気がして再帰全探索を書いちゃったんだけど、
　これ2通り遷移だからビット全探索でよかったよね。。
・やった。AC確認。やっぱこっちのがまださくっと書けるね。
*/
...
void solve() {
    ...
    for (auto& comb : combinations(B, 3)) {
        vector<ll> res;
        rep(bit, 1, 1<<3) {
            ll cur = 0;
            rep(i, 3) {
