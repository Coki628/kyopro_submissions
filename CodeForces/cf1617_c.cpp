...
・きっちり自力AC！
・modの性質
・mod取ると半分未満みたいになる性質を使う。
　具体的には「/2切り上げ-1以下」なら何でも作れるっぽい。それより大きいのは無理。
　元々順列に含まれる数はそのまま残して、
　余ったやつでその値の半分未満くらいの数は作れるはずなので、
　大きいやつ同士とか貪欲にペアにして無理になったらNG。
*/
...
void solve() {
    ...
    rep(a, 1, N+1) {
        if (!C[a]) {
            ans++;
            while (B.size() and ceil(B.back(), 2)-1 < a) {
                B.pop_back();
            }
            if (B.empty()) {
                print(-1);
