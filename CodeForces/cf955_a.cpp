/**
 * ・きっちり自力AC！
 * ・処理は書いてある通りを実装して、開始時と20時をやる。
 * 　20時の位置を見つけるのが面倒だったので24時間全探索した。
 */
...
ld calc(ll i, ll hh, ll mm, ll H, ll D, ld C, ll N) {
    H += i*D;
    mm += i;
    auto [d, m] = divmod(mm, 60);
    hh += d;
    mm = m;
    if (hh >= 24) {
        return INF;
...
int main() {
    ...
    ld ans = INF;
    rep(i, 24*60) {
        chmin(ans, calc(i, hh, mm, H, D, C, N));
    }
    print(ans);
    return 0;
}
