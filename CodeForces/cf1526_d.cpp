/*
参考：みんなのツイート
・自力ならず。まあC2がギリギリACだったので、こっちはちょっとしか考察しなかった。
　4文字順番全探索は最後の方でちらっと思ったんだけどね。
　確度全然なかったし、時間外で試すほどの気にはならなかったな。。
・順列全探索、転倒数
・隣接スワップだし転倒数みがあるなーとは思ったもののその先に進まず。
　正解は、同じ数は繋げて、4!通り全探索、だった。
　確かにこれだと4文字を0〜3にエンコードしとけば、
　それをソートするとみなして操作回数 = 転倒数にできるね。。
・順列全列挙も転倒数もライブラリがあるので、コードは結構すっきり。
　考察が生えなきゃ意味ないんだけど。。。
・計算量がNの10万*4!の24*logだから10万にlog2つ乗るくらいの感じで、
　AC0.83秒。2秒だからまだ大丈夫だけど結構かかってる。。
*/
...
void solve() {
    ...
    ll mx = -1;
    string ans;
    for (auto perm : permutations(vector<char>({'A', 'N', 'T', 'O'}))) {
        map<char, ll> mp;
        rep(i, 4) mp[perm[i]] = i;
        vector<ll> A(N);
        rep(i, N) A[i] = mp[S[i]];
        auto res = get_inversion(A);
        if (res > mx) {
            mx = res;
            ans = "";
            rep(i, N) {
