...
・疲れた。終了後、自力TLE。。
・再帰全探索
・文字を操作する順番は決まっているので、境界としてありうる所を全探索したらTLEした。
　失敗パターンは結構すぐNGが決まるはずなので行ける気もしたんだけどなぁ。。
　まあそれにしたってこの実装仕上げるのめちゃくちゃ時間かかったからね。しょうもない。
*/
...
void solve() {
    ...
    string ans;
    auto rec = [&](auto&& f, ll i, ll j, const string &prv) -> bool {
        ll r = j, cnt = 0;
        string cur;
        while (r >= 0 and (cnt == 0 or cur != prv)) {
            if (T[r] == seq[i]) {
                cnt++;
            } else {
                cur += T[r];
