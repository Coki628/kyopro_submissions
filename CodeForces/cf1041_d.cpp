 ...
 * ・自力AC！
 * ・区間、累積和にぶたん
 * ・区間に入っている間は高度が下がらないので、そこは全て飛距離に含められる。
 * 　区間外にいる間は高度が1ずつ下がるので、区間外を進む距離を全部足してH未満で収まるなら、
 * 　まだ着地しないで済むと言える。
 * ・ここから、i番目の区間から使い始める、を全探索すればいい。
 * 　区間外部分も累積和を取っておいて、これをにぶたんすれば、何個先の区間まで進めるか分かる。
 * 　あとは含められる区間の区間和+HをchmaxすればOK。
 * ・区間が1つしかない時、例外処理する。サンプルにあるのが優しい。
 */
...
void solve() {
    ...
    rep(i, N-1) {
        B[i] = seg[i+1].first-seg[i].second;
    }
    ...
    vector<ll> acc1(N+1), acc2(N);
    rep(i, N) acc1[i+1] = acc1[i]+A[i];
    rep(i, N-1) acc2[i+1] = acc2[i]+B[i];

    ll ans = 0;
    rep(l, N-1) {
        ll r = bisect_left(acc2, H+acc2[l]);
        chmax(ans, acc1[r]-acc1[l]+H);
    }
    print(ans);
}
