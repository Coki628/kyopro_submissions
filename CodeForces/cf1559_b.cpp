...
・自力AC！遠回りはした。。
・DP復元
・最適なのは1つ前の状態を持てばDPで求まる。でも構築なので復元が要る。。
　絶対貪欲あるだろうと思いながらも頑張ってDP復元実装した。
*/
...
pll calc(const pll &a, const pll &b) {
    if (a.first <= b.first) {
        return a;
    } else {
        return b;
...
void solve() {
    ...
    auto dp = list2d(N+1, 2, mkp(INF, -1LL));
    dp[0][0] = dp[0][1] = {0, -1};
    rep(i, N) {
        if (S[i] == 'R' or S[i] =='?') {
            dp[i+1][0] = calc(dp[i+1][0], {dp[i][0].first+1, 0});
            dp[i+1][0] = calc(dp[i+1][0], {dp[i][1].first, 1});
        }
        if (S[i] == 'B' or S[i] =='?') {
            dp[i+1][1] = calc(dp[i+1][1], {dp[i][1].first+1, 1});
            dp[i+1][1] = calc(dp[i+1][1], {dp[i][0].first, 0});
        }
    }
    string ans;
    ll cur = calc(dp[N][0], dp[N][1]) == dp[N][0] ? 0 : 1;
    vector<char> C = {'R', 'B'};
    rep(i, N, 0, -1) {
        ans += C[cur];
        cur = dp[i][cur].second;
