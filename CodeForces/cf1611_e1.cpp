/*
・きっちり自力AC！これは結構すんなり行ってよかった。
・中継地全探索、多点スタートBFS
・2回BFSして中継地全探索。これで自身が先に着く葉があればOKだしなければNG。
　友人たちは多点スタートBFSにすれば一度に全員チェックできる。
*/
...
void solve() {
    ...
    auto dist2 = bfs(nodes, A);

    rep(i, 1, N) {
        if (dist1[i] < dist2[i] and nodes[i].size() == 1) {
            YES();
            return;
        }
    }
