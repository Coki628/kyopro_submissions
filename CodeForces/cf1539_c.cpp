/*
・なんとか自力AC！
・にぶたんする。ソートして差分の列を持っておいて、基本的にそっちで話を進める。
　N=1がコーナーケース。1ペナ。。
　何人チームに分けるか決め打てば、追加が何人欲しいか分かるからそれがK以下か見る。
　にぶたんの上限、デバッグ用に小さくしたやつそのまま出しちゃったりでWAを叩いたのは反省。。
*/
...
void solve() {
    ...
    ll res = bisearch_min(0, N, [&](ll m) {
        ll cnt = 0;
        rep(i, N-1-(m-1)) {
            cnt += max(ceil(B[i], X)-1, 0LL);
        }
        return cnt <= K;
    });
    print(res);
