/*
・きっちり自力AC！これはまあ大丈夫だった。
・二分探索、等差数列の和
・山なりになるものを、上りと下りそれぞれ等差数列の和で一気に求める。
　X以上になる最小位置を二分探索で見つける。境界の添字ゲーが嫌になるやつ。
*/
...
void solve() {
    ...
    ll res = bisearch_min(-1, K-1, [&](ll m) {
        return arithmetic_sequence_sum(1LL, 1LL, m) >= X;
    });
    if (res < K-1) {
        print(res);
        return;
    }
    ll sm = arithmetic_sequence_sum(1LL, 1LL, K-1);
    res = bisearch_min(-1, K, [&](ll m) {
        return sm+arithmetic_sequence_sum(K, -1LL, m) >= X;
    });
    ll ans = K-1+res;
    print(ans);
}
