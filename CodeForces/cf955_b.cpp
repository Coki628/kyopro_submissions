/**
 * 参考：https://codeforces.com/blog/entry/58547
 * ・自力ならず。
 * ・題意が分からなかった。これみんな何で普通に分かるの？
 * ・2つのグループに違う文字が入ること、は書いてあるけど、
 * 　両グループとも2種類の文字で構成すること、ってどこ読めば解釈できるんだろ？
 * 　問題文どう読んでも3種類以上入っちゃダメって内容が汲み取れない。
 * ・やっと分かった。2つのグループに分けるみたいな話を、部分列2つに分けるの話だと思ってたら、
 * 　それが部分列内で2つの文字のグループに分けるみたいな話だったぽい。これだと筋が通る。
 */
...
int main() {
    ...
    if (C.size() >= 5) {
        ...
        Yes();
    } elif (C.size() == 3 and cnt2 >= 1) {
        Yes();
    } elif (C.size() == 2 and cnt2 >= 2) {
        Yes();
    } else {
        No();
    }
