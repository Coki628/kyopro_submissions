/*
参考：みんなのツイート
・自力ならず。。これ通したかったなー。。ほぼ考察パートはクリアしてたのに。
・セグ木、区間GCD取得、セグ木にぶたん
・考察を進めると、同じ素因数が連続している区間の最大長が欲しいとなる。
　これ自体は合ってたのに、素因数分解で頑張ろうとしてダメだった。
・どの素因数が残るかは問題でなく、どれでもいいから同じのがあればいいので、
　これは区間GCDが1にならない区間と見ることができる。これが出なかった。。
・そこが分かれば、GCDをセグ木に乗せて、左端固定で右端を伸ばすにぶたんとかやればOK。
・Nは20万で、GCDなので演算自体にlogが乗るし、セグ木のlogもあってにぶたんもしてるのに、AC0.12秒。速い。
*/
...
void solve() {
    ...
    auto seg = get_segment_tree([](int a, int b) { return gcd(a, b); }, 0);
    seg.build(A);
    ll ans = 0;
    rep(i, N*2) {
        // GCDが2以上である最大位置が欲しいので、1である最小位置を取る
        ll j = seg.find_first(i, [&](int x) { return x == 1; });
        if (j != -1) {
            chmax(ans, j-i);
        }
    }
