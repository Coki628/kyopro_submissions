/*
・終了後、一応思った貪欲を書いてみた。これもちゃんとACする。
・結局連続する?はRB交互にするのが絶対いいので、後はどっちになるか。
　?の手前か先を見て、1つでも合う方に寄せる。1つしか合わないのはどっちにしても1つなので。
　全部?だとこの方針ができないので、それだけ別処理。
　それ以外は、前と後ろから見ていって合う方に合わせていく。
・実装方針でそこそこ悩んでしまったけど、さすがにDP復元書くよりは速かったか？どうだろうな。。
*/
...
void solve() {
    ...
    string S;
    cin >> S;
    const vector<char> C = {'R', 'B'};
    map<char, ll> mp = {{'R', 0}, {'B', 1}};
