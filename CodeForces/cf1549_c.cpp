/*
・きっちり自力AC！
・題意のグラフと操作を整理していくと、自分より大きい頂点が隣接していたら結局やられそう、となる。
　その通りに生き残る頂点を加減していくと合う。2次元目をsetにした隣接リストを使うといい感じ。
*/
...
void solve() {
    ...
    rep(_, Q) {
        ...
        if (op == 1) {
            ...
            cin >> u >> v;
            u--; v--;
            if (not died[u] and v > u) {
                died[u] = 1;
                cnt--;
                
            }
            if (not died[v] and u > v) {
                died[v] = 1;
                cnt--;
            }
            nodes[u].insert(v);
            ...
            nodes[u].erase(v);
            nodes[v].erase(u);
            if (died[u] and *nodes[u].rbegin() <= u) {
                died[u] = 0;
                cnt++;
            }
            if (died[v] and *nodes[v].rbegin() <= v) {
                died[v] = 0;
                cnt++;
            }
        } else {
