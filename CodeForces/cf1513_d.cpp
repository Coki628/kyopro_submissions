/*
参考：https://twitter.com/butsurizuki/status/1381286273059414016
・自力ならず。。まあCに時間取られてあまり考察できてなかったのもあるけど。。
・UF、ソートして貪欲
・題意を満たすような辺は、隣接要素が倍数である限り広げていける。
　これをコストの小さい頂点から順に前後に向かってやっていけばOK。
・小さい方から貪欲にやるのはもう少し考察してたら思いついたかもしれないけど、
　手前側に向かってのチェックも必要なのは見落としてたかもしれない。
*/
...
void solve() {
    ...
    for (auto [a, i] : AI) {
        ll j = i+1;
        while (j < N and A[j]%A[i] == 0 and not uf.is_same(i, j)) {
            uf.merge(i, j);
            ans += min(a, P);
            j++;
        }
        j = i-1;
        while (j >= 0 and A[j]%A[i] == 0 and not uf.is_same(i, j)) {
            uf.merge(i, j);
            ans += min(a, P);
            j--;
        }
