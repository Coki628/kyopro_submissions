/*
・自力AC！ここまで通せてよかった。
・桁DPみたいな感じで、ある桁を元の値超にすれば下を自由にできる。
　どの桁でそれをやるか全探索。自由にできる下の桁は、
　上の桁で使ってる数字がK種類あるならその中の最小、K未満なら0で埋める。
　上の桁で使ってる数字の種類数はmapとかで管理できる。
・上界を999...とできるのが分かると、繰り上がりはないと判断できるので、
　だいぶ思考リソースを減らして貪欲方針を進めることができた。
・終了後ちょっと整理。for文ってcharでも回せるんだね。
　あと外ループ逆順にしなくてもよかった。
*/
...
void solve() {
    ...
    map<char, ll> C;
    rep(i, N) {
        for (char b=S[i]+1; b<='9'; b++) {
            C[b]++;
            if (C.size() < K) {
                string cur = S.substr(0, i) + b + string(N-i-1, '0');
                if (toint(cur) < toint(ans)) {
                    ans = cur;
                }
            } elif (C.size() == K) {
                string cur = S.substr(0, i) + b + string(N-i-1, C.begin()->first);
                if (toint(cur) < toint(ans)) {
                    ans = cur;
                }
            }
