/**
 * ・自力AC！
 * ・ナップザック系で必要な状態を持たせてDPする。
 * ・3000^2*4の状態を3次元で持ったらMLEした…。(256MB制限)
 * 　メモリ節約のあれ(1次元目をその場しか持たない)をやって無事AC。
 */
...
void solve() {
    ...
    auto dp = list2d(M+1, N, INF);
    dp[0][0] = 0;
    rep(i, N) {
        auto nxt = list2d(M+1, N, INF);
        rep(j, M+1) {
            rep(k, i+1) {
                chmin(nxt[j][k], dp[j][k]);
                if ((j == 0 or A[k] < A[i]) and j+1 <= M) {
                    chmin(nxt[j+1][i], dp[j][k]+B[i]);
                }
            }
        }
        dp = nxt;
