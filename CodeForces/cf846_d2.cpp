...
・ライブラリ整備：2Dセグ木
・ほぼ無駄な動きなんだけど、updateのちょうどいいverifyできる問題が見つからなかったのでこれで。
*/
...
void solve() {
    ...
    auto seg = get_segment_tree_2d(H, W, [](ll a, ll b) { return max(a, b); }, -INF);
    rep(i, H) rep(j, W) seg.set(i, j, INF);
    seg.build();
    rep(i, Q) {
        ll x, y, t;
        cin >> x >> y >> t;
