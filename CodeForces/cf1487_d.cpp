/**
 * ・ほんとに後1秒くらいのところで間に合わず。。。
 * ・実験エスパー、式変形
 * ・実際に条件に合うものを列挙していくと、規則的な形をしてる。
 * ・とりあえず b+c = a^2 の形のが考えやすい。で、規則的な形は、bは必ずc-1で、
 * 　aは結構離れた小さい数、でこのaは3,5,7,...みたいに3から始まって1つ飛ばしになってる。
 * 　N+(N-1)でb+cを作って、これをsqrt切り捨てにすれば、N以下で作れる最大のaが取れる。
 * 　これをaが3,4なら1、5,6なら2、...ってやろうとするとceil(a,2)-1になるのでそうする。
 */
