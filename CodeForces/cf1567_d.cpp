/*
参考：https://twitter.com/bayashiko_kpr/status/1434566533003370498
・自力ならず。。まあ時間もなかったけど。
・桁毎に考える、優先度付きキュー
・とりあえずサンプルの動きから察するに、繰り上がりが起きると損をしそう。
　なるべく繰り上がりが起きないように値をN個に分けていく方針を目指す。
・実装方針で結構悩んだ。多分、最近キューとかプリキューで管理してシミュする系が
　ぱっと思いつかなくて苦戦する傾向にある気がする。意識していきたい。
・桁和がN未満の間、最適に切り崩しを行っていきたいんだけど、
　この時残っている中でなるべく下位桁を使っていきたい。(1の桁は使えないので除外)
　ある桁を1つ切り崩すと、その1つ下の桁が10個作られるような仕組みなので、
　今ある最下位桁を取得、切り崩して生成された1つ下の桁10個を候補に再度戻す、
　みたいな処理を進めていきたい。こうやってまとめると、この処理にはプリキューが最適だと分かる。
　必要なだけ桁和が増えたら、後は答えに使うN個に1ずつ桁を割り振って、
　先頭とか適当な所に余ったやつを全部当てればいい。
*/
...
void solve() {
    ...
    ll M = S.size();
    vector<ll> A(M);
    rep(d, M) A[d] = toint(S[d]);

    // print(ntod(S, 11));

    reverse(ALL(A));
    ll dsm = sum(A);
    my_priority_queue<ll, vector<ll>, greater<ll>> que;
    rep(d, 1, M) {
        rep(_, A[d]) {
            que.push(d);
        }
