/*
・自力ならず。誤読。。どっちにしても提出は終了後だったけど。
・その時点での上下左右に最大でどこまで行ったかを持ちながらシミュれば、
　その範囲の長さがグリッドより大きくなったらアウトなのでそこで終了。
　最小位置によって始点をずらせば答えとして正しくなる。
・終了後ちょっと粘って、ここまでは辿り着いたんだけど、なぜかサンプル合わず。
　後日色々確かめた結果、単にU,Dの上下を逆に見てただけだった。。
　まあ問題文にfrom 1 to n from top to bottomって書いてあるんだけどさ、
　でもUだと+1でDだと-1するって思うじゃんね。。
*/
