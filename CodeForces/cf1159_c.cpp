/*
・自力AC！
・貪欲シミュする。Aが大きい方から、Bが大きいのを割り当てていく。
　ただしAへの割り当て最後の1つだけは、Aの最小値のルールを守るために、判定を別にする。
　Bへの割り当てがない分はA自身(最小値)にしていけばいい。
・嘘貪欲臭がプンプンしたが、通ったのでヨシ。。。
*/
...
void solve() {
    ...
    rep(i, M) que.push(B[i]);

    sort(A.rbegin(), A.rend());
    ll ans = 0;
    ll i = 0, cnt = 0;
    while (not que.empty()) {
        if (cnt < M-1 and A[i] <= que.top() or A[i] == que.top()) {
            ans += que.top(); que.pop();
            cnt++;
        } else {
            ans += A[i]*(M-cnt);
