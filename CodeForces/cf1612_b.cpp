/*
・自力AC。でも時間かかりすぎ。。今日の戦犯。
・a,bの大小で場合分けとか考えたけど、実際そんなの要らなくて、
　大きいのを貪欲に左に寄せてみて、いけるならいけるし、ダメならダメ。
*/
...
void solve() {
    ...
    ll mn = *min_element(ans.begin(), ans.begin()+N/2);
    ll mx = *max_element(ans.begin()+N/2, ans.end());
    if (mn != a or mx != b) {
        print(-1);
        return;
    }
