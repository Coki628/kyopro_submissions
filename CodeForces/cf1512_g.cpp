...
・きっちり自力AC！
・前計算、エラトステネス応用、約数の和
・約数の個数を全列挙するやつをちょっといじると、和も分かる。
　これを1000万まで前計算しておいて、最短出現位置もチェックしておく。
・1000万にlog乗せるからまあ重い。AC1.64秒。
　素数列挙は未到達のところしか内側ループ回さないけど、
　これは全部について内側も回すから素数列挙よりは遅いのかな、多分。
*/
...
void prep() {
    ...
    rep(i, 1, MX+1) {
        if (table[i] <= MX and ans[table[i]] == -1) {
            ans[table[i]] = i;
        }
    }
}
