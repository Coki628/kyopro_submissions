...
int main() {
    ...
    // 最小と最大の文字
    char mn = *can_use.begin();
    char mx = *can_use.rbegin();
    string ans;
    rep(i, min(N, K)) ans += S[i];

    // Kが大きかったら後ろに最小の文字足すだけ
    if (N < K) {
        rep(_, K-N) {
            ans += mn;
        }
    ...
    // 後ろから見ていく
    rep(i, K-1, -1, -1) {
        // ここが最大の文字でなければ、ここを辞書順で次の文字に書き換えて終了
        if (S[i] != mx) {
            ans[i] = *next(can_use.find(S[i]));
            break;
        // 無理ならここは最小の文字にして次へ
        } else {
            ans[i] = mn;
