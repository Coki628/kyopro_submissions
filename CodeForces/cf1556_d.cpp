...
・自力ならず。。
・インタラクティブ、ビット演算の式変形
・まず、(a | b) + (a & b) = a + b という性質を使う。
　これを知らないとスタートラインに立てない。(立てなかった)
　で、これが分かると3点で連立方程式作って個々の値が分かるので、その先も分かる。
・インタラクティブはデバッグがしにくい。結局、答えの出力をindex用に-1していたのを
　そのままで値を出力していたからWAになってたんだけど、
　こんなんでもボタン1つで試せないので、見つけるのが面倒だった。。
*/
...
void solve() {
    ...
    vector<ll> ans(N);
    ll ab = ask(0, 1, "or")+ask(0, 1, "and");
    ll bc = ask(1, 2, "or")+ask(1, 2, "and");
    ll ac = ask(0, 2, "or")+ask(0, 2, "and");
    ans[0] = (ab+ac-bc)/2;
    ans[1] = (ab+bc-ac)/2;
    ans[2] = (bc+ac-ab)/2;
    rep(i, 3, N) {
        ans[i] = ask(0, i, "or")+ask(0, i, "and")-ans[0];
    }
    sort(ALL(ans));
    answer(ans[K-1]);
}
