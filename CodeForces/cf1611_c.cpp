/*
・なんとか自力AC。。めちゃ時間かかった。今日の戦犯。
・色々と正しい逆操作を模索するが、NG確定する条件がうまく定まらず。。
　めちゃ通されてるから適当に、貪欲に復元してみて戻してみてうまく元と合ったらOK、
　ダメなら無理ってやったら通った。。
*/
...
void solve() {
    ...
    auto que3 = que2;
    que2.pb(tmp[0]);
    que3.push_front(tmp[0]);
    if (org == que2 or org == que3) print(ans);
    else print(-1);
}
